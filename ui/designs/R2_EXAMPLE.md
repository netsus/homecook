...
| 주제 | 공식 화면 ID | 문서 |
| --- | --- | --- |
| recording | `R2_RECORDING_EXAMPLE` | 이 문서 |
| homeflow | `R2_HOMEFLOW_EXAMPLE` | 이 문서 |
...
사용 예시 3/3
내 레시피 · 제육볶음 기록 예시
완성한 요리 1,180g 중 먹은 양 320g
487 kcal · 예시·추정치
탄수화물 31g · 단백질 39g · 지방 22g
영양 값 전체가 예시·추정치예요.
레시피의 재료와 양을 확인·수정하고, 완성한 요리의 무게와 먹은 양을 직접 입력하는 방식이에요. 영양 정보는 추정치예요.
[예시 확인 완료]
[이전 장면]
[베타 오픈 알림 받기]
[메뉴로 돌아가기]
...
장보기 요약 · 준비된 예시
구매: 돼지고기 목살 · 신김치
집에 있어 제외: 양파 · 고추장 · 고춧가루
요리에서 다음 식사로 · 예시
오늘 저녁 제육볶음 · 요리 완료
남은 제육볶음 → 다음 식사 계획
직접 표시한 상태를 잇는 예시예요.
먹을 요리를 고르고, 집에 있는 재료를 직접 확인하며, 요리 완료와 남은 요리 상태를 표시하는 방식이에요. 보유 재료가 자동으로 감지되지는 않아요.
[예시 확인 완료]
[이전 장면]
[베타 오픈 알림 받기]
[메뉴로 돌아가기]
...
세 장면을 직접 이전/다음으로 넘긴다. 장면 1에는 이전을 두지 않고, 장면 2부터 이전 장면 text 행동을 제공한다. 마지막은 '예시 확인 완료' 명시 버튼이며 페이지 열기·스크롤·시간 경과로 완료하지 않는다. 마지막 이전 버튼은 44px text다. 알림 직접 진입과 메뉴 복귀를 각 장면에 제공한다. 저장 중은 버튼 주변에 표시하고 실패는 마지막 장면을 유지한 RECOVERY다. 서버 성공 후 EXAMPLE_DONE. 이미 완료한 예시의 마지막 버튼은 '확인 완료 화면 보기'이며 완료를 다시 생성하지 않는다. 핵심 장면 자료가 없으면 재시도·메뉴를 제공하고 허위 완료하지 않는다.

## 구체적 결과와 fixture 출처

[기존 fixture](evidence/marketing-demand-validation-v2/source-0aaa282/src/Prototype.tsx.txt)의 ingredients와 DemoThree/Four/Five를 재사용한다. 제육볶음, 완성 1,180g, 먹은 320g, 487kcal, 탄수화물31g/단백질39g/지방22g은 기존 예시 수치다. 전부 '예시·추정치'로 인접 표시하며 새 영양 계산·정확성 검증값으로 주장하지 않는다. 재료 수정과 두 무게 직접 입력 조건을 유지한다.

homeflow는 동일 재료를 사용한 정적 배치 예시다. 구매(돼지고기 목살·신김치)와 보유 재료 제외(양파·고추장·고춧가루)를 두 그룹으로 나누고, 오늘 제육 요리 완료 → 남은 제육 → 다음 식사 계획 결과를 보여준다. 보유 여부·날짜·배치는 설계 예시이며 기존 서버 기록/DB fixture가 아니다. 실제 meals/shopping/pantry 필드·요청을 추가하지 않는다.

[세 장면 390 도면](evidence/marketing-demand-validation-round2/R2_example-scenes_390.png), [320 도면](evidence/marketing-demand-validation-round2/R2_example-scenes_320.png). 기존 '흐름 요약'만 있던 마지막 장면을 이 구체적 결과로 대체했다. critic의 해결 판정은 별도 대기다.
...
각 도면에서 `R2_RECORDING_EXAMPLE` / `R2_HOMEFLOW_EXAMPLE` label을 찾는다. 4열×2행, 첫 행 MENU/LEAD_DONE/EXAMPLE_DONE/SURVEY_DONE, 다음 행 EXAMPLE/SURVEY/LEAD/RECOVERY다. 셀 폭은 320/390 CSSpx이며 **높이는 전체 내용에 맞춘 정적 도면**이다. 긴 동의·질문을 844px 안에 축소하지 않는다. 화면·문서 카피는 [공통 시각 내용 JSON](evidence/marketing-demand-validation-round2/R2_design-content.json)의 같은 visibleCopy를 사용한다. 이는 API 스키마가 아니다.
...
판단: 직접 입력 조건과 구체적인 마지막 결과를 함께 보여줘 가치와 사용 노력을 연결한다. 공식 r2.1의 카피·보존·동의·복원 경계를 소비한 작성자 제안이다.

보완 요청: [해당 critique](critiques/R2_EXAMPLE-critique.md) 소유자가 최신 도면·문서를 재검토한다. 기존 critique와 수정 전 점수는 변경하지 않았다. 정적 보완은 author 작업이며 self-pass를 부여하지 않는다. 실서비스 검증·독립 Stage/authority가 완료될 때까지 draft/pending이다.
...
- [recording 320x568 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/recording-320x568-EXAMPLE.png)
- [recording 390x844 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/recording-390x844-EXAMPLE.png)
- [homeflow 320x568 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/homeflow-320x568-EXAMPLE.png)
- [homeflow 390x844 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/homeflow-390x844-EXAMPLE.png)
