...
Canonical 매핑: `R2_RECORDING_SURVEY_DONE` / `R2_HOMEFLOW_SURVEY_DONE` → [공유 설계 R2_SURVEY_DONE.md](../R2_SURVEY_DONE.md). [전체 16 ID 매핑](../R2_SCREEN_MAPPING.md)과 계약 §2에서 확인했다.

확인 결과: 두 topic의 정확한 감사 제목·주제 접수 본문·변경/재제출 불가, 메뉴 primary → 알림 secondary → 예시 text를 확인했다.

이전 finding 처리: 버튼 위계 major 및 접수 카피 차이 해결. 새 대표판에서 이전 마침표 단독 줄은 보이지 않는다. 아래 역사에 남은 '미해결/보류' 중 위에서 해결한 항목은 이 최신 결론으로 대체한다. 남은 필수 도면 수정 요청은 없다.

정책·후속 단계: 부정 응답에도 같은 감사 화면이며 유형/등급 결과가 없다. 실제 첫 제출·서버 복원·재제출 차단은 Stage4 pending이다.
...
- 두 topic generator: [R2_SURVEY_DONE.md](../R2_SURVEY_DONE.md)의 recording/homeflow 및 상태 정의. 공통 [설계 방향](../R2_DESIGN_DIRECTION.md)도 확인했다.
...
두 주제 모두 의견 접수·재제출/변경 불가·여기서 마쳐도 됨이 보인다. 유형·등급 결과가 없다.

## 해결

mascot 표시·320 전체 완료 변형의 알림 접수 확인/예시 다시 보기 해결.

## 남은 finding

major: 문서는 메뉴 우선, 도면은 알림 우선이다. minor: 주제별 접수 본문이 공통 문구로 축약됐다. 320의 마침표 단독 줄은 경미한 도면 제한으로 수용한다.

완료 위계가 다른 경우에는 종료 허용 다음 메뉴 복귀를 우선하는 generator 기준으로 맞추는 것을 권고한다. 원본은 수정하지 않았다. 실제 동작 결함으로 단정하지 않는다.

## 허용된 단계 제약과 Stage4 pending

서버 첫 제출 확인, 부정 응답 동일 처리, 이전 폼 재제출 방지, 부분 완료/390 변형, 짧은 화면 스크롤.
...
major 해결: homeflow 320 기본판과 두 topic 320 전체 완료 변형에서 메뉴 복귀가 blue+ink 주버튼, 알림/알림 접수 확인이 보조버튼임을 확인했다. 기존 버튼 위계 불일치는 확인 범위에서 종료한다. 주제별 접수 카피 차이와 320 마침표 고립의 경미한 도면 한계는 유지한다.
