# HOME 설계 리뷰

> 검토 대상: `ui/designs/HOME.md`
> 기준 문서: 화면정의서 v1.2 §1 HOME / 요구사항 v1.6 §1-1 / 유저Flow맵 v1.2 §① / 디자인 토큰 C2 명랑한 주방
> 검토일: 2026-03-21
> 검토자: design-critic

---

## 종합 평가

**등급**: 🟡 조건부 통과

**한 줄 요약**: 핵심 요구사항과 토큰 준수는 대부분 충족했으나, read-only 상태 미명시·로그인 게이트 return-to-action 누락·iOS safe-area 패딩 미기술 등 권장 수정 항목이 4건 있다.

---

## 크리티컬 이슈 (수정 필수)

없음

---

## 마이너 이슈 (권장 수정)

| # | 위치 | 문제 | 제안 |
|---|------|------|------|
| 1 | 로그인 게이트 모달 | [로그인하기] 탭 후 LOGIN → 완료 시 return-to-action 복귀 동작이 와이어프레임 주석에 언급만 됐고 플로우 다이어그램에 명시되지 않음. 화면정의서 §0-3, 유저Flow맵 §② 모두 return-to-action을 필수로 요구함 | 인터랙션 노트 표 "로그인 후 복귀" 행에 "로그인 완료 → 원래 액션(플래너/팬트리/마이페이지 이동) 자동 수행" 을 명시하고, 모달 와이어프레임 주석에 `returnTo: <원래 탭>` 파라미터를 추가 |
| 2 | 공통 상태 — read-only | HOME 자체는 read-only 제약이 없는 탐색 전용 화면이지만, 설계 내 공통 상태 목록(Loading / Empty / Error)에 unauthorized(비인가) 상태가 완전히 빠져 있음. 요청 명세에서 5개 필수 상태를 요구함 | "### 비인가 상태" 항목 추가. HOME은 비로그인 가능이므로 `unauthorized` 상태가 발생하지 않음을 한 줄로 명시하는 것으로 충분 (예: "HOME은 비로그인 가능 화면이므로 unauthorized 상태 없음") |
| 3 | 검색바 — iOS safe-area | design-critic 검토 요청 항목(§design-critic 검토 필요 항목)에서 노치/다이나믹 아일랜드 대응 패딩 미확정으로 명시함. 검색바 fixed 영역 상단 패딩이 `env(safe-area-inset-top)` 적용 기준 없이 "약 116px" 로만 표기됨 | 스크롤 & 레이아웃 동작 섹션에 `padding-top: calc(<검색바높이> + env(safe-area-inset-top))` 기준 명시. 구체적 수치는 구현 단계에서 확정하되 설계 단계에서 토큰 표기라도 반영 |
| 4 | 정렬 드롭다운 — z-index | design-critic 검토 요청 항목에서 드롭다운 열림 시 카드와의 z-index 충돌이 미결로 남아 있음. 설계 단계에서 레이어 정책이 결정되지 않으면 구현 시 충돌 발생 가능 | 드롭다운 열림 상태 레이어를 `z-index: 100` (탭바보다 낮고 모달보다 낮은 값)으로 명시하거나, 드롭다운 열림 시 그리드 영역 scroll-lock 여부를 결정해 주석으로 기술 |

---

## 체크리스트 결과

### A. 요구사항 정합성

- [x] 화면정의서 §1의 모든 컴포넌트(검색바, [재료로 검색] 버튼, 정렬 드롭다운, 테마 섹션, 레시피 그리드, 카드)가 와이어프레임에 포함됐는가
- [x] 문서에 없는 컴포넌트/필드/기능이 추가됐는가 — 없음. [재료로 검색] Out of Scope 처리 및 "준비 중" 토스트는 설계 결정 사항으로 적절히 문서화됨
- [x] 로그인 게이트 대상 액션(플래너/팬트리/마이페이지 탭 이동)이 모두 처리됐는가 — 하단 탭바 비로그인 LoginRequiredModal 처리 명시됨
- [x] read-only 상태(완료 장보기 등)가 올바르게 반영됐는가 — HOME은 read-only 해당 없는 탐색 전용 화면, 별도 처리 불필요
- [x] 삭제된 엔드포인트 `DELETE /recipes/{id}/save`가 UI에 등장하지 않는가 — HOME 설계 범위 내 저장 액션 없음, 해당 없음

### B. 공통 상태 커버리지

- [x] Loading 상태 (스켈레톤/인디케이터) 포함 — 스켈레톤 카드 6개(2×3) + pulse 애니메이션 + 섹션 헤더 스켈레톤 명시
- [x] Empty 상태 (안내 + CTA) 포함 — "조건에 맞는 레시피가 없어요" + [필터 초기화] --brand CTA 명시
- [x] Error 상태 (안내 + [다시 시도]) 포함 — "레시피를 불러오지 못했어요" + [다시 시도] --brand CTA 명시
- [x] read-only 상태가 필요한 화면에서 수정 UI 비노출 — 해당 없음 (HOME은 탐색 전용)
- [◻] unauthorized 상태 명시 — 5개 필수 상태 중 unauthorized가 설계 문서에 명시적으로 언급되지 않음 (마이너 이슈 #2)

### C. 내비게이션 & 플로우

- [x] 하단 탭 4개 (홈/플래너/팬트리/마이페이지) 구조 일관성 — --panel 배경, 4탭 배치, 활성/비활성 토큰 모두 명시
- [x] 유저 Flow맵과 진입/이탈 경로 일치 — 앱 실행 → HOME, 카드 탭 → RECIPE_DETAIL, [재료로 검색] → INGREDIENT_FILTER_MODAL(현재 Out of Scope 토스트) 모두 일치
- [x] 뒤로가기 동작 명시 여부 — HOME은 앱 진입점이라 별도 뒤로가기 불필요. "AppBar 없음" 설계 결정으로 암묵적 처리 가능하나, RECIPE_DETAIL → HOME 복귀 시 동작은 RECIPE_DETAIL 설계 범위
- [x] 플로우 단절 지점 없는가 — LoginRequiredModal [취소] → 이전 화면 유지, [로그인하기] → LOGIN 이동 명시. 단, return-to-action 명시가 미흡 (마이너 이슈 #1)

### D. UX 품질

- [x] 터치 타겟 최소 44px 준수 — 퀵 필터 행(44px 명시), 검색바 X버튼(44×44px), 하단 탭 각 탭(44×44px) 모두 명시
- [x] 모바일 퍼스트 (375px 기준) 레이아웃 — 375px 기준 2열 그리드, 카드 너비 계산식(165px) 명시
- [x] 핵심 액션이 시각적으로 명확한가 — 검색바가 최상단 고정, Empty/Error 시 --brand CTA 버튼 배치 명확
- [◻] 장보기 D&D (sort_order) UI — HOME 화면 해당 없음
- [◻] 팬트리 제외 섹션 2-영역 구조 — SHOPPING_DETAIL 전용 항목, HOME 해당 없음
- [x] AI스러운 제네릭 UI 사용 여부 — 글로우/과도한 그라디언트 없음. 박스섀도우는 `0 2px 10px rgba(0,0,0,0.08)` 토큰 기준 준수

### E. 도메인 규칙 정합성

- [x] `meals.status` 전이 표현 — HOME 탐색 화면이므로 meals 상태 전이 직접 노출 없음, 해당 없음
- [x] 독립 요리는 meals 상태를 바꾸지 않는 것이 UI에서 명확한가 — HOME 범위 밖, 해당 없음
- [x] 팬트리는 수량이 아닌 보유 여부만 표시 — HOME에서 팬트리 정보 미노출, 해당 없음
- [x] 요리 모드에서 인분 조절 UI가 없는가 — HOME 범위 밖, 해당 없음
- [x] 저장 가능한 레시피북 타입 (saved/custom) 만 표시 — HOME에서 레시피 저장 UI 없음, 해당 없음

### F. 디자인 토큰 준수

- [x] CTA 버튼에 `--brand (#FF6C3C)` 사용됐는가 — Empty [필터 초기화], Error [다시 시도], LoginRequiredModal [로그인하기] 모두 --brand 명시
- [x] 카드 배경에 `--surface (#ffffff)` 명시됐는가 — 레시피 카드 "--surface 카드 배경" 명시
- [x] 태그·칩에 `--olive (#2ea67a)` 사용됐는가 — 레시피 카드 태그 칩, [재료로 검색] 칩 모두 --olive 명시
- [x] 보조 텍스트에 `--muted (#999999)` 사용됐는가 — placeholder, 통계 수치, 더보기 링크, 비활성 탭 모두 --muted 명시
- [x] 카드 border-radius 16px 준수됐는가 — 레시피 카드 "border-radius: 16px" 명시
- [x] 수평 여백 16px(모바일) 기준 준수됐는가 — "수평 여백 16px (--space-4)" 와이어프레임 주석 명시
- [x] 확정 토큰 외 임의 색상(`#d56a3a`, `#6e7c4a` 등 구버전) 사용됐는가 — 설계 전체에서 확정 토큰 변수명(`--brand`, `--olive`, `--surface` 등)만 사용, 구버전 하드코딩 색상 없음

---

## design-generator 재작업 요청 항목

- [ ] 로그인 게이트 모달 — return-to-action 플로우를 인터랙션 노트와 와이어프레임 주석에 명시 (마이너 이슈 #1)
- [ ] 공통 상태 섹션 — unauthorized 상태 미해당 여부를 한 줄로 명시 (마이너 이슈 #2)
- [ ] 스크롤 & 레이아웃 동작 섹션 — 검색바 상단 패딩에 `env(safe-area-inset-top)` 적용 기준 추가 (마이너 이슈 #3)
- [ ] 정렬 드롭다운 — 열림 시 z-index 레이어 정책 주석 추가 (마이너 이슈 #4)

---

## 통과 조건

이 화면 설계가 구현으로 넘어가려면:
- [x] 크리티컬 이슈 0개
- [ ] 마이너 이슈 4건 처리 또는 수용 결정

---

## 부록: 설계 결정 검토 의견

design-generator가 `§ design-critic 검토 필요 항목`으로 위임한 6개 항목에 대한 판정:

| 항목 | 판정 | 근거 |
|------|------|------|
| 2열 그리드 카드 높이 균등화 | 구현 단계 결정 가능 — CSS `grid auto-rows` 사용 권장 (고정 높이 대비 유연성 확보) | 설계 단계에서 "최대 2줄 말줄임" 정의로 충분 |
| iOS safe-area-inset-top 패딩 | 설계에 토큰 수준 명시 필요 — 마이너 이슈 #3으로 등록 | 기기별 분기 발생 예상 |
| 정렬 드롭다운 z-index 충돌 | 설계에 레이어 정책 명시 필요 — 마이너 이슈 #4으로 등록 | 구현 시 테스트 항목 |
| 테마 섹션 0개 시 전체 Empty 처리 | 현행 설계("섹션이 0개면 전체 Empty 상태")로 충분 — 추가 수정 불필요 | 레시피 그리드 공통 상태와 일관성 있음 |
| [재료로 검색] 활성 시각 강조 | Out of Scope 해소 후 적용 — 현재 불필요 | 설계 결정 사항으로 적절히 문서화됨 |
| 스켈레톤 카드 수(6개) 적절성 | 구현 단계 조정 가능 — 설계 단계에서 "최소 6개, 뷰포트 크기에 따라 가변" 정도로 기술하면 충분 | 크리티컬 이슈 아님 |
