...
Canonical 매핑: `R2_RECORDING_LEAD_DONE` / `R2_HOMEFLOW_LEAD_DONE` → [공유 설계 R2_LEAD_DONE.md](../R2_LEAD_DONE.md). [전체 16 ID 매핑](../R2_SCREEN_MAPPING.md)과 계약 §2에서 확인했다.

확인 결과: 두 topic의 주제별 접수 본문, 메뉴 primary → 예시 secondary → 의견 text를 확인했다. 전체 완료 상태는 예시 다시 보기·의견 접수 확인으로 표시된다.

이전 finding 처리: 버튼 위계 major 및 주제 접수 카피·mascot 표시 관련 이전 finding 해결. 지정 크기의 실측은 승인하지 않는다. 아래 역사에 남은 '미해결/보류' 중 위에서 해결한 항목은 이 최신 결론으로 대체한다. 남은 필수 도면 수정 요청은 없다.

정책·후속 단계: 서버 영수증/완료 복원 후에만 표시하고 신규/기존 이메일에 같은 화면을 쓰는 설계다. 발송 완료나 이메일 소유 확인으로 표현하지 않는다.
...
- 두 topic generator: [R2_LEAD_DONE.md](../R2_LEAD_DONE.md)의 recording/homeflow 및 상태 정의. 공통 [설계 방향](../R2_DESIGN_DIRECTION.md)도 확인했다.
...
두 주제 모두 접수 확인과 선택적 종료가 명확하다. 메뉴/예시/의견으로 이동할 선택을 제공한다.

## 해결

작은 mascot 정상 표시. 320 전체 완료 변형에 예시 다시 보기·의견 접수 확인 반영.

## 남은 finding

major: generator는 메뉴 blue 주버튼·예시 outline, 도면은 메뉴 outline·예시 text로 위계가 다르다. minor: mascot 지정 치수와 접수 본문도 자료 간 다르다.

완료 위계가 다른 경우에는 종료 허용 다음 메뉴 복귀를 우선하는 generator 기준으로 맞추는 것을 권고한다. 원본은 수정하지 않았다. 실제 동작 결함으로 단정하지 않는다.

## 허용된 단계 제약과 Stage4 pending

서버 확인 후 완료 진입, 이메일 재입력 방지, 부분 완료/390 변형, 뒤로가기·키보드 닫힘·자동 복귀 없음.
...
major 해결: homeflow 320 기본판과 두 topic 320 전체 완료 변형에서 '메뉴로 돌아가기'가 blue+ink 주버튼, 예시/예시 다시 보기가 outline 보조버튼임을 확인했다. 기존 버튼 위계 불일치 finding은 확인 범위에서 종료한다. 접수 본문 카피·mascot 치수 표기 차이와 Stage4 검증은 유지한다.
