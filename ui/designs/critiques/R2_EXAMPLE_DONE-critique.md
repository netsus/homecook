...
Canonical 매핑: `R2_RECORDING_EXAMPLE_DONE` / `R2_HOMEFLOW_EXAMPLE_DONE` → [공유 설계 R2_EXAMPLE_DONE.md](../R2_EXAMPLE_DONE.md). [전체 16 ID 매핑](../R2_SCREEN_MAPPING.md)과 계약 §2에서 확인했다.

확인 결과: 두 topic의 결과 요약과 실서비스가 아닌 예시 안내, 메뉴 primary → 알림 secondary → 의견 text → 예시 다시 보기 순서를 확인했다. homeflow에 보유 재료 제외 장보기가 포함된다.

이전 finding 처리: 버튼 위계 major, homeflow 요약·예시 안내·밑줄·재보기 카피 관련 이전 finding 해결. 아래 역사에 남은 '미해결/보류' 중 위에서 해결한 항목은 이 최신 결론으로 대체한다. 남은 필수 도면 수정 요청은 없다.

정책·후속 단계: 전체 완료 상태는 알림/의견 접수 확인으로 이동하도록 설계됐다. 서버 저장 뒤 완료 진입·다시 보기 중 완료 유지·중복 생성 방지는 Stage4에서 검증한다.
...
- 두 topic generator: [R2_EXAMPLE_DONE.md](../R2_EXAMPLE_DONE.md)의 recording/homeflow 및 상태 정의. 공통 [설계 방향](../R2_DESIGN_DIRECTION.md)도 확인했다.
