...
- mixed-entry `390`, `320`, `1280` evidence에서 레시피 식사와 완제품 항목이 구분되고, 완제품에는 수량 변경/삭제만 제공된다.
- PATCH의 `NUTRITION_BASIS_MISMATCH`는 공식 UI 문구 `이 기준으로는 수량을 바꿀 수 없어요`로 표시되며 edit dialog를 유지한다. API raw message/status/code/fields는 presentation mapping으로부터 분리돼 있다.
- pinned nutrition basis와 partial/unavailable/missing-not-zero 표현을 유지한다.
