...
Canonical 매핑: `R2_RECORDING_LEAD` / `R2_HOMEFLOW_LEAD` → [공유 설계 R2_LEAD.md](../R2_LEAD.md). [전체 16 ID 매핑](../R2_SCREEN_MAPPING.md)과 계약 §2에서 확인했다.

확인 결과: 두 topic의 주제별 제목·설명과 신청 버튼 카피가 설계와 맞는다. §8의 전체 동의문, 2026년 11월 30일까지 보관·철회 삭제, 미동의자 예시/의견 이용, 만14세 안내, privacy, 미체크 동의를 도면에서 확인했다.

이전 finding 처리: 주제/버튼 카피 차이 해결. 동의·보관 placeholder는 승인 문구로 대체돼 이전 미승인 카피 보류 종료. 보안 슬롯은 검토용이며 실제 통과가 아니다. 아래 역사에 남은 '미해결/보류' 중 위에서 해결한 항목은 이 최신 결론으로 대체한다. 남은 필수 도면 수정 요청은 없다.

정책·후속 단계: 표시 보관일과 기술 경계 2026-12-01T00:00:00+09:00 exclusive를 설계가 구분한다. 실제 privacy/보안 readiness·키보드·동의 hit area·입력 오류·접수자 폼 우회는 Stage4/독립 authority pending이다.
...
- 두 topic generator: [R2_LEAD.md](../R2_LEAD.md)의 recording/homeflow 및 상태 정의. 공통 [설계 방향](../R2_DESIGN_DIRECTION.md)도 확인했다.
...
두 주제 모두 이메일 label, 미체크 동의, 별도 개인정보 링크, 보안 placeholder, 신청/메뉴 행동이 보인다.

## 해결

밝은 파랑 위 ink 및 설명 후 짧은 폼 구조 확인. 동의·보관 placeholder는 검토용으로 명시된다.

## 남은 finding

minor: 문서의 recording/homeflow별 제목·가치 설명이 도면에서는 공통 신청 문구로 바뀌었다. 신청 버튼 문구도 다르다.

완료 위계가 다른 경우에는 종료 허용 다음 메뉴 복귀를 우선하는 generator 기준으로 맞추는 것을 권고한다. 원본은 수정하지 않았다. 실제 동작 결함으로 단정하지 않는다.

## 허용된 단계 제약과 Stage4 pending

승인 동의문/보관일, 실제 보안 위젯·긴 이메일·키보드·동의 클릭 영역·오류 연결·이미 신청자 폼 우회. placeholder와 런타임 부재는 현 단계 제약이다.
...
이번 지정 이미지의 homeflow 320 LEAD에는 기존 주제 카피·버튼 문구 차이가 남는다. 새 해결 판정 없이 기존 minor와 Stage4 동의/보안/키보드 검증 pending을 유지한다.
