...
Canonical 매핑: `R2_RECORDING_RECOVERY` / `R2_HOMEFLOW_RECOVERY` → [공유 설계 R2_RECOVERY.md](../R2_RECOVERY.md). [전체 16 ID 매핑](../R2_SCREEN_MAPPING.md)과 계약 §2에서 확인했다.

확인 결과: 두 topic LEAD 기반 복구 도면의 정확한 동의문·주제 카피, 느낌표 오류 표시, 명확한 저장 실패, 열린 탭만 입력 유지·reload 폐기, 재시도·메뉴를 확인했다.

이전 finding 처리: 입력 보존 한계·오류 아이콘·카피 관련 이전 finding 해결. 대표 오류 외 모든 분기를 그리지 않은 것은 현 단계 필수 불일치로 세지 않는다. 아래 역사에 남은 '미해결/보류' 중 위에서 해결한 항목은 이 최신 결론으로 대체한다. 남은 필수 도면 수정 요청은 없다.

정책·후속 단계: 계약 §2의 cookie_resume/유효 참여 제출/쿠키·저장소 부재 시 draft만 허용/no-new-memory와 문서가 일치한다. 응답 유실은 접수 미확인으로 구분하며 알림 자동 재전송 금지·다른 완료 보존·410/409 처리는 실제 Stage4 검증 대상이다.
...
- 두 topic generator: [R2_RECOVERY.md](../R2_RECOVERY.md)의 recording/homeflow 및 상태 정의. 공통 [설계 방향](../R2_DESIGN_DIRECTION.md)도 확인했다.
