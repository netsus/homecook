...
Canonical 매핑: `R2_RECORDING_SURVEY` / `R2_HOMEFLOW_SURVEY` → [공유 설계 R2_SURVEY.md](../R2_SURVEY.md). [전체 16 ID 매핑](../R2_SCREEN_MAPPING.md)과 계약 §2에서 확인했다.

확인 결과: 두 topic × 네 문항을 직접 확인했다. Q1은 본인/가족·동거인 조리 식사 범위와 한 끼·배달/데우기/간식 제외 안내를 포함한다. recording Q2 no_record/reuse_saved/other, homeflow Q2 not_managing/other 및 양쪽 Q3 none, Q4 no/unsure와 직접 입력 조건이 계약 §3·설계 문서에 맞는다.

이전 finding 처리: Q1 가족 범위·초기 미선택·주제/이메일 불필요·다음 문항 카피 차이 해결. 이전 Q2/Q4 도면 부재도 해소됐다. 모든 옵션을 같은 선택 계열로 표시하고 other에 자유 텍스트를 붙이지 않는다. 아래 역사에 남은 '미해결/보류' 중 위에서 해결한 항목은 이 최신 결론으로 대체한다. 남은 필수 도면 수정 요청은 없다.

정책·후속 단계: value 매핑은 문서, label·순서는 PNG로 확인했다. radio/fieldset·오류·초점·최종 네 응답 제출·저장소 복원은 실제 검증하지 않았다.
...
- [recording 네 문항 320](../evidence/marketing-demand-validation-round2/R2_recording_survey_320.png), [homeflow 네 문항 320](../evidence/marketing-demand-validation-round2/R2_homeflow_survey_320.png).
...
- 두 topic generator: [R2_SURVEY.md](../R2_SURVEY.md)의 recording/homeflow 및 상태 정의. 공통 [설계 방향](../R2_DESIGN_DIRECTION.md)도 확인했다.
...
두 주제의 공통 Q1 도면은 같은 크기 옵션과 radio 선택 표시, 별도 다음 행동을 제공한다.

## 해결

색 외 선택 표시 확인. 검토자용 미승인 문항·옵션 안내가 있다.

## 남은 finding

minor: 도면 Q1 '직접 만든 식사'와 문서의 가족·동거인 포함 문장은 길이·의미가 다르다. 선택된 도면임을 명시하고 주제/이메일 없이 참여 안내·다음 문구를 맞출 필요가 있다.

완료 위계가 다른 경우에는 종료 허용 다음 메뉴 복귀를 우선하는 generator 기준으로 맞추는 것을 권고한다. 원본은 수정하지 않았다. 실제 동작 결함으로 단정하지 않는다.

## 허용된 단계 제약과 Stage4 pending

승인 후 최종 문항 동기화, 긴 Q2/Q4·오류·명시적 제출·키보드 접근·답변 복원. Q1 한 장면만 있는 것은 현 단계 허용 범위다.
...
이번 지정 이미지의 homeflow 320 설문은 기존 Q1 대표 도면이며 별도 해결 판정을 추가하지 않는다. Q1 문구·선택 후 상태 표시·주제 안내 차이는 유지하고 최종 카피/전체 문항/런타임은 허용된 Stage4 pending으로 남긴다.
