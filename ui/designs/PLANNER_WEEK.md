...
- keep the existing Planner route and bottom navigation item. Inside it, expose exactly `요리 계획 | 식사 기록`.
- `PLANNER_WEEK` owns only the `요리 계획` panel: future Recipe Meal plan, shopping and cooking status/actions.
- #12 `MEAL_LOG` owns the `식사 기록` panel body, totals, food-add/search and actual-intake mutations. Before #12 is deployed, show a fail-closed unavailable panel without weakening the plan panel.
- #11 owns COOK_MODE/LEFTOVERS batch-weight UI. #13 owns legacy compatibility telemetry and tombstone decisions.
- no new route, bottom tab, API, field, DB status, writer or HOME product/ingredient search is introduced.
...
│ HOMECOOK                            │
│ 플래너                              │
│ [ 요리 계획 ] [ 식사 기록 ]         │  44px min, tab semantics
...
│ 과거 완제품 계획 · 읽기 전용          │
│ 제품명 · 브랜드 · 1봉                │
│        [상세] [삭제]                 │  secondary → destructive tertiary
└─────────────────────────────────────┘
```
...
- `keep` renders the pinned title/ingredients/steps/nutrition snapshot. `legacy_backfill` renders `당시 상세 내용 미보존`.
...
- localized planner horizontal movement is allowed only where the approved prototype already requires it. Unintended page-level horizontal overflow is forbidden at 390px, 320px and desktop.
- visible focus, reduced motion, screen-reader state announcements and DOM order must not depend on color or animation.
...
| legacy read-only | pinned identity/quantity/detail plus confirmed owner delete only |
| #12 disabled | `식사 기록` explains temporary unavailability; `요리 계획` remains fully usable |

### CTA and language lock

- `registered`: `장보기` primary, `상세` secondary.
- `shopping_done`: `요리하기` primary, `상세` secondary.
- `cook_done`: `상세` only; never label the plan row consumed/eaten or show actual-intake totals.
...
## Historical Baseline and Superseded Evidence
