...
| 주제 | 공식 화면 ID | 문서 |
| --- | --- | --- |
| recording | `R2_RECORDING_SURVEY_DONE` | 이 문서 |
| homeflow | `R2_HOMEFLOW_SURVEY_DONE` | 이 문서 |
...
서버의 네 문항 첫 제출 성공 또는 확인된 완료 복원으로만 진입한다. 계약 제목과 주제별 접수 본문 후 '접수한 답변은 지금 변경하거나 다시 제출할 수 없어요.'를 표시한다. 부정 응답도 같은 감사 화면이다. 메뉴 primary → 알림 secondary → 예시 text이며 완료한 알림은 접수 확인만 제공한다. 유형·적합도·추천 결과를 만들지 않는다.
...
각 도면에서 `R2_RECORDING_SURVEY_DONE` / `R2_HOMEFLOW_SURVEY_DONE` label을 찾는다. 4열×2행, 첫 행 MENU/LEAD_DONE/EXAMPLE_DONE/SURVEY_DONE, 다음 행 EXAMPLE/SURVEY/LEAD/RECOVERY다. 셀 폭은 320/390 CSSpx이며 **높이는 전체 내용에 맞춘 정적 도면**이다. 긴 동의·질문을 844px 안에 축소하지 않는다. 화면·문서 카피는 [공통 시각 내용 JSON](evidence/marketing-demand-validation-round2/R2_design-content.json)의 같은 visibleCopy를 사용한다. 이는 API 스키마가 아니다.
...
보완 요청: [해당 critique](critiques/R2_SURVEY_DONE-critique.md) 소유자가 최신 도면·문서를 재검토한다. 기존 critique와 수정 전 점수는 변경하지 않았다. 정적 보완은 author 작업이며 self-pass를 부여하지 않는다. 실서비스 검증·독립 Stage/authority가 완료될 때까지 draft/pending이다.
...
- [recording 320x568 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/recording-320x568-SURVEY_DONE.png)
- [recording 390x844 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/recording-390x844-SURVEY_DONE.png)
- [homeflow 320x568 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/homeflow-320x568-SURVEY_DONE.png)
- [homeflow 390x844 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/homeflow-390x844-SURVEY_DONE.png)
