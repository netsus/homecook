...
| 주제 | 공식 화면 ID | 문서 |
| --- | --- | --- |
| recording | `R2_RECORDING_LEAD_DONE` | 이 문서 |
| homeflow | `R2_HOMEFLOW_LEAD_DONE` | 이 문서 |
...
서버 영수증 또는 같은 참여의 확인된 접수 상태가 있어야 진입한다. 제목은 '베타 오픈 알림 신청을 접수했어요'. 신규/기존 이메일에 동일한 화면이며 이메일 원문·중복 여부·발송 성공·소유 확인·계정 생성·베타 이용 권한 부여를 표시하지 않는다. 메뉴 primary → 예시 secondary → 의견 text이며 완료된 의견은 접수 확인만 열린다.
...
각 도면에서 `R2_RECORDING_LEAD_DONE` / `R2_HOMEFLOW_LEAD_DONE` label을 찾는다. 4열×2행, 첫 행 MENU/LEAD_DONE/EXAMPLE_DONE/SURVEY_DONE, 다음 행 EXAMPLE/SURVEY/LEAD/RECOVERY다. 셀 폭은 320/390 CSSpx이며 **높이는 전체 내용에 맞춘 정적 도면**이다. 긴 동의·질문을 844px 안에 축소하지 않는다. 화면·문서 카피는 [공통 시각 내용 JSON](evidence/marketing-demand-validation-round2/R2_design-content.json)의 같은 visibleCopy를 사용한다. 이는 API 스키마가 아니다.
...
보완 요청: [해당 critique](critiques/R2_LEAD_DONE-critique.md) 소유자가 최신 도면·문서를 재검토한다. 기존 critique와 수정 전 점수는 변경하지 않았다. 정적 보완은 author 작업이며 self-pass를 부여하지 않는다. 실서비스 검증·독립 Stage/authority가 완료될 때까지 draft/pending이다.
...
- [recording 320x568 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/recording-320x568-LEAD_DONE.png)
- [recording 390x844 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/recording-390x844-LEAD_DONE.png)
- [homeflow 320x568 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/homeflow-320x568-LEAD_DONE.png)
- [homeflow 390x844 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/homeflow-390x844-LEAD_DONE.png)
