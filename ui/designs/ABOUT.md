...
`/about`은 비로그인 사용자도 볼 수 있는 단일 서비스 설명서다. 처음 온 사용자는 집밥의 전체 흐름을 이해하고 레시피 또는 플래너로 이동하며, 기존 사용자는 기능별 설명과 FAQ를 찾는다.

- 화면 ID: `ABOUT_SERVICE_GUIDE`
- 공개 route: `/about`
- desktop nav label: `집밥 가이드`
- page H1: `집밥, 이렇게 써요`
- hero headline: `레시피에서 끝나지 않는 집밥 계획`
...
- global top nav는 `PRIMARY_WEB_NAV_ITEMS`를 사용하며 `/about`에서 `aria-current="page"`가 `집밥 가이드`에만 붙는다.

## 5. Mobile 390px

```text
┌──────────────────────────────────────┐
│ [←] 집밥 가이드                     │  sticky, 44px 이상 action
├──────────────────────────────────────┤
│ 레시피에서 끝나지 않는 집밥 계획    │
│ 집밥, 이렇게 써요                    │
│ 짧은 설명                            │
...
- eyebrow/headline/H1을 중복해서 모두 크게 만들지 않는다. `레시피에서 끝나지 않는 집밥 계획`을 핵심 가치 문장으로, `집밥, 이렇게 써요`를 유일한 H1으로 사용한다.
