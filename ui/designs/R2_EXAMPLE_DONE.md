...
예시 확인 완료가 서버에 저장된 경우에만 이 완료 화면을 표시한다. 메뉴와 세 완료 화면은 같은 흰 배경·로고·본문 폭·버튼 계열을 공유한다. '여기서 마쳐도 괜찮아요'가 종료 허용의 주 메시지이며 후속 활동은 전부 선택이다.
...
예시 확인은 실제 식사 기록·장보기·요리 완료가 아니다. 완료 제목 아래에 '베타 준비 중 · 실제 서비스가 아닌 사용 예시'를 유지한다.
...
'사용 예시 다시 보기'는 보조 선택 아래 44px text로 추가할 수 있다. 현재 서버 완료를 유지한 채 EXAMPLE의 첫 장면을 연다.
...
리더가 생성 완료를 보고한 4열×2행 정적 도면이다. 각 cell은 해당 320/390 CSSpx 폭이며 모든 8개 상태를 포함한다. 도면의 `R2_EXAMPLE_DONE` 상태 label로 대응한다. 이 generator는 새 도면을 직접 열어 검증하지 않았다.
