...
| 주제 | 공식 화면 ID | 문서 |
| --- | --- | --- |
| recording | `R2_RECORDING_EXAMPLE_DONE` | 이 문서 |
| homeflow | `R2_HOMEFLOW_EXAMPLE_DONE` | 이 문서 |
...
베타 준비 중 · 사용 예시
사용 예시를 확인했어요
내 레시피에서 먹은 분량의 추정 영양 기록으로 이어지는 예시를 보셨어요.
실제 서비스가 아닌 사용 예시예요.
여기서 마쳐도 괜찮아요.
원하시면 다른 활동도 살펴보세요.
[메뉴로 돌아가기]
...
베타 준비 중 · 사용 예시
사용 예시를 확인했어요
요리를 계획하고, 집에 있는 재료를 빼고 장보기부터 요리와 남은 요리 관리까지의 예시를 보셨어요.
실제 서비스가 아닌 사용 예시예요.
여기서 마쳐도 괜찮아요.
원하시면 다른 활동도 살펴보세요.
[메뉴로 돌아가기]
...
예시 확인 완료의 서버 저장 성공 또는 복원된 완료 상태로만 진입한다. 제목은 계약의 '사용 예시를 확인했어요'. 이 확인은 실제 식사·장보기·요리 완료가 아니다. 메뉴 primary → 알림 secondary → 의견 text → 예시 다시 보기 text다. 완료된 알림/의견은 접수 확인으로만 이동한다. 준비된 사용 예시 안내를 유지한다.
...
각 도면에서 `R2_RECORDING_EXAMPLE_DONE` / `R2_HOMEFLOW_EXAMPLE_DONE` label을 찾는다. 4열×2행, 첫 행 MENU/LEAD_DONE/EXAMPLE_DONE/SURVEY_DONE, 다음 행 EXAMPLE/SURVEY/LEAD/RECOVERY다. 셀 폭은 320/390 CSSpx이며 **높이는 전체 내용에 맞춘 정적 도면**이다. 긴 동의·질문을 844px 안에 축소하지 않는다. 화면·문서 카피는 [공통 시각 내용 JSON](evidence/marketing-demand-validation-round2/R2_design-content.json)의 같은 visibleCopy를 사용한다. 이는 API 스키마가 아니다.
...
판단: 확인된 완료와 종료 허용을 먼저 보여주고 메뉴를 주 행동으로 둔다. 공식 r2.1의 카피·보존·동의·복원 경계를 소비한 작성자 제안이다.

보완 요청: [해당 critique](critiques/R2_EXAMPLE_DONE-critique.md) 소유자가 최신 도면·문서를 재검토한다. 기존 critique와 수정 전 점수는 변경하지 않았다. 정적 보완은 author 작업이며 self-pass를 부여하지 않는다. 실서비스 검증·독립 Stage/authority가 완료될 때까지 draft/pending이다.
...
- [recording 320x568 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/recording-320x568-EXAMPLE_DONE.png)
- [recording 390x844 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/recording-390x844-EXAMPLE_DONE.png)
- [homeflow 320x568 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/homeflow-320x568-EXAMPLE_DONE.png)
- [homeflow 390x844 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/homeflow-390x844-EXAMPLE_DONE.png)
