...
| 주제 | 공식 화면 ID | 문서 |
| --- | --- | --- |
| recording | `R2_RECORDING_RECOVERY` | 이 문서 |
| homeflow | `R2_HOMEFLOW_RECOVERY` | 이 문서 |
...
이메일과 동의 입력은 이 탭을 열어 둔 동안만 유지돼요. 새로고침하면 지워져요.
[다시 시도]
[메뉴로 돌아가기]
```
...
별도 오류 페이지가 아닌 원 화면의 저장 영역 inline 상태다. 정적 대표 도면은 LEAD 입력·동의를 유지한 오류다. 명확한 저장 실패는 '! 아직 저장되지 않았어요.', 응답 유실은 '접수 여부를 확인하지 못했어요'/'저장 여부를 확인하지 못했어요'로 구분한다. 복구 중 메뉴·예시·질문은 계속 열린다. 재시도 성공은 원 활동의 DONE으로 이동하며 메뉴로 자동 이동하지 않는다. 반복 실패도 입력·다른 완료를 지우지 않는다. 입력 오류는 해당 필드, 보안 만료는 보안 영역에서 처리한다. 예시 실패는 마지막 장면과 읽음, 설문 실패는 선택을 유지하며 원인이 다르면 일반 네트워크 오류로 덮지 않는다.
...
각 도면에서 `R2_RECORDING_RECOVERY` / `R2_HOMEFLOW_RECOVERY` label을 찾는다. 4열×2행, 첫 행 MENU/LEAD_DONE/EXAMPLE_DONE/SURVEY_DONE, 다음 행 EXAMPLE/SURVEY/LEAD/RECOVERY다. 셀 폭은 320/390 CSSpx이며 **높이는 전체 내용에 맞춘 정적 도면**이다. 긴 동의·질문을 844px 안에 축소하지 않는다. 화면·문서 카피는 [공통 시각 내용 JSON](evidence/marketing-demand-validation-round2/R2_design-content.json)의 같은 visibleCopy를 사용한다. 이는 API 스키마가 아니다.
