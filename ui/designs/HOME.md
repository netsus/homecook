...
│  최대 2줄              │
│                       │
│  [조회 1.2k] [저장 32]│  ← compact muted stat row
└───────────────────────┘
...
- **인분 배치**: HOME 카드에서는 숨기고, 상세와 플래너 등록 흐름에서 확인한다.
- **통계 배치**: 카드 하단 muted row (`조회 / 저장`)
...
6. **통계 아이콘 텍스트 표기**: HOME 카드는 조회수(👁)/저장(🔖) 순서로 배치한다. 좋아요와 기본 인분은 상세 화면에서 확인한다.
