...
| 주제 | 공식 화면 ID | 문서 |
| --- | --- | --- |
| recording | `R2_RECORDING_LEAD` | 이 문서 |
| homeflow | `R2_HOMEFLOW_LEAD` | 이 문서 |
...
[이메일 주소 입력]
개인정보 수집·이용 동의 (필수)
무먹 베타 오픈 알림을 이메일로 받기 위해 이메일 주소와 신청 주제·동의 기록을 수집·이용하는 데 동의해요. 보관 기간은 2026년 11월 30일까지이며, 철회하면 해당 정보를 삭제해요.
동의하지 않아도 예시와 의견 남기기를 이용할 수 있어요
만 14세 이상인 경우에만 신청해 주세요
[개인정보처리방침]
[보안 확인: 실제 검증은 Stage 4]
[베타 오픈 알림 신청하기]
[메뉴로 돌아가기]
```
...
다른 활동 선행 없이 이메일·미체크 목적 동의·privacy·만 14세 안내·보안 확인을 읽고 신청한다. 이메일 type=email/autocomplete=email, 자동 대문자·맞춤법 교정 없음. 입력 최대 254자, r2.1 ASCII 이메일 규칙을 소비하며 서버가 정규화·중복을 판단한다. UTF-8 국제화/quoted local-part는 지원하지 않는다. 미선택 동의·이메일·보안 오류는 해당 필드에 연결한다. 같은 참여 접수를 확인하면 입력을 다시 요구하지 않고 LEAD_DONE으로 이동한다. 신규/기존 이메일 모두 같은 접수 화면이며 이메일 존재 여부를 공개 조회하지 않는다. 실패는 폼 주변 RECOVERY, 응답 유실은 접수 미확인이다.

## 승인 동의·보관·보안 조건

정확한 동의 label: **무먹 베타 오픈 알림을 이메일로 받기 위해 이메일 주소와 신청 주제·동의 기록을 수집·이용하는 데 동의해요. 보관 기간은 2026년 11월 30일까지이며, 철회하면 해당 정보를 삭제해요.**

필수 안내: '동의하지 않아도 예시와 의견 남기기를 이용할 수 있어요', '만 14세 이상인 경우에만 신청해 주세요'. 동의는 미체크로 시작하고 privacy 링크와 checkbox 동작을 분리한다. 목적은 해당 주제 베타 오픈 알림 1회와 접수 관리다. 동의 버전 `mumeok-r2-beta-notice-20260911`, 목적 `beta_open_notice`를 소비하며 일반 광고·뉴스레터·계정 가입으로 확장하지 않는다.

표시 보관일은 **2026년 11월 30일까지**, 기술적 종료 경계는 `2026-12-01T00:00:00+09:00` exclusive다. 캠페인 종료는 `2026-11-01T00:00:00+09:00` exclusive로 구분한다. 공개 privacy·위탁 사실 반영, 독립 개인정보 검토와 실제 lead readiness/Turnstile 검증은 운영 활성화 전제이며 정적 그림의 체크박스/보안 영역이 이를 충족하지 않는다. Turnstile 지원 모드는 후속 구현에서 실제 288px 내용 폭에 맞게 검증하고 그림을 축소해 통과시키지 않는다.
...
각 도면에서 `R2_RECORDING_LEAD` / `R2_HOMEFLOW_LEAD` label을 찾는다. 4열×2행, 첫 행 MENU/LEAD_DONE/EXAMPLE_DONE/SURVEY_DONE, 다음 행 EXAMPLE/SURVEY/LEAD/RECOVERY다. 셀 폭은 320/390 CSSpx이며 **높이는 전체 내용에 맞춘 정적 도면**이다. 긴 동의·질문을 844px 안에 축소하지 않는다. 화면·문서 카피는 [공통 시각 내용 JSON](evidence/marketing-demand-validation-round2/R2_design-content.json)의 같은 visibleCopy를 사용한다. 이는 API 스키마가 아니다.
...
보완 요청: [해당 critique](critiques/R2_LEAD-critique.md) 소유자가 최신 도면·문서를 재검토한다. 기존 critique와 수정 전 점수는 변경하지 않았다. 정적 보완은 author 작업이며 self-pass를 부여하지 않는다. 실서비스 검증·독립 Stage/authority가 완료될 때까지 draft/pending이다.
...
- [recording 320x568 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/recording-320x568-LEAD.png)
- [recording 390x844 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/recording-390x844-LEAD.png)
- [homeflow 320x568 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/homeflow-320x568-LEAD.png)
- [homeflow 390x844 production build 로컬 미리보기](evidence/marketing-demand-validation-round2/stage4/production-preview/homeflow-390x844-LEAD.png)

실제 위젯은 150×140 compact를 사용한다. preview는 widget/실제 신청 없이 preview@example.com만 사용하며, 실제 provider 성공과 실기기 키보드는 Manual Only다. 스크립트 오류 뒤 명시 보안 확인 재시도를 제공한다.
