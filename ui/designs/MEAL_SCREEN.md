...
## Prepared Food Standard Basis UX Addendum

> 추가일: 2026-07-19 KST
> workpack: `prepared-food-standard-basis-ux`
> 상태: Stage 1 temporary design contract / independent critique·implementation·authority pending

### Quantity Edit Semantics

- 이 addendum은 새 card, endpoint, 영양 환산을 만들지 않는다. 기존 `완제품 수량 변경` modal의 number input 의미만 picker와 맞춘다.
- 현재 선택 단위가 `g` 또는 `ml`이면 browser step은 `1`이다. 사용자는 100에서 시작한 수량을 1g/1mL씩 조절할 수 있다.
- 현재 선택 단위가 `serving` 또는 `package`이면 기존 자유 양수 입력을 유지한다. 이를 100g/100mL로 추정하거나 강제하지 않는다.
- unit select가 바뀌면 같은 input의 step 의미도 즉시 바뀐다. 값과 modal context는 보존한다.
- server의 exactly-one direct relation 검증과 `422 NUTRITION_BASIS_MISMATCH`가 최종 authority다. UI는 chained relation, 이름·브랜드·밀도, 임의 g↔mL를 추정하지 않는다.

### Layout / States

- 390px: 기존 2열 `수량 | 단위` 구조와 44px control 높이를 유지한다.
- 320px: number/select가 page-level horizontal overflow를 만들지 않고 error 문구가 modal 안에서 줄바꿈된다.
- 1280px: modal 폭·focus trap·ESC/닫기 후 product card focus 복원 동작을 유지한다.
- error: mismatch 시 modal, 입력량, 선택 단위, 배경 플래너 context를 유지한다.
- read-only/pin: current product를 다시 조회하거나 repin하지 않고 entry가 가진 version/relations만 사용한다.

### Evidence / Authority

- before/after: `ui/designs/evidence/prepared-food-standard-basis-ux/{before,after}/{390,320,1280}/`
- 각 viewport에서 public g, public ml, shared manual, legacy serving/package no-relation 중 필요한 sentinel을 확인한다.
- predecessor screenshot은 역사적 비교 자료일 뿐 새 exact implementation head authority를 대체하지 않는다.
- 최종 report: `ui/designs/authority/PLANNER_WEEK-prepared-food-standard-basis-ux-authority.md`.
