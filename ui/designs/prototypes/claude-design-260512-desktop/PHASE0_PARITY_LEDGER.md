...
| `screen:MENU_ADD` | stack `MENU_ADD`, `screens-3.jsx::MenuAddScreen` | `MenuAddScreen`, `DesktopMenuAddScreen` | Phase 4 | `verified` | Evidence: `menu-add-1024.png`, `menu-add-1280.png`, `menu-add-1440.png`, and `visual-qa-report.json`; all six entry cards route to real picker/create/import screens, including webpage import. |
...
| `screen:COOK_READY_LIST` | stack `COOK_READY_LIST`, `screens-3.jsx::CookReadyListScreen` | `CookListScreen`, `DesktopCookListScreen` | Phase 7 | `verified` | Evidence: `cook-ready-list-1024.png`, `cook-ready-list-1280.png`, `cook-ready-empty-1280.png`, and `visual-qa-report.json`; grouped cook-ready list, empty state, notice entry, and registered/shopped meal `요리 시작` activation verified. |
| `screen:COOK_MODE_PLANNER` | stack `COOK_MODE_PLANNER`, `screens-3.jsx::CookModePlannerScreen` | `CookRunScreen`, `DesktopCookRunScreen` | Phase 7 | `verified` | Evidence: `cook-mode-planner-1024.png`, `cook-mode-planner-1280.png`, and `visual-qa-report.json`; planner-context header, no serving adjustment UI, inline pantry deduction rail, registered/shopped cook completion path, and cooked-meal direct-entry guard verified. |
...
| `screen:WEB_IMPORT` | stack `WEB_IMPORT`, `screens-3.jsx::WebImportScreen` | `WebImportScreen`, `DesktopWebImportScreen` | Phase 4 follow-up | `verified` | Evidence: follow-up Playwright traversal; URL input, simulated webpage extraction review, editable ingredients, and planner handoff verified. |
...
| `surface:COOK_MODE::CookIngredientChecklist` | `screens-3.jsx::CookIngredientChecklist` | `ConsumedIngredientSheet`, `DesktopConsumedIngredientDialog` | Phase 7 | `verified` | Evidence: `cook-mode-planner-1280.png`, `cook-mode-standalone-1280.png`, and `visual-qa-report.json`; reachable inline pantry deduction rail, pantry-held preselection, unavailable ingredient disabled state, deduct-none completion, and selected deduction completion verified. |
...
| Cooking mode rules | Phase 7 | Phase 7 verified: no serving adjustment UI in cook mode; planner mode has meal status context and starts from `registered` or `shopped` for app parity; `cooked` direct entry is blocked; standalone mode has no meal status mutation path. |
