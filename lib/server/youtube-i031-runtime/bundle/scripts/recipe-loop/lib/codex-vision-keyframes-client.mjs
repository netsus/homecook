...
import {
  SCREEN_OCR_POLICY_VERSION,
  SCREEN_OCR_TIMELINE_VERSION,
  assessScreenOcrEvidence,
  classifyScreenOcrNeed,
  createMacOSVisionBatchRecognizer,
  formatScreenOcrTimelineForPrompt,
  runScreenOcrScout,
  screenOcrTimelineHash,
  selectScreenOcrRepresentativeFrames,
} from "./screen-ocr-scout.mjs";
...
const CLIENT_VERSION = "codex-vision-keyframes-client-v9-single-source-dedup";
const SINGLE_CLIENT_VERSION = "codex-vision-keyframes-client-v20-structured-final";

export const SINGLE_RECIPE_OUTPUT_SCHEMA = {
  type: "object",
        ...
        properties: {
          ...
          ingredients: {
            ...
            items: {
              type: "object",
              additionalProperties: false,
              required: ["name", "nameAliases", "amount", "unit", "amountBasis", "optional", "groupLabel"],
              properties: {
                name: { type: "string" },
                nameAliases: { type: "array", items: { type: "string" } },
                amount: { type: ["string", "null"] },
                unit: { type: ["string", "null"] },
                amountBasis: { type: ["string", "null"] },
...
const SINGLE_SCREEN_OCR_INTEGRATION_VERSION = "single-screen-ocr-integration-v2-additive-timeline";
const SELECTOR_PROMPT_VERSION = "keyframe-selector-v5-single-visual-annotator";
const SINGLE_SELECTOR_PROMPT_VERSION = "keyframe-selector-v6-single-compact-json";
const SINGLE_CANDIDATE_SAMPLER_VERSION = "single-candidate-sampler-v1-transcript-cue-bins";
const SINGLE_ANNOTATION_SUBSET_VERSION = "single-visual-annotation-subset-v2-candidate-index-output6";
const SINGLE_ANNOTATION_OUTPUT_LIMIT = 6;
const SINGLE_ACTION_CHECKLIST_VERSION = "single-action-checklist-v3-event-signature-merge";
const SINGLE_ACTION_CHECKLIST_LIMIT = 10;
const SINGLE_ACTION_CHECKLIST_TEXT_CHAR_LIMIT = 120;
const SINGLE_ACTION_CHECKLIST_SECTION_CHAR_LIMIT = 2400;
const SINGLE_ACTION_CHECKLIST_SECTION_BYTE_LIMIT = 7200;
const SINGLE_ATOMIC_STEP_AUDIT_VERSION = "single-atomic-step-audit-v4-explicit-action-clause";
const SINGLE_ATOMIC_STEP_AUDIT_CHAR_LIMIT = 1200;
const SINGLE_ATOMIC_STEP_AUDIT_BYTE_LIMIT = 3600;
const SINGLE_COMPACT_JSON_SERIALIZATION_VERSION = "single-compact-json-serialization-v2-two-phase";
const SINGLE_COMPACT_JSON_SERIALIZATION_CHAR_LIMIT = 240;
const SINGLE_COMPACT_JSON_SERIALIZATION_BYTE_LIMIT = 720;
const SINGLE_SELECTOR_COMPACT_JSON_SERIALIZATION_VERSION = "single-selector-compact-json-serialization-v1-two-phase";
const SINGLE_SELECTOR_COMPACT_JSON_SERIALIZATION_CHAR_LIMIT = 260;
const SINGLE_SELECTOR_COMPACT_JSON_SERIALIZATION_BYTE_LIMIT = 780;
...
const FINAL_PROMPT_VERSION = "keyframe-final-v36-single-bidirectional-coverage";
const SINGLE_FINAL_PROMPT_VERSION = "keyframe-final-v44-explicit-action-clause";
...
const SINGLE_CANDIDATE_AMOUNT_RE = /(?:\d+(?:[./]\d+)?|반)\s*(?:큰술|작은술|숟가락|스푼|컵|봉|팩|줌|개|장|줄|대|마리|알|g|kg|ml|l|L|T|t|그램|킬로|리터)/iu;
const ONSCREEN_AMOUNT_UNIT_RE = /(?:약\s*)?(\d+(?:\.\d+)?(?:\s*[~\-]\s*\d+(?:\.\d+)?)?|\d+\s*\/\s*\d+|반|한|하나|두|둘|세|셋|네|넷|다섯|여섯)\s*(큰\s*술|큰\s*스푼|큰\s*숟갈|큰\s*숟가락|밥\s*숟갈|밥\s*숟가락|작은\s*술|작은\s*스푼|작은\s*숟갈|작은\s*숟가락|티\s*스푼|숟갈|숟가락|스푼|컵|g|그램|kg|킬로그램|ml|밀리리터|l|리터|개|알|쪽|장|줄|팩|봉|줌|꼬집|모|인분|덩이|대|포기|송이|토막|조각|캔|통|병|공기|마리|꼬치|잎)/giu;
const ONSCREEN_KOREAN_AMOUNTS = new Map([
  ["반", "1/2"], ["한", "1"], ["하나", "1"], ["두", "2"], ["둘", "2"],
  ["세", "3"], ["셋", "3"], ["네", "4"], ["넷", "4"], ["다섯", "5"], ["여섯", "6"],
]);
const ONSCREEN_UNIT_ALIASES = new Map([
  ["큰스푼", "큰술"], ["큰숟갈", "큰술"], ["큰숟가락", "큰술"], ["밥숟갈", "큰술"],
  ["밥숟가락", "큰술"], ["숟갈", "큰술"], ["숟가락", "큰술"], ["스푼", "큰술"],
  ["작은스푼", "작은술"], ["작은숟갈", "작은술"], ["작은숟가락", "작은술"], ["티스푼", "작은술"],
  ["그램", "g"], ["킬로그램", "kg"], ["밀리리터", "ml"], ["리터", "l"],
]);
const SINGLE_CANDIDATE_ACTION_RE = /(?:썰(?:어|고|기|면|어서|어주|어\s*놓)|썬(?:다|뒤|후|것)|다지(?:고|기|면|세요|다가)|다져(?:\s|서|놓|주|요|야)|자르(?:고|기|면|세요|거나)|잘라(?:서|주|놓|요|야)|손질(?:하|해|해서|하고|한다)|씻(?:고|어|는|기|으|어서)|헹구(?:고|어|는|기|세요)|불리(?:고|어|는|기|면)|절이(?:고|어|는|기|면)|재우(?:고|어|는|기|면)|섞(?:고|어|는|기|으|는다)|버무리(?:고|어|는|기|면)|반죽(?:하|해|해서|하고|한다)|갈(?:아|고|기|면)|으깨(?:고|서|는|기|면)|체에\s*(?:치|거르)|거르(?:고|는|기|면|세요)|두르(?:고|는|기|면|세요)|둘러(?:서|주|요)|붓(?:고|는|기|는다)|부어(?:서|주|요)|넣(?:고|어|는|기|으|는다)|담(?:고|아|는|기|으|는다)|얹(?:고|어|는|기|는다)|올리(?:고|는|기|면|세요)|뿌리(?:고|는|기|면|세요)|바르(?:고|는|기|면|세요)|볶(?:고|아|는|기|으|는다)|굽(?:고|는|기|으|는다)|구워(?:서|주|요)|끓(?:이|여|고|는|기|으|인다)|삶(?:고|아|는|기|으|는다)|튀기(?:고|어|는|기|면)|익히(?:고|는|기|면|세요)|찌(?:고|는|기|면|세요|어)|졸이(?:고|는|기|면|세요)|졸여(?:서|주|요)|데치(?:고|는|기|면|세요)|식히(?:고|는|기|면|세요)|숙성(?:하|해|해서|시키|한다)|보관(?:하|해|해서|한다)|간하(?:고|는|기|면)|간을\s*맞(?:추|춰))/u;
const SINGLE_CANDIDATE_USAGE_PURPOSE_RE = /(?:(?:조미(?:료)?|양념|간|향|맛|색)\s*(?:(?:을|를|이|은|는|의|만)\s*)?(?:역할|목적|용도|조절|맞추|내기|더하|살리)|(?:취향|선택)\s*(?:에|대로|해서|하여|적으로|에\s*따라))/u;
const SINGLE_CANDIDATE_NOISE_RE = /(?:https?:\/\/|www\.|(?:^|[^\p{L}\p{N}])(?:구독|좋아요|댓글|이벤트|공지|협찬|광고|할인|쿠폰|구매|판매|배송|링크|인스타|instagram|email|이메일|BGM|music|음악|카메라|촬영장비))/iu;
const SINGLE_TRANSCRIPT_BLOCK_RE = /^transcript\([^\r\n)]*\)$/u;
const SINGLE_TRANSCRIPT_PREFIX_RE = /^\s*\[([^\]]+)\]\s+(.+?)\s*$/u;
const SINGLE_TRANSCRIPT_CLOCK_RE = /^(?:(\d{1,2}):)?(\d+):(\d{2})(?:\.(\d{1,3}))?$/u;
...
function normalizedSourceBlockText(value) {
  return String(value ?? "").replace(/\r\n?/g, "\n").trim();
}

function hasBoundedTextBlock(text, label, body) {
  const needle = `${label}\n${body}`;
  let offset = 0;
  while (offset <= text.length - needle.length) {
    ...
    const startsAtLineBoundary = index === 0 || text[index - 1] === "\n";
    const endsAtBlockBoundary = end === text.length || text.slice(end, end + 2) === "\n\n";
    if (startsAtLineBoundary && endsAtBlockBoundary) return true;
    offset = index + 1;
  }
  return false;
}

function promptHasExplicitSourceBlock(prompt, sourceText) {
  const normalizedPrompt = normalizedSourceBlockText(prompt);
  const normalizedSource = normalizedSourceBlockText(sourceText);
  if (!normalizedSource) {
    return hasBoundedTextBlock(normalizedPrompt, "텍스트 소스:", "(텍스트 소스 없음)");
  }
  if (!/^\[SOURCE:\s*[^\]\n]+\]/mu.test(normalizedSource)) return false;
  return hasBoundedTextBlock(normalizedPrompt, "텍스트 소스:", normalizedSource);
}

function singleSourceAppendLines(prompt, sourceText) {
  if (promptHasExplicitSourceBlock(prompt, sourceText)) return [];
  const normalizedSource = normalizedSourceBlockText(sourceText);
  return [
    "",
    "텍스트 소스:",
    normalizedSource || "(텍스트 소스 없음)",
  ];
}
...
function normalizedSingleCandidateFrames(frames) {
  const valid = [];
  const seen = new Set();
  ...
  for (const frame of Array.isArray(frames) ? frames : []) {
    const timestampSec = frameTimestampSec(frame);
    const framePath = String(frame?.path ?? "").trim();
    if (!Number.isFinite(timestampSec) || timestampSec < 0 || !framePath) {
      invalidFrameCount += 1;
      continue;
    }
    if (seen.has(framePath)) {
      duplicateFrameCount += 1;
      continue;
    }
    seen.add(framePath);
    valid.push({ frame, timestampSec, framePath });
  }

  valid.sort((left, right) => (
    left.timestampSec - right.timestampSec
    || (Number.isFinite(Number(left.frame?.index)) ? Number(left.frame.index) : Number.MAX_SAFE_INTEGER)
      - (Number.isFinite(Number(right.frame?.index)) ? Number(right.frame.index) : Number.MAX_SAFE_INTEGER)
    || left.framePath.localeCompare(right.framePath)
  ));
  return { valid, invalidFrameCount, duplicateFrameCount };
}

function parseSingleTranscriptLine(line) {
  const prefix = String(line ?? "").match(SINGLE_TRANSCRIPT_PREFIX_RE);
  if (!prefix) return { status: "none" };
  const clock = prefix[1].match(SINGLE_TRANSCRIPT_CLOCK_RE);
  if (!clock) return { status: "invalid" };
  const hasHours = clock[1] !== undefined;
  const hours = Number(clock[1] ?? 0);
  const minutes = Number(clock[2]);
  const seconds = Number(clock[3]);
  const fraction = Number(`0.${clock[4] ?? "0"}`);
  if (![hours, minutes, seconds, fraction].every(Number.isFinite)) return { status: "invalid" };
  if (hours < 0 || minutes < 0 || seconds < 0 || seconds >= 60 || (hasHours && minutes >= 60)) {
    return { status: "invalid" };
  }
  return {
    status: "valid",
    timestampSec: hours * 3600 + minutes * 60 + seconds + fraction,
    text: compact(prefix[2]),
  };
}

function singleTranscriptTemporalCues(sourceText, durationSec) {
  const cues = [];
  const counts = {
    parsedCueCount: 0,
    eligibleCueCount: 0,
    noiseCueCount: 0,
    neutralCueCount: 0,
    outOfRangeCueCount: 0,
    invalidCueCount: 0,
  };
  let sourceLineIndex = 0;

  for (const [sourceBlockIndex, block] of sourceBlocks(sourceText).entries()) {
    if (!SINGLE_TRANSCRIPT_BLOCK_RE.test(block.name)) continue;
    for (const [blockLineIndex, line] of block.lines.entries()) {
      const lineIndex = sourceLineIndex;
      sourceLineIndex += 1;
      const parsed = parseSingleTranscriptLine(line);
      if (parsed.status === "none") continue;
      counts.parsedCueCount += 1;
      if (parsed.status !== "valid") {
        counts.invalidCueCount += 1;
        continue;
      }
      const { timestampSec, text } = parsed;
      if (timestampSec > durationSec) {
        counts.outOfRangeCueCount += 1;
        continue;
      }
      if (SINGLE_CANDIDATE_NOISE_RE.test(text)) {
        counts.noiseCueCount += 1;
        continue;
      }
      const hasQuantity = SINGLE_CANDIDATE_AMOUNT_RE.test(text);
      const hasAction = SINGLE_CANDIDATE_ACTION_RE.test(text);
      const hasUsagePurpose = SINGLE_CANDIDATE_USAGE_PURPOSE_RE.test(text);
      if (!hasQuantity && !hasAction && !hasUsagePurpose) {
        counts.neutralCueCount += 1;
        continue;
      }
      const cueClass = hasQuantity && hasAction
        ? "quantity+action"
        : (hasQuantity ? "quantity" : (hasAction ? "action" : "usage-purpose"));
      cues.push({
        timestampSec,
        text,
        cueClass,
        score: hasQuantity && hasAction ? 3 : (hasQuantity ? 2 : (hasAction ? 1 : 0)),
        sourceBlockIndex,
        blockLineIndex,
        sourceLineIndex: lineIndex,
      });
      counts.eligibleCueCount += 1;
    }
  }
...
function singleActionCueComparator(left, right) {
  return left.timestampSec - right.timestampSec
    || left.sourceBlockIndex - right.sourceBlockIndex
    || left.blockLineIndex - right.blockLineIndex
    || left.sourceLineIndex - right.sourceLineIndex;
}

function resolvedSingleActionChecklistDuration(effectiveDurationSec, frameTimestamps) {
  const hasExplicitDuration = effectiveDurationSec !== null
    && effectiveDurationSec !== undefined
    && effectiveDurationSec !== "";
  const explicit = Number(effectiveDurationSec);
  if (hasExplicitDuration && Number.isFinite(explicit) && explicit >= 0) return explicit;
  const fallback = (Array.isArray(frameTimestamps) ? frameTimestamps : [])
    .map(Number)
...
function clipSingleActionChecklistText(value, charLimit = SINGLE_ACTION_CHECKLIST_TEXT_CHAR_LIMIT) {
  const characters = Array.from(compact(value));
  const safeLimit = Number.isFinite(Number(charLimit))
    ? Math.max(0, Math.floor(Number(charLimit)))
    : SINGLE_ACTION_CHECKLIST_TEXT_CHAR_LIMIT;
  ...
  if (safeLimit < separatorLength + 2) {
    return { text: characters.slice(0, safeLimit).join(""), truncated: true };
  }
  const available = safeLimit - separatorLength;
  const headLength = Math.ceil(available / 2);
  const tailLength = Math.floor(available / 2);
  return {
    text: `${characters.slice(0, headLength).join("").trimEnd()}${separator}${characters.slice(-tailLength).join("").trimStart()}`,
    truncated: true,
  };
}

function selectSingleActionChecklistCues(cues) {
  if (cues.length <= SINGLE_ACTION_CHECKLIST_LIMIT) return cues;
  const selected = [];
  const used = new Set();
  const firstTimestamp = cues[0].timestampSec;
  const lastTimestamp = cues.at(-1).timestampSec;
  for (let index = 0; index < SINGLE_ACTION_CHECKLIST_LIMIT; index += 1) {
    const targetTimestamp = firstTimestamp
      + ((lastTimestamp - firstTimestamp) * index) / (SINGLE_ACTION_CHECKLIST_LIMIT - 1);
    const choice = cues
      .map((cue, cueIndex) => ({ cue, cueIndex }))
      .filter(({ cueIndex }) => !used.has(cueIndex))
      .sort((left, right) => (
        Math.abs(left.cue.timestampSec - targetTimestamp) - Math.abs(right.cue.timestampSec - targetTimestamp)
        || singleActionCueComparator(left.cue, right.cue)
      ))[0];
    if (!choice) break;
    used.add(choice.cueIndex);
    selected.push(choice.cue);
  }
  return selected.sort(singleActionCueComparator);
}

function singleActionChecklistSectionText(checklist) {
  if (checklist.items.length === 0) return "";
  return [
    `[SOURCE-BACKED ACTION CHECKLIST version=${checklist.version} contentHash=${checklist.contentHash}]`,
    "이 목록은 정답이나 완전한 목록이 아닌 non-exhaustive attention aid다. 전체 source와 frame이 최종 authority다.",
    "각 안정 ID를 keep / merge / reject로 내부 판정하되 판정표 자체는 출력하지 않는다.",
    "keep은 인접한 full transcript 문맥에서 target recipe의 직접 action으로 확인한다. 서로 독립인 confirmed event만 별도 step으로 매핑하고, 같은 event를 반복 확인한 ID는 corroboration으로만 쓴다.",
    "merge는 source가 같은 occurrence의 같은 primary action과 같은 explicit modifier를 여러 대상에 직접 연결한 경우에만 허용한다. 같은 phase·목적이라는 이유만으로 다른 action·선후·modifier를 병합하지 않는다.",
    "reject는 잡담·광고·비교·다른 variant·standalone 거부/부정 대안·일반 추천과 corroboration 없는 noisy/uncertain ASR에 적용한다. confirmed action에 직접 붙은 금지/선택 modifier는 reject하지 않는다.",
    "대상이 다르다는 사실이나 시점 차이만으로 분리하지 않는다. 별도 투입·상태 전환·명시 순서처럼 독립 occurrence의 직접 근거가 있으면 source 순서대로 분리한다.",
    "부정 설명과 별도의 긍정 action이 한 줄에 있으면 줄 전체를 reject하지 않는다. truncated=true는 full transcript를 다시 확인한다.",
    "checklist가 비어 있거나 항목에 없는 것은 행동 부재의 negative evidence가 아니다.",
    ...checklist.items.map((item) => (
      `- ${item.id}: timestampSec=${item.timestampSec}, truncated=${item.truncated}; text=${JSON.stringify(item.text)}`
    )),
    "[/SOURCE-BACKED ACTION CHECKLIST]",
  ].join("\n");
}

function singleActionChecklistSectionMetrics(checklist) {
  const section = singleActionChecklistSectionText(checklist);
  return {
    section,
    charCount: Array.from(section).length,
    byteCount: Buffer.byteLength(section, "utf8"),
  };
}

function singleActionChecklistItemsWithinSectionLimits(items) {
  const { charCount, byteCount } = singleActionChecklistSectionMetrics({
    version: SINGLE_ACTION_CHECKLIST_VERSION,
    contentHash: "0".repeat(24),
    items,
  });
  return charCount <= SINGLE_ACTION_CHECKLIST_SECTION_CHAR_LIMIT
    && byteCount <= SINGLE_ACTION_CHECKLIST_SECTION_BYTE_LIMIT;
}

function fitSingleActionChecklistItemsToSection(rawItems) {
  let low = 0;
  let high = SINGLE_ACTION_CHECKLIST_TEXT_CHAR_LIMIT;
  let best = null;
  while (low <= high) {
    const charLimit = Math.floor((low + high) / 2);
    const items = rawItems.map((item) => ({
      id: item.id,
      timestampSec: item.timestampSec,
      ...clipSingleActionChecklistText(item.rawText, charLimit),
    }));
    if (singleActionChecklistItemsWithinSectionLimits(items)) {
      best = items;
      low = charLimit + 1;
    } else {
      high = charLimit - 1;
  ...
  if (best) return best;
  throw new Error("SINGLE_ACTION_CHECKLIST_SECTION_BASELINE_LIMIT");
}

function renderSingleActionChecklistSection(checklist) {
  const { section, charCount, byteCount } = singleActionChecklistSectionMetrics(checklist);
  if (
    charCount > SINGLE_ACTION_CHECKLIST_SECTION_CHAR_LIMIT
    || byteCount > SINGLE_ACTION_CHECKLIST_SECTION_BYTE_LIMIT
  ) {
    throw new Error(
      `SINGLE_ACTION_CHECKLIST_SECTION_LIMIT: ${charCount} chars / ${byteCount} bytes`,
    );
  }
  return section;
}

export function buildSingleTranscriptActionChecklist({
  sourceText,
  effectiveDurationSec = null,
  frameTimestamps = [],
}) {
  const durationSec = resolvedSingleActionChecklistDuration(effectiveDurationSec, frameTimestamps);
  const eligibleCues = durationSec === null
    ? []
    : singleTranscriptTemporalCues(sourceText, durationSec).cues
      .filter((cue) => cue.cueClass === "action" || cue.cueClass === "quantity+action")
      .sort(singleActionCueComparator);
  const seen = new Set();
  const dedupedCues = eligibleCues.filter((cue) => {
    const key = `${cue.timestampSec}\0${compact(cue.text)}`;
    if (seen.has(key)) return false;
    seen.add(key);
    return true;
  });
  const selectedCues = selectSingleActionChecklistCues(dedupedCues);
  const items = fitSingleActionChecklistItemsToSection(selectedCues.map((cue, index) => ({
      id: `A${String(index + 1).padStart(2, "0")}`,
      timestampSec: roundedMetric(cue.timestampSec),
      rawText: cue.text,
    })));
  const contentHash = hashText(JSON.stringify({
    version: SINGLE_ACTION_CHECKLIST_VERSION,
    limit: SINGLE_ACTION_CHECKLIST_LIMIT,
    textCharLimit: SINGLE_ACTION_CHECKLIST_TEXT_CHAR_LIMIT,
    items,
  }));
  const checklist = {
    version: SINGLE_ACTION_CHECKLIST_VERSION,
    limit: SINGLE_ACTION_CHECKLIST_LIMIT,
    textCharLimit: SINGLE_ACTION_CHECKLIST_TEXT_CHAR_LIMIT,
    sectionCharLimit: SINGLE_ACTION_CHECKLIST_SECTION_CHAR_LIMIT,
    sectionByteLimit: SINGLE_ACTION_CHECKLIST_SECTION_BYTE_LIMIT,
    effectiveDurationSec: roundedMetric(durationSec),
    count: items.length,
    contentHash,
    items,
  };
  const renderedSection = renderSingleActionChecklistSection(checklist);
  return {
    ...checklist,
    renderedSection,
    renderedSectionChars: Array.from(renderedSection).length,
    renderedSectionBytes: Buffer.byteLength(renderedSection, "utf8"),
  };
}

function closestUnusedSingleFrame(validFrames, selectedPaths, targetSec) {
  return validFrames
    .filter((entry) => !selectedPaths.has(entry.framePath))
    .sort((left, right) => (
      Math.abs(left.timestampSec - targetSec) - Math.abs(right.timestampSec - targetSec)
      || left.timestampSec - right.timestampSec
      || (Number.isFinite(Number(left.frame?.index)) ? Number(left.frame.index) : Number.MAX_SAFE_INTEGER)
        - (Number.isFinite(Number(right.frame?.index)) ? Number(right.frame.index) : Number.MAX_SAFE_INTEGER)
      || left.framePath.localeCompare(right.framePath)
...
export function selectSingleRecipeCandidateFrames({ frames, sourceText, limit, durationSec = null }) {
  const {
    valid: validFrames,
    invalidFrameCount,
    duplicateFrameCount,
  } = normalizedSingleCandidateFrames(frames);
  const safeLimit = Number.isFinite(Number(limit)) ? Math.max(0, Math.floor(Number(limit))) : 0;
  const targetCount = Math.min(safeLimit, validFrames.length);
  const maxFrameTimestamp = validFrames.reduce((max, entry) => Math.max(max, entry.timestampSec), 0);
  const explicitDuration = durationSec === null || durationSec === undefined || durationSec === ""
    ? null
    : Number(durationSec);
  const effectiveDurationSec = Number.isFinite(explicitDuration) && explicitDuration > 0
    ? explicitDuration
    : maxFrameTimestamp;
  const { cues, counts } = singleTranscriptTemporalCues(sourceText, effectiveDurationSec);
  const selectedPaths = new Set();
  const selected = [];
  const bins = [];

  for (let binIndex = 0; binIndex < targetCount; binIndex += 1) {
    const startSec = targetCount > 0 ? (effectiveDurationSec * binIndex) / targetCount : 0;
    const endSec = targetCount > 0 ? (effectiveDurationSec * (binIndex + 1)) / targetCount : effectiveDurationSec;
    const centerSec = (startSec + endSec) / 2;
    const isLast = binIndex === targetCount - 1;
    const binCues = cues
      .filter((cue) => cue.timestampSec >= startSec && (isLast ? cue.timestampSec <= endSec : cue.timestampSec < endSec))
      .sort((left, right) => (
        right.score - left.score
        || Math.abs(left.timestampSec - centerSec) - Math.abs(right.timestampSec - centerSec)
        || left.timestampSec - right.timestampSec
        || left.sourceLineIndex - right.sourceLineIndex
      ));
    const cue = binCues[0] ?? null;
    const targetSec = cue?.timestampSec ?? centerSec;
    const chosen = closestUnusedSingleFrame(validFrames, selectedPaths, targetSec);
    if (!chosen) continue;
    selectedPaths.add(chosen.framePath);
    const targetKind = cue ? "cue" : "center";
    const fallbackReason = cue
      ? null
      : (cues.length === 0 ? "no-eligible-transcript-cues" : "no-eligible-cue-in-bin");
    const candidateSampling = {
      binIndex,
      startSec: roundedMetric(startSec),
      endSec: roundedMetric(endSec),
      targetKind,
      cueClass: cue?.cueClass ?? null,
      cueTimestampSec: cue ? roundedMetric(cue.timestampSec) : null,
      frameTimestampSec: roundedMetric(chosen.timestampSec),
      deltaSec: roundedMetric(Math.abs(chosen.timestampSec - targetSec)),
      fallbackReason,
    };
    selected.push({ ...chosen.frame, candidateSampling });
    bins.push({
      ...candidateSampling,
      frameFile: path.basename(chosen.framePath),
    });
  }

  const candidates = selected.sort((left, right) => (
    frameTimestampSec(left) - frameTimestampSec(right)
    || String(left.path ?? "").localeCompare(String(right.path ?? ""))
  ));
  const timestamps = candidates.map((frame) => frameTimestampSec(frame)).filter(Number.isFinite);
  const candidateTimelineCoverageRatio = timestamps.length <= 1 || effectiveDurationSec <= 0
    ? (timestamps.length === 1 ? 1 : 0)
    : (timestamps.at(-1) - timestamps[0]) / effectiveDurationSec;
  let candidateMaxGapSec = 0;
  for (let index = 1; index < timestamps.length; index += 1) {
    candidateMaxGapSec = Math.max(candidateMaxGapSec, timestamps[index] - timestamps[index - 1]);
  }
  const selectionHash = hashText(JSON.stringify(candidates.map((frame) => ({
    file: path.basename(frame.path ?? ""),
    timestampSec: frameTimestampSec(frame),
    candidateSampling: frame.candidateSampling,
  }))));
  ...
  return {
    ...
    effectiveDurationSec,
    diagnostics: {
      version: SINGLE_CANDIDATE_SAMPLER_VERSION,
      durationSec: roundedMetric(effectiveDurationSec),
      requestedLimit: safeLimit,
      validFrameCount: validFrames.length,
      invalidFrameCount,
      duplicateFrameCount,
      ...counts,
      cueAlignedCandidateCount: bins.filter((bin) => bin.targetKind === "cue").length,
      centerFallbackCandidateCount: bins.filter((bin) => bin.targetKind === "center").length,
      candidateTimelineCoverageRatio: roundedMetric(candidateTimelineCoverageRatio),
      candidateMaxGapSec: roundedMetric(candidateMaxGapSec),
      selectionHash,
      bins,
    },
  };
...
export function buildSelectorPrompt({
  ...
  singleAnnotationOutputLimit = null,
}) {
  if (singleRecipeOnly) {
    ...
    const fullBudget = candidateFrames.length > 0 && requestedFrameCount === candidateFrames.length;
    const annotationSubsetActive = fullBudget
      && Number.isInteger(singleAnnotationOutputLimit)
      && singleAnnotationOutputLimit > 0
      && singleAnnotationOutputLimit < candidateFrames.length;
    if (annotationSubsetActive) {
      return [
        "너는 단일 요리 영상 후보 프레임의 시각 annotator다.",
        `첨부된 후보 이미지 ${candidateFrames.length}장을 모두 시간순으로 직접 확인하되 최종 레시피는 작성하지 않는다.`,
        `전체 후보 중 정보가 가장 많은 프레임을 selectedFrames에 정확히 ${singleAnnotationOutputLimit}개만 시간순으로 출력한다.`,
        "재료명과 수량·단위가 함께 보이는 실제 quantity-bearing frame을 가장 먼저 보존한다.",
        "실제 quantity-bearing frame이 출력 한도 이하면 가능한 한 모두 보존한다.",
        "그다음 서로 다른 실제 재료·투입·상태 변화가 보이는 근거와 시간축 다양성을 우선한다.",
        "같은 재료·같은 화면 글자·비슷한 완성 화면의 반복, 얼굴·광고·인트로·아웃트로·저정보 화면은 후순위로 둔다.",
        "각 선택 프레임의 실제 화면에서 확인되는 재료·수량·단위·조리 상태와 화면 원문만 visualEvidence에 기록한다.",
        "candidateIndex=N은 images[N-1], N번째 첨부 이미지, candidateFrames[N-1], 후보 목록 N번을 모두 뜻한다.",
        `candidateIndex는 1부터 ${candidateFrames.length}까지의 정수만 쓰고, 서로 다른 값 정확히 ${singleAnnotationOutputLimit}개를 시간순으로 출력한다.`,
        "file·filename·path·image·imagePath·frame·frameFile·index·timestamp·timestamp_sec·timeSec 같은 다른 identity 필드는 출력하지 않는다.",
        ...
        "  \"selectedFrames\": [{",
        "    \"candidateIndex\": 1,",
        "    \"reason\": \"화면에서 직접 확인한 짧은 근거\",",
        "    \"visualEvidence\": {",
        "      \"observed\": [\"화면에서 보인 대상\"],",
        "      \"onscreenText\": [\"화면 원문\"],",
        "      \"quantityCues\": [\"재료명과 수량이 함께 보인 원문\"],",
        "      \"confidence\": 0.9",
        "    }",
        "  }]",
        "}",
        ...
        buildSingleSelectorCompactJsonSerializationContract().line,
      ].join("\n");
    }
    if (fullBudget) {
      return [
        "너는 단일 요리 영상 후보 프레임의 시각 annotator다.",
        "첨부된 모든 후보 이미지를 시간순으로 확인하되 최종 레시피는 작성하지 않는다.",
        "각 후보의 실제 화면에서 확인되는 재료·수량·단위·조리 상태와 화면 원문만 visualEvidence에 기록한다.",
        "얼굴·광고·인트로·아웃트로·저정보 화면도 파일을 생략하지 말고 세 evidence 배열을 비우고 confidence는 null로 둔다.",
        "파일명은 후보 목록의 file 값을 정확히 복사하고 새 파일명을 만들지 않는다.",
        `selectedFrames는 정확히 ${requestedFrameCount}개를 시간순으로 출력한다.`,
        "observed, onscreenText, quantityCues에는 실제 화면에서 읽거나 확인한 원문만 넣고 추측하지 않는다.",
        "confidence는 0~1이다. 화면 글자가 없으면 세 배열을 비우고 confidence는 null로 둔다.",
        "",
        "출력은 설명 없이 JSON만 한다.",
    ...
    return [
      ...
      "파일명은 후보 목록의 file 값을 정확히 복사하고 새 파일명을 만들지 않는다.",
      `관련 있는 후보가 충분하면 selectedFrames는 정확히 ${requestedFrameCount}개를 고른다. 광고·인트로·아웃트로·저정보 화면만 남았다면 억지로 채우지 않는다.`,
      "같은 재료·같은 화면 글자·비슷한 공정의 반복보다 서로 다른 재료·분량·조리 단계 근거를 우선한다.",
      "선택한 각 프레임의 visualEvidence를 빠짐없이 작성한다.",
      "시간축 앞·중간·끝의 실제 공정 근거를 고르게 남긴다.",
      "observed, onscreenText, quantityCues에는 실제 화면에서 읽거나 확인한 원문만 넣고 추측하지 않는다.",
      "confidence는 0~1이다. 화면 글자가 없으면 세 배열을 비우고 confidence는 null로 둔다.",
      "",
      "출력은 설명 없이 JSON만 한다.",
...
function formatVisualEvidenceLine(frame, index) {
  if (frame.completionReason === "single-full-budget") {
    return [
      `- ${index + 1}. file=${frameBasename(frame)}, timestamp=${frame.timestamp ?? frame.timestamp_sec ?? "?"}, resolutionSource=${frame.resolutionSource ?? "none"}, completionReason=single-full-budget`,
      "  visualEvidence=미주석(unannotated); 첨부 원본 이미지를 직접 확인",
    ].join("\n");
  }
  const evidence = frame.visualEvidence ?? { observed: [], onscreenText: [], quantityCues: [], confidence: null };
  return [
    `- ${index + 1}. file=${frameBasename(frame)}, timestamp=${frame.timestamp ?? frame.timestamp_sec ?? "?"}, resolutionSource=${frame.resolutionSource ?? "none"}, completionReason=${frame.completionReason ?? "none"}`,
...
export function buildSingleAtomicStepAuditPromptParts() {
  const ledgerLines = [
    "9. occurrence anchor(source event identity, 명시 time/phase/order)+필수 target|primary action+선택 order/state/polarity/modality다. anchor 자체도 clean/독립 근거이고 각 field도 근거가 필요하다.",
    "10. clean description/comment/transcript/frame이 target+action을 직접 연결한다. noisy transcript는 인접 문맥/독립 source/frame 보강이 필요하고 action 근거가 보이지 않는 modifier를 승인하지 않는다.",
    "11. event에 직접 붙고 자체 근거가 있는 order/state·금지 polarity·필수/선택/권장 modality만 action에 종속해 보존한다. standalone 추천·variant·comparison·거부 대안, 깨진 ASR·같은 occurrence 재진술은 signature가 아니다.",
    "12. same-time/action/modifier group의 같은 occurrence 조각은 모으고 다른 action/modifier는 분리한다. 한 steps[] 문자열에 서로 다른 주행동이 둘 이상이면 action occurrence별로 분리한다. 별도 투입·상태 전환·명시 순서로 확인된 반복만 별도 occurrence다. timestamp 차이만으로 나누지 않는다.",
    "13. action/명시 modifier를 삭제·반전·이동·순서변경하지 않는다. modifier-only step·새 action/time을 만들지 않고 깨진 ASR은 복사하지 않는다.",
  ];
  const auditLines = [
    `[BIDIRECTIONAL ATOMICITY AUDIT version=${SINGLE_ATOMIC_STEP_AUDIT_VERSION}]`,
    "forward: signature마다 draft step 하나인지 확인한다.",
    "reverse: step마다 signature/group 하나이고 action/modifier가 보존됐는지 확인한다.",
    "violation repair: 같은 occurrence 조각은 모으고 다른 signature는 분리하며 근거·field·행동·시간을 만들지 않는다.",
    "[/BIDIRECTIONAL ATOMICITY AUDIT]",
  ];
  const rendered = [...ledgerLines, ...auditLines].join("\n");
  const renderedChars = Array.from(rendered).length;
  const renderedBytes = Buffer.byteLength(rendered, "utf8");
  if (
    renderedChars > SINGLE_ATOMIC_STEP_AUDIT_CHAR_LIMIT
    || renderedBytes > SINGLE_ATOMIC_STEP_AUDIT_BYTE_LIMIT
  ) {
    throw new Error(
      `SINGLE_ATOMIC_STEP_AUDIT_LIMIT: ${renderedChars} chars / ${renderedBytes} bytes`,
    );
  }
  return {
    version: SINGLE_ATOMIC_STEP_AUDIT_VERSION,
    charLimit: SINGLE_ATOMIC_STEP_AUDIT_CHAR_LIMIT,
    byteLimit: SINGLE_ATOMIC_STEP_AUDIT_BYTE_LIMIT,
    ledgerLines,
    auditLines,
    rendered,
    renderedChars,
...
export function buildSingleCompactJsonSerializationContract() {
  const line = "직렬화 계약: 먼저 위 규칙으로 모든 JSON field/value/content를 완성하고 compact는 whitespace만 뜻하며 ingredients/steps 축약이 아니므로 그 다음 serialization 단계에서만 내용을 변경하지 않고 문자열 JSON escape를 허용하되 설명·Markdown code fence·formatting 줄바꿈·들여쓰기 없는 UTF-8 한 줄 compact JSON으로 출력한다.";
  const chars = Array.from(line).length;
  const bytes = Buffer.byteLength(line, "utf8");
  const promptDeltaChars = chars + 1;
  const promptDeltaBytes = bytes + 1;
  if (
    promptDeltaChars > SINGLE_COMPACT_JSON_SERIALIZATION_CHAR_LIMIT
    || promptDeltaBytes > SINGLE_COMPACT_JSON_SERIALIZATION_BYTE_LIMIT
  ) {
    throw new Error(
      `SINGLE_COMPACT_JSON_SERIALIZATION_LIMIT: ${promptDeltaChars} chars / ${promptDeltaBytes} bytes`,
    );
  }
  return {
    version: SINGLE_COMPACT_JSON_SERIALIZATION_VERSION,
    charLimit: SINGLE_COMPACT_JSON_SERIALIZATION_CHAR_LIMIT,
    byteLimit: SINGLE_COMPACT_JSON_SERIALIZATION_BYTE_LIMIT,
    line,
    chars,
    bytes,
    promptDeltaChars,
...
export function buildSingleSelectorCompactJsonSerializationContract() {
  const line = "selector 직렬화 계약: 먼저 요구된 selectedFrames와 모든 visualEvidence field/value/content를 완성하고 개수·identity·evidence 내용은 변경·생략·축약하지 않고 그 다음 serialization 단계에서만 JSON escape를 허용하되 설명·Markdown code fence·formatting 줄바꿈·들여쓰기 없는 유효한 UTF-8 한 줄 compact JSON으로 출력한다.";
  const chars = Array.from(line).length;
  const bytes = Buffer.byteLength(line, "utf8");
  const promptDeltaChars = chars + 1;
  const promptDeltaBytes = bytes + 1;
  if (
    promptDeltaChars > SINGLE_SELECTOR_COMPACT_JSON_SERIALIZATION_CHAR_LIMIT
    || promptDeltaBytes > SINGLE_SELECTOR_COMPACT_JSON_SERIALIZATION_BYTE_LIMIT
  ) {
    throw new Error(
      `SINGLE_SELECTOR_COMPACT_JSON_SERIALIZATION_LIMIT: ${promptDeltaChars} chars / ${promptDeltaBytes} bytes`,
    );
  }
  return {
    version: SINGLE_SELECTOR_COMPACT_JSON_SERIALIZATION_VERSION,
    charLimit: SINGLE_SELECTOR_COMPACT_JSON_SERIALIZATION_CHAR_LIMIT,
    byteLimit: SINGLE_SELECTOR_COMPACT_JSON_SERIALIZATION_BYTE_LIMIT,
    line,
    chars,
    bytes,
    promptDeltaChars,
...
export function buildFinalPrompt({
  ...
  screenOcrTimelineText = "",
  selectorBypassed = false,
}) {
  const selectedFrameLines = selectedFrames.map((frame, index) => frameDisplayLine(frame, index));
  if (singleRecipeOnly) {
    const sourceAppendLines = singleSourceAppendLines(prompt, sourceText);
    const atomicStepAudit = buildSingleAtomicStepAuditPromptParts();
    const compactJsonSerialization = buildSingleCompactJsonSerializationContract();
    return [
      prompt,
      "",
      selectorBypassed
        ? "추가 입력: 화면 OCR 이벤트에서 결정적으로 고른 실제 프레임과 그 프레임에 연결된 화면 근거다."
      ...
      "completionReason=single-full-budget인 프레임은 selector가 상세 주석하지 않은 미주석(unannotated) 프레임이다. 빈 visualEvidence는 화면 정보가 없다는 negative evidence가 아니므로 첨부 원본 이미지를 직접 확인한다.",
      "영상에 없는 재료·단계는 요리 상식으로 추가하지 않고 recipes[]에는 정확히 한 레시피만 출력한다.",
      "출력 직전에 다음 ingredient coverage audit를 내부적으로 수행하되, audit 내용은 JSON에 따로 출력하지 않는다.",
      "1. 모든 소스의 재료 후보를 actual-use, optional-use, rejected-or-discussion-only, uncertain으로 분류한다.",
      "2. actual-use는 재료명과 직접 손질·투입·혼합·마무리 동작이 함께 있거나, 명시 재료 목록과 일치하는 화면·단계 근거가 있을 때만 인정한다.",
      "3. noisy ASR의 고립된 재료명이나 동작·순서가 불명확한 단일 언급은 uncertain으로 두고 출력하지 않는다.",
      "4. optional-use는 이 target recipe에 추가하라고 명시되고 실제 시연 또는 일치 근거가 있는 선택 첨가물만 허용하며 optional: true로 출력한다.",
      "5. 대체재·다른 variant·단순 추천·비교·거부된 항목은 실제 투입 근거가 따로 없으면 rejected-or-discussion-only로 두고 실제 재료와 동시에 출력하지 않는다.",
      "6. actual-use가 단계 요약에서 빠졌다면 ingredients에서 삭제하지 말고 근거가 있는 기존 시간대·단계에 재료명만 복구한다. coverage를 맞추기 위해 새 동작·시간·순서를 만들지 않는다.",
      "7. 서로 다른 구체 재료명을 넓은 상위 범주 하나로 합치지 않는다. 반대로 소스 근거 없이 재료를 세분화하지 않는다.",
      "8. 각 actual-use 재료의 name은 한국어 표준명으로 쓰고, 소스에서 같은 actual-use 재료 identity를 가리키는 정확한 원문 재료명 span이 다르면 nameAliases에 보존한다. 수량·단위·조리동사·전체 문장·section label이나 여러 재료를 묶은 구절은 넣지 않고 alias로 서로 다른 재료 identity를 합치지 않는다. 소스에 없는 동의어와 uncertain 또는 rejected-or-discussion-only 표기는 alias로 만들지 않는다.",
      "출력 직전에 chronological action ledger를 내부적으로 만들되 ledger 자체는 JSON에 출력하지 않는다.",
      ...atomicStepAudit.ledgerLines,
      "",
      "선택 프레임 화면 근거:",
      ...selectedFrames.map(formatVisualEvidenceLine),
      ...(screenOcrTimelineText ? ["", screenOcrTimelineText] : []),
      ...sourceAppendLines,
      "",
      "checklist가 비어 있거나 항목에 없는 것은 행동 부재의 negative evidence가 아니다. 전체 source와 frame을 계속 확인한다.",
      ...(actionChecklist?.count > 0 ? actionChecklist.renderedSection.split("\n") : []),
      ...atomicStepAudit.auditLines,
      "",
      "JSON 출력 직전에 bidirectional ingredient-step coverage check를 수행하고 check 자체는 출력하지 않는다.",
      "14. actual-use 또는 자격을 갖춘 optional-use 각각에 대해 ingredient row ↔ evidence-backed step reference가 양방향으로 연결됐는지 확인한다.",
      "14-1. draft step 자체는 ingredient 승격 근거가 아니다. source·frame·transcript가 target recipe의 물질과 사용 action·목적·선택 지시를 직접 연결할 때만 actual-use 또는 qualified optional-use로 검토한다.",
      "14-2. 위 직접 근거가 확인된 물질은 팬 코팅·양념·간 조절·마무리처럼 소량이어도 evidence-backed step과 ingredient row 양쪽에 보존한다.",
      "14-3. 인접한 반복 ASR은 corroboration일 뿐 독립 근거가 아니다. 정확한 동사가 깨졌을 때 직접 연결을 보강할 수 있지만 새 identity·action·time을 만들지 않는다.",
      "14-4. ASR 반복은 rejected·comparison·negation 판정과 비교·거부·다른 variant·일반 추천을 뒤집지 않는다.",
      "15. source·frame·transcript가 해당 재료와 action을 직접 연결할 때만, 빠진 재료명을 그 evidence-backed step에 복구한다. 시간상 가깝다는 이유만으로 연결하지 않고 새 행동·시간·순서를 만들지 않는다.",
      "16. reverse check에서 cookware·중간 산출물·generic mixture명이나 rejected·variant·comparison·negation 언급을 ingredient row로 승격하지 않는다.",
      "17. source에 명시된 마무리·보관·숙성 행동이 먹는 품질이나 안전에 영향을 주면 근거 있는 별도 step으로 보존한다.",
      "18. 최소 대기와 권장 숙성·보관이 함께 명시되면 최소·권장 관계와 modality를 원문대로 유지한다. 권장을 필수 안전 대기로 바꾸거나 단위 환산이나 새 시간을 만들지 않는다.",
      "",
      "위 정보를 바탕으로 기존 스키마의 JSON만 출력한다.",
      compactJsonSerialization.line,
    ].join("\n");
  }
...
function buildTitleVisualBridgeFacts() {
  // Strict i031 is global single-recipe mode. Disable segmented recipe-specific bridges.
  return [];
}
...
export function buildCodexVisionKeyframesCacheKey({
  ...
  frameOptions,
  singleRecipeOnly,
  singleCandidateSamplerVersion = null,
  singleAnnotationSubsetVersion = null,
  singleAnnotationOutputLimit = null,
  singleActionChecklistVersion = null,
  singleActionChecklistLimit = null,
  singleActionChecklistTextCharLimit = null,
  singleActionChecklistSectionCharLimit = null,
  singleActionChecklistSectionByteLimit = null,
  singleActionChecklistHash = null,
  singleAtomicStepAuditVersion = null,
  screenOcrIdentity = null,
  sourceFingerprint,
}) {
  const keyPayload = {
    provider: PROVIDER,
    ...
    singleRecipeOnly,
    sourceFingerprint,
    clientVersion: singleRecipeOnly ? SINGLE_CLIENT_VERSION : CLIENT_VERSION,
    selectorPromptVersion: singleRecipeOnly ? SINGLE_SELECTOR_PROMPT_VERSION : SELECTOR_PROMPT_VERSION,
    finalPromptVersion: singleRecipeOnly ? SINGLE_FINAL_PROMPT_VERSION : FINAL_PROMPT_VERSION,
    modelSourceBoundaryVersion: MODEL_SOURCE_BOUNDARY_VERSION,
  };
  if (singleRecipeOnly) {
    keyPayload.singleCandidateSamplerVersion = singleCandidateSamplerVersion;
    keyPayload.singleAnnotationSubsetVersion = singleAnnotationSubsetVersion;
    keyPayload.singleAnnotationOutputLimit = singleAnnotationOutputLimit;
    keyPayload.singleActionChecklistVersion = singleActionChecklistVersion;
    keyPayload.singleActionChecklistLimit = singleActionChecklistLimit;
    keyPayload.singleActionChecklistTextCharLimit = singleActionChecklistTextCharLimit;
    keyPayload.singleActionChecklistSectionCharLimit = singleActionChecklistSectionCharLimit;
    keyPayload.singleActionChecklistSectionByteLimit = singleActionChecklistSectionByteLimit;
    keyPayload.singleActionChecklistHash = singleActionChecklistHash;
    keyPayload.singleAtomicStepAuditVersion = singleAtomicStepAuditVersion;
    if (screenOcrIdentity) keyPayload.screenOcrIdentity = screenOcrIdentity;
  }
...
function mapSingleAnnotationSubset(selectorJson, candidateFrames, expectedCount) {
  const rawEntries = Array.isArray(selectorJson?.selectedFrames)
    ? selectorJson.selectedFrames
    : [];
  ...
  const validEvidence = (evidence) => isObject(evidence)
    && Array.isArray(evidence.observed)
    && evidence.observed.every((value) => typeof value === "string")
    && Array.isArray(evidence.onscreenText)
    && evidence.onscreenText.every((value) => typeof value === "string")
    && Array.isArray(evidence.quantityCues)
    && evidence.quantityCues.every((value) => typeof value === "string")
    && (
  ...
  const schemaValid = rawEntries.every((entry) => (
    isObject(entry)
    && Number.isInteger(entry.candidateIndex)
    && entry.candidateIndex >= 1
    && entry.candidateIndex <= candidateFrames.length
    && forbiddenIdentityFields.every((field) => !Object.prototype.hasOwnProperty.call(entry, field))
    && typeof entry.reason === "string"
    && entry.reason.trim().length > 0
    && validEvidence(entry.visualEvidence)
  ));
  const uniqueCandidateIndices = new Set(rawEntries.map((entry) => entry?.candidateIndex));

  if (
    rawEntries.length !== expectedCount
    || !schemaValid
    || uniqueCandidateIndices.size !== expectedCount
  ) {
    throw new Error(
      `SINGLE_ANNOTATION_CONTRACT: expected exactly ${expectedCount} unique candidateIndex annotations within 1..${candidateFrames.length}, received ${rawEntries.length}`,
    );
  }

  return {
    ...selectorJson,
    selectedFrames: rawEntries.map((entry) => ({
      ...entry,
      file: frameBasename(candidateFrames[entry.candidateIndex - 1]),
    })),
  };
}

function completeSingleFullBudgetFrames(
  selectedFrames,
  candidateFrames,
  { singleRecipeOnly, totalLimit, perRecipeLimit },
) {
  const fullBudget = singleRecipeOnly
    && candidateFrames.length > 0
    && totalLimit >= candidateFrames.length
    && perRecipeLimit >= candidateFrames.length;
  if (!fullBudget) {
    return { selectedFrames, deterministicCandidateCompletionCount: 0 };
  }

  const selectedByPath = new Map(selectedFrames.map((frame) => [frame.path, frame]));
  let deterministicCandidateCompletionCount = 0;
  const completed = candidateFrames.map((candidate) => {
    const selected = selectedByPath.get(candidate.path);
    if (selected) return { ...selected, completionReason: null };
    deterministicCandidateCompletionCount += 1;
    return {
      ...candidate,
      selectionReason: "single full-budget deterministic completion",
      resolutionSource: "exact",
      completionReason: "single-full-budget",
      visualEvidence: {
        observed: [],
        onscreenText: [],
        quantityCues: [],
  ...
  return { selectedFrames: completed, deterministicCandidateCompletionCount };
}
...
function ingredientCueKeys(ingredient, { minLength = 1 } = {}) {
  return [ingredient?.name, ...(Array.isArray(ingredient?.nameAliases) ? ingredient.nameAliases : [])]
    .map((value) => keyOf(value))
    .filter((value) => value.length >= minLength);
}

function parseOnscreenAmountUnits(text) {
  const pairs = [];
  for (const match of String(text ?? "").matchAll(ONSCREEN_AMOUNT_UNIT_RE)) {
    const rawAmount = compact(match[1]).replace(/\s+/gu, "");
    const rawUnit = compact(match[2]).replace(/\s+/gu, "");
    const amount = ONSCREEN_KOREAN_AMOUNTS.get(rawAmount) ?? rawAmount;
    const unit = ONSCREEN_UNIT_ALIASES.get(rawUnit) ?? rawUnit;
    pairs.push({
      amount,
      unit,
      start: match.index,
...
function normalizedOnscreenAmount(value) {
  const raw = compact(value).replace(/^약\s*/u, "").replace(/\s+/gu, "");
  return ONSCREEN_KOREAN_AMOUNTS.get(raw) ?? raw;
}

function normalizedOnscreenUnit(value) {
  const raw = compact(value).replace(/\s+/gu, "");
  return ONSCREEN_UNIT_ALIASES.get(raw) ?? raw;
}

function isLocalQuantityBridge(value) {
  const bridge = compact(value).replace(/[:：,，.·=~\-–—()[\]]/gu, " ").replace(/\s+/gu, "").trim();
  return /^(?:(?:은|는|이|가|을|를|의|약|대략|총|각|각각|기준|분량)){0,3}$/u.test(bridge);
}

function cueLocallyLinksIngredientAndPair(text, ingredient, pair) {
  const cue = String(text ?? "");
  const lowerCue = cue.toLocaleLowerCase();
  const labels = [ingredient?.name, ...(Array.isArray(ingredient?.nameAliases) ? ingredient.nameAliases : [])]
  ...
  for (const label of labels) {
    ...
    let offset = 0;
    while (offset <= lowerCue.length - lowerLabel.length) {
      const labelStart = lowerCue.indexOf(lowerLabel, offset);
      if (labelStart < 0) break;
      const labelEnd = labelStart + lowerLabel.length;
      const previousCharacter = labelStart > 0 ? cue[labelStart - 1] : "";
      if (previousCharacter && /[\p{L}\p{N}]/u.test(previousCharacter)) {
        offset = labelStart + 1;
        continue;
      }
      if (labelEnd <= pair.start && isLocalQuantityBridge(cue.slice(labelEnd, pair.start))) return true;
      if (pair.end <= labelStart && isLocalQuantityBridge(cue.slice(pair.end, labelStart))) return true;
      offset = labelStart + 1;
    }
  }
  return false;
...
export function reconcileOnscreenAmountEvidence(json, selectedFrames, { recoverMissing = false } = {}) {
  ...
  const conflicts = [];
  const recoveries = [];
  const ingredientEntries = (json.recipes ?? []).flatMap((recipe, recipeIndex) => (
    (recipe.ingredients ?? []).map((ingredient, ingredientIndex) => ({
      recipeIndex,
      ingredientIndex,
      ingredient,
      keys: ingredientCueKeys(ingredient, { minLength: 2 }),
    }))
  ));
  const initiallyMissingIds = new Set(ingredientEntries
    .filter((entry) => (
      (entry.ingredient?.amount === null || entry.ingredient?.amount === undefined)
      && (entry.ingredient?.unit === null || entry.ingredient?.unit === undefined)
    ))
    .map((entry) => `${entry.recipeIndex}:${entry.ingredientIndex}`));
  ...
  for (const [recipeIndex, recipe] of (json.recipes ?? []).entries()) {
    for (const [ingredientIndex, ingredient] of (recipe.ingredients ?? []).entries()) {
      ...
      const ingredientKeys = ingredientCueKeys(ingredient);
      const supportingCue = cueTexts.find((text) => {
        const cueKey = keyOf(text);
        if (!ingredientKeys.some((ingredientKey) => cueKey.includes(ingredientKey))) return false;
        return parseOnscreenAmountUnits(text).some((pair) => (
          cueLocallyLinksIngredientAndPair(text, ingredient, pair)
          && pair.amount === normalizedOnscreenAmount(ingredient.amount)
          && pair.unit === normalizedOnscreenUnit(ingredient.unit)
        ));
      });
  ...
  if (recoverMissing) {
    const pairsByIngredient = new Map();
    for (const text of cueTexts) {
      const cueKey = keyOf(text);
      const targetEntries = ingredientEntries.filter((entry) => (
        entry.keys.some((ingredientKey) => cueKey.includes(ingredientKey))
      ));
      const pairs = parseOnscreenAmountUnits(text);
      if (targetEntries.length !== 1 || pairs.length !== 1) continue;
      const target = targetEntries[0];
      if (!cueLocallyLinksIngredientAndPair(text, target.ingredient, pairs[0])) continue;
      const id = `${target.recipeIndex}:${target.ingredientIndex}`;
      const existing = pairsByIngredient.get(id) ?? [];
      existing.push(pairs[0]);
      pairsByIngredient.set(id, existing);
    }

    for (const entry of ingredientEntries) {
      const ingredient = entry.ingredient;
      const id = `${entry.recipeIndex}:${entry.ingredientIndex}`;
      if (
        !initiallyMissingIds.has(id)
        || (ingredient?.amount !== null && ingredient?.amount !== undefined)
        || (ingredient?.unit !== null && ingredient?.unit !== undefined)
      ) continue;
      const candidates = pairsByIngredient.get(id) ?? [];
      const unique = new Map(candidates.map((pair) => [`${pair.amount}:${pair.unit}`, pair]));
      if (unique.size !== 1) continue;
      const recovered = [...unique.values()][0];
      ingredient.amount = recovered.amount;
      ingredient.unit = recovered.unit;
      ingredient.amountBasis = "onscreen";
      recoveries.push({
  ...
  return { json, conflicts, recoveries };
}
...
export function createCodexVisionKeyframesClient(options = {}) {
  ...
  const screenOcrMode = singleRecipeOnly
    ? String(options.screenOcrMode ?? process.env.RECIPE_LOOP_CODEX_VISION_SCREEN_OCR_MODE ?? "off").trim().toLowerCase()
    : "off";
  classifyScreenOcrNeed({ mode: screenOcrMode, sourceText: "" });
  const recognizeScreenOcrBatch = options.recognizeScreenOcrBatch ?? createMacOSVisionBatchRecognizer({
    cacheRoot: cacheDir,
    timeoutMs,
  });
  const screenOcrScout = options.screenOcrScout ?? ((input) => runScreenOcrScout({
    ...input,
    recognizeBatch: recognizeScreenOcrBatch,
  }));
  const executionConfigSignature = hashText(JSON.stringify({
    ...
    singleRecipeOnly,
    ...(singleRecipeOnly ? {
      singleCandidateSamplerVersion: SINGLE_CANDIDATE_SAMPLER_VERSION,
      singleAnnotationSubsetVersion: SINGLE_ANNOTATION_SUBSET_VERSION,
      singleAnnotationOutputLimit: SINGLE_ANNOTATION_OUTPUT_LIMIT,
      singleActionChecklistVersion: SINGLE_ACTION_CHECKLIST_VERSION,
      singleActionChecklistLimit: SINGLE_ACTION_CHECKLIST_LIMIT,
      singleActionChecklistTextCharLimit: SINGLE_ACTION_CHECKLIST_TEXT_CHAR_LIMIT,
      singleActionChecklistSectionCharLimit: SINGLE_ACTION_CHECKLIST_SECTION_CHAR_LIMIT,
      singleActionChecklistSectionByteLimit: SINGLE_ACTION_CHECKLIST_SECTION_BYTE_LIMIT,
      singleAtomicStepAuditVersion: SINGLE_ATOMIC_STEP_AUDIT_VERSION,
    } : {}),
    keyframeMode,
    ...
    ...(screenOcrMode !== "off" ? {
      screenOcrMode,
      screenOcrPolicyVersion: SCREEN_OCR_POLICY_VERSION,
      screenOcrIntegrationVersion: SINGLE_SCREEN_OCR_INTEGRATION_VERSION,
    } : {}),
  }));
  ...
  return {
    async generate({ prompt, videoUrl = null, cacheText = "", evidencePacketBundle = null }) {
      ...
      const ocrNeedStartedAt = Date.now();
      const screenOcrDecision = classifyScreenOcrNeed({ mode: screenOcrMode, sourceText: cacheText });
      const ocrNeedClassificationMs = Date.now() - ocrNeedStartedAt;
      const effectiveFrameOptions = screenOcrDecision.enabled
        ? {
          ...frameOptions,
          screenOcrScan: true,
          screenOcrPolicyVersion: SCREEN_OCR_POLICY_VERSION,
      ...
      const frameResult = await extractFrames({
        ...
        videoId,
        cacheDir,
        frameOptions: effectiveFrameOptions,
        timeoutMs,
      ...
      const durationSeconds = Number.isFinite(frameResult.extractionStats?.duration_sec)
        ? Math.ceil(frameResult.extractionStats.duration_sec)
        : null;
      try {
        options.onProgress?.(
          "model_analysis",
        ...
        // Progress telemetry is best-effort and never changes extraction.
      }

      let screenOcrResult = {
        status: "skipped",
        cached: false,
        fallbackReason: screenOcrDecision.reason,
        events: [],
        timings: {},
        diagnostics: {},
      };
      let screenOcrTimeline = { version: SCREEN_OCR_TIMELINE_VERSION, events: [] };
      let screenOcrAssessment = { sufficient: false, reason: screenOcrDecision.reason, diagnostics: {} };
      let screenOcrSelection = { frames: [], selectedEventIds: [] };
      let normalizedScreenOcrTimelineHash = null;
      if (singleRecipeOnly && screenOcrDecision.enabled) {
        const screenOcrCandidates = Array.isArray(frameResult.screenOcrCandidates)
          ? frameResult.screenOcrCandidates
          : frames.filter((frame) => String(frame.reason ?? "").includes("screen-ocr"));
        try {
          screenOcrResult = await screenOcrScout({
            candidates: screenOcrCandidates,
            cacheDir,
            cacheKey: hashKey({
              sourceFingerprint: frameResult.sourceFingerprint ?? videoId,
              screenOcrPolicyVersion: SCREEN_OCR_POLICY_VERSION,
              candidates: screenOcrCandidates.map((candidate) => ({
                file: path.basename(candidate.path ?? ""),
                timestampSec: frameTimestampSec(candidate),
                region: candidate.region ?? candidate.screen_ocr_region ?? null,
              })),
            }),
            durationSec: frameResult.extractionStats?.duration_sec ?? null,
            timeoutMs,
            noCache: Boolean(options.noCache),
            frameResult,
          });
        ...
        const rawEvents = Array.isArray(screenOcrResult?.events)
          ? screenOcrResult.events
          : Array.isArray(screenOcrResult?.timeline?.events)
            ? screenOcrResult.timeline.events
            : [];
        screenOcrTimeline = {
          version: screenOcrResult?.timelineVersion ?? screenOcrResult?.timeline?.version ?? SCREEN_OCR_TIMELINE_VERSION,
          events: rawEvents,
        };
        normalizedScreenOcrTimelineHash = screenOcrTimelineHash(screenOcrTimeline);
        screenOcrAssessment = screenOcrResult.status === "ok"
          ? assessScreenOcrEvidence({
            events: screenOcrTimeline.events,
            durationSec: frameResult.extractionStats?.duration_sec ?? null,
          })
          : {
            sufficient: false,
            reason: screenOcrResult.fallbackReason ?? `screen-ocr-${screenOcrResult.status ?? "failed"}`,
            diagnostics: {},
          };
        screenOcrSelection = selectScreenOcrRepresentativeFrames({
          events: screenOcrTimeline.events,
          durationSec: frameResult.extractionStats?.duration_sec ?? null,
          limit: Math.min(keyframeTotalLimit, keyframesPerRecipe),
        });
      }
      const selectorBypassed = singleRecipeOnly
        && screenOcrDecision.enabled
        && screenOcrAssessment.sufficient
        && screenOcrSelection.frames.length > 0;
      const screenOcrTimelineText = screenOcrTimeline.events.length
        ? formatScreenOcrTimelineForPrompt(screenOcrTimeline)
        : "";
      ...
      const candidateSamplerStartedAt = Date.now();
      const singleCandidateSelection = singleRecipeOnly
        ? selectSingleRecipeCandidateFrames({
          frames,
          sourceText: cacheText,
        ...
        : null;
      const sampledCandidateFrames = singleCandidateSelection?.candidates ?? sampleEvenly(frames, selectorCandidateLimit);
      const candidateFrames = selectorBypassed ? screenOcrSelection.frames : sampledCandidateFrames;
      const samplerMs = singleRecipeOnly ? Date.now() - candidateSamplerStartedAt : null;
      const candidateSamplerDiagnostics = singleCandidateSelection
        ? { ...singleCandidateSelection.diagnostics, sampler_ms: samplerMs }
        : null;
      const actionChecklist = singleRecipeOnly
        ? buildSingleTranscriptActionChecklist({
          sourceText: cacheText,
          effectiveDurationSec: singleCandidateSelection?.effectiveDurationSec
            ?? candidateSamplerDiagnostics?.durationSec
            ?? null,
          frameTimestamps: frames.map((frame) => frameTimestampSec(frame)),
        })
        : null;
      ...
      const resultKey = buildCodexVisionKeyframesCacheKey({
        ...
        frameOptions: effectiveFrameOptions,
        singleRecipeOnly,
        singleCandidateSamplerVersion: singleRecipeOnly ? SINGLE_CANDIDATE_SAMPLER_VERSION : null,
        singleAnnotationSubsetVersion: singleRecipeOnly ? SINGLE_ANNOTATION_SUBSET_VERSION : null,
        singleAnnotationOutputLimit: singleRecipeOnly ? SINGLE_ANNOTATION_OUTPUT_LIMIT : null,
        singleActionChecklistVersion: singleRecipeOnly ? SINGLE_ACTION_CHECKLIST_VERSION : null,
        singleActionChecklistLimit: singleRecipeOnly ? SINGLE_ACTION_CHECKLIST_LIMIT : null,
        singleActionChecklistTextCharLimit: singleRecipeOnly ? SINGLE_ACTION_CHECKLIST_TEXT_CHAR_LIMIT : null,
        singleActionChecklistSectionCharLimit: singleRecipeOnly ? SINGLE_ACTION_CHECKLIST_SECTION_CHAR_LIMIT : null,
        singleActionChecklistSectionByteLimit: singleRecipeOnly ? SINGLE_ACTION_CHECKLIST_SECTION_BYTE_LIMIT : null,
        singleActionChecklistHash: actionChecklist?.contentHash ?? null,
        singleAtomicStepAuditVersion: singleRecipeOnly ? SINGLE_ATOMIC_STEP_AUDIT_VERSION : null,
        screenOcrIdentity: screenOcrDecision.enabled ? {
          mode: screenOcrMode,
          policyVersion: SCREEN_OCR_POLICY_VERSION,
          integrationVersion: SINGLE_SCREEN_OCR_INTEGRATION_VERSION,
          timelineVersion: screenOcrTimeline.version,
          timelineHash: normalizedScreenOcrTimelineHash,
          representativeFrameManifest: frameManifestHash(screenOcrSelection.frames),
          selectorBypassed,
          fallbackReason: screenOcrAssessment.reason,
        } : null,
        sourceFingerprint: frameResult.sourceFingerprint ?? null,
      });
      const resultDir = resultDirectory(cacheDir, resultKey, Boolean(options.noCache));
      ...
      if (!options.noCache && !options.refreshFinal && existsSync(finalJsonPath)) {
        ...
        return {
          ...
          meta: {
            ...
            ...runCacheMeta({
              frameResult,
              selectorCacheHit: !Boolean(cached.meta?.selectorBypassed),
              visionCacheHit: true,
              declaredRunType,
            }),
            ...
            ...(singleRecipeOnly ? { sampler_ms: samplerMs } : {}),
            selector_ms: null,
            final_ms: null,
            total_fresh_ms: null,
            codexVisionKeyframesCacheDir: resultDir,
      ...
      await mkdir(resultDir, { recursive: true });
      if (candidateSamplerDiagnostics) {
        await writeFile(
          path.join(resultDir, "candidate_sampler.json"),
          JSON.stringify(candidateSamplerDiagnostics, null, 2) + "\n",
          "utf8",
        );
      }
      await copyIfExists(path.join(frameResult.frameDir, "frames.json"), path.join(resultDir, "frames.json"));
      await copyIfExists(path.join(frameResult.frameDir, "extraction_stats.json"), path.join(resultDir, "extraction_stats.json"));
      if (screenOcrDecision.enabled) {
        await writeFile(
          path.join(resultDir, "screen_ocr_timeline.json"),
          JSON.stringify({
            version: screenOcrTimeline.version,
            timelineHash: normalizedScreenOcrTimelineHash,
            status: screenOcrResult.status,
            assessment: screenOcrAssessment,
            events: screenOcrTimeline.events,
            timings: screenOcrResult.timings ?? {},
            diagnostics: screenOcrResult.diagnostics ?? {},
          }, null, 2) + "\n",
          "utf8",
        );
      }
      ...
      try {
        ...
        if (selectorBypassed) {
          selectorJson = {
            selectedFrames: candidateFrames.map((frame, index) => ({
              candidateIndex: index + 1,
              frame: frameBasename(frame),
              reason: frame.selectionReason ?? frame.reason ?? "screen OCR event",
              visualEvidence: frame.visualEvidence,
            })),
          };
          await writeFile(
            path.join(resultDir, "selector_bypass.json"),
            JSON.stringify({
              version: SINGLE_SCREEN_OCR_INTEGRATION_VERSION,
              reason: screenOcrAssessment.reason,
              selectedEventIds: screenOcrSelection.selectedEventIds,
              selectedFrameCount: candidateFrames.length,
            }, null, 2) + "\n",
            "utf8",
          );
        } else {
          ...
          selectorMs = selectorCacheHit ? null : Date.now() - selectorStartedAt;
        }

        const singleAnnotationSubsetActive = singleRecipeOnly
          && !selectorBypassed
          && candidateFrames.length > SINGLE_ANNOTATION_OUTPUT_LIMIT
          && keyframeTotalLimit >= candidateFrames.length
          && keyframesPerRecipe >= candidateFrames.length;
        const selectorJsonForNormalization = singleAnnotationSubsetActive
          ? mapSingleAnnotationSubset(
            selectorJson,
            candidateFrames,
            SINGLE_ANNOTATION_OUTPUT_LIMIT,
          )
          : selectorJson;
        const selectedCandidateIndices = singleAnnotationSubsetActive
          ? selectorJson.selectedFrames.map((entry) => entry.candidateIndex)
          : null;
        const normalizedSelectedFrames = normalizeSelectedFrames(selectorJsonForNormalization, candidateFrames, {
          totalLimit: keyframeTotalLimit,
          perRecipeLimit: keyframesPerRecipe,
        });
        if (
          singleAnnotationSubsetActive
          && normalizedSelectedFrames.length !== SINGLE_ANNOTATION_OUTPUT_LIMIT
        ) {
          throw new Error(
            `SINGLE_ANNOTATION_CONTRACT: expected exactly ${SINGLE_ANNOTATION_OUTPUT_LIMIT} valid unique annotated frames, received ${normalizedSelectedFrames.length}`,
          );
        }
        const annotationFrameHash = frameManifestHash(normalizedSelectedFrames);
        const annotatedFrameEvidenceSummary = selectedFrameEvidenceSummary(normalizedSelectedFrames);
        const {
          selectedFrames,
          deterministicCandidateCompletionCount,
        } = completeSingleFullBudgetFrames(normalizedSelectedFrames, candidateFrames, {
          singleRecipeOnly,
          totalLimit: keyframeTotalLimit,
          perRecipeLimit: keyframesPerRecipe,
        });
        const selectedFrameHash = frameManifestHash(selectedFrames);
        if (singleAnnotationSubsetActive) {
          await writeFile(
            path.join(resultDir, "annotation_subset.json"),
            JSON.stringify({
              version: SINGLE_ANNOTATION_SUBSET_VERSION,
              outputLimit: SINGLE_ANNOTATION_OUTPUT_LIMIT,
              selectorInputImageCount: candidateFrames.length,
              selectorAnnotatedFrameCount: normalizedSelectedFrames.length,
              selectedCandidateIndices,
              annotationFrameHash,
              annotatedEvidenceSummary: annotatedFrameEvidenceSummary,
              deterministicCandidateCompletionCount,
              finalInputImageCount: selectedFrames.length,
            }, null, 2) + "\n",
            "utf8",
          );
        }
        await writeFile(
          ...
          JSON.stringify({
            ...
            selectedFrames: selectedFrames.map((frame) => ({
              ...
              completionReason: frame.completionReason ?? null,
              ...(frame.candidateSampling ? { candidateSampling: frame.candidateSampling } : {}),
              visualEvidence: frame.visualEvidence ?? {
                observed: [],
                onscreenText: [],
                quantityCues: [],
        ...
        const finalRaw = await codexExec({
          prompt: finalPrompt,
          ...(singleRecipeOnly ? { outputSchema: SINGLE_RECIPE_OUTPUT_SCHEMA } : {}),
        ...
        const validation = reconcileOnscreenAmountEvidence(rawJson, selectedFrames, {
          recoverMissing: singleRecipeOnly,
        });
        ...
        const meta = {
          ...
          requestedKeyframeMode,
          singleRecipeOnly,
          singleRecipeProfile: singleRecipeOnly ? "fast12-v13-index6-event-signature" : null,
          clientVersion: singleRecipeOnly ? SINGLE_CLIENT_VERSION : CLIENT_VERSION,
          ...(singleRecipeOnly ? {
            singleCandidateSamplerVersion: SINGLE_CANDIDATE_SAMPLER_VERSION,
            singleAnnotationSubsetVersion: SINGLE_ANNOTATION_SUBSET_VERSION,
            ...
            singleActionChecklistSectionByteLimit: SINGLE_ACTION_CHECKLIST_SECTION_BYTE_LIMIT,
            singleAtomicStepAuditVersion: SINGLE_ATOMIC_STEP_AUDIT_VERSION,
            singleActionChecklistCount: actionChecklist.count,
            singleActionChecklistHash: actionChecklist.contentHash,
            singleActionChecklistEffectiveDurationSec: actionChecklist.effectiveDurationSec,
            singleActionChecklistSectionChars: actionChecklist.renderedSectionChars,
            singleActionChecklistSectionBytes: actionChecklist.renderedSectionBytes,
          } : {}),
          selectorPromptVersion: singleRecipeOnly ? SINGLE_SELECTOR_PROMPT_VERSION : SELECTOR_PROMPT_VERSION,
          finalPromptVersion: singleRecipeOnly ? SINGLE_FINAL_PROMPT_VERSION : FINAL_PROMPT_VERSION,
          ...
          candidateFrameCount: candidateFrames.length,
          selectorCandidateLimit,
          selectorInputImageCount: selectorBypassed ? 0 : candidateFrames.length,
          ...(screenOcrDecision.enabled ? {
            screenOcrMode,
            screenOcrPolicyVersion: SCREEN_OCR_POLICY_VERSION,
            screenOcrIntegrationVersion: SINGLE_SCREEN_OCR_INTEGRATION_VERSION,
            screenOcrStatus: screenOcrResult.status,
            screenOcrNeedReasons: [screenOcrDecision.reason],
            screenOcrTimelineVersion: screenOcrTimeline.version,
            screenOcrTimelineHash: normalizedScreenOcrTimelineHash,
            screenOcrEventCount: screenOcrTimeline.events.length,
            screenOcrEligibleEventCount: screenOcrAssessment.diagnostics?.eligibleEventCount
              ?? screenOcrResult.diagnostics?.eligibleEventCount
              ?? 0,
            screenOcrRepresentativeFrameCount: screenOcrSelection.frames.length,
            screenOcrRecognitionCallCount: screenOcrResult.diagnostics?.recognitionCallCount ?? 0,
            screenOcrScannedFrameCount: frameResult.extractionStats?.screen_ocr_scan_sample_count
              ?? screenOcrResult.diagnostics?.scannedFrameCount
              ?? 0,
            screenOcrCandidateCount: frameResult.extractionStats?.screen_ocr_candidate_count
              ?? screenOcrResult.diagnostics?.scannedFrameCount
              ?? 0,
            selectorBypassed,
            selectorBypassReason: screenOcrAssessment.reason,
            ocr_need_classification_ms: ocrNeedClassificationMs,
            ocr_region_scan_ms: screenOcrResult.timings?.regionScanMs
              ?? frameResult.runTimings?.screen_ocr_region_scan_ms
              ?? frameResult.extractionStats?.screen_ocr_region_scan_ms
              ?? null,
            ocr_recognition_ms: screenOcrResult.timings?.recognitionMs ?? null,
            ocr_dedupe_ms: screenOcrResult.timings?.dedupeMs ?? null,
            ocr_total_ms: screenOcrResult.timings?.totalMs ?? null,
          } : {}),
          ...(singleRecipeOnly ? {
            selectorAnnotatedFrameCount: normalizedSelectedFrames.length,
            ...(singleAnnotationSubsetActive ? { selectedCandidateIndices } : {}),
            annotationFrameHash,
            annotatedFrameEvidenceSummary,
            finalInputImageCount: selectedFrames.length,
          } : {}),
          selectedFrameCount: selectedFrames.length,
          deterministicCandidateCompletionCount,
          ...
          onscreenAmountRecoveryCount: validation.recoveries.length,
          ...(singleRecipeOnly ? {
            candidateSamplerDurationSec: candidateSamplerDiagnostics?.durationSec ?? null,
            candidateSamplerParsedCueCount: candidateSamplerDiagnostics?.parsedCueCount ?? null,
            candidateSamplerEligibleCueCount: candidateSamplerDiagnostics?.eligibleCueCount ?? null,
            candidateSamplerNoiseCueCount: candidateSamplerDiagnostics?.noiseCueCount ?? null,
            candidateSamplerNeutralCueCount: candidateSamplerDiagnostics?.neutralCueCount ?? null,
            candidateSamplerOutOfRangeCueCount: candidateSamplerDiagnostics?.outOfRangeCueCount ?? null,
            candidateSamplerInvalidCueCount: candidateSamplerDiagnostics?.invalidCueCount ?? null,
            candidateSamplerInvalidFrameCount: candidateSamplerDiagnostics?.invalidFrameCount ?? null,
            candidateSamplerDuplicateFrameCount: candidateSamplerDiagnostics?.duplicateFrameCount ?? null,
            candidateSamplerCueAlignedCount: candidateSamplerDiagnostics?.cueAlignedCandidateCount ?? null,
            candidateSamplerCenterFallbackCount: candidateSamplerDiagnostics?.centerFallbackCandidateCount ?? null,
            candidateSamplerTimelineCoverageRatio: candidateSamplerDiagnostics?.candidateTimelineCoverageRatio ?? null,
            candidateSamplerMaxGapSec: candidateSamplerDiagnostics?.candidateMaxGapSec ?? null,
            candidateSamplerSelectionHash: candidateSamplerDiagnostics?.selectionHash ?? null,
          } : {}),
        ...
        await writeFailure(resultDir, error, {
          ...
          selectorModel,
          ...(singleRecipeOnly ? {
            clientVersion: SINGLE_CLIENT_VERSION,
            selectorPromptVersion: SINGLE_SELECTOR_PROMPT_VERSION,
            finalPromptVersion: SINGLE_FINAL_PROMPT_VERSION,
            executionConfigSignature,
            singleRecipeProfile: "fast12-v13-index6-event-signature",
          } : {}),
          frameCacheDir: frameResult.frameDir,
          frameCount: frames.length,
          candidateFrameCount: candidateFrames.length,
