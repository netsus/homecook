...
const FRAME_PROGRESS_PREFIX = "__HOMECOOK_PROGRESS__";
const FRAME_PROGRESS_STAGES = new Set(["video_download", "frame_extraction"]);
...
async function runCommand(command, args, {
  ...
  return new Promise((resolve, reject) => {
    ...
    const publishLine = (line) => {
      if (typeof onOutputLine !== "function") return;
      try {
        onOutputLine(line);
      } catch {
        // Output telemetry callbacks must not change command execution.
      }
    };
    ...
    child.on("close", async (code) => {
      clearTimeout(timer);
      if (settled) return;
      for (const line of Object.values(lineBuffers)) {
        if (line) publishLine(line);
      }
...
export function buildLeakSafeSandboxProfile({
  safeDir,
  homeDir = homedir(),
  codexBinary = process.env.YOUTUBE_I031_CODEX_BIN || "codex",
} = {}) {
  ...
  if (path.isAbsolute(codexBinary)) {
    systemReadRoots.push(path.resolve(path.dirname(codexBinary), ".."));
  }
  return {
    blockedRoots,
    profile: [
...
export async function runCodexExec({
  ...
  const boundaryPath = `${outputPath}.model-read-boundary.json`;
    const { profile, blockedRoots } = buildLeakSafeSandboxProfile({ safeDir, codexBinary });
  try {
    const safeImages = [];
    ...
    if (outputSchema) {
      const schemaPath = path.join(safeDir, "output-schema.json");
    ...
    await runCommandImpl(sandboxExecPath, ["-p", profile, codexBinary, ...codexArgs], {
      input: prompt,
      cwd: safeDir,
      timeoutMs,
      logPath,
...
export async function defaultExtractFrames({
  ...
  if (frameOptions.screenOcrScan) {
    args.push(
      "--screen-ocr-scan",
      "--screen-ocr-candidate-limit",
      String(frameOptions.screenOcrCandidateLimit ?? 48),
      "--screen-ocr-change-threshold",
      String(frameOptions.screenOcrChangeThreshold ?? 0.08),
      "--screen-ocr-min-gap",
      String(frameOptions.screenOcrMinGapSec ?? 0.4),
    );
  }
  if (noCache) args.push("--no-cache");
  await runCommandImpl("python3", args, {
    cwd: PROJECT_ROOT,
    timeoutMs,
    logPath,
    onOutputLine(line) {
      if (!line.startsWith(FRAME_PROGRESS_PREFIX) || typeof onProgress !== "function") return;
      try {
        const progress = JSON.parse(line.slice(FRAME_PROGRESS_PREFIX.length));
        if (!FRAME_PROGRESS_STAGES.has(progress.stage)) return;
        const duration = Number.isInteger(progress.videoDurationSeconds)
          && progress.videoDurationSeconds >= 1
          && progress.videoDurationSeconds <= 86_400
          ? progress.videoDurationSeconds
          : null;
        onProgress(progress.stage, duration);
      } catch {
  ...
  const screenOcrCandidates = frames
    .filter((frame) => String(frame.reason ?? "").startsWith("screen-ocr:"))
    .map((frame) => ({
      ...frame,
      region: String(frame.reason).slice("screen-ocr:".length).split(":", 1)[0] || "unknown",
    }));
  return {
    ...
    runTimings: managedResult.runTimings ?? {
      ...
      scene_scan_ms: null,
      frame_write_ms: null,
      screen_ocr_region_scan_ms: null,
    },
  };
}
...
export function createCodexVisionClient(options = {}) {
  ...
  return {
    async generate({ prompt, videoUrl = null, cacheText = "" }) {
      ...
      try {
        options.onProgress?.("model_analysis", null);
      } catch {
        // Progress telemetry is best-effort and never changes extraction.
      }
