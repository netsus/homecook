...
const INGREDIENT_TOKEN_RE = /(새우|들기름|고추장|된장|진간장|간장|액젓|쯔유|맛술|알룰로스|마늘|고춧가루|후추|소금|부추|미나리|메밀|연어|오이|달걀|계란|곱창|항정살|마늘쫑|도토리묵|묵|열무|칼국수|멸치)/u;
...
function normalizeRecipeTitle(rawTitle) {
...
function timelineCandidates(refs) {
  const rows = refs
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    for (const part of parts) {
      const titleHint = normalizeRecipeTitle(part);
...
function titleAndTagCandidates(input, refs, contextText, candidates, seen) {
  ...
  for (const raw of [...titleParts, ...tagParts, ...descriptionTimelineTitles]) {
    const titleHint = normalizeRecipeTitle(raw);
...
export function buildEvidencePacketBundle(input = {}) {
  ...
  const { candidates, seen } = timelineCandidates(refs);
