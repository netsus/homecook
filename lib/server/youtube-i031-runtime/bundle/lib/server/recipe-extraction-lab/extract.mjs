...
const UNIT_NORMALIZER_VERSION = "unit-normalizer-v2-parenthetical-reference";
...
const PARENTHETICAL_REFERENCE_UNIT_RE = new RegExp(
  String.raw`^\s*(${UNIT_WORD_SOURCE})\s*\(\s*(?:약\s*)?${AMOUNT_RANGE_RE_SOURCE}\s*(?:${UNIT_WORD_SOURCE})\s*\)\s*$`,
  "i",
);
...
const STEW_SEASONING_RECOVERY_RULES = [
  { name: "설탕", amount: null, unit: null, optional: false, groupLabel: null, sourceRe: /설탕/ },
  { name: "양파", amount: null, unit: null, optional: false, groupLabel: null, sourceRe: /양파/ },
  { name: "고춧가루", amount: null, unit: null, optional: false, groupLabel: null, sourceRe: /고춧가루/ },
  { name: "소금", amount: null, unit: null, optional: true, groupLabel: "간 맞추기", sourceRe: /소금/ },
  { name: "새우젓", amount: null, unit: null, optional: true, groupLabel: "간 맞추기", sourceRe: /새우젓/ },
...
function normalizeUnit(unit) {
  ...
  const referenceUnit = text.match(PARENTHETICAL_REFERENCE_UNIT_RE)?.[1] ?? text;
  const squashed = referenceUnit.replace(/\s+/g, "");
...
function hasIngredient(recipe, name) {
  ...
  return recipe.ingredients.some((ingredient) => {
    const ingredientKey = keyOf(ingredient.name);
    if (ingredientKey === targetKey) return true;
    return targetKey.length >= 2
      && ingredientKey.length >= 2
      && (ingredientKey.includes(targetKey) || targetKey.includes(ingredientKey));
  });
}
  ...
  for (const recipe of recipes) {
    ...
    if (addedNames.size > 0) {
      ...
      if (addedNames.has("소금") && !stepBlob.includes(keyOf("소금"))) {
        const seasoningChoices = ["소금"];
        if (hasIngredient(recipe, "새우젓")) seasoningChoices.push("새우젓");
        if (hasIngredient(recipe, "김치 국물")) seasoningChoices.push("김치 국물");
        recipe.steps.push(`모자란 간은 ${seasoningChoices.join("이나 ")}으로 맞춘다.`);
      }
...
export async function extractRecipeFromSources(input, deps) {
  ...
  const singleRecipeMode = recipeMode === "single";
  const recipeHints = singleRecipeMode ? [] : buildRecipeCandidateHints(input);
  const effectiveUseEvidencePackets = !singleRecipeMode && useEvidencePackets;
  const sourceText = publicSourceMode
    ? (publicSourceBundle?.publicSourceText ?? "")
    : buildSourceText(input, { recipeMode });
  const evidencePacketBundle = !publicSourceMode && effectiveUseEvidencePackets ? buildEvidencePacketBundle(input) : null;
  const sendVideoUrl = Boolean(useVisual && youtubeUrl && !publicSourceMode && !(effectiveUseEvidencePackets && packetPromptTextOnly));
  ...
  if (singleRecipeMode && recipes.length !== 1) {
    throw new Error(`SINGLE_RECIPE_CONTRACT: expected exactly 1 recipe, received ${recipes.length}`);
  }
  const splitResult = singleRecipeMode
    ? { recipes, splitCount: 0 }
    : splitCombinedRecipes(recipes, recipeHints);
  recipes = splitResult.recipes;
  const derivativeMergeResult = singleRecipeMode
    ? { recipes, mergeCount: 0 }
    : mergeDerivativeRecipes(recipes);
  ...
  const stewSeasoningRecoveryResult = singleRecipeMode
    ? { recipes, recoveryCount: 0 }
    : recoverSourceMentionedStewSeasonings(recipes, sourceText, {
      publicSourceBundle: publicSourceMode ? publicSourceBundle : null,
    });
  recipes = stewSeasoningRecoveryResult.recipes;
  const packetIngredientStepRecoveryResult = singleRecipeMode
    ? { recipes, recoveryCount: 0 }
    : recoverPacketIngredientStepMentions(recipes);
  ...
  if (singleRecipeMode && recipes.length !== 1) {
    throw new Error(`SINGLE_RECIPE_CONTRACT: expected one non-empty recipe, received ${recipes.length}`);
  }
  ...
  return {
    ...
    meta: {
      ...
      ...generationMeta,
      recipeMode,
      singleRecipeOnly: singleRecipeMode,
      unitNormalizerVersion: UNIT_NORMALIZER_VERSION,
      evidencePacketMode: Boolean(effectiveUseEvidencePackets),
