...
export const PROMPT_VERSION = "step-process-density-1";
...
6. 만들기(steps): 명령형 한국어 한 문장씩, 영상 진행 순서대로 쓰되 실제 공정·상태 전환 1개당 한 단계가 되게 한다. 손질, 양념 만들기, 기구 세팅, 가열 조리, 휴지, 성형, 담기처럼 목적·대상·열 상태가 바뀌면 단계를 나누고, 서로 다른 공정을 억지로 병합하지 않는다. 반대로 같은 팬·냄비·볼에서 한 공정을 이어가는 재료 투입, 섞기, 뒤집기, 불 세기·온도·시간 조절, 졸이기 정도 확인은 여러 얇은 단계로 쪼개지 말고 한 단계 안에 모아 핵심 명사가 흩어지지 않게 한다. 각 단계에는 핵심 재료명과 구체 동작을 쓰고, 기구·온도·시간·불세기·상태 기준 중 화면이나 발화에서 보이는 명사를 1개 이상 함께 넣는다. 너무 짧은 "넣는다", "섞는다" 단독 단계는 피한다. 재료 투입 순서가 맛에 영향을 주면 영상의 실제 순서를 따르고, 재료가 처음 들어가는 단계에는 그 재료명을 명시한다. 인사말·잡담·시식 멘트는 제외한다.
