...
export const PROMPT_VERSION = "multi-recipe-hints-1";
...
5. 분량(amount/unit): 텍스트 명시 > 발화 > 화면 자막 > 시각 추정 순으로 최대한 채운다. amount는 숫자·범위·분수만 넣고, "적당량/약간/취향껏/넉넉히/1:1 비율" 같은 정성·비율 표현은 amount에 넣지 않는다. 단위는 가능하면 개, g, ml, 큰술, 작은술, 컵, 장, 줌, 팩처럼 표준 단위로 분리한다. amountBasis로 근거를 표기한다(stated/spoken/onscreen/visual-estimate). 정말 판단 불가일 때만 amount를 null로 둔다.
6. 만들기(steps): 명령형 한국어 한 문장씩, 영상 진행 순서대로. 조리기구 설정(오븐/에어프라이어 온도·시간), 불 세기, 시간, 상태 판단 기준을 포함한다. 재료 투입 순서가 맛에 영향을 주면 영상의 실제 순서를 따른다. 인사말·잡담·시식 멘트는 제외한다.
