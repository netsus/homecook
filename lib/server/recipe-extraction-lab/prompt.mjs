...
export const PROMPT_VERSION = "iter12-train-diagnostic-recovery-3";
...
4. 재료(ingredients): 조리에 실제로 쓰이는 식재료·양념만. name은 한국어 표준명, 외국어·원문 표기는 nameAliases에. 만들기 단계에 등장하지 않는 재료는 넣지 마라. 설명란·댓글·자막에 "재료명 수량단위" 목록이 있으면 먼저 그 목록을 훑고, 실제 조리 장면에 쓰인 항목만 recipes[].ingredients에 반영한다. 이벤트, 구매 인증, 댓글 이벤트, 선물, 쿠폰, 할인, 배송, 상품 출고, 추첨 안내의 수량은 실제 조리 투입량이 아니므로 재료 분량으로 쓰지 않는다. 출력 직전 각 recipes[]마다 ingredients[]를 steps[]와 대조해, 양념장·고춧가루 종류·젓갈류(예: 새우젓)·간 조절 재료·곁들임 소스·고명·마무리 향채·마무리 토핑처럼 단계에서 쓰인 재료가 빠졌는지 점검하고 빠진 항목은 보충한다. 국/찌개는 자동자막이 깨져도 영상 화면·자막 카드·투입 장면을 우선 확인해 양파·설탕·소금·후추·고추류처럼 단맛/간/향을 조절하는 부재료가 보이면 소량이어도 남긴다. 특히 김치찌개·돼지고기찌개류는 김치·고기·쌀뜨물만으로 끝내지 말고, 국간장 뒤에 들어가는 설탕/양파, 간 맞추기용 소금/새우젓, 마지막 후추, 청양고추와 별도로 보이는 붉은 고추를 최종 체크리스트로 재대조한다. 붉은 곁들임 소스나 파채소스처럼 색이 있는 소스에 가루 양념을 넣는 장면이 보이면 고춧가루·후추 같은 분말 양념도 누락하지 않는다. 냉파스타·국수·샐러드의 고명은 실제로 올린 재료만 남기고, 비슷해 보인다는 이유만으로 들깨가루·쪽파 같은 고명을 추가하지 않는다. 반대로 단계 근거가 전혀 없는 재료는 보충하지 않는다.
