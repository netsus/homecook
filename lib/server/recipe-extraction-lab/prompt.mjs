...
export const PROMPT_VERSION = "iter17-evidence-packet-artifacts";
export const URL_ONLY_PROMPT_VERSION = "url-only-gpt-direct-v1";
...
export function buildExtractionPrompt({ video, sourceText, useVisual, evidencePackets }) {
  ...
  const packetClause = Array.isArray(evidencePackets) && evidencePackets.length > 0
...
export function buildUrlOnlyExtractionPrompt({ youtubeUrl }) {
  return `영상 속 여러 레시피의 재료와 조리 단계를 최대한 정확히 추출해 주세요.

대상 링크: ${youtubeUrl ?? ""}

중요 조건:
- 이 저장소의 golden.json, train/validation 정답, 이전 추출 결과, grade 파일은 절대 읽지 마세요. 데이터 누수를 막기 위함입니다.
- 시작 입력은 위 YouTube URL 하나라고 가정하세요. 공개적으로 접근 가능한 영상 제목/설명/자막/챕터/화면 단서만 사용하세요.
- 여러 레시피가 있으면 반드시 레시피별로 분리하세요.
- 재료는 name, nameAliases, amount, unit, amountBasis, optional, groupLabel 형식으로 최대한 정확히 적으세요.
- amountBasis는 stated, spoken, onscreen, visual-estimate 중 하나만 사용하세요.
- 수량이 명시되지 않으면 영상/자막/설명 근거로 추정 가능한 경우만 visual-estimate라고 표시하고, 정말 모르면 amount와 unit을 null로 두세요.
- source_note 같은 근거 메모가 필요하면 추가 필드로 넣어도 되지만, 필수 구조는 아래 JSON 스키마를 따르세요.
- 조리 단계는 초보자가 따라 할 수 있게 순서대로, 한 단계에 하나의 핵심 공정이 드러나게 작성하세요.
- 출력은 한국어로 작성하세요.
- 파일 수정은 하지 마세요. 최종 답변은 JSON만 출력하세요.

레시피 분리 규칙:
1. 영상에 실제 조리 과정이 있는 요리만 recipes[]에 넣으세요.
2. 한 영상에 여러 요리가 있으면 제목, 설명, 챕터, 자막, 화면 흐름을 대조해 빠짐없이 분리하세요.
3. 한 요리의 소스, 토핑, 곁들임만 따로 나온 장면은 별도 recipe로 만들지 말고 관련 recipe의 재료와 단계에 합치세요.
4. 사이드 메뉴가 독립 조리 과정을 가진 별도 요리인지, 본체 요리의 곁들임인지 구분하세요.

재료 규칙:
1. 조리에 실제로 쓰인 식재료와 양념만 넣으세요.
2. "초록색 줄기채소", "갈색 소스" 같은 색/상태 이름은 최후의 수단입니다. 화면, 자막, 발화, 설명에서 구체 재료명이 보이면 구체명으로 쓰세요.
3. 완제품 소스, 양념장, 원물 채소, 분말 양념을 서로 마음대로 바꾸지 마세요.
4. 재료 목록과 만들기 단계를 마지막에 대조해서, 단계에 쓰인 핵심 재료가 ingredients[]에서 빠지지 않게 하세요.
5. 단계에 전혀 쓰이지 않는 재료는 넣지 마세요.

분량 규칙:
1. 설명/자막/챕터의 명시값 > 발화 > 화면 자막 > 시각 추정 순서로 믿으세요.
2. amount는 숫자, 범위, 분수만 넣고 "약간", "적당량", "취향껏" 같은 말은 amount에 넣지 마세요.
3. unit은 큰술, 작은술, 컵, g, ml, 개, 장, 줌, 팩처럼 분리해서 쓰세요.
4. 영상으로 추정한 값은 amountBasis를 반드시 visual-estimate로 두세요.

만들기 단계 규칙:
1. 실제 공정과 상태 변화가 드러나게 쓰세요.
2. "넣는다", "섞는다"처럼 대상이 없는 문장은 피하고, 핵심 재료명이나 조리 대상을 문장에 넣으세요.
3. 영상 진행 순서를 따르세요.
4. 인사말, 잡담, 시식 멘트, 단순 완성 멘트는 제외하세요.
