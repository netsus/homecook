...
export const PROMPT_VERSION = "multi-recipe-hints-2";
...
5. 분량(amount/unit): 텍스트 명시 > 발화 > 화면 자막 > 시각 추정 순으로 최대한 채운다. amount는 숫자·범위·분수만 넣고, "적당량/약간/취향껏/넉넉히/1:1 비율" 같은 정성·비율 표현은 amount에 넣지 않는다. 단위는 출처에 나온 단위를 amount와 unit로 분리해 보존하고, g↔ml, 컵↔ml, 숟가락↔g처럼 다른 단위로 환산하지 않는다. amountBasis로 근거를 표기한다(stated/spoken/onscreen/visual-estimate). 정말 판단 불가일 때만 amount를 null로 둔다.
6. 만들기(steps): 명령형 한국어 한 문장씩, 영상 진행 순서대로. 한 단계에는 하나의 조리 동작만 담고, 손질·투입·섞기·가열·담기처럼 여러 동작이 이어지면 단계 수를 늘려 나눈다. 조리기구 설정(오븐/에어프라이어 온도·시간), 불 세기, 시간, 상태 판단 기준을 포함한다. 재료 투입 순서가 맛에 영향을 주면 영상의 실제 순서를 따른다. 인사말·잡담·시식 멘트는 제외한다.
