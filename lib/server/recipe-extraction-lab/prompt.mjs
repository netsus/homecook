...
export const PROMPT_VERSION = "multi-recipe-step-token-recall-1";
...
5. 분량(amount/unit): 텍스트 명시 > 발화 > 화면 자막 > 시각 추정 순으로 최대한 채워 amount null을 피한다. amount는 숫자·범위·분수만 넣고, "적당량/약간/취향껏/넉넉히/1:1 비율" 같은 정성·비율 표현은 amount에 넣지 않는다. 정확한 수치가 없어도 영상에서 개수, 스푼 수, 컵·줌·꼬집·팩·봉·모 같은 단위, 한 번에 넣는 대략적 양이 보이면 거짓 정밀값 대신 정직한 근사값을 amount에 넣고 amountBasis는 반드시 "visual-estimate"로 둔다. visual-estimate 재료도 조리에 실제로 쓰인 경우에만 ingredients에 넣고, 그 재료가 처음 투입되는 만들기 단계에는 재료명을 반드시 적는다. 단위는 출처나 화면에서 판단한 단위를 amount와 unit로 분리해 보존하고, g↔ml, 컵↔ml, 숟가락↔g처럼 다른 단위로 환산하지 않는다. amountBasis로 근거를 표기한다(stated/spoken/onscreen/visual-estimate). 텍스트·발화·자막·영상 어느 쪽에서도 수량 단서가 전혀 없을 때만 amount를 null로 둔다.
6. 만들기(steps): 명령형 한국어 한 문장씩, 영상 진행 순서대로 쓰되 별개 행위와 공정 전환을 놓치지 않게 나눈다. 새 재료 투입, 손질, 양념 만들기, 기구 세팅, 가열 시작, 불 세기·온도·시간 변경, 휴지, 성형, 뒤집기, 졸이기, 담기처럼 채점자가 별도 공정으로 셀 수 있는 장면은 서로 병합하지 않는다. 단순히 같은 그릇 안에서 이어지는 동작이라도 새 핵심 재료가 들어가거나 상태가 바뀌면 별도 단계로 둔다. 너무 짧은 "넣는다", "섞는다" 단독 단계는 피하되, 각 단계에는 핵심 재료명 + 구체 동작 + 기구·온도·시간·불세기·상태 기준 중 보이는 정보를 함께 포함한다. 재료 투입 순서가 맛에 영향을 주면 영상의 실제 순서를 따르고, 재료가 처음 들어가는 단계에는 그 재료명을 명시한다. 인사말·잡담·시식 멘트는 제외한다.
