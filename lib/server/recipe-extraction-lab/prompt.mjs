...
export const PROMPT_VERSION = "iter11-semantic-count-recovery-1";
...
2. 다중 레시피 영상이면 [SOURCE: recipe_candidate_hints], 제목, 설명란 타임라인을 체크리스트처럼 대조하고, 먼저 실제 조리 장면이 있는 후보를 내부적으로 전부 열거한 뒤 recipes[]에 빠짐없이 넣는다. 후보가 A&B, A/B, A·B, A+B처럼 결합되어 있으면 실제 조리 장면이 각각 있는 한 별도 recipes[]로 분리한다. 제목이 "A와 B" 또는 "A과 B"처럼 와/과로 결합되어 있고 양쪽이 각각 요리명 후보라면 같은 기준으로 분리한다. 단, 한 요리의 소스·토핑·곁들임만 뜻하면 과분리하지 않는다.
3. 자막이 빈약하거나 노이즈가 있으면 영상 화면·발화·조리 흐름으로 누락 구간을 채운다. 설명란 타임라인에 있는 후보를 뺐다면, 그 후보가 조리 장면이 없거나 시식/구매/외식 언급뿐인 경우여야 한다. 여러 후보가 있을 때는 후보별로 제목, 재료, 단계 근거를 따로 모으고, 각 후보의 근거끼리 섞지 않는다.
4. 재료(ingredients): 조리에 실제로 쓰이는 식재료·양념만. name은 한국어 표준명, 외국어·원문 표기는 nameAliases에. 만들기 단계에 등장하지 않는 재료는 넣지 마라. 설명란·댓글·자막에 "재료명 수량단위" 목록이 있으면 먼저 그 목록을 훑고, 실제 조리 장면에 쓰인 항목만 recipes[].ingredients에 반영한다. 출력 직전 각 recipes[]마다 ingredients[]를 steps[]와 대조해, 양념장·고춧가루 종류·젓갈류(예: 새우젓)·간 조절 재료·마무리 토핑처럼 단계에서 쓰인 재료가 빠졌는지 점검하고 빠진 항목은 보충한다. 반대로 단계 근거가 전혀 없는 재료는 보충하지 않는다.
5. 분량(amount/unit): 텍스트 명시 > 발화 > 화면 자막 > 시각 추정 순으로 최대한 채워 amount null을 피한다. amount와 unit은 같은 출처에서 한 쌍으로 고르고, 더 높은 우선순위 출처가 값과 단위 또는 단위 family(g/ml/컵/큰술/작은술/개 등)를 명시하면 화면상 양이 다르게 보여도 그 명시 단위를 unit에 쓴다. "굴소스 1큰술", "양파 반 개", "물 500ml", "두부 1모"처럼 재료명과 수량단위가 붙어 있거나 괄호 안에 있으면 amount에는 숫자·범위·분수만, unit에는 원 단위 family를 표준 한국어 단위로 분리한다. amount는 숫자·범위·분수만 넣고, "적당량/약간/취향껏/넉넉히/1:1 비율" 같은 정성·비율 표현은 amount에 넣지 않는다. 다만 말로는 정성 표현뿐이어도 화면에서 투입량이 보이면 1 꼬집, 1 작은술, 1 큰술, 1 줌, 1/2 컵처럼 관찰 가능한 단위와 근사 숫자를 넣고 amountBasis는 반드시 "visual-estimate"로 둔다. 정확한 수치가 없어도 영상에서 개수, 스푼 수, 컵·줌·꼬집·팩·봉·모 같은 단위, 한 번에 넣는 대략적 양이 보이면 거짓 정밀값 대신 정직한 근사값을 amount에 넣는다. 작은술/티스푼/tsp/teaspoon은 unit "작은술", 큰술/밥숟가락/T/tbsp/tablespoon은 unit "큰술"로 구분한다. visual-estimate 재료도 조리에 실제로 쓰인 경우에만 ingredients에 넣고, 그 재료가 처음 투입되는 만들기 단계에는 재료명을 반드시 적는다. 단위는 선택한 출처의 단위를 amount와 unit로 분리해 보존하고, 다른 출처의 시각 단위와 섞거나 g↔ml, 컵↔ml, 숟가락↔g처럼 다른 단위로 환산하지 않는다. amountBasis로 근거를 표기한다(stated/spoken/onscreen/visual-estimate). 텍스트·발화·자막·영상 어느 쪽에서도 수량 단서가 전혀 없을 때만 amount를 null로 둔다.
