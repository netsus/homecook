...
const UNIT_WORD_RE = /(큰\s*술|큰스푼|큰숟갈|큰숟가락|밥숟갈|밥숟가락|숟갈|숟가락|스푼|테이블\s*스푼|작은\s*술|작은스푼|작은숟갈|작은숟가락|티스푼|tbsp\.?|tbsps\.?|tablespoons?|tsp\.?|tsps\.?|teaspoons?|T|t|컵|cups?|g|그램|kg|ml|밀리리터|l|리터|개|알|쪽|장|매|줄|팩|봉|줌|꼬집|모|인분|덩이|대|포기|송이|토막|조각|캔|병|공기)/i;
