...
import {
  ...
  buildSourceText,
  buildUrlOnlyExtractionPrompt,
  PROMPT_VERSION,
  URL_ONLY_PROMPT_VERSION,
} from "./prompt.mjs";
 ...
 * @param {object} deps   { llm: createCodexVisionKeyframesClient(), useVisual?:boolean, sourceMode?:"source-text"|"url-only" }
...
export async function extractRecipeFromSources(input, deps) {
  ...
  const urlOnlyMode = sourceMode === "url-only";
  const recipeHints = urlOnlyMode ? [] : buildRecipeCandidateHints(input);
  const sourceText = urlOnlyMode ? "" : buildSourceText(input);
  const evidencePacketBundle = !urlOnlyMode && useEvidencePackets ? buildEvidencePacketBundle(input) : null;
  const sendVideoUrl = urlOnlyMode
    ? Boolean(useVisual && youtubeUrl)
    : Boolean(useVisual && youtubeUrl && !(useEvidencePackets && packetPromptTextOnly));
  const prompt = urlOnlyMode
    ? buildUrlOnlyExtractionPrompt({ youtubeUrl })
    : buildExtractionPrompt({
      video,
      sourceText,
  ...
  const generation = await llm.generate({
    ...
    cacheText: urlOnlyMode ? `url-only:${youtubeUrl ?? ""}` : sourceText,
  });
  ...
  return {
    ...
    meta: {
      ...
      promptVersion: urlOnlyMode ? URL_ONLY_PROMPT_VERSION : PROMPT_VERSION,
      model,
      cached,
