...
function isRecipeThemesRead({ method, path, search, body }) {
  ...
  const orders = params.getAll("order").flatMap((order) => order.split(","));
...
function isRecipeListRead({ method, path, search, body }) {
  ...
  if (params.has("or")) {
    ...
    // PostgREST's SDK wraps the complete OR expression in parentheses.
    const expression = cursor[0]?.startsWith("(") && cursor[0]?.endsWith(")")
      ? cursor[0].slice(1, -1)
      : cursor[0];
    if (
      cursor.length !== 1
