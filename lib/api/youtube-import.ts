...
export async function createYoutubeCandidateDraft(
  body: YoutubeCandidateDraftBody,
): Promise<ApiResponse<YoutubeCandidateDraftData>> {
  try {
    const response = await fetch(
      "/api/v1/recipes/youtube/candidate-drafts",
      withE2EAuthOverrideHeaders({
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
    ...
    let payload: ApiResponse<YoutubeCandidateDraftData> | null = null;

    try {
      payload = (await response.json()) as ApiResponse<YoutubeCandidateDraftData>;
    } catch {
      return {
        success: false,
        data: null,
