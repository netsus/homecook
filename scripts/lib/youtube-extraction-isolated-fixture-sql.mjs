...
export function buildIsolatedYoutubeWorkerSyntheticFixtureSql({
  allowedSnapshotDigest,
  extractorMode = DEFAULT_YOUTUBE_EXTRACTION_WORKER_EXTRACTOR_MODE,
  fingerprintKeyVersion = DEFAULT_YOUTUBE_EXTRACTION_WORKER_FINGERPRINT_KEY_VERSION,
  jobId,
  jtiHash,
  nowEpoch,
  ...
  if (
    ...
    || typeof fingerprintKeyVersion !== "string"
    || fingerprintKeyVersion.length === 0
  ) throw new Error("isolated fixture policy authority is invalid");
  return Object.freeze({
    sql: [
      "insert into private.youtube_extraction_current_policy(policy_key,policy_version,extractor_mode,pipeline_identity,result_affecting_options,fingerprint_key_version,enabled) values ('primary',:'policy_version',:'extractor_mode',:'pipeline_identity',:'result_affecting_options'::jsonb,:'fingerprint_key_version',true);",
      ...
      "insert into public.youtube_extraction_jobs(id,user_id,youtube_video_id,request_fingerprint,request_fingerprint_key_version,release_policy_key,policy_version,policy_snapshot_digest,extractor_mode,pipeline_identity,result_affecting_options,submission_mode,status,attempt_count,max_attempts,available_at) values (:'job_id',:'user_id','synthetic01',:'request_fingerprint',:'fingerprint_key_version','primary',:'policy_version',:'allowed_snapshot_digest',:'extractor_mode',:'pipeline_identity',:'result_affecting_options'::jsonb,'background_notify','queued',0,3,to_timestamp(:'now_epoch'));",
    ].join("\n"),
    variables: Object.freeze({ allowed_snapshot_digest: allowedSnapshotDigest, extractor_mode: extractorMode, fingerprint_key_version: fingerprintKeyVersion, job_id: jobId, jti_hash: jtiHash, nickname: `r2-${runIdentity.slice(0, 8)}`, now_epoch: String(nowEpoch), pipeline_identity: pipelineIdentity, policy_version: String(policyVersion), release_sha: releaseSha, request_fingerprint: allowedSnapshotDigest, result_affecting_options: JSON.stringify(resultAffectingOptions), schema_identity: schemaIdentity, social_id: `r2-${runIdentity}`, user_id: userId }),
    allowedVariableNames: Object.freeze(["allowed_snapshot_digest", "extractor_mode", "fingerprint_key_version", "job_id", "jti_hash", "nickname", "now_epoch", "pipeline_identity", "policy_version", "release_sha", "request_fingerprint", "result_affecting_options", "schema_identity", "social_id", "user_id"]),
  });
}
