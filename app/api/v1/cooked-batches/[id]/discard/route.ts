...
import { callCookedBatchRpc, isUuid, parseBatchDiscardRequest, projectCookedBatchMutationData } from "@/lib/server/cooked-batches";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const parsed = parseBatchDiscardRequest(body.value); if (!parsed.ok) return fail("VALIDATION_ERROR", "요청 값을 확인해 주세요.", 422, parsed.fields);
  const result = await callCookedBatchRpc(authorized.client, "discard_cooked_batch", {
    ...authorized.authorityArgs, p_batch_id: id, p_idempotency_key: key.key,
    p_discarded_g: parsed.value.discardedG, p_reason: parsed.value.reason,
  ...
  return data ? ok(data) : fail("INTERNAL_ERROR", "폐기 결과를 확인하지 못했어요.", 500);
}
