...
import { isUuid, type LeftoverDishRow } from "@/lib/server/leftovers";
import {
  ensurePublicUserRow,
...
interface LeftoverKeepUpdateQuery {
  eq(column: string, value: string): LeftoverKeepUpdateQuery;
  select(columns: string): LeftoverKeepUpdateQuery;
  maybeSingle(): MaybeSingleResult<LeftoverKeepData>;
}

interface LeftoverDishesTable {
  select(columns: string): LeftoverSelectQuery;
  update(values: { stale_reviewed_at: string }): LeftoverKeepUpdateQuery;
}

interface LeftoverKeepDbClient {
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const dbClient = (createServiceRoleClient() ?? routeClient) as unknown as
    LeftoverKeepDbClient & UserBootstrapDbClient;
  ...
  const leftoverResult = await dbClient
    ...
    .select("id, user_id, recipe_id, status, cooked_at, eaten_at, auto_hide_at, stale_reviewed_at")
    .eq("id", leftoverId)
    .maybeSingle();
  ...
  const updateResult = await dbClient
    ...
    .eq("id", leftoverId)
    .eq("user_id", user.id)
    .select("id, status, stale_reviewed_at")
    .maybeSingle();
  ...
  return ok({
    ...
    stale_reviewed_at: updateResult.data.stale_reviewed_at,
  });
}
