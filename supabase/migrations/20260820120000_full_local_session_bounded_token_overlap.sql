...
as $function$
declare
  v_bounded_overlap boolean := false;
...
begin
  ...
  if not v_bounded_overlap and (
    p_access_token_expires_at < v_binding.binding_expires_at
    or p_binding_expires_at < v_binding.binding_expires_at
  ) then
    raise exception 'ACCOUNT_SESSION_STALE'
      using errcode = '55000',
        detail = 'HOMECOOK_SESSION_AUTHORITY_REASON::non_monotonic';
