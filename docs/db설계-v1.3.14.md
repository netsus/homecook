...
> 기준 문서: 요구사항 기준선 v1.7.9 / 화면정의서 v1.5.16 / API 설계 v1.2.18 / 유저 Flow맵 v1.3.16
>
>
> 작성: 킴실장
>
...
- v2 등급 band는 level 1-3 `새싹 집밥러`, 4-7 `집밥 러너`, 8-12 `주방 탐험가`, 13-20 `한상 메이커`, 21-34 `집밥 장인`, 35-49 `식탁 큐레이터`, 50+ `집밥 명장`이다.
...
사용자별 quest/tutorial quest 진행 상태. Quest definition은 MVP에서 서버 코드 상수로 관리한다.
...
| quest_type | text | NOT NULL | `standard` / `tutorial` |
...
- tutorial dismiss는 XP, level, badge award를 변경하지 않는다.
- quest completion은 별도 claim reward를 만들지 않는다.

## 11-2e. user_progress_notifications `33c-gamification`

XP toast와 badge/quest new 상태 표시를 위한 사용자별 notification outbox. Source action 성공 피드백을 보강하지만, notification 생성/seen 실패는 원래 action 결과를 바꾸지 않는다.
...
| notification_type | text | NOT NULL | `level_up` / `xp_awarded` / `badge_unlocked` / `quest_completed` |
| source_event_id | uuid | FK → user_progress_events(id), NULL 허용 | XP source event |
| payload_json | jsonb | NOT NULL, DEFAULT '{}' | toast 표시용 label/xp/badge/quest 요약 |
| priority | integer | NOT NULL, DEFAULT 4 | 표시 우선순위. `level_up=1`, `badge_unlocked=2`, `quest_completed=3`, `xp_awarded=4` |
...
- seen 처리는 멱등해야 한다.
- toast stack 우선순위는 서버가 정렬한다. 클라이언트는 `level_up > badge_unlocked > quest_completed > xp_awarded` 순서를 바꾸지 않는다.
...
| 25 | user_growth_activity_events | 사용자 성장/activity `growth-leveling-v2` |
| 26 | user_badge_awards | 사용자 성장/배지 `33c-gamification` |
| 27 | user_quest_progress | 사용자 성장/퀘스트 `33c-gamification` |
| 28 | user_progress_notifications | 사용자 성장/notification `33c-gamification` |
| 29 | admin_members | Admin Foundation `v1.3.8` |
| 30 | operational_events | Admin Foundation `v1.3.8` |
| 31 | admin_audit_logs | Admin Foundation `v1.3.8` |
| 32 | youtube_extraction_candidates | YouTube 다중 후보 `2026-05-30` |
