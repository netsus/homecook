...
| CR-004 | P0/P1 | `MANUAL_ONLY_RELEASE_CHECK` | live OAuth, 실제 YouTube API, 실제 Supabase/production seed, 모바일 실기기 확인 | 구현 계약과 별개로 release 전에 사람이 확인해야 하는 항목이다. RC-MO-01 live OAuth / return-to-action, RC-MO-02 real-device mobile smoke, RC-MO-03 live YouTube registration smoke, RC-MO-04 staging DB compatibility smoke, RC-MO-05 stale Manual Only classification, RC-MO-07 Web Share OS sheet는 evidence로 통과했다. RC-MO-06 account deletion/rejoin은 탈퇴/재로그인/화면 에러 없음은 통과했지만, 삭제 안내 문구와 달리 기존 저장/플래너 데이터가 그대로 보여 release 전 정책 결정 또는 수정이 필요하다. RC-MO-08 wake-lock은 실제 OS/browser 정책 의존 확인으로 별도 남아 있다. | `docs/workpacks/beta-release-manual-checklist-2026-05-04.md`의 `CR-004 Release Candidate Tracking` 표로 이관했다. delete-account action은 public beta 전에 숨기거나, 실제 데이터 삭제로 고치거나, recoverable soft-deactivation으로 공식 계약/문구를 바꿔야 한다. wake-lock은 cooking-heavy beta 또는 public launch 전에 별도 실기기 확인이 필요하다. |
...
| 13-3 | `DELETE /api/v1/users/me` | SETTINGS | 예 | 예 | 예 | 실패 기록 | RC-MO-06에서 탈퇴/재로그인/화면 에러 없음은 통과. 그러나 삭제 안내와 달리 기존 저장/플래너 데이터가 재로그인 후 유지되어 delete-account 정책 결정 또는 수정 필요. |
...
| 계정 삭제 / 동일 소셜 재가입 | `DELETE /users/me` | RC-MO-06 확인. 탈퇴/재로그인/화면 에러 없음은 통과했지만 삭제 안내와 달리 기존 저장/플래너 데이터가 유지되어 public beta 전에 hide/fix/contract-change 중 하나를 결정해야 한다. |
...
8. 확인완료(2026-05-24): account deletion/rejoin은 탈퇴/재로그인/화면 에러 없음까지 확인했다. 단, 기존 저장/플래너 데이터가 유지되어 delete-account 정책/구현/문구 불일치가 새 release decision으로 남았다.
9. 확인완료(2026-05-24): Web Share OS sheet는 Android Chrome과 iOS Safari에서 열림과 앱 복귀 정상까지 확인했다.
10. 다음: public beta 전에 delete-account action을 숨길지, 실제 삭제로 고칠지, recoverable soft-deactivation으로 공식 계약/문구를 바꿀지 결정한다.
