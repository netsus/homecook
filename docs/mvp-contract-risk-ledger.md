...
| CR-004 | P0/P1 | `MANUAL_ONLY_RELEASE_CHECK` | live OAuth, 실제 YouTube API, 실제 Supabase/production seed, 모바일 실기기 확인 | 구현 계약과 별개로 release 전에 사람이 확인해야 하는 항목이다. RC-MO-01 live OAuth / return-to-action, RC-MO-02 real-device mobile smoke, RC-MO-03 live YouTube registration smoke, RC-MO-04 staging DB compatibility smoke, RC-MO-05 stale Manual Only classification은 evidence로 통과했다. 원본 Manual Only 123개는 이미 확인됨, 아직 필요함, deferred, stale-candidate로 분류했고, 자동화 가능한 일부 항목은 테스트/문서로 닫았다. | `docs/workpacks/beta-release-manual-checklist-2026-05-04.md`의 `CR-004 Release Candidate Tracking` 표로 이관했다. 남은 release-gate 후보는 account deletion/rejoin처럼 실제로 보이는 destructive action만 안전한 테스트 계정으로 확인하거나 명시적으로 defer/hide 결정한다. |
...
| 오래된 acceptance 정리 | recipe save, shopping 10a/10b/11 일부, `_template`, 과거 wave/design polish 항목 | RC-MO-05 통과. recipe save/remove, shopping placeholder, leftovers auto-hide, YouTube URL shape, YouTube all-`절이기` 회귀는 닫았고, 디자인/미래방향은 deferred, 진짜 남은 destructive action은 별도 release-gate 후보로 분리. |
...
8. 다음: public beta 전에 보이는 destructive action만 안전한 테스트 계정으로 확인한다. 우선 후보는 account deletion/rejoin이다.
9. 다음: YouTube quota, 광범위한 URL/영상 품질 매트릭스, synonym matching 개선은 별도 hardening 작업으로 분리한다.
