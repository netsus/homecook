...
| CR-004 | P0/P1 | `MANUAL_ONLY_RELEASE_CHECK` | live OAuth, 실제 YouTube API, 실제 Supabase/production seed, 모바일 실기기 확인 | 구현 계약과 별개로 release 전에 사람이 확인해야 하는 항목이 남아 있다. | `docs/workpacks/beta-release-manual-checklist-2026-05-04.md`의 `CR-004 Release Candidate Tracking` 표로 이관했다. 남은 일은 각 항목의 실제 evidence를 채우는 것이다. |
...
2. 다음: staging/beta deploy SHA가 정해지면 `RC-MO-01`부터 `RC-MO-04`까지 실제 evidence를 채운다.
3. 다음: stale로 보이는 Manual Only는 `RC-MO-05`에서 후속 slice로 이미 닫혔는지 확인한 뒤 acceptance 정리 후보로 분리한다.
