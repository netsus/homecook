...
| CR-004 | P0/P1 | `MANUAL_ONLY_RELEASE_CHECK` | live OAuth, 실제 YouTube API, 실제 Supabase/production seed, 모바일 실기기 확인 | 구현 계약과 별개로 release 전에 사람이 확인해야 하는 항목이다. RC-MO-01 live OAuth / return-to-action, RC-MO-02 real-device mobile smoke, RC-MO-03 live YouTube registration smoke, RC-MO-04 staging DB compatibility smoke, RC-MO-05 stale Manual Only classification, RC-MO-07 Web Share OS sheet는 evidence로 통과했다. RC-MO-06 account deletion/rejoin은 정책을 private data cleanup + authored recipe 보존으로 확정하고 구현했다. redeploy 후 같은 smoke 재확인이 필요하다. RC-MO-08 wake-lock은 실제 OS/browser 정책 의존 확인으로 별도 남아 있다. | `docs/workpacks/beta-release-manual-checklist-2026-05-04.md`의 `CR-004 Release Candidate Tracking` 표로 이관했다. account deletion은 이 PR에서 구현/문서/문구를 정렬했고, 배포 후 staging smoke로 closed 처리한다. wake-lock은 cooking-heavy beta 또는 public launch 전에 별도 실기기 확인이 필요하다. |
...
| 13-3 | `DELETE /api/v1/users/me` | SETTINGS | 예 | 예 | 예 | 재확인 필요 | RC-MO-06에서 탈퇴/재로그인/화면 에러 없음은 통과했고, 이 PR에서 private data cleanup + authored recipe 보존 정책으로 구현/문서/문구를 정렬했다. redeploy 후 기존 저장/플래너 데이터가 보이지 않는지 재확인 필요. |
...
| 모바일 실기기 / visual | shopping complete popup, cook mode, leftovers, cooking method color | RC-MO-02 통과. 막히는 화면, 안 눌리는 버튼, 하단 CTA 가림, 서버 에러, 무한 로딩 없음. 세부 visual/taste/animation은 design backlog로 분리. settings wake lock의 실제 브라우저 정책 동작은 BETA-QA-008로 별도 유지. |
...
| 계정 삭제 / 동일 소셜 재가입 | `DELETE /users/me` | RC-MO-06 policy fix 구현. 탈퇴 시 개인 기록은 삭제하고 직접/유튜브 등록 레시피는 작성자 정보 없이 보존한다. redeploy 후 같은 소셜 계정 재로그인 smoke 재확인 필요. |
| Wake lock 실제 브라우저 동작 | `PATCH /users/me/settings`, COOK_MODE | RC-MO-08 todo. 일반 모바일 blocker smoke와 별개로, 기기 자동 잠금 시간을 짧게 두고 화면 꺼짐 방지 ON/OFF 차이를 확인해야 한다. |
...
8. 수정완료(2026-05-24): account deletion/rejoin 정책을 private data cleanup + authored recipe 보존으로 확정하고 구현/문서/문구를 정렬했다. redeploy 후 기존 저장/플래너 데이터가 보이지 않는지 재확인한다.
9. 확인완료(2026-05-24): Web Share OS sheet는 Android Chrome과 iOS Safari에서 열림과 앱 복귀 정상까지 확인했다.
10. 다음: account deletion fix 배포 후 동일 소셜 계정 재가입 smoke를 다시 실행하고 RC-MO-06을 closed로 바꾼다.
11. 다음: cooking-heavy beta 또는 public launch 전에 RC-MO-08 wake-lock 실기기 장시간 확인을 한다.
