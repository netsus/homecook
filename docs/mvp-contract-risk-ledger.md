...
| API | `docs/api문서-v1.2.8.md` |

## 대조 방법

| 항목 | 기준 |
| --- | --- |
| API 문서 endpoint | `docs/api문서-v1.2.8.md`의 `엔드포인트 전체 목록` 표 |
...
| 분류 | 결과 |
| --- | --- |
| API 문서 active method/path 수 | 57개 |
| 실제 `app/api/v1` method/path 수 | 57개 |
...
| 문서 내부 카운트 | `docs/api문서-v1.2.8.md`에서 active 57개로 정리 |
...
| CR-002 | P1 | `NO_DIRECT_UI` | `POST /api/v1/auth/login`, `PATCH /api/v1/auth/profile` | route와 테스트는 있으나 현재 웹 로그인 화면은 Supabase client OAuth/password 흐름을 직접 사용한다. | 이 API가 모바일/외부 client용인지, 웹도 이 route를 써야 하는지 결정한다. |
| CR-003 | 완료 | `DOC_META_DRIFT` | `엔드포인트 전체 목록` | v1.2.7 표 제목은 `52개`였지만 active method/path는 58개였다. | `docs/api문서-v1.2.8.md`에서 active 57개로 정리 완료. |
| CR-004 | P0/P1 | `MANUAL_ONLY_RELEASE_CHECK` | live OAuth, 실제 YouTube API, 실제 Supabase/production seed, 모바일 실기기 확인 | 구현 계약과 별개로 release 전에 사람이 확인해야 하는 항목이 남아 있다. | RC 체크리스트로 옮겨 owner/date/status를 붙인다. |

## Endpoint 목록표

| # | API 문서 endpoint | 화면 | route 있음? | 화면에서 씀? | 테스트 있음? | Manual Only? | 위험 유형 / 메모 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 0-1 | `POST /api/v1/auth/login` | LOGIN | 예 | 아니오 | 예 | 완료 기록 | `NO_DIRECT_UI`: 웹 로그인은 Supabase client를 직접 사용한다. |
| 0-2 | `PATCH /api/v1/auth/profile` | LOGIN | 예 | 아니오 | 예 | 없음 | `NO_DIRECT_UI`: 현재 화면 연결 여부를 확인해야 한다. |
...
| CR-003 | `엔드포인트 전체 목록 (52개)` | `docs/api문서-v1.2.8.md`에서 active 57개로 정리 | `auth/refresh` 제거 후 공식 API endpoint 수와 실제 `app/api/v1` method/path 수가 57개로 일치한다. |
...
| --- | --- | --- |
| live OAuth / return-to-action | auth, like/save, meals, pantry, cooking, leftovers, mypage/settings | 실제 소셜 로그인 후 원래 액션으로 복귀하는지 확인 |
| 실제 YouTube API | `POST /recipes/youtube/*` | `YOUTUBE_API_KEY`가 있는 환경에서 validate/extract/register live smoke |
| 실제 DB / 운영 seed | ingredients, pantry bundles, planner columns, shopping, recipe books | local/production seed 정합성과 기존 사용자 데이터 호환성 확인 |
| 모바일 실기기 / visual | shopping complete popup, cook mode, leftovers, settings wake lock, cooking method color | 작은 화면, 터치, 화면 꺼짐 방지, 색상 구분 확인 |
| 오래된 acceptance 정리 | recipe save, shopping 10a/10b/11 일부 | 이미 후속 slice에서 닫힌 항목인지 확인하고 stale Manual Only면 체크/정리 |

## A 담당 다음 순서

1. `POST /api/v1/auth/login`, `PATCH /api/v1/auth/profile`이 현재 웹에서 직접 쓰이지 않는 것이 의도인지 확인한다.
2. `Manual Only 묶음`을 RC 체크리스트로 옮기고, 각 항목에 owner/date/status를 붙인다.
3. stale로 보이는 Manual Only는 후속 slice에서 이미 닫혔는지 확인한 뒤 acceptance 정리 후보로 분리한다.
