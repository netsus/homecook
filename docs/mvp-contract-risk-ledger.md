...
| 구분 | 공식 문서 |
| --- | --- |
| 요구사항 | `docs/요구사항기준선-v1.7.0.md` |
| 화면 | `docs/화면정의서-v1.5.7.md` |
| 유저 flow | `docs/유저flow맵-v1.3.7.md` |
| DB | `docs/db설계-v1.3.6.md` |
| API | `docs/api문서-v1.2.10.md` |

## 대조 방법

| 항목 | 기준 |
| --- | --- |
| API 문서 endpoint | `docs/api문서-v1.2.10.md`의 `엔드포인트 전체 목록` 표 |
...
| 문서 내부 카운트 | `docs/api문서-v1.2.10.md`에서 active 55개로 정리 |
...
| CR-004 | 완료 | `MANUAL_ONLY_RELEASE_CHECK` | live OAuth, 실제 YouTube API, 실제 Supabase/production seed, 모바일 실기기 확인 | 구현 계약과 별개로 release 전에 사람이 확인해야 하는 항목이다. RC-MO-01 live OAuth / return-to-action, RC-MO-02 real-device mobile smoke, RC-MO-03 live YouTube registration smoke, RC-MO-04 staging DB compatibility smoke, RC-MO-05 stale Manual Only classification, RC-MO-06 account deletion/rejoin, RC-MO-07 Web Share OS sheet, RC-MO-08 wake-lock cooking mode는 evidence로 통과했다. | `docs/workpacks/beta-release-manual-checklist-2026-05-04.md`의 `CR-004 Release Candidate Tracking` 표로 닫았다. MVP 1차 release-blocking Manual Only는 closed이며, YouTube quota/광범위 URL 품질/production cutover 같은 항목은 별도 hardening 또는 production 운영 체크로 분리한다. |
...
| 8-6 | `GET /api/v1/shopping/lists/{id}/share-text` | SHOPPING_DETAIL | 예 | 예 | 예 | 완료 기록 | RC-MO-07에서 Android Chrome/iOS Safari OS 공유 시트 호출과 앱 복귀 정상 확인. |
...
| 13-1 | `PATCH /api/v1/users/me/settings` | SETTINGS | 예 | 예 | 예 | 있음 | RC-MO-08 통과. iOS Safari와 Android Chrome에서 화면 꺼짐 방지 ON 상태로 요리모드 화면이 꺼지지 않음을 staging에서 확인했다. |
| 13-2 | `PATCH /api/v1/users/me` | SETTINGS | 예 | 예 | 예 | 있음 | live OAuth 설정 화면 확인 필요. |
| 13-3 | `DELETE /api/v1/users/me` | SETTINGS | 예 | 예 | 예 | 있음 | RC-MO-06 통과. Supabase migration 적용 후 탈퇴/재가입 smoke에서 기존 저장/플래너 개인 데이터는 보이지 않고, 유튜브로 등록한 레시피는 보존되며 "내가 추가한 레시피"는 0개로 표시됨을 확인했다. |
...
| 실제 YouTube API | `POST /recipes/youtube/*` | RC-MO-03 통과. staging에서 실제 YouTube 레시피 등록까지 완료. URL shape matrix는 자동화로 고정했고, all-`절이기` cooking-method regression은 수정했다. Quota와 광범위한 extraction 품질 평가는 YouTube hardening backlog로 분리. |
...
| 모바일 실기기 / visual | shopping complete popup, cook mode, leftovers, cooking method color | RC-MO-02 통과. 막히는 화면, 안 눌리는 버튼, 하단 CTA 가림, 서버 에러, 무한 로딩 없음. 세부 visual/taste/animation은 design backlog로 분리. Web Share는 RC-MO-07, wake-lock 실제 브라우저 정책은 RC-MO-08에서 닫았다. |
| Web Share OS sheet | `GET /shopping/lists/{id}/share-text` | RC-MO-07 통과. Android Chrome과 iOS Safari에서 OS 공유 시트가 열리고 닫은 뒤 앱이 정상 동작했다. |
| 계정 삭제 / 동일 소셜 재가입 | `DELETE /users/me` | RC-MO-06 통과. 탈퇴 시 개인 기록은 삭제하고 직접/유튜브 등록 레시피는 작성자 정보 없이 보존한다. 동일 소셜 계정 재로그인 후 기존 저장/플래너 데이터가 보이지 않고 "내가 추가한 레시피"는 0개로 표시됨을 staging에서 확인했다. |
| Wake lock 실제 브라우저 동작 | `PATCH /users/me/settings`, COOK_MODE | RC-MO-08 통과. PR #579 / `8c2a2b8b` 배포 후 iOS Safari와 Android Chrome에서 화면 꺼짐 방지 ON 상태의 요리모드가 꺼지지 않음을 실기기로 확인했다. |
| 오래된 acceptance 정리 | recipe save, shopping 10a/10b/11 일부, `_template`, 과거 wave/design polish 항목 | RC-MO-05 통과. recipe save/remove, shopping placeholder, leftovers auto-hide, YouTube URL shape, YouTube all-`절이기` 회귀는 닫았고, 디자인/미래방향은 deferred로 분리했다. |
...
7. 완료(2026-05-24): 자동화 가능한 Manual Only 후속 항목을 정리했다. recipe save/remove 문서 stale, shopping placeholder, leftovers auto-hide, YouTube URL shape, YouTube all-`절이기` 회귀를 닫았다.
8. 완료(2026-05-24): account deletion/rejoin 정책을 private data cleanup + authored recipe 보존으로 확정하고 구현/문서/문구를 정렬했다. Supabase migration 적용 후 재테스트에서 기존 저장/플래너 데이터 제거와 작성 레시피 보존을 확인했다.
9. 확인완료(2026-05-24): Web Share OS sheet는 Android Chrome과 iOS Safari에서 열림과 앱 복귀 정상까지 확인했다.
10. 완료(2026-05-24): RC-MO-06 동일 소셜 계정 재가입 smoke를 다시 실행했고 closed 처리했다.
11. 완료(2026-05-24): RC-MO-08 wake-lock은 PR #579 / `8c2a2b8b` 배포 후 iOS Safari와 Android Chrome 실기기에서 화면이 꺼지지 않음을 확인했고 closed 처리했다.
12. 다음: YouTube quota, 광범위한 URL/영상 품질 매트릭스, synonym matching 개선은 별도 hardening 작업으로 분리한다.
