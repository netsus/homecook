...
> 기준 문서: 요구사항 기준선 v1.6.3 / 화면정의서 v1.3.0 / DB 설계 v1.3.1 / API 설계 v1.2.2
>
> 작성: Claude (H2 Stage 1 contract-evolution)
>
> v1.2.3 → v1.3.0 변경: PLANNER_WEEK day-card slot row 전환에 따른 §③ 갱신
> - 세로 스크롤 중심 탐색 명시 (가로 스크롤 없음)
...
| # | 변경 내용 | 영향 범위 | 상태 |
|---|----------|----------|------|
| 1 | `PLANNER_WEEK` 탐색 방식을 세로 스크롤로 명시하고 가로 스크롤 없음을 선언 | ③ 식단 계획 여정 | ✅ |
...
LOGIN (로그인 화면)
  ...
  └─ [구글 로그인]
     ...
     소셜 인증 완료
       │
       ├─ (신규 회원) → 닉네임 입력 → 가입 완료
       │    └─ 자동 생성: meal_plan_columns ×4, recipe_books ×3
       │
       └─ (기존 회원) → 로그인 완료
            │
            ▼
...
PLANNER_WEEK (식단 플래너)
  ...
  ├─ 필요 시 [이번주로 가기]
  ├─ 세로 스크롤로 날짜 카드 탐색 (가로 스크롤 없음)
  ├─ 하루 카드 단위로 식단 확인
  │    └─ 카드 안에 아침 / 점심 / 간식 / 저녁 4끼 고정 노출 (세로 slot row)
