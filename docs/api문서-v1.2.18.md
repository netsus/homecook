...
### 12-10. 사용자 성장/배지/퀘스트 조회 `33c-gamification`
...
- `grade`는 서버가 level band 기준으로 계산한다. 클라이언트는 label을 계산하지 않는다.
...
- `badges.earned` / `badges.locked`는 badge guide modal 또는 확장 surface에서만 사용한다. MYPAGE 첫 viewport에 locked grid를 강제하지 않는다.
- `quests.active`는 현재 진행 중인 standard quest, `quests.completed_recent`는 최근 완료된 quest다. 별도 reward claim은 없다.
- `tutorial.active_steps`는 tutorial quest subset이다. dismiss는 UX 상태만 바꾼다.
- `notifications.unseen`은 XP toast 또는 badge/quest new 상태 표시용이다.
- `notifications.priority_unseen`은 서버 우선순위 정렬 결과다. 우선순위는 `level_up > badge_unlocked > quest_completed > xp_awarded`이다.
...
### 12-11b. 사용자 성장 알림 보관함 조회 `growth-leveling-v2`
...
- XP, level, badge award, source progress event는 변경하지 않는다.
...
| 12-10    | GET        | /users/me/gamification                 | MYPAGE                   | 🔒     | 33c gamification                 |
| 12-11    | POST       | /users/me/gamification/notifications/seen | MYPAGE / shell        | 🔒     | 33c notification seen            |
| 12-11b   | GET        | /users/me/gamification/archive         | MYPAGE / 성장 보관함     | 🔒     | growth-leveling-v2 신규          |
| 12-12    | POST       | /users/me/gamification/tutorial-quests/{quest_key}/dismiss | MYPAGE | 🔒     | 33c tutorial dismiss             |
