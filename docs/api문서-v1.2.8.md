...
# 집밥 서비스 — API 설계 v1.2.8

> 기준 문서: 요구사항 기준선 v1.6.9 / 화면정의서 v1.5.6 / DB 설계 v1.3.5 / 유저 Flow맵 v1.3.6
> 작성: 킴실장
...
> 회원가입 완료 시 서버 자동 생성: meal_plan_columns ×3(`아침 / 점심 / 저녁`), recipe_books ×3
...
검수 단계에서 `unresolved` 또는 `needs_review` 재료가 기존 검색으로 해결되지 않을 때, 사용자가 확인한 표준명/카테고리로 새 재료를 등록하거나 이미 존재하는 표준 재료를 재사용한다. 성공 시 서버는 `draft_json`을 수정하지 않고, 클라이언트가 반환값으로 현재 row를 `resolved`로 바꾼다.
...
| Body | category | string | `채소` / `육류` / `해산물` / `양념` / `유제품` / `곡류` / `기타` |
