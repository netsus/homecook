...
> 기준 문서: 요구사항 기준선 v1.7.9 / 화면정의서 v1.5.16 / DB 설계 v1.3.14 / API 설계 v1.2.18
>
...
[XP source events]
  ...
  └─ 플래너 등록 (meals INSERT 성공) → planner_registered XP
```
...
## ⑪-b 사용자 성장/배지/퀘스트 확인 (Gamification) `growth-leveling-v2`
...
- official contract-evolution으로 33c API/DB 계약이 확정됨

### MYPAGE 확인 플로우

```
MYPAGE (Bottom Tab) → 계정 섹션
  └─ 프로필 영역 통합 성장 surface
       ├─ 등급명 + 레벨 + XP progress
       ├─ 대표/최근 배지 row
       ├─ 배지 안내/전체 보기 버튼 → badge guide modal/bottom sheet
       ├─ active quest 1~2개
       ├─ tutorial quest 1개
       └─ 최근 성장 기록 preview → archive surface
```

전체 badge inventory, 전체 quest list, 전체 성장 기록은 프로필 아래 secondary surface 또는 별도 상세 진입으로 이동한다. 첫 viewport에는 별도 대형 성장 card를 두지 않는다.
...
XP source action 성공
  ...
  │    ├─ 성공: level/badge/quest/notification 갱신
  │    └─ 실패: 원래 source action 성공은 유지, gamification soft-fail 기록
  └─ FE notification refresh
       ├─ priority_unseen 조회
       ├─ level_up > badge_unlocked > quest_completed > xp_awarded 순서로 toast stack 표시
...
- 팬트리 항목 추가 (`pantry_item_added`)
- 남은요리 다먹음 (`leftover_eaten`)
...
- invalid quest key 또는 notification id → 본인 리소스만 처리, 타인 리소스 정보 노출 금지
- backfill/recompute 이후 첫 로그인 → 과거 notification toast burst 없음. Historical state는 `is_new=false`로 취급.

### 종료 조건

- MYPAGE에서 대표 배지/퀘스트/tutorial quest가 렌더링되거나 soft-fail 상태로 안전하게 축소됨
- XP source action 직후 toast가 표시되거나, 표시할 unseen notification이 없음을 확인함
- archive surface에서 cutover 이후 live 성장 기록이 최신순으로 조회됨
