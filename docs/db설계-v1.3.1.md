...
> 기준 문서: 요구사항 기준선 v1.6.2 / 화면정의서 v1.2.2 / API 설계 v1.2.2 / 유저 Flow맵 v1.2.2
> 
> 
> 작성: 킴실장
> 
...
| name | varchar(30) | NOT NULL | 아침 / 점심 / 간식 / 저녁 |
| sort_order | int | NOT NULL | 컬럼 순서 |
| created_at | timestamptz | NOT NULL |  |
- 공개 제품 계약상 planner slot은 유저 편집 없이 **4개 고정**이다.
- 서버는 유저별 canonical slot `아침 / 점심 / 간식 / 저녁`을 보장한다.
- `meal_plan_columns`는 API에 직접 노출되는 CRUD 대상이 아니라 planner slot 식별용 내부 저장소다.
...
| 대상 | 내용 |
| --- | --- |
| meal_plan_columns × 4 | 아침 / 점심 / 간식 / 저녁 |
