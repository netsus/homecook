...
## Happy Path
- [ ] MYPAGE 레시피북 탭에서 레시피북 탭 → RECIPEBOOK_DETAIL 진입, 레시피 목록 표시 <!-- omo:id=accept-happy-path;stage=4;scope=frontend;review=5,6 -->
...
- [ ] 시스템 row 자동 생성이 필요한 슬라이스면 owning flow와 기대 결과가 명시되어 있다 (17a에서 검증됨) <!-- omo:id=accept-bootstrap-owning-flow;stage=2;scope=shared;review=3,6 -->
...
### Playwright
- [ ] RECIPEBOOK_DETAIL 진입, 레시피 목록 표시, 제거, empty 전환 브라우저 테스트 <!-- omo:id=accept-playwright-flow;stage=4;scope=frontend;review=5,6 -->
