...
- [ ] launchd 300-second tick, 15-minute heartbeat/oldest-due and 3-failure/dead-letter alerts, 24-hour cleanup target, mode 600 secret, 10MB × 5 logs, recovery and Manual Only gates are recorded <!-- omo:id=delivery-maintenance-operations;stage=2;scope=shared;review=3,6 -->
  ...
  - The aggregate remains unchecked: power/login/sleep, approved external heartbeat/alert receiver and actual external 15-minute gap/alert fault evidence, 24-hour cleanup target, and 10MB × 5 rotation are not yet proven.
...
- [ ] local existing/fresh/replay, live Storage fixtures and merged-exact-SHA remote read-only evidence are green <!-- omo:id=delivery-visibility-verification;stage=2;scope=shared;review=3,6 -->
  - 2026-07-28 partial local evidence: focused Vitest passed 601 tests, PostgreSQL existing/fresh/replay passed 75 tests, local migration replay reported up to date, and the two real local Storage tests passed. Remote merged-exact-SHA evidence remains open.
  - 2026-07-28 KST orchestrator rerun on clean merged-exact head `0410b0cb6b4c4e3204ab462783f2cf7431b055d1`: `pnpm test:recipe-visibility-read-hardening:focused` passed 56 files / 601 tests, `pnpm test:recipe-visibility-read-hardening:postgres` passed 75 tests, local Supabase-backed `pnpm test:recipe-visibility-read-hardening:storage-live` passed 2 files / 2 tests, and `pnpm verify:backend` completed lint, typecheck, product tests 2298 passed / 102 skipped, production build and security Playwright 12/12. The remote verifier dry run confirmed merged-exact/clean/read-only prerequisites for `0410b0cb`, while the full remote read-only verifier failed closed at the protected `psql` execution step without exposing captured output. This item remains unchecked until remote merged-exact-SHA evidence is collected.
  - 2026-07-28 KST repair branch evidence: RED tests reproduced service-owned legacy `thumbnail_url` acceptance and the missing service-role tag usage reconcile path, then GREEN tests passed focused 56 files / 601 tests, PostgreSQL 75 tests, related unit 85 tests, and `pnpm verify:backend`. Independent code and security/DB reviewers rechecked the repair with P0/P1/P2 findings 0. Remote merged-exact-SHA, launchd runtime, full live Storage and final stage promotion evidence remain open, so this item stays unchecked.
  - 2026-07-28 KST post-merge remote evidence on exact source `ff0998d6f862027e81ce6d4aee57916600cba9e2`: dry-run passed merged-exact/clean/read-only prerequisites, then the full linked remote verifier failed because the remote database is still missing required Stage 2 image/guard relations and routines. The narrowed read-only diagnostic reported remote writes 0. This remains a service-owner migration application blocker, not a verifier write.
  - 2026-07-28 KST live Storage matrix completion: `tests/recipe-image-legacy-visibility-storage.live.test.ts` now covers managed upload/replay, takeover, compensation, attach, cancel/drain, first-404 quarantine and late-object requeue against the local Supabase Storage stack. With `HOMECOOK_STORAGE_LIVE_URL`, `HOMECOOK_STORAGE_LIVE_SERVICE_ROLE_KEY`, `HOMECOOK_STORAGE_LIVE_DB_URL` and `HOMECOOK_STORAGE_LIVE_LOCAL_ONLY=1` loaded from local Supabase, `pnpm test:recipe-visibility-read-hardening:storage-live` passed 2 files / 5 tests. The live tests fail closed if Storage/DB URLs and service-role key do not exactly match `supabase status -o env` for the default local Supabase endpoints, and cleanup verifies Storage removal errors. The aggregate remains unchecked because merged-exact remote evidence is still blocked by the remote migration gap.
  ...
  - 2026-07-29 KST closeout rerun on exact source `539a9f593ef5d26563f496c33a577210583688f5`: `pnpm lint`, `pnpm typecheck`, `pnpm test:recipe-visibility-read-hardening:focused` (57 files / 606 tests), `pnpm test:recipe-visibility-read-hardening:postgres` (75 tests), `pnpm exec vitest run tests/recipe-image-storage-live-guard.test.ts` (5 tests), local-Supabase `pnpm test:recipe-visibility-read-hardening:storage-live` (2 files / 5 tests), source-of-truth/workflow-v2/workpack/automation-spec/OMO bookkeeping/closeout validators and `git diff --check` passed. `node scripts/verify-recipe-visibility-read-hardening-remote.mjs --mode post-merge-read-only --dry-run` passed, but the full read-only verifier failed closed; a narrowed read-only diagnostic found the linked remote still missing the Stage 2 image/guard routines and relations while `remote_writes_probe` stayed 0. Reconstructed PR #1205 RED evidence also failed on parent `0410b0cb6b4c4e3204ab462783f2cf7431b055d1` when only the repair tests were applied: service-owned legacy `thumbnail_url` returned `201` instead of expected `422`, and tag usage-count guard matches were `0` instead of `>=3`. The aggregate remains unchecked because remote migration application, Manual Only launchd/heartbeat evidence, complete historical RED artifacts and successor-owned historical reader evidence remain open.
