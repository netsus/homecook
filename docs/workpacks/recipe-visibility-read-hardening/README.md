...
- [ ] MANUAL_RECIPE_CREATE consumes object ID/cancel/signed-read states with no browser direct Storage mutation <!-- omo:id=delivery-image-existing-consumer;stage=4;scope=frontend;review=5,6 -->
  - 2026-07-29 KST partial Stage 4 evidence: the `generation_active` consumer sends a UUID `Idempotency-Key`, preserves the exact compressed upload payload for same-key replay, attaches only `image_object_id`, refreshes an expired signed preview from the replay `read_url`, and calls the owner cancel endpoint for remove/replace/unmount/stale completions. Focused Vitest passed 3 files / 34 tests and the dedicated desktop/mobile Playwright path passed 2 tests.
  - This aggregate stays unchecked because the server intentionally still returns the legacy public `{ thumbnail_url, storage_path }` shape while capability is `legacy`, and the cancel route is hidden in that state. Removing browser Storage cleanup before the approved F0+#3 joint activation would orphan abandoned public uploads; retiring that final compatibility cleanup therefore remains an explicit production activation/rollout gate, not a claim of completed Stage 4.
- [ ] existing screen composition, accessibility, loading/error/read-only and return-to-action behavior remain unchanged <!-- omo:id=delivery-visibility-ui-regression;stage=4;scope=frontend;review=5,6 -->
  - 2026-07-29 KST partial regression evidence: `pnpm verify:frontend:pr` passed lint, typecheck, 2,316 product tests with 105 skips, production build, 59 smoke tests with 10 skips, 8 accessibility tests with 1 skip and 12 visual-core tests before the final exact-payload replay repair; after that one-line payload preservation repair, focused lint/typecheck/Vitest and dedicated Playwright reran green. Full Stage 4 promotion remains blocked by the legacy cleanup retirement gate above.
