...
- [ ] service-only DB reservation authority atomically locks the active generation/session, server-derived MIME/hash/size, exact request/byte/active/backlog gates and durable key/attempt/path; same-key replay or takeover does not recharge quota <!-- omo:id=accept-image-preput-reservation-authority;stage=2;scope=backend;review=3,6 -->
