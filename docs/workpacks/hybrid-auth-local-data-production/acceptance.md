...
- Stage 4 production bundle: `pnpm build` 통과 후 `pnpm verify:hybrid-browser-bundle`로 `.next/static`의 Storage SDK dot/bracket alias 및 호출 단위 Storage REST URL/method/options alias mutation match `0`; bounded value lattice가 `if/else`·`switch` case entry/fallthrough/break/default/no-match·`try/catch/finally`·`while`/`for`/`do…while`·conditional/short-circuit expression의 가능한 값 집합과 unknown control-flow taint를 call site에서 병합하고 template/concatenation URL fragment를 추적하며 `fetch`/`window.fetch`/`globalThis.fetch`/fetch alias와 실제 minified positive/negative bundle fixture를 검사한다. 모든 경로가 GET/HEAD인 branch, `do…while(false)`의 확정 1회 body, unrelated API POST, Storage 문서 문자열은 false positive `0`, server-only env 노출 경계는 기존 `tests/hybrid-supabase-env.test.ts`와 전체 product suite 통과로 검증
- Stage 4 static gate performance: default repository inventory를 suite `beforeAll`에서 1회 계산하고 adversarial fixture inventory는 독립 유지해 source/bundle gate `50 passed`를 3회 반복했으며 wall time `2.94s / 2.94s / 2.89s`로 기본 timeout 안에 통과
...
- full regression: `pnpm lint`, `pnpm typecheck`, `pnpm test` (4639 passed, 211 skipped), `pnpm test:product` (2416 passed, 120 skipped), `pnpm build`, `git diff --check`
