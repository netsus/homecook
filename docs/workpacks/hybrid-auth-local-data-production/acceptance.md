...
- Stage 4 production bundle: `pnpm build` 통과 후 `pnpm verify:hybrid-browser-bundle`로 `.next/static`의 Storage SDK dot/bracket alias 및 호출 단위 Storage REST URL/method/options alias mutation match `0`; bounded value lattice가 `if/else`·`switch` case entry/fallthrough/break/default/no-match·`try/catch/finally`·`while`/`for`/`do…while`·conditional/short-circuit expression의 가능한 값 집합과 unknown control-flow taint를 call site에서 병합하고 template/concatenation URL fragment를 추적하며 `fetch`/`window.fetch`/`globalThis.fetch`/fetch alias와 실제 minified positive/negative bundle fixture를 검사한다. identifier 평가는 현재 lexical binding map에서 초기화 완료가 증명된 parameter/import/function/var/선행 local만 non-throwing으로 보고 unresolved/TDZ, await thenable assimilation, class definition을 exceptional outcome으로 보존한다. iteration label 소유권은 loop executor가 소비하여 labeled continue가 do-while condition 또는 for increment/condition을 거쳐 정상 exit하는 경로를 유지하고 nested switch/loop의 다른 label completion은 전파한다. statement completion은 `normal/throw/return/break/continue`로 분리하며 finally를 각각에 적용한 뒤 원래 abrupt completion을 다시 전파해 도달 불가능한 fetch를 정상 경로에 합치지 않는다. source inventory는 별도 literal fetch 판정을 제거하고 같은 TypeScript AST/value-lattice scanner를 사용하여 client runtime graph의 fetch/URL/template/options/method alias·spread·escape를 fail-closed하며 confirmed GET은 제외한다. 모든 경로가 GET/HEAD인 branch, `do…while(false)`의 확정 1회 body, unrelated API POST, Storage 문서 문자열은 false positive `0`, server-only env 노출 경계는 기존 `tests/hybrid-supabase-env.test.ts`와 전체 product suite 통과로 검증
- Stage 4 static gate performance: default repository inventory를 suite `beforeAll`에서 1회 계산하고 adversarial fixture inventory는 독립 유지해 source/bundle gate `98 passed`를 3회 반복했으며 wall time `4.33s / 4.49s / 4.30s`로 기본 timeout 안에 통과
...
- full regression: `pnpm lint`, `pnpm typecheck`, `pnpm test` (4697 passed, 219 skipped), `pnpm test:product` (2420 passed, 128 skipped), `pnpm build`, `git diff --check`
