...
- Stage 4 source/runtime/data authority: client-reachable runtime import/re-export/literal dynamic import에서 `@supabase/ssr`, `@supabase/supabase-js`, `@supabase/storage-js`는 `lib/supabase/browser.ts` 한 파일 외 violation이다. loopback gateway의 `/storage/v1/**` POST/PUT/PATCH/DELETE는 exact `DATA_SUPABASE_SECRET_KEY`와 `x-homecook-internal-scope: recipe-image` 및 기존 path/method allowlist가 모두 맞아야 upstream에 도달한다. final recipe image buckets와 isolated runtime의 `anon`/`authenticated` mutation policy count는 0이며 server-only upload/sign/remove는 service role bypass와 기존 image lifecycle 계약을 유지한다.
...
- architecture-pivot focused regression: 변경 연관 `27 files`에서 `608 passed / 2 skipped`, auth facade·gateway·Storage policy 경계 `25 passed`, source/import + bundle 보조 canary `19 passed`를 3회 반복(`3.18s / 3.14s / 3.11s`)
- full regression: `pnpm lint`, `pnpm typecheck`, `pnpm test` (`4697 passed / 226 skipped`), `pnpm test:product` (`2421 passed / 128 skipped`), `pnpm build`, `pnpm validate:workpack`, `git diff --check`
