...
- Stage 4 frontend image lifecycle + Stage 5 remediation: `pnpm exec vitest run tests/account-session-generation-inventory.test.ts tests/manual-recipe-create-screen.test.tsx tests/recipe-image-upload-client.test.tsx tests/recipe-images-route.test.ts tests/recipe-image-managed-upload.test.ts tests/recipe-image-cancel-route.test.ts tests/recipe-image-managed-cancel.test.ts tests/hybrid-supabase-static-gate.test.ts tests/hybrid-browser-bundle-gate.test.ts` — `150 passed`; managed-only upload/cancel의 strict wrapper/status, same-key replay, interrupted expired read URL 재시도, upload/cancel/unmount/save/refresh/replace/remove stale completion, saving 중 navigation/image mutation guard, browser direct Storage source/bundle inventory `[]`를 검증
...
- Stage 4 production bundle: `pnpm build` 통과 후 `pnpm verify:hybrid-browser-bundle`로 `.next/static`의 Storage SDK dot/bracket alias 및 `/storage/v1/object` mutation match `0`; server-only env 노출 경계는 기존 `tests/hybrid-supabase-env.test.ts`와 전체 product suite 통과로 검증
...
- full regression: `pnpm lint`, `pnpm typecheck`, `pnpm test` (4581 passed, 211 skipped), `pnpm test:product` (2405 passed, 120 skipped), `pnpm build`, `git diff --check`
