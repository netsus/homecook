...
- [ ] `HOMECOOK_DATA_AUTHORITY=remote` 기본값에서 기존 remote-only 동작이 유지된다 <!-- omo:id=accept-hybrid-remote-default;stage=2;scope=backend;review=3,6 -->
- [ ] `local-shadow`는 safe GET digest만 비교하고 사용자 response/write authority는 remote에 남긴다 <!-- omo:id=accept-hybrid-shadow-read-only;stage=2;scope=backend;review=3,6 -->
...
- [ ] account-generation capability는 `legacy`이고 canonical generation activation이 열리지 않는다 <!-- omo:id=accept-hybrid-generation-legacy-only;stage=2;scope=shared;review=3,6 -->
...
- [ ] remote exact `iss`, `aud=authenticated`, `role=authenticated`, UUID `sub`, UUID `session_id`, `iat`/`nbf`/`exp`, allowlisted `alg`/`kid`가 모두 통과 조건이다 <!-- omo:id=accept-hybrid-exact-claim-guard;stage=2;scope=backend;review=3,6 -->
...
- [ ] historical audit actor는 set-null 또는 write-time snapshot policy로 처리되어 deleted/recreated epoch owner 권한으로 되살아나지 않는다 <!-- omo:id=accept-hybrid-historical-audit-policy;stage=2;scope=backend;review=3,6 -->
...
- [ ] user-scoped local Data client가 service role/secret key 없이 동작한다 <!-- omo:id=accept-hybrid-user-client-no-service-role;stage=2;scope=backend;review=3,6 -->
...
- [ ] 신규 public endpoint/field/status/error code가 추가되지 않는다 <!-- omo:id=accept-hybrid-no-public-shape-change;stage=2;scope=shared;review=3,6 -->
...
- [ ] local restore 뒤 `auth.users=0`은 의도된 invariant로 유지된다 <!-- omo:id=accept-hybrid-local-auth-users-zero;stage=2;scope=backend;review=3,6 -->
- [ ] `pg_constraint`/`pg_depend`/`pg_proc`/policy에서 application-owned `auth.users` 잔존 count가 0이다 <!-- omo:id=accept-hybrid-auth-users-residual-zero;stage=2;scope=backend;review=3,6 -->
- [ ] known fixture `public.users=5`, `admin missing=1`, `audit missing=99`가 교정 전 fail, 교정 후 pass한다 <!-- omo:id=accept-hybrid-known-semantic-fixture;stage=2;scope=backend;review=3,6 -->
- [ ] remote DB roles/schema/data dump와 local restore rehearsal이 성공한다 <!-- omo:id=accept-hybrid-db-restore-rehearsal;stage=2;scope=backend;review=3,6 -->
...
- [ ] Storage object count/bytes/MIME/hash/reference manifest mismatch가 0이다 <!-- omo:id=accept-hybrid-storage-manifest;stage=2;scope=backend;review=3,6 -->
...
- [ ] Auth/Data env split과 public/server-only exposure가 테스트된다 <!-- omo:id=accept-hybrid-vitest-env-split;stage=2;scope=backend;review=3,6 -->
- [ ] issuer/audience/sub/session/expiry validation이 테스트된다 <!-- omo:id=accept-hybrid-vitest-token-validation;stage=2;scope=backend;review=3,6 -->
- [ ] no service role user client fallback이 테스트된다 <!-- omo:id=accept-hybrid-vitest-no-service-role;stage=2;scope=backend;review=3,6 -->
- [ ] mirror payload allowlist와 secret-free invariant가 테스트된다 <!-- omo:id=accept-hybrid-vitest-mirror-allowlist;stage=2;scope=backend;review=3,6 -->
- [ ] route/helper service-role allowlist AST/static gate가 테스트된다 <!-- omo:id=accept-hybrid-vitest-service-role-static;stage=2;scope=backend;review=3,6 -->
