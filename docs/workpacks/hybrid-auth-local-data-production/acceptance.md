...
- Stage 4 frontend image lifecycle + Stage 5 remediation: `pnpm exec vitest run tests/account-session-generation-inventory.test.ts tests/manual-recipe-create-screen.test.tsx tests/recipe-image-upload-client.test.tsx tests/recipe-images-route.test.ts tests/recipe-image-managed-upload.test.ts tests/recipe-image-cancel-route.test.ts tests/recipe-image-managed-cancel.test.ts tests/hybrid-supabase-static-gate.test.ts tests/hybrid-browser-bundle-gate.test.ts` — `171 passed`; managed-only upload/cancel의 strict wrapper/status, same-key replay, upload-owned/create-owned 소유권 전환, mounted/unmounted create의 success/definitive `IMAGE_*`/unknown 결과별 cancel `0/1/0`, StrictMode와 later unmount의 exactly-once cleanup, interrupted expired read URL 재시도, upload/cancel/unmount/save/refresh/replace/remove stale completion, saving 중 navigation/image mutation guard, dynamic import/re-export client graph와 type-only edge 제외, browser direct Storage source/bundle inventory `[]`를 검증
...
- Stage 4 production bundle: `pnpm build` 통과 후 `pnpm verify:hybrid-browser-bundle`로 `.next/static`의 Storage SDK dot/bracket alias 및 호출 단위 Storage REST URL/method/options alias mutation match `0`; method 재할당은 fetch call 시점의 마지막 값을 사용하고 unknown은 fail-closed하며 `fetch`/`window.fetch`/`globalThis.fetch`/fetch alias와 실제 minified bundle fixture를 탐지한다. unrelated API POST, Storage 문서 문자열, call 전에 DELETE에서 GET으로 재할당된 값은 false positive `0`, server-only env 노출 경계는 기존 `tests/hybrid-supabase-env.test.ts`와 전체 product suite 통과로 검증
...
- full regression: `pnpm lint`, `pnpm typecheck`, `pnpm test` (4613 passed, 211 skipped), `pnpm test:product` (2416 passed, 120 skipped), `pnpm build`, `git diff --check`
