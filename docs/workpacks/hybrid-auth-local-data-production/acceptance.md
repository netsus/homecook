...
- measured isolated runtime: `node scripts/verify-hybrid-supabase.mjs --mode isolated-runtime-measured` — PG 17.6.1.136/PostgREST 14.12/Storage 1.60.4/gateway를 host port 없이 기동하고 DB setting 주입, 역할·schema·extension init-complete healthcheck 뒤 clean-volume bootstrap 3회 연속 성공, remote-shaped ES256 `sub=auth.uid()`, 공식 익명 `/recipes/themes`·`/tags`·`/ingredients`·`/cooking-methods` downstream query와 `HEAD` 거부, User A/B DB RLS, Storage owner mutation, direct anon/service_role upstream bypass 차단, Google/Naver/Kakao remote-shaped callback fixture의 RPC-only user/bootstrap 생성, callback RPC의 capability-race maintenance와 stale 오류 보존, admin member/비회원 조회, 404 feedback/operational event 실제 insert와 scope·method·path 오용 차단, revoke 후 동일 JWT DB/Storage mutation 0, 409/503 outage mapping을 실제 HTTP/DB 상태로 검증
...
- Stage 4 frontend image lifecycle + Stage 5 remediation: `pnpm exec vitest run tests/account-session-generation-inventory.test.ts tests/manual-recipe-create-screen.test.tsx tests/recipe-image-upload-client.test.tsx tests/recipe-images-route.test.ts tests/recipe-image-managed-upload.test.ts tests/recipe-image-cancel-route.test.ts tests/recipe-image-managed-cancel.test.ts tests/hybrid-supabase-static-gate.test.ts tests/hybrid-browser-bundle-gate.test.ts` — `302 passed`; managed-only upload/cancel의 strict wrapper/status, same-key replay, upload-owned/create-owned 소유권 전환, mounted/unmounted create의 success/definitive `IMAGE_*`/unknown 결과별 cancel `0/1/0`, StrictMode와 later unmount의 exactly-once cleanup, interrupted expired read URL 재시도, upload/cancel/unmount/save/refresh/replace/remove stale completion, saving 중 navigation/image mutation guard, dynamic import/re-export와 repo-local `stores/`/`features/` runtime graph 및 type-only/server-only edge 제외, browser direct Storage source/bundle inventory `[]`를 검증
...
- Stage 4 production bundle: `pnpm build` 통과 후 `pnpm verify:hybrid-browser-bundle`로 `.next/static`의 executable Storage SDK mutation과 호출 단위 Storage REST mutation match `0`; bounded value lattice는 binding 초기화 상태를 `uninitialized | initialized | unknown`으로 분리하고 block/switch 진입 시 `let`/`const`/`class`를 TDZ 상태로 선등록한다. 따라서 outer binding을 가리는 nested lexical 선언과 `typeof` TDZ는 exceptional outcome이고, lexical binding이 없는 `typeof unresolved`만 non-throwing이다. `ThisKeyword`는 non-throwing으로 처리한다. REST 판정은 `fetch`/`window.fetch`/`globalThis.fetch`/constant-computed global fetch, generic object property/element, object destructuring, comma callee, member assignment와 보수적 escape를 같은 call의 URL/options/method 흐름과 함께 추적하며 production bundle에서도 확정 local non-fetch function을 제외한 unknown callee가 Storage URL과 mutating/unknown options를 받으면 fail-closed한다. source inventory는 `fetch | function | Storage namespace/from/bucket/mutator | dynamic module namespace | object/array composite | constant | unknown` summary lattice를 runtime static import, literal dynamic import namespace/destructuring, barrel `export *`, named re-export를 포함한 client graph 전체에 SCC 호환 고정점으로 전파한다. literal dynamic import는 registry summary를 namespace/destructuring/`.then()` callback에 전달하고, element access나 computed destructuring의 key binding이 현재 lattice에서 exact string으로 확인되면 해당 module property descriptor를 alias call까지 보존한다. 확정 safe literal helper는 제외하며, client-reachable nonliteral 또는 해석할 수 없는 dynamic import는 unknown-capability namespace로 유지해 `upload/remove/update/move/copy/createSignedUploadUrl` 호출을 fail-closed한다. source module summary는 top-level assignment/update/destructuring assignment를 실행 순서대로 반영하고, identifier-rooted nested member assignment의 statically resolved property path를 재귀 갱신한다. known object/array composite의 alias identity는 현재 branch-local 분석 state에서 persistent old→new reference replacement로 유지한다. 이 identity 집합에는 direct alias뿐 아니라 `||`/`??`/`&&` 결과의 가능한 operand와 lattice가 배열임을 확정한 array spread의 원소 참조도 포함한다. 따라서 alias property에 mutator를 대입하면 원래 object path의 같은-file/cross-module 호출에도 반영되고, 확정 safe helper 대입은 safe로 남으며, 배열/iterable 또는 operand가 불명확하면 known safe로 축소하지 않고 unknown을 유지한다. computed constant property key와 safe→mutator·mutator→safe·분기/unknown live binding 결과도 import/re-export 고정점에 전달된다. registry가 없는 production bundle에서도 `import()`의 await/destructuring/element alias/`.then()` namespace에서 비롯된 mutation 이름 호출은 fail-closed하지만, 확정 local safe object와 unrelated `remove` method는 제외한다. SDK 판정은 regex를 사용하지 않고 executable TypeScript AST `CallExpression`의 `.storage.from(...).upload/remove/...` 체인, object/array destructuring assignment, tuple numeric access, bucket/method alias만 추적하므로 문자열·주석과 확정 safe tuple/function은 violation이 아니다.
- Stage 4 static gate의 보장 범위: 이 분석기는 fixture로 고정된 known executable path와 분석 불명확 경로를 CI에서 fail-closed하는 방어이며, 임의 JavaScript 의미 전체에 대한 형식 증명은 아니다. production의 독립 방어는 기존 `next.config.ts` CSP `connect-src` origin allowlist, browser에 노출되지 않는 server-only local gateway/env, Storage owner RLS·private bucket 권한, Playwright request 관찰의 `/storage/v1/object` mutation `0`이다. CSP는 허용된 remote Supabase origin 내부의 Auth/Storage path를 구분하지 않고 owner RLS도 유효 사용자의 자기 object mutation을 blanket deny하지 않으므로 어느 하나도 static gate나 server image API authority를 대체한다고 주장하지 않는다.
- Stage 4 static gate performance: default repository inventory를 suite `beforeAll`에서 1회 계산하고 adversarial fixture inventory는 독립 유지해 source+bundle `155 passed`를 3회 반복했으며 wall time `4.68s / 4.70s / 5.06s`로 기본 timeout 안에 통과했다. soundness를 약화할 수 있는 token/AST prefilter는 채택하지 않았고 최종 production build의 `.next/static` 전체 AST scan은 `12.70s`, mutation `0`이었다. 현재 workflow에는 이 검사를 중단시키는 별도 짧은 timeout이 없어 CI timeout 변경은 하지 않았다.
...
- full regression: `pnpm lint`, `pnpm typecheck`, `pnpm test` (4819 passed, 226 skipped), `pnpm test:product` (2421 passed, 128 skipped), `pnpm build`, `pnpm validate:workpack`, `git diff --check`
