...
- real DB smoke:
  ...
  - Stage 2 source authority gate는 client-reachable ESM/CommonJS runtime graph(`.ts/.tsx/.mts/.cts/.js/.jsx/.mjs/.cjs`, index 및 TypeScript runtime extension 대입 포함)에서 `@supabase/ssr`, `@supabase/supabase-js`, `@supabase/storage-js`를 단일 Auth adapter 외에 금지하고 해석 불가능한 runtime loader를 fail closed한다. dynamic import는 import attributes/options와 무관하게 첫 specifier를 검사하며, 같은 basename에 여러 runtime 후보가 있으면 단일 우선순위를 가정하지 않고 가능한 후보를 모두 inventory한다
