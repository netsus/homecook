...
- The target-DB security inventory is exact: function ACL entries include grant-option state, protected tables include FORCE RLS state, and policy permissiveness plus boolean expression structure are compared against the existing migration-derived contract. Policy canonicalization normalizes only unquoted PostgreSQL syntax; string/dollar literals and quoted identifiers retain their exact case and spacing. The isolated active path applies the snapshot migration and verifies all 12 protected tables and 10 policies; this does not complete merged-exact or Manual Only evidence.
