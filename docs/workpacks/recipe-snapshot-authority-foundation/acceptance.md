...
- [ ] the planned Stage 2 merged-exact-SHA full-local snapshot verifier proves `self-hosted-local-auth-db-storage-single-authority`, stable UUID, pre-expiry local session revoke, `auth.uid()` RLS, cross-owner denial, delete/recreate cleanup, official S3/rclone restore, and app+Auth-only public exposure; `tests/full-local-auth-db-foundation.test.ts` and `tests/full-local-auth-db-foundation-postgres.integration.test.ts` are existing foundation evidence, not completion of this pending snapshot gate <!-- omo:id=accept-snapshot-remote;stage=2;scope=shared;review=3,6 -->
...
- [ ] provider live login/link, final public cutover, production-scale compatibility observation, full actual-DB cleanup rehearsal, official S3/rclone restore, and current-head release evidence are closed as Manual Only gates <!-- omo:id=accept-snapshot-full-local-manual;stage=2;scope=shared;review=3,6 -->
