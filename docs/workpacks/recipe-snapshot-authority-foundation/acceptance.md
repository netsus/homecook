...
- [ ] the planned Stage 2 merged-exact-SHA full-local snapshot verifier proves `self-hosted-local-auth-db-storage-single-authority`, stable UUID, pre-expiry local session revoke, `auth.uid()` RLS, cross-owner denial, delete/recreate cleanup, official S3/rclone restore, and app+Auth-only public exposure; PR #1263 merged the Stage 3 deployable app/runtime authority and existing foundation tests, but neither completes this pending snapshot gate <!-- omo:id=accept-snapshot-remote;stage=2;scope=shared;review=3,6 -->
