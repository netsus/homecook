...
- A later full-diff review found one remaining P1: the dedicated allowlist was exact only at file level, so the mixed public `GET` / private mutation recipe route could not fail closed on handler drift. It also found two P2 workflow projection errors: a nonexistent `local-first` verifier path and PR #1278 attached to an unrelated slice. Three focused RED failures reproduced these gaps. The inventory now records the enclosing function and permits the future-propagation client only in preview `POST` and recipe `PATCH` / `DELETE`; client creation moved into those handlers. Workflow commands now use the existing `local-rehearsal` verifier and PR bookkeeping belongs only to this slice. Focused tests passed `19/19`.
- Fresh `pnpm verify:backend` passed lint, typecheck, product tests `209 files / 2,606 pass / 140 intended skip`, production build and security Playwright `12/12`. Fresh full Vitest passed `504 files / 5,154 tests` with `28 files / 320 tests` intentionally skipped. Workflow, bookkeeping, closeout, source-of-truth, workpack, automation, generated inventory, security-function contract and isolated PostgreSQL security-function checks passed.
...
## Formal Stage 3 finding repair

- The separate formal Stage 3 task `019fc718-de6e-7422-9d68-db397420d23d` reviewed exact head `3873ded8ed18927b78b0967bfb581974e6e71190` and returned `REQUEST_CHANGES`, P0/P1/P2 `0/3/3`. The six findings covered missing full-local scoped RPC clients, standalone public-owner visibility, snapshot-v2 cook-mode projection/scaling, description drift binding, exact diff-check drift and executable multi-recipe concurrency evidence.
- Repair TDD remained test-first. RED commits `1da8f8be`, `bcd7d863`, `6821c58b` and `55b1a993` locked the scoped-client, projection, description, hidden-owner and lifecycle-race failures before their corresponding runtime repairs. GREEN commits `54c6f858`, `a6373d22` and `59556837` introduced least-privilege scoped clients, exact `CookingModeRecipe` normalization, description-aware preview/PATCH hashing and serialized source-owner lifecycle visibility. Test-only commit `7a407059` then locked the exact concurrent shopping terminal projection.
- The final executable PostgreSQL matrix uses separate `psql` connections and a barrier. It proves opposite-order multi-recipe writers complete without deadlock; each fixture Meal maps to the exact returned shopping list; each list has the exact `(recipe_id, recipe_content_snapshot_id)` grouping; cross-list misassociation, duplicate item identity and orphan list counts are zero; and post-preview Meal insertion returns stale without partial writes.
- A fresh security review found and repaired a source-owner lifecycle race beyond the original six findings. Transition-first and start-first races for `deleting` and `quarantined` owners now serialize through sorted requester/source-owner locks, and durable snapshot-v2 reads fail closed after the source owner becomes non-visible. Exact race RED is `55b1a993`; exact GREEN is `59556837`.
- Fresh repair reviews approved the product/test head with no unresolved findings: `/root/slice7_stage3_repair_code_review` and `/root/slice7_stage3_repair_security_review` each returned P0/P1/P2 `0/0/0`. Both reviewers then reconfirmed exact local head `51a46e069c26bb1925becb14da65be468c512e5a`; the final delta from `7a407059` removes one EOF-only blank line from each of three migrations and changes no SQL, ACL or authority semantics. These auxiliary repair approvals do not replace the separate formal Stage 3 re-review.
- Exact local head `51a46e069c26bb1925becb14da65be468c512e5a` passed full Vitest `504 files / 5,157 tests` with `28 files / 328 tests` intentionally skipped. `pnpm verify:backend` passed lint, typecheck, product tests `209 files / 2,606 tests` with `11 files / 148 tests` intentionally skipped, production build and security Playwright `12/12`.
- The composite PostgreSQL runner passed predecessor `15 pass / 1 intended skip`, #7 follow-up `18/18`, active full-local inventory `31 pass / 17 intended skip`, and replay/companion `16/16`, `18/18`, `31 pass / 17 intended skip`. `git diff --check origin/master...HEAD`, source/workflow/workpack/automation/bookkeeping validators and audit passed; audit retained one pre-existing low advisory and high/critical `0`.
- The exact-head isolated collector compared `51a46e069c26bb1925becb14da65be468c512e5a` with official immediate previous release `53ebcc325665da1d7f0c2c304d4b3e73c0d7612c`. Its sanitized report was `2060` bytes, mode `0600`, validated successfully and cleaned isolated resources in `finally`. Two-owner missing/other-owner preview and PATCH remained identical `404 RESOURCE_NOT_FOUND`; recipe/content/Meal/shopping/session/claim digests were unchanged `6/6`; both releases kept personal-recipe-v2 and snapshot-v2 creation flags off, all personal/v2 row and idempotency deltas at zero, and the legacy-v1 shape intact. Production/staging/remote application writes remained `0/0/0`.
- The formal Stage 3 re-review, final docs-head current-check snapshot and merge remain pending. The broader real-Auth/Data stale and active-claim row, merged-exact server-Mac read-only evidence, Stage 4, Manual Only and #8 R/R+1-to-R+2 activation boundaries remain unchecked.
