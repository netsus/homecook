...
- [x] non-owner/public/unauthenticated recipe detail omits the whole `edit_context` field, while other-owner private/deleted/quarantined resources preserve existing 404 non-disclosure <!-- omo:id=accept-future-entrypoint-context-omit;stage=4;scope=shared;review=5,6 -->
...
- [x] `image_object_id` is a verified managed object identity or explicit `null`, is never inferred from a URL, and thumbnail/signed/external/legacy URLs never become managed identity <!-- omo:id=accept-future-entrypoint-managed-image;stage=4;scope=shared;review=5,6 -->
- [x] Recipe Meal `GET /meals.data.items[]` alone exposes positive `revision`; `product_entries[]` stays unchanged and the transaction remains final stale/authority validation <!-- omo:id=accept-future-entrypoint-meal-revision;stage=4;scope=shared;review=5,6 -->
