...
- [x] 플래너 화면 진입 시 기본 주간 범위(오늘부터 7일)로 플래너 조회 성공
...
- [x] 요일 스트립을 좌우로 넘기면 이전 주 / 다음 주로 이동한다
- [x] 이전 주 / 다음 주 버튼은 노출하지 않고, 필요 시 `이번주로 가기`만 보조 액션으로 제공한다
- [x] 범위 정보는 한 번만 보이고, `ready` 같은 개발용 상태 문구는 사용자 화면에 노출되지 않는다
- [x] 상단 CTA `[장보기] [요리하기] [남은요리]`가 disabled 상태로 노출됨
- [x] API 응답 형식이 `{ success, data, error }` 래퍼를 따름
- [x] 백엔드 계약과 프론트엔드 타입이 일치함 (고정 slots + meals 구조)
- [x] 식사 상태 표시(registered / shopping_done / cook_done)가 시각적으로 구분됨
- [x] 스와이프 또는 `이번주로 가기` 동작 시 새로운 범위로 플래너 재조회 성공
...
1. **플래너 화면 진입 및 조회**
   ...
   - 기본 주간 범위(오늘부터 7일)로 고정 4끼 슬롯과 식사 목록 조회 확인
...
4. **주간 이동**
   - 요일 스트립을 좌우로 넘겨 이전 주 / 다음 주 이동 → 새로운 범위로 플래너 재조회 확인
   - 비기본 범위에서 [이번주로 가기] 노출 및 복귀 동작 확인
   - 로딩 상태 표시 확인
...
- [x] 주간 이동 대상(요일 스트립)이 플래너 본문과 붙어 있는지 확인
- [x] 같은 날짜의 4끼가 한 카드 안에서 읽히는지 확인
- [x] 비로그인 시 플래너 탭 진입 → 로그인 게이트 확인
- [x] 로그인 후 플래너로 복귀 확인 (return-to-action)
- [x] 식사 상태 표시 시각적 구분 확인 (registered / shopping_done / cook_done)
- [x] 요일 스트립 스와이프 → 플래너 재조회 확인
