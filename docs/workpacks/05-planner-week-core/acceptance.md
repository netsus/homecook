...
- [x] drag handle 기반 끼니 컬럼 순서 변경 인터랙션 (드롭 저장, 실패 시 원복, 화살표 제거) (`2026-04-09`, `pnpm exec vitest run tests/planner-week-screen.test.tsx`, `pnpm exec playwright test tests/e2e/slice-05-planner-week-core.spec.ts`, drag handle reorder + 새로고침 유지 + 저장 실패 원복/오류 안내 + 화살표 제거 확인)
