...
- manual/YouTube writer와 FoodSafety backfill은 같은 bounded predecessor loader와 순수 calculator를 사용한다. recipe ingredient별 query를 추가하지 않고 batch마다 정규화·중복 제거한 ingredient ID에 대해 nutrition link 1회와 conversion assignment 1회만 읽는다.
...
- 격리 PostgreSQL 14.5에서 승인 chain count `1 → 2(모호) → 1(revoked) → 0`과 기존 snapshot/RLS/backfill/concurrency를 포함해 9/9가 통과했다. Supabase PostgreSQL 17 컨테이너 `supabase_db_homecook`는 image `public.ecr.aws/supabase/postgres:17.6.1.106`의 `created` 상태에서 시작하지 못해 full FoodSafety seed/owner bootstrap smoke는 수행하지 않았고 아래 real DB 항목은 계속 미완료다.
