...
- 2차 repair current-head에서 비-PG 영양 Vitest 6 files 71/71, DB contract 7/7, 기존 container/volume과 분리된 `/tmp` PostgreSQL 14.5 integration 16/16, typecheck가 통과했다. 전체 lint는 error 0이고 기존 backfill test의 unused parameter warning 4건이 남아 있다.
...
- plain/percent/double/partial-percent encoded credential query key는 TypeScript와 PostgreSQL 모두 fail-closed한다. `service_role`의 recipe/predecessor input table `TRUNCATE` 권한도 모두 제거됐음을 실제 DB에서 확인했다.
