...
- public recipe의 current nutrition projection은 기존 공개 read 정책 안에서 읽을 수 있다. 사용자 생성 recipe는 기존 `created_by` 생성 소유권을 유지하며, 이 슬라이스는 공식 계약에 없는 recipe edit/delete endpoint를 추가하지 않는다.
...
- `POST /meals`는 column/leftover 소유권, recipe 존재·공개 read 정책과 기존 401/403/404/409/422 경계를 유지한다. client가 보낸 snapshot ID는 받지 않는다.
...
| 타 사용자 column/leftover/Meal mutation | 기존 `403` |
| recipe 대상 없음 | 기존 `404` |
...
- Visual artifact: `ui/designs/evidence/recipe-nutrition-calculation/`의 390px, 320px, desktop before/after 및 상태별 screenshot
- Authority report: `ui/designs/authority/RECIPE_DETAIL-recipe-nutrition-calculation-authority.md`
- Authority status: `reviewed`
- Notes:
  - 독립 Codex 최종 visual verdict는 `96/100`, blocker `0`, finding `0`이다.
...
- [x] 확정 (confirmed) — Stage 5/final authority `96/100`, blocker `0`
- [ ] N/A
...
- recipe 공개 read와 사용자 생성 `created_by` 회귀, anon/authenticated snapshot RLS, column/leftover/Meal service guard, concurrent current switch, transaction injection rollback을 실제 DB에서 확인한다.
...
- 격리 plain PostgreSQL 17.10 검증은 통과했지만 full Supabase/PostgREST 환경과 동등하다는 뜻은 아니며, production-scale contention은 아직 검증하지 않았다.
...
- 위 1·2차 repair 증거만으로는 full migration + FoodSafety-30 seed + 실제 owner/bootstrap smoke, production-scale contention, Supabase PostgreSQL 17 동등성을 증명하지 못했다. 후속 closeout에서 격리 PostgreSQL 17 검증을 추가했으며 production/staging write는 계속 0이고 secret/raw provider row는 출력·커밋하지 않았다.
- 최종 current-head는 기존 사용자 DB/container/volume과 분리된 격리 PostgreSQL 17.10에 전체 migration 66/66과 FoodSafety 30 recipe/124 canonical ingredient seed를 적용했다. guarded exact predecessor apply/replay 뒤 active approved primary link 13개와 최소 1개 link가 있는 recipe 21/30을 보존했다.
- actual closeout 4/4에서 FoodSafety-30 backfill dry-run/apply/replay/current switch/rollback과 실제 `ensurePublicUserRow` + `ensureUserBootstrapState`를 함께 실행했다. 사용자 A의 기본 `아침/점심/저녁` column을 확인했고, A가 사용자 B의 column/leftover로 `POST /meals`를 시도하면 403과 0 writes, A의 정상 요청은 201과 current snapshot pin, A가 B의 Meal을 `PATCH/DELETE`하면 403과 row 불변임을 확인했다.
- PostgreSQL 17 minimal integration 18/18에서 constraint, authenticated/service direct snapshot mutation 거부, append-only/current uniqueness, concurrency, transaction rollback을 확인했다. port guard 9/9, runner 1/1, 관련 회귀 61/61, root unit/API 28/28, typecheck, lint, source-of-truth/workflow/automation/workpack/draft-closeout validator도 통과했다. 종료 뒤 test user/Meal/snapshot row와 임시 DB가 0임을 확인했다.
- 이 검증은 plain PostgreSQL 17 기준이므로 full Supabase/PostgREST 동등성과 production-scale contention을 증명하지 않는다. 현재 sparse baseline 13/124와 21/30, active conversion/piece assignment 부족, 조리 손실 미모델링 한계도 유지한다. 이 변경과 무관한 기존 backfill CLI spawn test는 로컬 기본 5초 제한에서 timeout이 있었고, 30초를 명시한 단독 실행은 12/12 통과했다.

## Stage 4/5 Pre-Merge Evidence `2026-07-16`

- Stage 4는 Recipe Detail에 additive `예상 영양` 카드와 선택 인분 계산을 연결했다. 선택값은 nutrient별 `scalable * selected / base + fixed`를 사용하며 fixed 기여를 비례시키지 않는다. optional nutrient는 관측된 숫자 key만 표시하고 결측을 0으로 만들지 않는다.
- complete/partial/normal unavailable/missing/temporarily unavailable/loading/low-quality를 서로 다른 한국어 문구와 구조로 표시한다. `temporarily_unavailable`에만 영양 영역 재시도를 제공하며 기존 상세 본문과 CTA는 유지한다.
- 초기 상세 조회와 영양 재시도는 request sequence와 recipe ID를 함께 확인해 이전 recipe의 늦은 응답이 새 화면을 덮지 못하게 했다. 출처 URL은 HTTP(S)만 허용하고 공식 6-field projection만 렌더링한다.
- 관련 Vitest 2 files 80/80, TypeScript, targeted ESLint, corrected slice Playwright command가 통과했다. Playwright는 mobile-chrome 7 pass/2 skip, desktop-chrome 2 pass/7 skip, mobile-ios-small 명시 실행 2 pass/7 skip이며 390/320/desktop과 상태별 evidence를 남겼다. 390px guest login return-to-action도 실제 브라우저에서 pending planner action 재개까지 확인했다.
- `ui/designs/authority/RECIPE_DETAIL-recipe-nutrition-calculation-authority.md`의 독립 Stage 5/final visual verdict는 `96/100`, blocker `0`, finding `0`이다. 첫 캡처의 Chromium compositor 손상은 제품 결함으로 숨기지 않고 증거를 거부한 뒤 직접 viewport 재캡처로 교체하고, 일반 회귀가 다시 덮어쓰지 않는 승인 경로로 분리했다.
- 독립 Codex 코드 재리뷰는 공식 계약 밖 API/field/status, 계산식, availability 상태, race guard, source/XSS 경계, 44px, COOK_MODE, E2E glob/명령을 확인하고 수정 필요 finding `0건`으로 판정했다.
- 독립 Codex Stage 6은 exact PR head `7d73d0da90a523abe316211a9a6ce3e79930b709`을 재검토해 Blocker/Important/Suggestion `0/0/0`으로 승인했다. 같은 head에서 시작된 GitHub checks는 의도된 full-regression skip 외 모두 성공했고 pending/fail/cancel은 `0`이다.
- known limitations는 approved coverage `13/124`, 최소 한 link recipe `21/30`, conversion/piece sparse, 조리 손실 미모델링, plain PostgreSQL 17과 full Supabase/PostgREST 차이, production-scale contention 미검증이다. production/staging write는 0이다.
...
- [x] DB constraint/RLS/ownership/append-only/current uniqueness 테스트 — PostgreSQL 17 minimal 18/18과 실제 cross-owner Meal route guard/row 불변 smoke로 확인 <!-- omo:id=delivery-db-security;stage=2;scope=backend;review=3,6 -->
- [x] FoodSafety-30 bounded backfill/report/replay/rollback과 real DB smoke — PostgreSQL 17 actual closeout 4/4로 확인 <!-- omo:id=delivery-backfill-smoke;stage=2;scope=backend;review=3,6 -->
...
- [x] Recipe Detail loading/complete/partial/unavailable/error/low-quality UI 연결 <!-- omo:id=delivery-recipe-ui-states;stage=4;scope=frontend;review=5,6 -->
- [x] selected servings vector UI와 COOK_MODE control 비추가 확인 <!-- omo:id=delivery-serving-ui;stage=4;scope=frontend;review=5,6 -->
- [x] 한국어 `1인분 기준 예상 영양`/`약`/`최소`/`정보 준비 중`과 attribution 안내 확인 <!-- omo:id=delivery-korean-copy;stage=4;scope=frontend;review=5,6 -->
- [x] Vitest/Playwright/manual QA 범위와 fixture/real DB smoke 분리 <!-- omo:id=delivery-test-split;stage=4;scope=shared;review=6 -->
- [x] 390px/320px/desktop before-after와 상태별 screenshot evidence <!-- omo:id=delivery-authority-evidence;stage=4;scope=frontend;review=5,6 -->
- [x] anchor authority report와 final authority blocker 0 <!-- omo:id=delivery-authority-gate;stage=4;scope=frontend;review=5,6 -->
- [x] secret/raw data leak 0, N+1 방지, bounded backfill 증거 <!-- omo:id=delivery-security-performance;stage=2;scope=shared;review=3,6 -->
- [x] rollout/rollback/known-limitations와 current-head closeout evidence 동기화 <!-- omo:id=delivery-closeout;stage=4;scope=shared;review=6 -->
