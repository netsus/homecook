...
- [ ] actual edible-use amount와 exactly-one approved/current predecessor/conversion selection, exact piece fail-closed, unit priority 고정 <!-- omo:id=delivery-approved-unit-paths;stage=2;scope=backend;review=3,6 -->
...
- [ ] `GET /recipes/{id}` additive nutrition projection과 `availability_reason` normal/missing/temporary 구분 구현 <!-- omo:id=delivery-recipe-api;stage=2;scope=backend;review=3,6 -->
