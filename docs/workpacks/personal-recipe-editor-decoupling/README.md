...
Stage 2 implementation evidence is retained at [`evidence/2026-08-02-stage2-full-local-verifier.md`](./evidence/2026-08-02-stage2-full-local-verifier.md). The locked test first failed as `1 failed suite / 0 tests` because the implementation module did not exist, then passed `8/8` after the minimum #4-composed verifier and CLI were added. The first Stage 3 findings were repaired from `10 failed / 20 passed` to `30/30`; fresh re-review findings were then repaired from `9 failed / 23 passed` to `32/32`, and the final permission/contract/#4/#5 bundle passes `55/55`. Isolated full-local PostgreSQL passes `16` core cases with `28` snapshot-owned skips; fresh snapshot is `15 pass / 1 intended skip`, replay snapshot is `16 pass / 0 skip`, and the active Auth/Storage inventory is `28 pass / 16 skip` in both modes. The exact restore manifest input shape is not promoted into a contract and remains pending/manual. This is implementation evidence only: the feature head is not merged, fresh Stage 3 re-review is pending and no merged-exact success is claimed.
