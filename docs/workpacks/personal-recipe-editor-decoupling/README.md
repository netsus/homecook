...
Stage 2 implementation evidence is retained at [`evidence/2026-08-02-stage2-full-local-verifier.md`](./evidence/2026-08-02-stage2-full-local-verifier.md). The locked test first failed as `1 failed suite / 0 tests` because the implementation module did not exist, then passed `8/8` after the minimum #4-composed verifier and CLI were added. Stage 3 findings were repaired from an actual `10 failed / 20 passed` RED to `30/30`, and the final focused permission/contract/#4/#5 bundle passes `53/53`. Isolated full-local PostgreSQL passes `16` core cases; snapshot fresh/replay plus the active observed Auth/Storage authority pass in both modes. The exact restore manifest input shape is not promoted into a contract and remains pending/manual. This is implementation evidence only: the feature head is not merged, fresh Stage 3 re-review is pending and no merged-exact success is claimed.
