...
The first replacement current-head CI run found two further regression-test failures: the generated account-session inventory had not yet recorded the new method/title worker RPC calls, and the immutable subprocess integration timed out only on Linux CI. The inventory is now regenerated from the exact call sites. A second CI run proved the timeout was not slow execution: the fixture omitted the exact `yt-dlp` preflight executable, so Linux correctly failed closed before claim while macOS accidentally found a host-installed binary. The fixture now supplies every exact tool, rejects immediately if the child exits before finalize, and keeps a bounded 30-second integration budget while retaining claim/persist/finalize/SIGTERM assertions. The focused pair passes `18/18`, and the local full Vitest suite passes under parallel load.
