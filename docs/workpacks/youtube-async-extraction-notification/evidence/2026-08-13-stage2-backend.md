...
The first replacement current-head CI run found two further regression-test failures: the generated account-session inventory had not yet recorded the new method/title worker RPC calls, and the immutable subprocess integration exceeded Vitest's default five-second timeout only under the full parallel suite. The inventory is now regenerated from the exact call sites. A second CI run proved that GitHub's shared parallel load could also exceed 30 seconds while the same real subprocess completed locally in about two seconds, so the test now has a bounded 120-second integration budget while retaining its claim/persist/finalize/SIGTERM assertions. The focused pair passes `18/18`, and the local full Vitest suite passes under parallel load.
