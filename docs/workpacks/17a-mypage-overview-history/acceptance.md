...
- [ ] 2026-05-12 계약 기준 장보기 기록 카드에 완료 시각이 표시되고 완료된 항목은 `다시열기` CTA로 `SHOPPING_DETAIL` read-only 모드로 이동한다 <!-- omo:id=accept-shopping-detail-reopen-v124;stage=4;scope=frontend;review=5,6 -->
...
- [ ] GET /shopping/lists 최신순, cursor pagination, `completed_at` 포함 계약 검증 <!-- omo:id=accept-vitest-shopping-lists-v124;stage=2;scope=backend;review=3,6 -->
...
- [ ] 장보기 기록 탭 → 완료 항목 `다시열기` 클릭 → SHOPPING_DETAIL read-only 이동 <!-- omo:id=accept-playwright-shopping-reopen-v124;stage=4;scope=frontend;review=5,6 -->
