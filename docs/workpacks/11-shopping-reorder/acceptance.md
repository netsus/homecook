...
- [x] read-only 정책이 지켜진다 <!-- omo:id=accept-read-only;stage=2;scope=shared;review=3,6 -->
  ...
  - 프론트 read-only 모드에서 드래그 핸들 비활성화 또는 숨김
...
- scenarios:
  1. 장보기 상세(미완료) 진입 → 재료 카드 드래그 시작
  2. 드롭 완료 → 화면 순서 변경 확인
  3. 재진입 → 순서 유지 확인
  4. 완료된 리스트 재열람 → 드래그 핸들 비활성화 또는 숨김 확인
...
- [ ] 실제 사용자 흐름, 라우팅, 모달, 권한 게이트가 브라우저 테스트로 고정되어 있다 <!-- omo:id=accept-playwright-flow;stage=4;scope=frontend;review=5,6 -->
  - 장보기 상세 진입 → 드래그&드롭 → 순서 변경 → 재진입 → 순서 유지
  - 완료된 리스트 재열람 → 드래그 핸들 비활성화
