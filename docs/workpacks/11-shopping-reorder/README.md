...
## In Scope
- 화면:
  - `SHOPPING_DETAIL` (미완료 리스트 순서 변경 컨트롤 UI)
  - `SHOPPING_DETAIL` (완료 리스트 재열람 시 순서 변경 컨트롤 비활성화 확인)
- API:
...
- 완료된 리스트의 read-only UI 전반 (체크/제외 비활성화) — slice 10a에서 이미 처리됨, 이 슬라이스는 순서 변경 컨트롤 비활성화만 추가
...
- 필수 상태:
  ...
  - `empty`: N/A (장보기 리스트 자체는 slice 09/10a에서 처리, 이 슬라이스는 순서 변경만 담당)
  - `error`: reorder API 호출 실패
  - `read-only`: 완료된 리스트(`is_completed=true`)에서 순서 변경 컨트롤 비활성화 또는 숨김
...
- Affected screen: 기존 confirmed `SHOPPING_DETAIL` 화면에 순서 변경 컨트롤 추가
- Visual artifact: 기존 `ui/designs/SHOPPING_DETAIL.md` (slice 10a에서 생성) 참조, 위/아래 이동 버튼 추가 정도의 경미한 UI 변경
- Authority status: `not-required`
- Notes: 기존 confirmed 화면의 low-risk UI change (위/아래 이동 버튼 추가, 순서 변경 인터랙션)이므로 design-generator/design-critic/authority review 불필요. Stage 4 PR에 low-risk 판단 근거 명시.
...
> 2026-04-27 Codex Stage 5/6: 기존 confirmed `SHOPPING_DETAIL`의 low-risk 순서 변경 컨트롤 추가로 lightweight design check와 Stage 6 PR review를 수행했고, 5개 상태 / 화면정의서 액션 / token·spacing drift / 44×44px 터치 타겟 / a11y 기본 요소에서 blocker 0개를 확인했다. Authority-required가 아니므로 Claude final authority gate는 필요 없다. Internal 6.5 closeout projection에서 `confirmed`로 전환한다.

## Source Links
- `docs/sync/CURRENT_SOURCE_OF_TRUTH.md`
- `docs/workpacks/README.md`
- `docs/요구사항기준선-v1.6.3.md` — 장보기 순서 변경 요구사항
- `docs/화면정의서-v1.5.0.md` § 12 SHOPPING_DETAIL (순서 변경 컨트롤 위치는 구현 판단)
- `docs/api문서-v1.2.2.md` § 8-4b PATCH /shopping/lists/{id}/items/reorder
- `docs/db설계-v1.3.1.md` (shopping_lists, shopping_list_items)
- `docs/reference/wireframes/jibhap-wireframe-session2.md` (순서 변경 UX 참고)
...
- **Real DB smoke 경로**:
  ...
  - seed script로 위 baseline 데이터 생성 후 브라우저에서 SHOPPING_DETAIL → 위/아래 이동 버튼으로 순서 변경 확인 → 재진입 → 순서 유지 확인
  - 완료된 리스트 재열람 → 순서 변경 컨트롤 비활성화 또는 숨김 확인
- **Bootstrap 요구사항**:
...
- **미완료 리스트만 순서 변경 가능**: `shopping_lists.is_completed=false`인 리스트만 reorder API 호출 가능
  ...
  - 프론트는 완료된 리스트 재열람 시 순서 변경 컨트롤 비활성화 또는 숨김 (UI 레벨 차단 + 서버 409 이중 방어)
...
- **순서 변경 UX 구현**:
  - 접근 가능한 위/아래 이동 버튼 사용 (기존 의존성만 사용, 새 패키지 추가 없음)
  - 버튼 클릭 → 새 순서 계산 → reorder API 호출 → 성공 시 로컬 상태 반영
...
2. 장보기 상세에서 재료 카드의 위/아래 이동 버튼 클릭 → 순서 변경
3. 새 순서가 서버에 저장되고 화면에 반영됨
4. 앱을 종료했다가 다시 장보기 상세 진입 → 순서가 그대로 유지됨
5. 마이페이지 장보기 기록에서 완료된 리스트 재열람 → 이동 버튼 비활성화 또는 숨김 확인
...
> 신규 GPT-only 실행에서는 reviewer waiver를 만들지 않는다. rebuttal이 수용되면 독립 Codex 검토 작업의 task ID, evidence, verdict를 남기고 closeout coordinator가 checklist를 갱신한다. 과거 `waived_by=claude` metadata는 merged 이력 호환용으로만 보존한다.
