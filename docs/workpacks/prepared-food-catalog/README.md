...
승인된 stable product key를 가진 public 완제품과 사용자가 직접 등록한 private 완제품을 하나의 검색 catalog로 제공한다. public 제품은 검수된 영양 version을 read-only로 노출하고, private manual 제품은 owner만 생성·조회·수정·soft-delete할 수 있다. 영양 수정은 기존 값을 덮어쓰지 않고 새 immutable version을 원자적으로 current로 전환해 과거 planner pin을 보존한다.
...
- runtime provider 호출, 신규 public source 수집·promotion, raw provider payload 저장
...
- 나머지 핵심/optional nutrient는 생략 또는 null 가능하다. 제공된 숫자는 finite `>= 0`이어야 한다.
...
| invalid basis, 음수/비정상 nutrient, 잘못된 body | 기존 `422` 계약 |
...
- 승인 stable key·완전한 핵심 영양·안전한 attribution을 가진 public product
...
- energy는 필수 `>= 0`, 다른 nutrient의 null/missing은 0이 아니다.
...
1. 로그인 사용자가 catalog를 검색하면 active public 제품과 자신의 active private 제품만 cursor page로 받는다.
...
- [ ] 네 food-products endpoint의 공식 request/response/error 계약 고정 <!-- omo:id=delivery-product-api-contract;stage=2;scope=backend;review=3,6 -->
- [ ] public stable-key approved subset과 name-brand no-merge 고정 <!-- omo:id=delivery-public-admission;stage=2;scope=backend;review=3,6 -->
- [ ] private manual owner-only create/read/update/delete 고정 <!-- omo:id=delivery-private-owner-scope;stage=2;scope=backend;review=3,6 -->
- [ ] basis·energy·nullable nutrient·missing-not-zero validation 구현 <!-- omo:id=delivery-manual-nutrition-validation;stage=2;scope=backend;review=3,6 -->
- [ ] first version/profile/value/current pointer atomic create 구현 <!-- omo:id=delivery-product-atomic-create;stage=2;scope=backend;review=3,6 -->
- [ ] nutrition 새 immutable version과 metadata-only version preservation 구현 <!-- omo:id=delivery-version-update-semantics;stage=2;scope=backend;review=3,6 -->
- [ ] concurrent current switch conflict와 rollback 구현 <!-- omo:id=delivery-version-concurrency;stage=2;scope=backend;review=3,6 -->
- [ ] idempotent soft-delete와 history/pin retention·new-entry block prerequisite 구현 <!-- omo:id=delivery-product-soft-delete;stage=2;scope=backend;review=3,6 -->
- [ ] public mutation·cross-owner·direct table mutation RLS/permission guard 구현 <!-- omo:id=delivery-product-security;stage=2;scope=backend;review=3,6 -->
- [ ] safe public attribution과 secret/raw provider leak 0 검증 <!-- omo:id=delivery-product-provenance;stage=2;scope=backend;review=3,6 -->
- [ ] cursor scope/pagination과 bounded read query 검증 <!-- omo:id=delivery-product-read-performance;stage=2;scope=backend;review=3,6 -->
- [ ] isolated PostgreSQL constraint/RLS/concurrency/idempotency/rollback gate 통과 <!-- omo:id=delivery-product-real-db;stage=2;scope=backend;review=3,6 -->
- [ ] production/staging write 0과 cleanup evidence 기록 <!-- omo:id=delivery-product-zero-write;stage=2;scope=backend;review=3,6 -->
- [ ] Stage 3 독립 Codex review finding 0 또는 수용·반박 기록 <!-- omo:id=delivery-product-stage3-review;stage=2;scope=backend;review=3,6 -->
- [ ] planner-entry/UI 비포함과 design authority N/A closeout 동기화 <!-- omo:id=delivery-product-scope-closeout;stage=2;scope=shared;review=3,6 -->
