...
- [ ] 레시피북에서 레시피 선택 → Meal 생성 흐름이 정상 동작한다 <!-- omo:id=accept-recipebook-to-meal;stage=4;scope=frontend;review=6 -->
- [ ] 팬트리 추천에서 레시피 선택 → Meal 생성 흐름이 정상 동작한다 <!-- omo:id=accept-pantry-to-meal;stage=4;scope=frontend;review=6 -->
...
## Error / Permission
- [ ] loading 상태가 있다 (레시피북 목록, 레시피북 내 레시피, 팬트리 추천, Meal 생성) <!-- omo:id=accept-loading;stage=4;scope=frontend;review=6 -->
- [ ] empty 상태가 있다 (레시피북 없음, 레시피북 내 레시피 없음, 팬트리 추천 없음) <!-- omo:id=accept-empty;stage=4;scope=frontend;review=6 -->
- [ ] error 상태가 있다 (네트워크 오류, 서버 오류, 권한 오류) <!-- omo:id=accept-error;stage=4;scope=frontend;review=6 -->
- [ ] unauthorized 처리 흐름이 있다 (비로그인 시 로그인 게이트) <!-- omo:id=accept-unauthorized;stage=4;scope=frontend;review=6 -->
- [ ] 로그인 게이트 후 return-to-action이 맞다 <!-- omo:id=accept-return-to-action;stage=4;scope=frontend;review=6 -->
...
### Playwright
- [ ] 레시피북 버튼 → 레시피북 선택 → 레시피 선택 → Meal 생성 흐름이 브라우저 테스트로 고정되어 있다 <!-- omo:id=accept-playwright-recipebook-flow;stage=4;scope=frontend;review=6 -->
- [ ] 팬트리 버튼 → 팬트리 추천 레시피 선택 → Meal 생성 흐름이 브라우저 테스트로 고정되어 있다 <!-- omo:id=accept-playwright-pantry-flow;stage=4;scope=frontend;review=6 -->
- [ ] 로그인 게이트 + return-to-action 흐름이 브라우저 테스트로 고정되어 있다 <!-- omo:id=accept-playwright-login-gate;stage=4;scope=frontend;review=6 -->
- [ ] empty 상태 (레시피북 없음, 레시피 없음, 팬트리 추천 없음)가 브라우저 테스트로 고정되어 있다 <!-- omo:id=accept-playwright-empty;stage=4;scope=frontend;review=6 -->
