...
> 신규 GPT-only 실행에서는 reviewer waiver를 새로 만들지 않는다. rebuttal이 수용되면 독립 Codex 검토 작업의 task ID, evidence, verdict를 남기고 closeout coordinator가 checklist를 갱신한다. 과거 `waived_by=claude` metadata는 merged 이력 호환용으로만 보존한다.
