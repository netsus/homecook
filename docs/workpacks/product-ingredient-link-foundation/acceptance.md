...
- [x] PostgreSQL integration covers isolated fresh/replay with the official snapshot-authority migration, dirty-row preflight, FK/check/partial unique, RLS/grants, concurrent promotion, two-session shopping create conflict and immutable snapshot-preserving split clone authority <!-- omo:id=accept-link-postgres-targets;stage=2;scope=backend;review=3,6 -->
