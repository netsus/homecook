...
3. **인분 아이콘 / 좋아요·저장·요리완료 컨트롤 / 인분 `+`/`−` 버튼 색상 prototype 정합**
   ...
   - **인분 `+` 버튼**: prototype(`detail.jsx`)은 `+` 버튼 bg를 `T.mint` (#2AC1BC)로 사용. 현재 코드는 `var(--brand)`를 사용하며, 현재 앱 런타임 토큰 `--brand-primary`는 파란색 `#00A1FF`다. prototype과 다르지만 현재 앱 브랜드 기준은 런타임 토큰을 따른다.
...
|-----------|-----------|------------------|-----------|-----------|
| `--brand-primary` = `#00A1FF` (blue) | prototype `T.mint` = `#2AC1BC` (bright mint) 전역 적용 | prototype 계열과 전역 색상 일치 | `docs/design/design-tokens.md`, `app/globals.css` | 미승인 — 현재 앱 브랜드 파란색을 유지 |
...
- Color parity notes: 모바일 인분 stepper `+` 버튼은 앱 팔레트 역할 토큰 `var(--brand)`를 사용한다. 현재 런타임 값은 `#00A1FF`다. Prototype `T.mint` (#2AC1BC)와 다르지만, 현재 앱 brand 역할 토큰과 정합한다. 모달 stepper는 `var(--wave1-ink)` (#212529)로 prototype `T.ink`와 정합. 히어로 메트릭은 white text + drop-shadow로 prototype 정합 완료. `ServingsIcon`은 `currentColor` 상속으로 정상.
