...
- `custom:naver` provision은 exact `--confirm-local-auth-mutation PROVISION_LOCAL_OAUTH_PROVIDERS` gate 뒤 create/update하는 명령으로 구현했지만 실행하지 않았다. 2026-08-08 contract-evolution 1A는 production domain을 `mumeok.kr`로 교정했다. runtime/example/provider console의 `.com` 잔존 참조는 별도 domain-alignment PR과 Manual Only 검증이 끝나기 전까지 미완료다.
- 2026-08-08 장애 교정 계약은 JWT 1시간과 로그인 유지기간을 분리하고 stable session identity와 rotating token evidence를 구분한다. 현재 이 acceptance는 구현 전이므로 완료 표시하지 않으며 1시간 경계 canary가 통과한 뒤에만 체크한다.
- Stage 0 baseline은 dirty 운영 checkout을 수정하지 않고 Git·Docker·launchctl·public origin의 read-only provenance만 수집한다. 운영 checkout에만 있던 완료 표시는 canonical `[ ]` 상태로 역전파하지 않으며, nullable incident 시각과 hash-only dirty fingerprint는 실제 T+65분 canary 전의 미완료 기준선을 뜻한다.
