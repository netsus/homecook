...
- production `validate`/`start`/`status`는 HMAC 인증된 mode `0600` readiness와 canonical owner/mode-`0700` parent를 required gate로 사용한다. 24시간 안의 인증·복호화된 primary/off-Mac archive, signed clean restore, archive와 다른 두 번째 매체의 signed replacement-Mac key recovery, 그 evidence가 결합한 actual AES-256-GCM escrow envelope의 exact path/hash/HMAC/device, exact production identity가 하나라도 없으면 fail closed한다. `start`는 offline preflight 뒤에만 writer를 기동하고 live identity 2차 실패 시 그 attempt가 새로 올린 writer만 정지한다.
