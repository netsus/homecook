...
- internal operator command:
  ...
  - `all-active`는 provider별 approved/pinned handoff를 반복 `--bundle <file>`로 받아 한 run 안에서 atomic apply/disable 한다. `foodsafety-30`은 기존 single-bundle 경로를 유지한다.
...
- Schema Change:
  - [x] table/column/status 없음 → 기존 table, constraint, RLS를 유지하고, multi-source all-active를 위해 follow-up migration에서 internal `apply_ingredient_nutrition_model`/`disable_ingredient_nutrition_model` 함수만 교체한다
  - [ ] 있음
  - 신규 table/column/index/status는 추가하지 않는다. 기존 apply payload의 checksum·candidate identity·approval guard를 재사용하고, 공식 schema 확장이 필요해지면 이 슬라이스를 중단해 별도 contract-evolution으로 분리한다.
...
| apply local | 승인 link/version/run registry만 atomic write | explicit decision + checkpoint, `(provider_code, external_item_key, fingerprint)` exact target, duplicate/conflict 0, mixed-provider all-active는 bundle order canonicalization |
...
| disable | payload delete 0 | apply와 별개의 disable approval; run에 포함된 affected source ids 전부를 한 번에 revoke하고 감사 이력 유지 |
...
- 실제 inventory 전수의 source 선택 검수와 local promotion은 2026-07-18 evidence에서 완료했다. production/staging write는 수행하지 않았으며 별도 operator approval이 필요한 Manual Only로 유지한다.
- 전수 local evidence: `evidence/2026-07-18-local-full-coverage.md`
- local inventory `845`개 = approved exactly once `838` + strict excluded `7`; `eligible_without_profile=0`, `unclassified=0`, `classification_conflict=0`, `multiple_qualified_primary=0`
- 최초 apply `writes_committed=7,049`, 동일 입력 replay `writes_committed=0`, production/staging write `0`
- exact MFDS basis는 `100g 157` + `100mL 7`, RDA edible 100g source item `652`; 임의 edible percent와 missing→0 변환은 `0`
