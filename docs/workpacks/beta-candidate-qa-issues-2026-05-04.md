...
| BETA-QA-008 | P2 | Passed on real devices | Wake-lock UX and cooking-mode screen behavior were verified on iOS Safari and Android Chrome after PR #579 / `8c2a2b8b` production deployment. | Screen wake lock is browser/device specific and important while cooking. | Closed for MVP real-device browsers. Rerun if cook-mode wake-lock code, settings persistence, deploy domain, or browser support assumptions change. |
...
| BETA-QA-011 | P1 | Passed on staging | Account deletion was changed to private app-data cleanup plus authored recipe anonymized preservation, then verified after Supabase migration on staging. | Delete-account copy and data behavior must match user trust expectations. | Closed for MVP staging: same-social rejoin starts without previous saved/planner data, while authored YouTube recipes remain available without re-linking to the rejoined user. |
...
- `BETA-QA-008`: passed on 2026-05-24 after PR #579 / `8c2a2b8b` production deployment. iOS Safari and Android Chrome both kept cook mode awake with the setting enabled.
