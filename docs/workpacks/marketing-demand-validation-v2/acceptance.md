...
- [x] lead readiness가 닫혀도 result/experience/planner payoff는 read-only로 유지된다 <!-- omo:id=accept-read-only;stage=2;scope=shared;review=3,6 -->

> Stage 2 evidence scope: lead gate가 anonymous result/experience stage transition을 막지 않음을 backend test로 확인했다. Stage 4 UI evidence는 기존 `accept-result-before-email`, `accept-error` 항목에 남아 있다.
