...
- [x] Stage 6 운영 테스트가 PII 비노출 stdout, safe artifact path, CSV injection 방지, dry-run/confirm purge, paid allowlist cohort, Wilson interval, retention internal scope를 고정한다 <!-- omo:id=accept-stage6-operations-lock;stage=2;scope=shared;review=6 -->
...
- [x] `docs/marketing/demand-validation-analysis.sql`이 paid allowlisted `weekly_nutrition_v2` cohort만 대상으로 unique 세션, accepted/duplicate lead, Green/Yellow/Red, Wilson 95%, target mismatch, result/target/planner 진단 분포를 direct identifier 없이 계산한다 <!-- omo:id=accept-stage6-analysis-sql;stage=2;scope=backend;review=6 -->
- [x] `scripts/export-marketing-leads.mjs`는 consent가 있는 accepted lead의 연락 필드 `email / consent_version / consented_at`만 CSV로 export하고 `.artifacts/marketing-validation/*.csv` 밖 경로를 거부하며 stdout에 email/session 식별자를 남기지 않는다 <!-- omo:id=accept-stage6-lead-export-safe-path;stage=2;scope=backend;review=6 -->
...
- [x] `supabase/migrations/20260831110000_marketing_validation_retention_operations.sql`는 기존 내부 scope를 유지하면서 `marketing-validation-export` exact GET과 `marketing-validation-purge` exact GET/DELETE path만 추가한다 <!-- omo:id=accept-stage6-retention-scope-migration;stage=2;scope=backend;review=6 -->
- [x] `docs/marketing/demand-validation-result-template.md`가 accepted/duplicate/deliverable 분리, primary threshold, Manual Only readiness sign-off를 고정한다 <!-- omo:id=accept-stage6-result-template;stage=2;scope=shared;review=6 -->
- [x] Stage 6 운영 테스트 `tests/marketing-validation-operations.test.ts`가 export/purge/analysis/migration 경계를 회귀로 고정한다 <!-- omo:id=accept-stage6-operations-tests;stage=2;scope=shared;review=6 -->
