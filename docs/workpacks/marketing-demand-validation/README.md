...
- 운영 테스트는 PII 비노출 stdout, `.artifacts/marketing-validation` safe path 강제, consented accepted-only export, CSV injection escape, default dry-run + explicit confirm purge, mock fixture 전용 `--now`, non-PII `--operator-id`, `retention_until < now` strict expiry, paid allowlisted `weekly_nutrition_v2` denominator, Wilson 95%, target rule literal sync, export exact GET 및 purge exact GET/DELETE internal scope를 고정한다.
