...
사용자 승인으로 비로그인은 개인 API를 호출하지 않는 명시적 공개 예시를 둘러보고 mutation 선택 시 로그인 안내를 받는다. 상단 날짜 줄은 간결하게, 선택 날짜 카드 안에 서버 합계 4타일과 음식별 kcal/단백질/실제 양을 표시하며 펼침 그래프를 제공한다. 저장/환산/불완전 영양 정책과 실제 서버 합계 권한은 유지한다. 기존 guest 전체 화면 차단의 시각 정책은 이 범위에서 대체된다. planner-shell/evidence/2026-09-06-prelaunch-ui-plan.md를 따른다.
