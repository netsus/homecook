...
## Happy Path
- [ ] 관리자가 `/admin`에 진입하여 대시보드를 볼 수 있다 <!-- omo:id=accept-happy-path;stage=4;scope=frontend;review=5,6 -->
- [ ] 관리자가 사용자 목록을 조회하고 검색/필터/페이지네이션할 수 있다 <!-- omo:id=accept-admin-users-flow;stage=4;scope=frontend;review=5,6 -->
- [ ] 관리자가 운영 이벤트 로그를 필터/페이지네이션하여 조회할 수 있다 <!-- omo:id=accept-admin-events-flow;stage=4;scope=frontend;review=5,6 -->
...
- [ ] empty 상태가 있다 (데이터 없을 때) <!-- omo:id=accept-empty;stage=4;scope=frontend;review=5,6 -->
- [ ] error 상태가 있다 (API 에러 시) <!-- omo:id=accept-error;stage=4;scope=frontend;review=5,6 -->
- [ ] unauthorized 처리 흐름이 있다 (미인증 시 로그인 유도 또는 차단) <!-- omo:id=accept-unauthorized;stage=4;scope=frontend;review=5,6 -->
- [ ] forbidden(403) 상태가 비관리자에게 적절히 표시된다 (선택적 추가 상태) <!-- omo:id=accept-forbidden;stage=4;scope=frontend;review=5,6 -->
...
- [ ] 비관리자가 Admin 데이터를 볼 수 없는 것 확인 <!-- omo:id=accept-playwright-non-admin;stage=4;scope=frontend;review=5,6 -->
- [ ] 관리자가 대시보드를 로드하고, 사용자를 검색하고, 로그를 확인할 수 있는 것 확인 <!-- omo:id=accept-playwright-admin-flow;stage=4;scope=frontend;review=5,6 -->
- [ ] disabled future nav 항목이 있으면 미구현 페이지로 이동하지 않는 것 확인 <!-- omo:id=accept-playwright-future-nav;stage=4;scope=frontend;review=5,6 -->
