...
- [x] Web Share API가 가능한 환경에서는 OS 공유 시트가 열린다. (2026-05-24 staging, Android Chrome/iOS Safari 확인.)
...
- [ ] 완료 전 팝업 제목 `팬트리에 반영할까요?`가 보인다.
- [ ] 구매 체크 항목과 `이미있음`으로 표시한 팬트리 제외 항목이 팝업 선택 대상에 보인다.
- [ ] 기본 선택은 모든 반영 후보가 선택된 상태이며 API body는 미전달 또는 `undefined` 의미를 유지한다.
- [ ] 기본 선택으로 완료하면 유효 항목이 팬트리에 반영되고 성공 toast에 팬트리 반영 개수가 보인다.
- [ ] 일부 항목만 선택하면 선택한 항목만 `add_to_pantry_item_ids`로 전달된다.
- [ ] 선택을 모두 해제하면 완료 버튼이 비활성화된다.
- [ ] `반영 안 함`으로 완료하면 `add_to_pantry_item_ids: []` 의미가 유지되고 팬트리 반영 개수는 표시되지 않는다.
- [ ] 반영 후보가 없으면 `반영 안 함`만 활성화되고 `N개 반영하기`는 비활성화된다.
