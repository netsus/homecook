...
| **S7** | **사용자 승인: D1/D2/D3/D4 결정 확인** | **⬜ 미완료** |
...
| F2 | 정렬 컨트롤이 discovery panel 보조 행에 위치함 (재료 필터와 같은 행) | `HOME-after-mobile.png` |
| F3 | 테마 섹션이 compact carousel strip으로 구현됨 (1.5장 peek, 가로 스크롤) | `HOME-carousel-strip.png` |
| F4 | first viewport에 레시피 카드 최소 2장 이상 보임 (390px 기준) | `HOME-after-mobile.png` |
...
| # | 기준 | 검증 |
|---|------|------|
| M1 | 390px first viewport: 브랜드 헤더 + discovery panel + carousel strip + 레시피 카드 2장 이상 | `HOME-after-mobile.png` |
| M2 | 320px에서 discovery panel 보조 행(재료 필터 + 정렬) 잘림·중첩 없음 | `HOME-after-narrow.png` |
...
| V2 | 검색/필터 컨트롤이 first viewport 안에 전부 보임 | `HOME-after-mobile.png` |
...
| CB2 | 390px first viewport에서 레시피 카드가 0장 (개선 없음) |
| CB3 | 320px에서 discovery panel 보조 행 잘림 또는 중첩 |
| CB4 | carousel strip이 page-level 가로 스크롤처럼 느껴짐 |
| CB5 | GET /recipes 또는 theme API 계약 변경 발생 |
| CB6 | 정렬 컨트롤이 discovery panel 보조 행에 없음 (D1 미이행) |
