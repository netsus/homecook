...
- [x] 기존 레시피북을 선택하고 [저장] 클릭 시 레시피가 해당 책에 저장된다
...
- [x] 저장 후 user_status.saved_book_ids에 저장한 book_id가 포함된다
- [x] API 응답 형식이 `{ success, data, error }` 래퍼를 따른다
- [x] 백엔드 계약과 프론트 타입이 일치한다

## State / Policy
- [x] book_type이 'saved' 또는 'custom'인 레시피북만 저장 대상으로 노출된다
- [x] book_type이 'my_added' 또는 'liked'인 레시피북으로 저장 시도 시 409 CONFLICT 반환된다
- [x] 동일 레시피를 동일 레시피북에 재저장 시 409 CONFLICT 반환된다
- [x] 다른 유저의 레시피북으로 저장 시도 시 403 FORBIDDEN 반환된다
- [x] 레시피북 생성 시 book_type은 자동으로 'custom'으로 설정된다
- [x] 저장 성공 시 recipes.save_count가 1 증가한다

## Error / Permission
- [x] SAVE_MODAL 로딩 상태가 있다 (레시피북 목록 조회 중)
- [x] 저장 처리 로딩 상태가 있다 (저장 API 호출 중)
- [x] 레시피북 목록 조회 실패 시 에러 메시지가 노출된다
- [x] 저장 실패 시 에러 메시지가 노출되고 모달이 닫히지 않는다
- [x] 비로그인 상태에서 [저장] 버튼 클릭 시 로그인 게이트 모달이 노출된다
- [x] 로그인 게이트 모달에서 [로그인] 선택 시 로그인 화면으로 이동한다
- [x] 로그인 완료 후 SAVE_MODAL이 자동으로 오픈된다 (return-to-action)
- [x] 존재하지 않는 레시피에 저장 시도 시 404 RESOURCE_NOT_FOUND 반환된다
- [x] 존재하지 않는 레시피북에 저장 시도 시 404 RESOURCE_NOT_FOUND 반환된다

## Data Integrity
- [x] 타인의 레시피북에 저장할 수 없다 (403)
- [x] 레시피북 생성 시 name 필드가 누락되면 422 VALIDATION_ERROR 반환된다
...
- [x] 중복 저장 시도 시 recipe_book_items UNIQUE 제약으로 409 반환된다
