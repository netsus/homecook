...
- Stage 2는 narrow additive migration으로 no new table or column을 유지한 채, planner complete와 standalone complete의 기존 공개 경로를 위한 scoped `SECURITY DEFINER` RPC만 보정한다. existing planner and standalone public endpoint/body/response는 그대로 유지한다. 정확한 identity signature는 `public.complete_cooking_session(uuid, timestamptz, text, integer, timestamptz, uuid, uuid[], uuid, timestamptz)`와 `public.complete_standalone_cooking(uuid, timestamptz, text, integer, timestamptz, uuid, integer, uuid[], uuid, timestamptz)`다. 전자는 `(p_owner_uuid, p_auth_identity_created_at_snapshot, p_session_key_hash, p_hmac_key_version, p_session_issued_at, p_session_id, p_consumed_ingredient_ids, p_idempotency_key, p_now)`, 후자는 같은 authority prefix 뒤 `(p_recipe_id, p_cooking_servings, p_consumed_ingredient_ids, p_idempotency_key, p_now)`만 받는다. `p_now`는 DB default이며 public request가 owner, generation, time authority를 주입하는 경로는 없다.
- PostgreSQL overload는 replace가 아니다. legacy `public.complete_cooking_session(uuid, uuid, uuid[])`와 `public.complete_standalone_cooking(uuid, uuid, integer, uuid[])`는 새 9/10-인자 signature와 별개로 남는다. 따라서 same Stage 2 migration/transaction as the route cutover에서 route를 new signature의 internal client로 전환하고, both old exact signatures에는 `REVOKE EXECUTE on both old exact signatures from PUBLIC, anon, authenticated, service_role`를 적용한다. old exact signatures are absent from the callable inventory이며, 물리 `pg_proc` 보존이 필요하면 privilege 없는 disabled legacy overload로만 남긴다. existing public HTTP endpoint/body/response는 유지한다.
- 두 RPC는 `auth.role() = service_role`인 server-only call만 허용하고, route가 `public.assert_recipe_future_session_authority`에 쓸 server-verified JWT sub/session_id/iat evidence를 기존 내부 client에서 전달한다. 함수는 이 evidence와 auth identity epoch, active expected account generation을 같은 transaction에서 재검증하고 generation을 결과에서만 취한다. owner + account generation + stored legacy_v1 predicate는 caller owner/generation만으로 신뢰하지 않으며, request JWT/role·session binding 또는 `iat`가 stale/revoked/mismatched이면 claim 전 write 0이다.
