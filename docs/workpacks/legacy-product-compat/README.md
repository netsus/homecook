...
- privilege barrier: negative privilege tests deny PUBLIC/anon/authenticated execute and service-role direct DML, while the exact internal client may execute only the two inventoried RPC signatures. search path, owner, control class/effect/exposure/principal inventory, `auth.role()` and server-verified session authority are each inspected independently.
