...
- [ ] phased cutover를 따른다: additive DB phase creates new scoped RPC/core while old overload remains only for existing optional-key/no-key compatibility. old overload는 `public.complete_cooking_session(uuid, uuid, uuid[])`와 `public.complete_standalone_cooking(uuid, uuid, integer, uuid[])`다. deploy new route callers to new RPC; prove current head and drain old server instances. under approved maintenance/write fence and explicit server-Mac/OAuth/manual authority, revoke/drop old 3/4-arg overload execute와 `REVOKE EXECUTE on both old exact signatures from PUBLIC, anon, authenticated, service_role`를 적용한다. 그 후 old exact signatures are absent from the callable inventory이며 function inventory privilege test는 새 two-signature service_role execute만 통과시키고 old authenticated self-call is denied with mutation 0 및 old overload replay/rollback mutation 0을 검증한다. rollback before revoke can return route to old optional-key flow. after revoke rollback must not restore legacy grants or bypass; instead re-enable last safe new-route/RPC version under fence. required-key transition cannot activate until old overload revoke/drain evidence. all remote application/fence/activation remains Manual pending and is not executed by Stage2 automation <!-- omo:id=accept-legacy-compat-legacy-overload-cutover;stage=2;scope=backend;review=3,6 -->
...
- [ ] server tests가 cursor/idempotency/telemetry barrier와 mutation/removal 0을 deterministic하게 잠근다. `MAINTENANCE/QUARANTINED/DELETING and malformed/reused/missing/version/owner rejection`의 `checksum/delta 0 across users, recipe_books, meal_plan_columns, ledger, completion, progress events/summaries`를 확인한다. negative privilege tests deny PUBLIC/anon/authenticated execute and service-role direct DML이며 two exact RPC signature의 service-role execute만 허용한다. function inventory privilege test, old authenticated self-call is denied with mutation 0, old overload replay/rollback mutation 0과 두 completion endpoint의 세 lifecycle exact wrapper를 함께 고정한다. route deploy/current-head drain과 Manual fenced revoke/drop은 자동 fixture와 분리하고 activation을 완료로 표시하지 않는다 <!-- omo:id=accept-legacy-compat-vitest-server;stage=2;scope=backend;review=3,6 -->
