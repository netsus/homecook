...
> Stage 2/3 backend checkpoint는 PR #1369 merge `50e31293e6740b3fdc56d022e12d3b9fe8be4cf7`로 merged-green이다. Stage 4 author task `01a01e01-0a0e-7f70-97dd-2e6c8f0012af`의 local implementation과 browser evidence는 Draft PR #1371에서 GREEN이다. 현재 lifecycle은 `in_progress`, approval/evaluation은 `not_started`, verification은 `pending`, auto-merge는 `false`이며 fresh Stage 5/6, Manual/activation/tombstone authority는 없다.
...
- [ ] `full-release no-key 0 is telemetry evidence only; missing Idempotency-Key remains compatible success until a separately approved exact required-key activation`이다 <!-- omo:id=accept-legacy-compat-no-key-intermediate-server;stage=2;scope=backend;review=3,6 -->
- [ ] `only after separately approved exact required-key activation following full-release no-key 0 may missing Idempotency-Key return 428 IDEMPOTENCY_KEY_REQUIRED mutation 0`이다 <!-- omo:id=accept-legacy-compat-required-key-server;stage=2;scope=backend;review=3,6 -->
