...
- [ ] same Stage 2 migration/transaction as the route cutover에서 legacy `public.complete_cooking_session(uuid, uuid, uuid[])`와 `public.complete_standalone_cooking(uuid, uuid, integer, uuid[])`를 새 signature로 replace하지 않고 execute 경로에서 제거한다. `REVOKE EXECUTE on both old exact signatures from PUBLIC, anon, authenticated, service_role`를 적용해 old exact signatures are absent from the callable inventory로 만든다. function inventory privilege test는 새 two-signature service_role execute만 통과시키고 old authenticated self-call is denied with mutation 0 및 old overload의 replay/rollback mutation 0을 검증한다 <!-- omo:id=accept-legacy-compat-legacy-overload-cutover;stage=2;scope=backend;review=3,6 -->
