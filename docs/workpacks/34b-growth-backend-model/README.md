...
34a에서 merge된 성장/레벨링 v2 공식 계약을 실제 서버 모델로 구현한다. 사용자는 플래너 등록을 포함한 5개 XP source에서 첫/반복 경험치를 분리 적립받고, 장기 레벨 곡선 v2와 등급명 기준으로 자신의 성장을 조회할 수 있게 된다. 이 slice는 backend model 전용이며, additive DB migration, XP writer v2, non-XP activity ledger, level curve v2/grade, backfill no-toast, notification priority/archive server logic, API additive response를 닫는다. 런타임 UI 표시는 34c/34d가 담당한다.
...
- API:
  - `GET /api/v1/users/me/progress` additive 확장 (공식 표기 `GET /users/me/progress`): `event_counts.planner_registered_first`, `event_counts.planner_registered_repeat` 추가. progress-only 계약 유지
...
- 상태 전이:
  - 5개 XP source의 첫/반복 XP 분리 적립과 KST repeat cap 적용
  ...
  - non-XP activity 7종을 `user_growth_activity_events`에 idempotent append
  - historical/backfill recompute는 XP/레벨/등급/배지/퀘스트 상태만 조용히 반영하고 notification/archive row를 만들지 않음
- DB 영향 (모두 additive migration):
  - `user_progress_events`: `event_type` 허용값에 `planner_registered` 추가, `source_meta_json` 컬럼 추가
  - `user_progress_summary`: `level_curve_version` 컬럼 추가, `event_counts`에 `planner_registered_first` / `planner_registered_repeat` 키 추가
...
- cap window는 서버가 `Asia/Seoul` (KST, UTC+9 고정) 기준으로 계산한다. day window는 KST calendar date, week window는 KST 기준 월요일 시작 ISO week다.
- cap 초과 반복 액션은 `user_progress_events` row를 만들지 않는다 (`xp_delta > 0` 제약과 일관). source action 성공은 유지되고 notification도 만들지 않는다.
- cap 계산의 단일 권한자는 서버다. `source_meta_json`의 cap window key와 ledger 조회로 같은 결과가 재현돼야 한다.
...
| abuse 방지 | meal 삭제 후 재생성은 새 `meal_id`로 repeat award가 가능하지만 KST 3/day, 12/week cap 안에서만 적립된다 |
...
- cap 적용 이벤트면 cap window key (예: `cap_day_key` = KST date, `cap_week_key` = KST ISO week)
...
XP를 지급하지 않는 활동 truth. badge/quest/최근 성장 기록의 데이터 기반이며 `user_progress_events`의 XP 경제와 분리한다.
...
- priority 값: `level_up=1`, `badge_unlocked=2`, `quest_completed=3`, `xp_awarded=4`. 정렬은 서버 권한이며 클라이언트가 순서를 바꾸지 않는다.
...
- deterministic order: `planner_registered` backfill은 `meals.created_at ASC, meals.id ASC` 순서로 first XP와 cap 적용 repeat XP를 계산한다
...
- fixture baseline:
  ...
  - 5개 XP source 각각 first/repeat가 섞인 사용자
  - planner repeat cap 경계: KST 같은 날 repeat 3회 적립 후 4번째 미적립, 주간 12회 적립 후 13번째 미적립, KST 자정/주 경계(일→월) 직전·직후 케이스
  - custom book repeat cap 경계: KST 같은 날 repeat 2회 후 3번째 미적립
  - first XP가 repeat cap을 소비하지 않는 케이스 (같은 날 first 1 + repeat 3 = 4건 적립)
  - meal 삭제 후 재생성 abuse 케이스 (cap 안에서만 적립)
  ...
  - non-XP activity 7종 각각의 중복 시도 케이스
...
- blocker 조건:
  ...
  - planner repeat cap(KST 3/day, 12/week) 또는 first/repeat 분리가 빠진 설계
...
2. `user_progress_events`는 XP 지급 ledger, `user_growth_activity_events`는 non-XP activity ledger다. 두 책임을 섞지 않는다.
3. XP/level/grade/badge/quest/notification 계산의 단일 권한자는 서버다.
4. 첫 XP와 반복 XP는 분리하며, repeat cap은 KST(Asia/Seoul) 기준으로 서버가 계산한다. first는 repeat cap을 소비하지 않는다.
...
2. 서버가 XP source 5종은 `user_progress_events`에 첫/반복 XP 분리와 cap 적용으로 기록하고, non-XP activity 7종은 `user_growth_activity_events`에 idempotent하게 기록한다.
