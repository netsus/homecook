...
| `youtube-async-extraction-notification` | in-progress | 기존 i031 추출을 request-independent worker job으로 실행하고 이탈·재실행·재로그인 뒤에도 owner-scoped 결과를 toast, badge, durable list로 복구하는 Stage 2 백엔드 구현 |
...
| `cooking-meal-log-cross-slice-release-qa` | in-progress | Stage 2 verification-only가 `afb1b31aa6c95ba974f7484d31fa123439d5fcd6`에서 시작됐다. F0/#1~#13과 Stage 1/repair merge ancestry preflight는 통과했고 pinned isolated local full preflight는 green이다. Stage 4는 approved disposable isolated rehearsal only이며 reserved production-shaped HTTPS issuer claim과 loopback-only transport를 쓴다. Stage 3 rereview, Stage 4 `FINAL_EVIDENCE_SHA` same-head rerun/browser bundle, controlled full-local, Stage 4~6, Manual/activation은 pending이다 |
...
| 14 | F | `cooking-meal-log-cross-slice-release-qa` | in-progress | Stage 2 verification-only가 `afb1b31aa6c95ba974f7484d31fa123439d5fcd6`에서 시작됐다. predecessor 및 Stage 1/repair ancestry preflight는 통과했고 pinned isolated local full preflight는 green이다. Stage 4는 approved disposable isolated rehearsal only이며 reserved production-shaped HTTPS issuer claim과 loopback-only transport를 쓴다. Stage 3 rereview, Stage 4 `FINAL_EVIDENCE_SHA` same-head rerun/browser bundle, controlled full-local, Stage 4~6, Manual/server-Mac/OAuth/device/AT/full-WCAG/local-production/rehearsal/backup-restore/cutover/capability/R/R+1/R+2/required-key/activation은 pending이다 |
