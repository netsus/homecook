...
| `product-ingredient-link-foundation` | in-progress | 승인 Contract Evolution PR #1254는 `8b4287f`로 merged. Stage 2 backend/data 구현은 `feature/be-product-ingredient-link-contract-runtime`에서 RED→GREEN, local PostgreSQL fresh/replay와 backend gate까지 완료했으며 Stage 3 독립 리뷰 대기 |
...
| 2 | B | `product-ingredient-link-foundation` | in-progress | F0 + #3, link-only subset/verifier, Contract Evolution merged; Stage 2 backend/data Draft 구현 완료 후 Stage 3 독립 리뷰 대기. production account cleanup activation은 Manual Only |
