...
| `marketing-demand-validation` | in-progress | Stage 1 계약 병합 후 Stage 2/3 백엔드 완료: 단일 session table/route, UUID cookie, ordered first-write-wins, server target rule, duplicate generic success, Turnstile/privacy fail-closed를 구현·독립 검토했으며 frontend/authority/production activation은 후속이다 |
