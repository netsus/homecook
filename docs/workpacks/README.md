...
| `recipe-content-snapshot-future-propagation` | in-progress | Stage 2/3 PR #1278 reviewed head `9eb1ebd8`는 `ef5903b1`로 merge됐다. Draft Stage 4 PR #1281 code commit `8856184a`는 실제 entrypoint/owner editor와 AUTH-M01/M02 focus·recovery repair를 포함한다. Earlier final authority는 HOLD `0/2/0`이므로 Design Status는 pending-review/temporary이며 fresh final authority, Stage 6, Manual/server-Mac, activation은 pending이다 |
