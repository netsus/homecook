...
| `legacy-product-compat` | in-progress | #13 Stage 2 backend author GREEN: Draft PR #1369에 inactive-owner visibility repair `ae517053`와 actual-route evidence repair `d6032a59`를 추가했다. Expanded focused `10 files / 76 tests`, isolated PostgreSQL `12/12`, full Vitest `6,106 pass / 415 intended skip`, product `2,756 / 175`, security E2E `12/12`가 통과했다. Exact head `b6c5be31` security/DB review는 `APPROVE 0/0/0`; code/quality review `REQUEST_CHANGES 0/1/2`의 runtime/evidence findings는 위 두 commits로 수리했고 PR-body terminal projection은 successor push 뒤 갱신한다. Fresh exact-head re-review와 successor current-head CI 전 approval/evaluation은 `not_started`, verification은 `pending`, auto-merge는 false다. #10/#12 runtime dependency fulfilled; Manual/server-Mac/OAuth, drain/fence/old overload revoke, deployed inventory, Stage 4~6, R/R+1/R+2, production/activation은 pending이다 |
...
| 13 | E | `legacy-product-compat` | in-progress | #10 + #12 runtime dependency fulfilled; broader Manual/activation pending |
