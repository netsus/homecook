...
| `prepared-food-standard-basis-ux` | docs-in-progress | predecessor가 구현한 100g/100mL 비교·source/label·추정 금지를 교차 잠그고, 남은 MEAL_SCREEN g/mL 수량 변경 step 불일치를 TDD로 고친다. Stage 2/3 backend는 N/A |
...
| `prepared-food-standard-basis-ux` | docs-in-progress | `community-prepared-food-catalog` = merged, `prepared-food-planner-entry` = merged, `planner-nutrition-summary` = merged | Successor 5; Stage 1 draft는 residual FE/authority만 소유하며 독립 internal 1.5 승인과 docs PR merge 전 구현 금지 |
