...
| `33-youtube-i031-direct-extraction` | in-progress | 기존 YT_IMPORT에서 exact cv-goal-i031-ocr selector/final 추출을 localhost strict mode로 직접 실행하고 기존 재료 표준명 검수 흐름에 연결 |
...
| `recipe-content-snapshot-future-propagation` | in-progress | Stage 2/3 PR #1278 reviewed head `9eb1ebd8`는 `ef5903b1`로 merge됐다. Draft Stage 4 PR #1281 exact implementation head `1096494a`는 실제 entrypoint/owner editor와 AUTH-M01/M02/AUTH-M03 repair를 포함한다. final authority와 fresh Stage 6 task `019fca98-d8a1-7613-a34f-39cc836de385`가 `0/0/0`으로 승인했고 Design Status는 confirmed다. PR #1281 merge와 별개로 Manual/server-Mac/OAuth, #8 gate와 activation이 남아 전체 slice lifecycle은 in-progress다 |
...
| 7 | C | `recipe-content-snapshot-future-propagation` | in-progress | #4 PR #1218 + #6 PR #1274 + `cook-mode-whole-board` PR #711 runtime merged; Stage 2/3 PR #1278 merge `ef5903b1`; Contract Evolution PR #1282 + re-lock PR #1283의 공식 tuple을 Draft Stage 4 PR #1281 exact implementation head `1096494a`에서 actual owner editor와 AUTH-M01/M02/AUTH-M03 repair까지 구현했다. final authority와 fresh Stage 6가 `0/0/0`으로 승인했으며 Design Status는 confirmed다. PR #1281 merge 뒤에도 Manual/server-Mac/OAuth, #8 gate와 activation이 남아 전체 lifecycle은 in-progress다 |
