...
| `ingredient-nutrition-full-coverage` | in-progress | 모든 active canonical ingredient를 승인 nutrition profile 또는 명시적 제외 사유로 분류하고 coverage denominator와 unclassified 0 기준을 닫는다 |
...
| `public-prepared-food-catalog-import` | in-progress | 검수된 공공 완제품을 batch-only local catalog로 승격하고 stable key, attribution, rollback을 보존한 검색 경로를 연다 |
...
| `prepared-food-standard-basis-ux` | planned | 제품 비교를 100g/100mL 기준으로 통일하고 planner 수량을 1g/1mL 단위로 조절하며 원 라벨 basis와 source tag를 일관되게 표시한다 |
...
| `ingredient-nutrition-full-coverage` | in-progress | 2026-07-17 public-sharing official docs = merged, `public-nutrition-source-acquisition` = merged, `ingredient-nutrition-conversion-model` = merged | Successor 1; Stage 2/3 implementation tree merged in PR #1030 and independent review approved it. Actual inventory-wide source decisions and external promotion remain Manual Only, so the slice stays in-progress |
...
| `public-prepared-food-catalog-import` | in-progress | 2026-07-17 public-sharing official docs = merged, `prepared-food-catalog` = merged | Successor 3; Stage 1 docs are merged, Stage 2 implementation and local 10k/full promotion evidence are complete, independent Stage 3/current-head merge gate remains |
