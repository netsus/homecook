...
| `33a-user-progress-foundation` | planned | 전용 progress ledger/read model과 `GET /users/me/progress` contract를 backend-first로 구현. 4개 canonical event writer/backfill/reconcile. `GET /users/me`는 profile-only 유지 |
