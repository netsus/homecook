...
| `personal-recipe-editor-decoupling` | in-progress | PR #1238 backend 경계와 PR #1243 capability-off shell/UI evidence, PR #1246 hybrid verifier는 역사로 보존한다. Active gate는 full-local Stage 2 read-only verifier → 독립 Stage 3 → existing shell Stage 4 재검증 → lightweight Stage 5 → merged-exact Stage 6이며 activation/Manual Only 증거는 pending이다 |
...
| 5 | C | `personal-recipe-editor-decoupling` | in-progress | #3; `31-recipe-media-tags` merged; `36e-recipe-tags-frontend` merged; #4는 predecessor 아님. PR #1238, capability-off PR #1243 merge `6565c2a8`, hybrid verifier PR #1246은 historical evidence. Full-local Stage 2→6 재검증과 Manual Only activation은 pending |
