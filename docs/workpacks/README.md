...
| `34a-growth-model-contract-evolution` | docs | 33c 이후 성장/레벨링 v2 공식 계약 잠금. `planner_registered` XP source, non-XP activity ledger, 레벨 곡선 v2, 등급명, toast stack, archive, MYPAGE profile integration, badge shape/image concept 원칙 |
| `34b-growth-backend-model` | planned | 34a 계약 기반 backend model 구현. XP writer v2, `user_growth_activity_events`, level curve v2/grade, backfill no-toast, notification priority/archive server logic |
