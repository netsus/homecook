...
| `recipe-nutrition-calculation` | in-progress | 레시피 재료·인분·대표 환산으로 completeness/quality와 scalable/fixed vectors를 계산하고 immutable snapshot을 생성/pin/backfill하며 Recipe Detail additive API와 최소 상태 UI를 제공한다 |
...
| `recipe-nutrition-calculation` | in-progress | `ingredient-nutrition-conversion-model` + PR #1005 pilot + PR #1006 official contract = merged |
