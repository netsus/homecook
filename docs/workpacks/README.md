...
| `personal-recipe-customization-write-core` | in-progress | Stage 2 dormant backend core가 TDD와 disposable PostgreSQL fresh/replay를 통과했다. 독립 Stage 3, current-head CI, Manual Only와 #7/#8 activation은 pending이다 |
...
| 6 | C | `personal-recipe-customization-write-core` | in-progress | #2 + #3 + #4 + #5 runtime merged; Stage 2 Draft PR/독립 Stage 3 pending |
