...
| `personal-recipe-editor-decoupling` | in-progress | 공개 원본 불변 fork 및 owner-only 개인 레시피 편집 진입을 RECIPE_DETAIL 중심으로 분리하고 기존 MYPAGE/RECIPEBOOK 상세과의 소유권 충돌을 피한다. Stage 2는 기존 backend 계약을 테스트로 잠갔고, Stage 4는 capability-off 공유 편집 shell과 local-fixture 증거를 구현 중이다. 새 write runtime API는 추가하지 않는다 |
...
| 5 | C | `personal-recipe-editor-decoupling` | in-progress | #3; `31-recipe-media-tags` merged; `36e-recipe-tags-frontend` merged; Stage 2/3 evidence PR #1238 merged, Stage 4 capability-off client shell과 local-fixture 검증 진행 중 |
