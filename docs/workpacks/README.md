...
| `product-ingredient-link-foundation` | in-progress | link-only Stage 2 subset·hybrid verifier는 merged. 2026-07-31 승인 Contract Evolution의 internal 1.5 request_changes를 같은 Draft PR에서 repair 중이며 fresh re-review 전 구현은 시작하지 않는다 |
...
| 2 | B | `product-ingredient-link-foundation` | in-progress | F0 + #3 및 link-only subset/verifier merged; reader authenticated-self, shopping provenance automation, successor ownership finding repair 뒤 fresh Stage 1 internal 1.5 re-review pending |
