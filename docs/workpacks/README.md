...
| `legacy-product-compat` | in-progress | #13 Stage 2 backend author GREEN: Draft PR #1369 product/evidence head `4bb7f65c`, focused `8 files / 54 tests`, isolated PostgreSQL `11/11`, full Vitest `6,103 pass / 414 intended skip`, product `2,756 / 175`, security E2E `12/12`와 unique current-head checks `12 success + 2 intended skip`가 통과했다. Frozen `be65e6df` security/DB review는 runtime finding `0`이고 이 projection drift만 P2로 남겼다. Fresh exact-head Stage 3와 successor current-head CI 전 approval/evaluation은 `not_started`, verification은 `pending`, auto-merge는 false다. #10/#12 runtime dependency fulfilled; Manual/server-Mac/OAuth, drain/fence/old overload revoke, deployed inventory, Stage 4~6, R/R+1/R+2, production/activation은 pending이다 |
