...
| `product-ingredient-link-foundation` | in-progress | link-only Stage 2 subset·hybrid verifier는 merged. 2026-07-31 승인된 pantry/shopping product provenance Contract Evolution Stage 1 docs는 독립 internal 1.5 review 전이며 구현은 아직 시작하지 않는다 |
...
| 2 | B | `product-ingredient-link-foundation` | in-progress | F0 + #3 및 link-only subset/verifier merged; 승인된 exact pantry product/version, shared reader, shopping provenance, RESTRICT FK 계약의 Stage 1 docs internal 1.5 review pending |
