...
| `H1-home-first-impression` | `HOME` | 정렬 위치(D1), 테마 섹션 형태(D2), 재료 필터 행 배치(D3), 전체 방향(D4) — 안 C(compact carousel hybrid) 권고 | ⬜ 대기 중 (S7 승인 필요) | `h1-home-first-impression` |
...
| `h1-home-first-impression` | docs | H1 ⬜ | HOME compact carousel hybrid(안 C) — 정렬 보조 행 이동, 테마 carousel strip, first viewport 레시피 밀도 개선 |
