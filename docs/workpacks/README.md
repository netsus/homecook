...
- `v2` nutrition products/planner all-recipe Stage 1 docs (2026-07-18)
  - `ingredient-nutrition-full-coverage`를 PR #1038 merge `3c737eae` 근거로 `merged` projection으로 정정하고, `all-recipe-nutrition-recalculation`은 Stage 1 docs PR #1039가 열려 roadmap을 `docs`로 전환했다. workflow-v2 구현 lifecycle은 Stage 2 전까지 `planned`를 유지한다.
...
| `all-recipe-nutrition-recalculation` | docs | 전수 승격된 ingredient nutrition 결과로 checkpoint 시점 `public.recipes` 전체의 current nutrition snapshot을 재계산하고 predecessor `missing_reasons` / canonical ordered `warnings_json` report를 고정한다 |
...
| `all-recipe-nutrition-recalculation` | docs | `ingredient-nutrition-full-coverage` = merged, `recipe-nutrition-calculation` = merged | Successor 2; Stage 1 docs PR #1039 open and independently approved; all-public-recipes checkpoint snapshot backfill must not start before that PR merges |
