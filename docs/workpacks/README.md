...
| `recipe-content-snapshot-future-propagation` | in-progress | Stage 2/3 PR #1278 reviewed head `9eb1ebd8`는 `ef5903b1`로 merge됐다. Contract Evolution PR #1282와 Stage 1 re-lock PR #1283의 공식 tuple을 Draft Stage 4 PR #1281 code head `7ca6a5dd`에서 실제 entrypoint, owner editor와 보안·hybrid authority inventory로 구현했다. Design Status는 pending-review/temporary이며 fresh authority, Stage 5/6, Manual/server-Mac, activation은 pending이다 |
...
| 7 | C | `recipe-content-snapshot-future-propagation` | in-progress | #4 PR #1218 + #6 PR #1274 + `cook-mode-whole-board` PR #711 runtime merged; Stage 2/3 PR #1278 merge `ef5903b1`; Contract Evolution PR #1282 + re-lock PR #1283의 v1.7.29/v1.5.33/v1.3.31/DB1.3.31/API1.2.35를 Draft Stage 4 PR #1281 code head `7ca6a5dd`에서 실제 owner editor까지 구현하고 생성형 authority inventory를 동기화했다. fresh authority/Stage 5/6, Manual/server-Mac, activation pending |
