...
| `planner-shell` | in-progress | PR #1326의 Stage 1 docs/relock 뒤 Stage 2는 backend implementation N/A로 유지하고 `planner-shell-compatibility` test-only 계약을 추가했다. Planner reads, 계획 영양 compatibility, legacy read/delete·v1 cursor, owner boundary와 완료 장보기 read-only를 잠그며 #10 frontend runtime과 fresh Stage 3/4 이후 review는 pending이다 |
...
| 10 | E | `planner-shell` | in-progress | #9. Stage 2 backend implementation N/A; compatibility contract tests only. Frontend runtime과 fresh Stage 3/4 이후 review는 pending |
