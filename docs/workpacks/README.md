...
| `cooking-meal-log-cross-slice-release-qa` | docs | F0/#1~#13 automated/runtime merge evidence는 `c5b21315` 기준 green이다. Stage 1 relock/reviews 뒤 pinned isolated local과 권한 있는 controlled full-local read-only 검증을 수행하며 Manual/activation은 pending으로 분리한다 |
...
| 14 | F | `cooking-meal-log-cross-slice-release-qa` | docs | F0/#1~#13 automated/runtime merges retained green at `c5b21315`; overall Manual/server-Mac/OAuth/device/AT/full-WCAG/local-production/rehearsal/backup-restore/cutover/capability/R/R+1/R+2/required-key/activation pending |
