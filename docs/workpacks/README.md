...
| `all-recipe-nutrition-recalculation` | in-progress | 전수 승격된 ingredient nutrition 결과로 checkpoint 시점 `public.recipes` 전체의 current nutrition snapshot을 재계산하고 predecessor `missing_reasons` / canonical ordered `warnings_json` report를 고정한다 |
