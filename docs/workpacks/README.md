...
| `account-session-generation-foundation` | in-progress | JWT session-bound account generation, lifecycle watermark, DB cutover fence/Auth Hook/quarantine/outbox와 personal-writer inventory를 feature-off foundation으로 잠근다 |
...
| ---: | --- | --- | --- | --- |
| F0 | B | `account-session-generation-foundation` | in-progress | contract gate; security hotfix merged and deployed |
