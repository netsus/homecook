...
| `12a-shopping-complete`        | in-progress   | 장보기 완료 core, `shopping_done` 전이, `is_completed=true`, 완료 직후 read-only lock, 멱등성                   |
...
| `baemin-style-planner-week-retrofit` | merged | h6 ✅ | PLANNER_WEEK anchor screen 배민 스타일 시각적 리트로핏 — H2/H4 day-card contract 보존, 토큰 교체, STATUS_META color-mix() 전환, glass-panel 제거 |
| `baemin-style-modal-system-fit` | docs | h6 ✅ | Modal/sheet 오버레이 패밀리 배민 스타일 시각 정합 — LoginGateModal H5 합류(ModalHeader, eyebrow 제거, icon-only close), interaction modal surface/shadow/radius/footer 토큰 일관성 |
