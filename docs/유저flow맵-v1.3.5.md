...
> 기준 문서: 요구사항 기준선 v1.6.8 / 화면정의서 v1.5.5 / DB 설계 v1.3.4 / API 설계 v1.2.6
>
...
Step 3) 결과 검수/수정
  ...
  │ │         needs_review → 경고 배지, candidate 선택/교체 전 저장 차단
  │ │         unresolved  → 에러 배지 (저장 차단)
  │ ├─ 스텝 리스트 (추가/삭제/수정)
...
| 레시피 등록 (RPC) | recipes | INSERT (source_type='youtube') |
