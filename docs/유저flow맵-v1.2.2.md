...
> 기준 문서: 요구사항 기준선 v1.6.2 / 화면정의서 v1.2.2 / DB 설계 v1.3.1 / API 설계 v1.2.2
>
> 작성: 킴실장
>
> v1.2.1 → v1.2.2 변경: 플래너를 4끼 고정 day-card 구조로 재정의

---

## v1.2.1 → v1.2.2 변경 체크리스트

| #   | 변경 내용                                                 | 영향 범위                   | 상태 |
| --- | --------------------------------------------------------- | --------------------------- | ---- |
| 1   | `PLANNER_WEEK` 끼니 컬럼 CRUD를 제거하고 4끼 고정 day-card 구조로 재정의 | ③ 식단 계획 여정 | ✅ |
...
```
PLANNER_WEEK (식단 플래너)
  ├─ 주간 범위 확인 (이번 주 / 다음 주 / 지난 주)
  ├─ [이전 주] [다음 주] [이번주로 가기]
  ├─ 하루 카드 단위로 식단 확인
  │    └─ 카드 안에 아침 / 점심 / 간식 / 저녁 4끼 고정 노출
