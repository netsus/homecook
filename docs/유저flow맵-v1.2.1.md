...
> 기준 문서: 요구사항 기준선 v1.6.1 / 화면정의서 v1.2.1 / DB 설계 v1.3 / API 설계 v1.2.1
>
> 작성: 킴실장
>
> v1.2 → v1.2.1 변경: 플래너 끼니 컬럼 drag reorder UX 반영

---

## v1.2 → v1.2.1 변경 체크리스트

| #   | 변경 내용                                                 | 영향 범위                   | 상태 |
| --- | --------------------------------------------------------- | --------------------------- | ---- |
| 1   | `PLANNER_WEEK` 끼니 컬럼을 drag handle로 순서 변경        | ③ 식단 계획 여정           | ✅   |
...
PLANNER_WEEK (식단 플래너)
  ├─ 위아래 무한 스크롤로 주차 이동
  ├─ 끼니 컬럼 관리 ([+] 추가, 최대 5개)
  │    ├─ drag handle로 좌우 순서 변경
  │    ├─ 드롭 직후 저장
  │    └─ 실패 시 직전 순서로 원복 + 오류 안내
