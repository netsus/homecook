...
- 테스트 범위는 현재 `AGENTS.md`를 우선한다. 관련 회귀 테스트가 필요한 변경은 구현 전에 실패 테스트를 먼저 설계한다.
- 브라우저 사용자 흐름과 외부 연동 검증은 `docs/engineering/playwright-e2e.md` 기준을 따른다.

## 실행 모음

- `pnpm test`는 `test:product`를 실행한다. 일반 변경은 관련 파일을 `pnpm exec vitest run --config vitest.product.config.ts tests/<관련-test>.test.ts`로 선택 실행한다.
...
- `tests/vitest-suite-partition.test.ts`가 경로 존재·완전성·중복·신규 파일 기본값을 검증한다.
  전체 제품 테스트 실행은 매 작업의 의무가 아니다. 자동 CI는 사용하지 않는다.
