...
| 에이전트 | 역할 |
|----------|------|
| **Claude** | Workpack 문서 작성 (1단계), 백엔드 리뷰 (3단계), 프론트엔드 구현 (4단계), authority-required slice의 final authority gate, CI 디버깅 |
| **Codex** | TDD 기반 백엔드 구현 (2단계), authority precheck, 디자인 리뷰 (5단계), 프론트엔드 PR 리뷰/closeout (6단계), API 계약 고정, 커밋/PR 생성 |
...
product slice의 기본 public stage 경로에서는 사용하지 않는다.
slice workflow에는 이미 Stage 3, 5, 6의 정식 리뷰/authority/closeout 단계가 있으므로, product slice에서 review loop를 기본 또는 권장 경로로 중복 추가하지 않는다.
