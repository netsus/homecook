...
> **사용 중지:** 2026-09-14부터 이 감사 절차와 자동 수리는 신규 작업에 사용하지 않는다. 아래는 과거 설계 기록이며 현재 작업 절차는 `AGENTS.md`가 우선한다.
...
- stable set에 남아 있는 finding ID가 곧 current active blocker를 뜻하지는 않는다.
