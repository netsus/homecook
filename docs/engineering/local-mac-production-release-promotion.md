...
- preflight가 읽은 full-local config와 worker config/credential/token/provider secret root는 `O_NOFOLLOW` FD identity, owner/private mode, nlink, containment와 digest를 다시 확인해 scratch의 private `runtime-inputs`에 freeze한다. 원본 source identity와 frozen inventory digest만 prelock authority에 넣으며 raw secret bytes/path는 receipt/manifest/metadata/log에 기록하지 않는다. lock 이후 start/install/readiness/final probe는 이 frozen private path만 소비하고 original external path를 다시 열지 않는다.
