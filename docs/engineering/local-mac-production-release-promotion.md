...
| `all_context_check_run_instances_digest` | self-suite만 제외한 모든 external context의 sorted unique check-run/App/suite instance와 GitHub Actions `run_id/event/run_attempt` binding digest |
...
| `all_context_workflow_runs` | external check set에 결합된 sorted unique non-secret `{run_id, check_suite_id, event, run_attempt}` 보고 projection |
...
- complete sorted suite ID set의 `all_check_suite_count`와 `all_check_suite_ids_digest`, external sorted check-run instance와 workflow-run binding set의 `all_context_check_run_instances_digest`를 subject/predicate/production manifest/server readback에 exact cross-bind한다. `filter=all`의 모든 check-run은 terminal `success` 또는 optional exact `skipped`여야 하고 external App check도 집합에서 제거하지 않는다. GitHub Actions suite마다 same-head workflow API의 `run_id`, `check_suite_id`, repository, `event`, status/conclusion, `run_attempt`을 검증한다. 서로 다른 first-attempt run/event의 동일 context는 허용하고 actual rerun은 `run_attempt > 1`로만 차단하며, metadata 누락·불일치와 first-attempt same-run duplicate context는 fail closed한다. `self-referential suite exception`은 exact `GITHUB_RUN_ID`의 current `workflow_check_suite_id` 하나뿐이다. complete suite count/digest에는 self-suite를 포함하되 external terminal-result set에서만 exact current suite를 제외한다. current self-run API readback은 workflow id/path, event, master ref, repository, `workflow_head_sha`, `run_id`, `run_attempt`을 모두 검증한다. `release_sha != workflow_head_sha`이면 selected release check set에 self-suite가 없으므로 exclusion은 empty다. 과거 retry suite나 동일 context name 전체를 제외하지 않는다.
