...
모든 inventory full-detail entry는 `source`와 `source_type`을 명시한다. repository-origin entry는 source가 exact `netsus/homecook`, source_type이 `Repository`이고 `bypass_actors`까지 읽혀야 한다. parent-origin에서만 unreadable `bypass_actors` omission과 official `exempt` bypass mode를 허용한다. Organization inherited conditions는 `repository_name | repository_id | repository_property` 중 정확히 하나를 허용한다. Enterprise inherited conditions는 `organization_name | organization_id | organization_property` 중 하나와 `repository_name | repository_property` 중 하나를 조합하며 enterprise `repository_id`는 허용하지 않는다. `repository_name.protected`, repository property entry의 `source=custom|system`, inherited `EnterpriseOwner`/`EnterpriseRole` actor type 등 공식 필드만 허용한다. Integration/RepositoryRole/Team/User는 positive ID, DeployKey는 null ID여야 하고 EnterpriseRole ID는 optional이다. `pull_request` bypass는 branch-only이며 DeployKey에는 금지한다. ambiguous union/unknown field는 fail closed한다. C2 execute는 first mutation 전에 `includes_parents=true` effective inventory까지 읽고 이 schema와 parent/canonical conflict를 닫는다.
