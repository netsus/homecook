...
- dependency lifecycle/install과 Next build는 absolute trusted argv를 macOS `sandbox-exec` deny-default profile에서 실행한다. process inspection과 unrestricted Mach lookup은 허용하지 않는다. signal은 sandboxed build가 직접 생성한 child에만 `(target children)`으로 허용하며 sibling/기존 process, launchctl, production process 제어는 계속 거부한다. macOS Node가 정상 startup 중 alias root를 확인할 때 관측된 exact literal `/etc`와 `/var`에는 `file-read-metadata`만 허용한다. `file-read-data`, directory listing, subpath read, write 또는 `/private/etc`·`/private/var`에 대한 추가 완화로 확장하지 않는다. broad `/private/etc`와 `/private/var/db` content subtree read는 제거하고, system content exception은 Node/Next가 실제 요구한 exact lexical `zoneinfo/posixrules` regular file과 그 current realpath 하나뿐이다. 그 두 file까지의 directory chain은 exact metadata literal만 허용하며 `/etc/hosts`, `zoneinfo/UTC`, directory listing과 다른 physical alias child는 실제 sandbox에서 deny로 고정한다. deterministic child env는 global module search와 locale preference daemon을 비활성화한다. macOS가 denial fallback으로 시도하는 exact dtracehelper write, DNS/logd/notification/opendirectory Mach name, notification shared-memory read와 passwd read는 capability allow가 아니라 closed `deny ... (with no-log)`로 유지하며, 그 외 operation/path/name의 denial은 하나라도 unified log에 나타나면 실패한다. exact Git source는 parent verifier에만 read-only이고 sandbox child read root에는 넣지 않는다. child는 그 source에서 미리 복제·봉인한 `build-work` tracked bytes만 읽으며 tracked root 자체는 deny-default다. 실제 `node_modules`와 `.next` directory는 tracked bytes 복제 뒤 `build-work` parent를 `0500`으로 봉인하기 전에 먼저 create하고, 봉인 뒤 exact path/FD identity를 잡는다. 두 directory와 candidate-private home/tmp만 writable이며 adjacent tracked path와 run root는 writable이 아니다. 모든 writable directory는 current uid, private parent, canonical realpath, held `O_DIRECTORY|O_NOFOLLOW` FD identity와 pre/post readback을 요구하고 swapped path, symlink/hardlink/path replacement를 거부한다. offline install 뒤 `node_modules/next`는 direct directory나 임의 symlink로 허용하지 않는다. exact relative `.pnpm/next@15.5.21.../node_modules/next` lexical shape, candidate-private `.pnpm` containment, 각 directory segment와 `package.json`/`dist/bin/next`의 owner·mode·nlink·FD identity를 실행 전후 고정하고, package/entrypoint bytes를 generated inventory와 sandbox authority에 교차 결합한 표준 pnpm symlink만 허용한다. production releases/locks/config/logs, LaunchAgents, production env/config, Docker socket과 network는 read/write deny하고 fallback은 없다. install과 Next build는 서로 다른 profile digest를 사용하며 install-phase pre-spawn guard는 index seal 뒤 재사용을 거부한다. 두 stage 모두 production observer의 complete wall-clock unified-log query를 통과하고 recorded denial count가 각각 0이어야 한다. child stderr/exit는 denial authority가 아니다. sandbox spawn 직전의 guarded wall-clock start부터 child 종료 뒤 audit flush가 끝난 end보다 최소 1초 뒤의 보수적 ceil boundary까지를 exact child process name/PID predicate의 `log show --start/--end`로 조회한다. millisecond truncation이나 고정 `--last` window는 사용하지 않으며, denial event가 하나라도 있거나 log가 unavailable/truncated이면 candidate를 실패시킨다. child가 초반 또는 종료 직전 EPERM을 삼킨 뒤 exit 0/stderr empty여도 동일하다.
- approved local pnpm v10 store는 candidate가 직접 쓰거나 child read root로 사용하지 않는다. broad writable `build-home` 밖의 `<runRoot>/pnpm-store/v10`에 `files/index` 전체를 APFS clone-or-copy로 create-only materialize하고 모든 file을 private nlink-1 canonical path로 고정한다. original과 private snapshot은 sorted full inventory의 content identity, dev/inode/mode/uid/gid/nlink/size/ctime/mtime digest를 copy 전후와 build 종료에 다시 비교한다. original same-user mutation은 source drift로 실패하며 이미 복제된 private bytes에는 전파되지 않는다. offline pnpm install 전용 working phase에서만 exact candidate-owned `v10/index`, `projects`, `tmp`를 writable로 열고 `files`, original/global store와 unknown root child는 계속 write deny한다. install 종료 직후 transition authority는 held parent/root FD identity를 확인한 뒤 exact `index/projects/tmp`를 먼저 unguessable run-owned quarantine으로 atomic rename한다. moved object identity가 기존 held FD와 일치할 때만 index/files child mode를 열린 FD의 `fchmod`로 봉인하며, scratch도 quarantined identity를 다시 확인한 뒤 제거한다. index를 exact original child로 복귀시키고 quarantine residue 0, root child set exact `files,index`, parent/root pre/post identity를 모두 확인하기 전에는 Next build profile을 만들지 않는다. child/scratch/storeRoot/storePath swap 또는 swap+restore, symlink/hardlink, unknown child와 external sentinel 접촉은 fail closed한다. 그 뒤 full final index의 path/type/mode/size/content inventory와 dev/inode/owner/nlink/ctime/mtime physical identity digest를 계산한다. Next build는 index/store write root가 전혀 없는 새 profile에서만 실행한다. seal 이후 index mutation, source/global store drift, `files` write, unknown child, path swap, hardlink, symlink 또는 예상하지 않은 child는 completed callback·reader와 build 후 재검증 중 하나에서 fail closed한다.
