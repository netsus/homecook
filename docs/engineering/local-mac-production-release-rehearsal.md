...
- approved local pnpm v10 store는 candidate가 직접 쓰거나 child read root로 사용하지 않는다. broad writable `build-home` 밖의 `<runRoot>/pnpm-store/v10`에 `files/index` 전체를 APFS clone-or-copy로 create-only materialize하고 모든 file을 private nlink-1 canonical path로 고정한다. original과 private snapshot은 sorted full inventory의 content identity, dev/inode/mode/uid/gid/nlink/size/ctime/mtime digest를 copy 전후와 build 종료에 다시 비교한다. original same-user mutation은 source drift로 실패하며 이미 복제된 private bytes에는 전파되지 않는다. store/view root와 `files/index` directory는 `0500`, private `index`는 `0400/0500`으로 유지한다. `files`는 pnpm의 patched dependency copy 호환 mode로 materialize하되 sandbox에서 항상 write deny하고 callback 종료 즉시 `0400/0500`으로 봉인한다. sandbox write allow는 exact `projects`와 `tmp` 두 directory뿐이며 original store와 private `files/index`, store root의 다른 경로는 default/explicit deny다.
- seal 전 `file_inventory`의 `source_kind='generated_build'`는 app component의 `.next/**`와 `node_modules/**`만 허용한다. 두 root가 모두 nonempty여야 하며 exact filtered inventory의 `generated_build_inventory_digest`를 candidate schema, bundle authority manifest와 `bundle_manifest_digest`에 교차 결합한다. sealed private store의 portable `pnpm_store_snapshot_inventory_digest`는 `files/index`의 path, type, mode, size, content identity만 candidate/bundle/schema에 교차 결합하고 completed reader가 각 root에서 재계산한다. device/inode/owner/nlink/ctime/mtime를 포함한 physical identity digest는 candidate/bundle portable authority에 넣지 않고 exact candidate path에 묶인 create-only root-local physical authority로만 검증한다. 원본 authority는 exact sibling `<candidateRoot>.physical-authority.json`, R2 authority는 private run root의 `execution-candidate.physical-authority.json`이며 둘 다 authority 자체의 `0400`, nlink 1, canonical path, stable `O_NOFOLLOW` FD read를 요구한다. 따라서 정상 filesystem copy는 같은 portable authority를 유지하되 원본 physical seal을 복사본의 실행 권한으로 재사용할 수 없다.
