...
- candidate root는 repository 밖 `<home>/.homecook/rehearsal/attempts/<cryptorandom-run-id>`를 `mkdir` create-only로 한 번 예약한다. global destination rename/replace 없이, 모든 검증 뒤 `complete.json`을 `wx`로 쓰며 실패 시 다른 내용을 제거하고 fixed-schema `failed.json`만 `wx`로 남긴다. 두 marker가 함께 있거나 marker가 없는 root는 reusable candidate가 아니다.
...
- dependency lifecycle/install과 Next build는 absolute trusted argv를 macOS `sandbox-exec` deny-default profile에서 실행한다. file read는 exact materialized source/run root, offline package store, snapshotted tool과 system runtime allowlist만 허용하고 write는 run root만 허용한다. production releases/locks/config/logs, source checkout, LaunchAgents, production env/config, Docker socket과 network는 명시적으로 deny하며 sandbox 부재/실패의 fallback은 없다. `--frozen-lockfile --offline --package-import-method=copy`만 허용한다.
...
- tool identity는 Git/GitHub/Node/pnpm/Docker/Supabase/sandbox/candidate builder를 first use 전에 owner/mode/realpath/bytes로 snapshot하고 마지막 use 뒤 재검증한다. `scripts/config/local-mac-production-rehearsal-toolchain-lock.json`은 공식 npm integrity와 darwin-arm64 Supabase CLI `2.110.0` binary digest, full-local exact service/image set을 canonical JCS로 고정한다.
- Compose `services` 전체를 열거해 각 service가 exact `image@sha256`와 platform expression을 가지는지 lock과 비교한다. tag-only/mixed/build/missing/extra service는 누락시키지 않고 거부하며, candidate Docker 호출은 exact version 및 local `image inspect` argv allowlist뿐이다. pull 경로는 없다.
- app/full-local/worker는 한 번 만든 bytes를 기존 production execution-tree copier/mode normalizer와 existing worker artifact materializer로 같은 bundle에 복사한다. `sealed_bundle_digest`는 file bytes, executable mode, contained symlink target+dereferenced bytes인 physical execution bytes만 뜻한다. `bundle_manifest_digest`는 physical inventory에 build ID, CI projection, source manifest, sandbox policy, tool lock/toolchain, image/service, final sealed migration, build-env override, production guard를 결합한다. `candidate_identity_digest`는 두 digest를 묶어 candidate identity가 되며 timestamp, run ID, absolute path, inode는 physical digest에서 제외된다.
