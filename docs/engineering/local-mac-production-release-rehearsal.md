...
- 로컬 macOS 원증거는 비밀·raw provider payload 없이 canonical `homecook.local-mac-production-release-evidence.v1` JSON을 exact tested HEAD의 Git note `refs/notes/homecook-release-evidence`에 붙인다. note는 branch tree를 바꾸지 않으므로 final exact head를 자기참조 없이 결합한다. `pnpm verify:local-mac-production-release-evidence`는 current HEAD/tree, darwin-arm64, exact release-suite/actual-build command result와 stdout/stderr digest, 28-file test-name inventory/count, residue 0, secret-shaped value 부재, self digest를 검증한다. note ref를 fetch하지 못한 reviewer에게 evidence가 있다고 주장하지 않으며 PR body에는 exact fetch/verify command와 note object ID를 남긴다.
