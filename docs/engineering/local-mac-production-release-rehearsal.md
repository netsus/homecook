...
- current-head GitHub check-suites, `check-runs?filter=all`, `actions/runs?head_sha=<release_sha>`와 remote `origin/master`는 trusted read-only adapter가 full pagination으로 build 전후 두 번 읽는다. workflow-run search의 1,000-result boundary, page/count/ID 중복, exact SHA/repository 불일치, `run_attempt>1`, missing suite→run mapping은 fail closed한다. commit statuses는 exact empty여야 한다. candidate는 attestation과 같은 shared normalizer와 `EXPECTED_RELEASE_CONTEXTS`를 사용한다. exact GitGuardian allowlist projection은 safe CI evidence에 별도로 보존하고 `ci_snapshot_digest`로 봉인하며 completed reader가 같은 shared normalizer에 재입력한다. 동일 Actions context 복수는 distinct run/suite IDs, 동일 workflow ID/path, 각 first attempt일 때만 정상 `push`/`schedule` 실행으로 허용하고 `rerun=0`으로 유지한다. 같은 run/suite 내부 중복과 cross-workflow owner collision은 거부한다. normalized projection과 suite/run/provenance set이 전후 exact match할 때만 create-only `ci-evidence.json`과 digest를 candidate에 묶으며 provider raw payload는 저장하지 않는다.
- `ci_check_summary_digest`는 shared normalizer가 만든 canonical summary만, `ci_snapshot_digest`는 optional intended skip과 empty external suite를 포함한 전체 safe projection만 각각 독립 hashing한다. `ci_suite_run_set_digest`는 complete suite와 started check-run identity set을, `ci_workflow_run_provenance_digest`는 normalized run/suite/workflow/path/event/attempt/repository provenance를 별도로 묶는다. candidate schema는 v2, bundle authority schema는 v2이며 이전 v1 authority는 current candidate로 재사용하지 않는다.
