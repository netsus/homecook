...
- selection 없이 candidate는 실행 시점 `origin/master`가 가리키는 exact full SHA여야 한다. selection이 있으면 verified selected ancestor SHA를 사용하며 current master가 selection의 observed master를 계속 포함해야 한다. 두 경우 모두 selected SHA의 7개 `EXPECTED_RELEASE_CONTEXTS`의 모든 started instance는 trusted GitHub Actions App의 exact `success`여야 한다. 추가 non-required context는 terminal `success` 또는 exact `skipped`만 허용하고 `skipped`만 `intended_skip`으로 기록한다. `neutral`, pending/queued, failure/cancelled, non-Actions started check는 candidate를 거부한다. 동일 context 복수는 distinct first-attempt run이 같은 workflow ID/path를 소유할 때만 허용한다.
