...
- Compose `services` 전체를 strict closed subset으로 열거해 각 service가 exact `image@sha256`와 platform expression을 가지는지 lock과 비교한다. service key와 service body의 모든 single/double-quoted mapping key, flow style, anchor/alias/merge/extension, env interpolation, tab, multiline scalar, duplicate normalized key/section처럼 완전한 열거가 모호한 문법은 field 인식 전에 fail closed한다. tag-only/mixed/build/missing/extra service는 거부하며, candidate Docker 호출은 exact version 및 local `image inspect` argv allowlist뿐이다. pull 경로는 없다.
