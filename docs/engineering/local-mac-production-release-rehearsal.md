...
- selection 없이 candidate는 실행 시점 `origin/master`가 가리키는 exact full SHA여야 한다. selection이 있으면 verified selected ancestor SHA를 사용하며 current master가 selection의 observed master를 계속 포함해야 한다. 두 경우 모두 selected SHA의 7개 `EXPECTED_RELEASE_CONTEXTS`는 trusted GitHub Actions App의 latest exact `success`여야 한다. 추가 non-required context는 terminal `success` 또는 exact `skipped`만 허용하고 `skipped`는 `intended_skip`으로 기록한다. `filter=all`의 과거/current check-run 중 `neutral`, pending/queued, failure/cancelled가 하나라도 있으면 candidate를 거부한다. 동일 SHA의 서로 다른 first-attempt workflow run은 context 이름이 겹쳐도 허용하며 actual rerun은 workflow run `run_attempt > 1`로만 판정한다.
...
- current-head GitHub check-runs, same-head workflow runs, commit statuses 및 remote `origin/master`는 trusted read-only adapter가 full pagination으로 build 전후 두 번 읽는다. 각 check REST item의 actual `head_sha`/`sha`, check-run ID, suite ID와 App integration ID를 보존하고, GitHub Actions suite는 same-head workflow run의 exact `run_id`, `check_suite_id`, repository, `event`, status/conclusion, `run_attempt`에 결합한다. metadata 누락·중복·head/repository/suite 불일치와 nonterminal workflow run은 fail closed한다. candidate는 `normalizeGitHubProductionReleaseCheckSummary`와 shared `EXPECTED_RELEASE_CONTEXTS`만 사용한다. 필수 7개 context는 trusted GitHub Actions App의 exact `success`, 추가 non-required context는 external App check를 포함해 terminal `success|skipped`만 가능하고 exact `skipped`만 `intended_skip`이다. `neutral`, pending/queued, failure/cancelled, `run_attempt > 1`, first-attempt same-run duplicate context는 fail closed한다. 서로 다른 first-attempt `run_id`/event의 같은 context 이름은 rerun이 아니다. normalized projection과 workflow/suite/run set이 전후 exact match할 때만 create-only `ci-evidence.json`과 digest를 candidate에 묶으며 provider raw payload는 저장하지 않는다. completed candidate reader도 이 evidence file 하나만 허용하고 fatal UTF-8/JCS, closed identity/state, candidate의 세 CI digest를 다시 검증한다.
- `ci_check_summary_digest`는 shared normalizer가 만든 canonical summary만, `ci_snapshot_digest`는 optional intended skip과 비밀 없는 workflow-run projection을 포함한 전체 safe projection만 각각 독립 hashing한다. `ci_suite_run_set_digest`는 stored check-run/App/suite ID와 workflow `run_id/event/run_attempt/check_suite_id`의 exact ordered set을 묶고, 세 field를 서로의 alias로 사용하지 않는다.
- completed reader는 `complete.json`, `candidate.json`, `candidate-identity.json`, `ci-evidence.json`, physical manifest, bundle authority manifest를 모두 current-owner exact mode `0400`, nlink-1 `O_NOFOLLOW` FD로 읽고 lstat/open/fstat/read/fstat/path-post identity와 containment를 검증한다. realpath를 먼저 적용하지 않고 authority root부터 original absolute lexical path의 각 directory segment를 `lstat`해 symlink가 아님을 확인하며, 각 segment를 `O_DIRECTORY|O_NOFOLLOW` FD로 읽기 종료까지 고정한다. 그 뒤의 realpath는 lexical canonical path와 같아야 하고 전체 chain을 post-read에 다시 검증한다. authority root/parent directory mode는 lifecycle에 따라 exact `0500` 또는 `0700`만 허용하며 group/world bit가 있는 `0444`/`0440`/`0040` file이나 `0555`/`0755` parent를 읽기 위해 자동 chmod하지 않는다. stored CI summary는 safe check/status array를 shared normalizer에 다시 넣어 재계산하고 stored summary와 exact 비교한 뒤 candidate의 summary/snapshot/suite-run digest를 각각 재검증한다. hardcoded `rerun:0` 또는 all-success projection은 허용하지 않는다. candidate와 bundle의 repository/source ref/SHA/tree/build/toolchain/images/migration/artifacts/file inventory/physical·provenance·input digest도 exact 교차 비교하고, sealed private pnpm `files/index` 전체 inventory digest를 candidate root에서 다시 계산한다.
...
- 실제 current-master형 fixture의 필수 7개 success + unique scope context + optional QA skip acceptance, 서로 다른 first-attempt `push`/`schedule`의 same-name context acceptance, `run_attempt > 1` 및 same-run generic `scope` 중복 rejection
