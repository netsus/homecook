...
- `ci_check_summary_digest`는 shared normalizer가 만든 canonical summary만, `ci_snapshot_digest`는 optional intended skip과 allowlisted external suite를 포함한 전체 safe projection만 각각 독립 hashing한다. `ci_suite_run_set_digest`는 complete suite ID/App/repository/head와 started check-run identity set을, `ci_workflow_run_provenance_digest`는 normalized run/suite/workflow/path/event/attempt/repository provenance를 별도로 묶는다. candidate schema는 v2, bundle authority schema는 v2이며 이전 v1 authority는 current candidate로 재사용하지 않는다.
