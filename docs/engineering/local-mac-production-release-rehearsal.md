...
- Compose `services` 전체를 strict closed line/token grammar로 열거한다. 모든 non-empty/non-comment line을 indentation별로 소비하고 plain service key와 service body `image|platform|build`만 허용한다. image inventory에 필요 없는 `environment`와 그 밖의 mapping/nested/list key는 부분 해석하지 않고 거부한다. quoted/tagged/empty/explicit key, flow, anchor/alias/merge/extension, interpolation, tab, multiline scalar, duplicate normalized key/section, colon ambiguity는 field 인식 전에 fail closed한다. 각 service의 exact `image@sha256`와 platform expression을 lock과 비교하며 tag-only/mixed/build/missing/extra service도 거부한다. candidate Docker 호출은 exact version 및 local `image inspect` argv allowlist뿐이고 pull 경로는 없다.
