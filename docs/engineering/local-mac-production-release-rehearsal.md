...
- dependency lifecycle/install과 Next build는 absolute trusted argv를 macOS `sandbox-exec` deny-default profile에서 실행한다. process inspection과 unrestricted Mach lookup은 허용하지 않는다. signal은 sandboxed build가 직접 생성한 child에만 `(target children)`으로 허용하며 sibling/기존 process, launchctl, production process 제어는 계속 거부한다. exact Git source는 parent verifier에만 read-only이고 sandbox child read root에는 넣지 않는다. child는 그 source에서 미리 복제·봉인한 `build-work` tracked bytes만 읽으며 tracked root 자체는 deny-default다. 그 안의 실제 directory인 exact `node_modules`와 `.next`, candidate-private home/tmp만 writable이며 adjacent tracked path와 run root는 writable이 아니다. 모든 writable directory는 current uid, private parent, canonical realpath, held `O_DIRECTORY|O_NOFOLLOW` FD identity와 pre/post readback을 요구하고 symlink/hardlink/path replacement를 거부한다. production releases/locks/config/logs, LaunchAgents, production env/config, Docker socket과 network는 read/write deny하고 fallback은 없다. child stderr/exit는 denial authority가 아니다. sandbox spawn 직전의 guarded wall-clock start부터 child 종료 뒤 audit flush가 끝난 end보다 최소 1초 뒤의 보수적 ceil boundary까지를 exact child process name/PID predicate의 `log show --start/--end`로 조회한다. millisecond truncation이나 고정 `--last` window는 사용하지 않으며, denial event가 하나라도 있거나 log가 unavailable/truncated이면 candidate를 실패시킨다. child가 초반 또는 종료 직전 EPERM을 삼킨 뒤 exit 0/stderr empty여도 동일하다.
- approved local pnpm v10 store는 candidate가 직접 쓰지 않는다. candidate-private home 아래 create-only `pnpm-store/v10` view를 만들고 `projects/tmp`만 run-owned writable metadata로 둔다. `files`는 exact source directory의 current-user ownership, non-symlink type, safe mode, canonical realpath와 held FD identity를 전후 고정한 read-only link다. `index`는 source execution-tree digest와 같은 private sealed copy만 사용하고 build 전후 source/private digest를 모두 재검증한다. pnpm의 기본 store-integrity 검증은 이 private index에 pin된 file integrity로 original `files` blob을 검사한다. sandbox는 original store와 private index write를 명시적으로 deny하며 source/view identity 또는 digest가 바뀌거나 source write가 관측되면 candidate를 실패시킨다.
- seal 전 `file_inventory`의 `source_kind='generated_build'`는 app component의 `.next/**`와 `node_modules/**`만 허용한다. 두 root가 모두 nonempty여야 하며 exact filtered inventory의 `generated_build_inventory_digest`를 candidate schema, bundle authority manifest와 `bundle_manifest_digest`에 교차 결합한다. 따라서 writable build output은 검증된 selected/current payload에서 복사·seal된 실제 bytes와 inventory를 벗어나 재사용될 수 없다.
