...
- dependency install과 Next build는 pinned Node clone이 trusted preload를 마친 뒤 같은 process 안에서 macOS deny-default profile을 적용하고, 그 이후 `process-fork`를 전면 거부한다. pnpm lifecycle script는 계속 `--ignore-scripts`로 금지한다. offline DNS projection은 pinned pnpm install stage에만 exact 활성화하고 Next/application build에는 주입하지 않는다. pinned pnpm bytes는 바꾸지 않으며 CommonJS `dns|node:dns|dns/promises|node:dns/promises`, `process.getBuiltinModule`, ESM static/dynamic import로 얻은 모든 lookup/resolve/Resolver 사용은 witness `network` attempt를 기록하고 즉시 fail closed한다. rehearsal 전용 Next config는 `workerThreads=true`, `cpus=1`, `webpackBuildWorker=false`, parallel server compile/trace false를 고정하므로 정상 병렬 작업은 thread로만 수행한다. child, nested child, `setsid` escape는 생성 자체가 kernel에서 거부되며, 일반 `child_process`/signal API 시도는 preload witness pipe에도 기록되어 호출자가 오류를 삼켜도 stage가 실패한다. Node startup 중 발생하던 platform noise를 숨기기 위해 no-log를 넓히지 않는다. exact `(deny mach-lookup (global-name "com.apple.diagnosticd") (with no-log))` 한 줄만 승인 예외이고, DNS/logd/notification/opendirectory/shared-memory/passwd/dtracehelper 및 다른 file/network/Mach/process/signal denial은 모두 관찰 실패다. permission allow로 바꾸지 않는다. macOS root alias metadata와 exact `zoneinfo/posixrules`, source/global/production/Docker/network, install/build profile 분리와 index-seal 재사용 금지 계약은 유지한다.
- 각 stage root는 candidate-private nlink-1 mode `0500` Node clone이며, trusted N-API witness가 untrusted pnpm/Next code보다 먼저 profile을 적용하고 자기 `audit_token_t`를 읽는다. `(pid,pidversion)`과 microsecond start time, PID/PPID/PGID, process name, `proc_pidpath_audittoken` path, executable dev/inode/size/ctime/SHA-256을 parent에게 단회 보고한다. parent는 spawn 전후 exact clone/witness/preload/instance-signal-helper bytes와 보고값을 교차 검증하며, path/basename 또는 `lsof` 빈 결과로 실행 vnode를 추정하지 않는다. witness 누락·malformed·path swap·bytes drift·root 외 process identity·process attempt 1개 이상은 fail closed다. witness 검증 전 오류는 PID-only signal을 보내지 않는다. 검증 후 timeout/output overflow 종료는 helper가 pidversion, microsecond start, executable vnode/bytes를 다시 확인하고 `proc_signal_with_audittoken`으로 exact process instance에만 보낸다. PID가 재사용되면 새 instance는 signal하지 않고 fail closed한다. unified log event는 registered PID라도 unique process name이 다르면 제외하고, event가 audit token을 제공하면 exact registered token이 다를 때 제외하며, microsecond 실행 구간 밖 PID reuse event도 제외한다. unregistered sub-poll event는 exact clone name과 execution-scoped marker가 함께 있을 때만 포함한다. audit digest는 v3 enforcement, stage, interval, root PID/PGID, witness identity, exact audit token/process instance/executable digest, process-attempt 0과 denial 0을 결합한다. legacy polling observer는 회귀 fixture의 fail-closed 검증에만 남고 실제 install/Next authority가 아니다.
