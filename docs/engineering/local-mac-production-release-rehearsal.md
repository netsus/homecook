...
상태: **canonical / candidate failure diagnostic repair author complete / fresh independent review pending**
...
- production release root 전체를 recursive content authority로 취급하지 않는다. root 자체는 owner/mode/dev/inode/size/ctime/mtime의 metadata-only identity로 고정한다. validated canonical `current.json`/`previous.json`이 exact `execution-snapshots/<64-hex>`를 참조한 경우에만 그 snapshot root를 shared entry/depth/byte limit 안에서 deep digest한다. descriptor reference는 canonical app/worker path, unique snapshot digest, known `evidence.json` schema, descriptor와 exact 같은 release SHA/tree/promotion/rehearsal identity가 모두 맞아야 한다. 검증 순서는 bounded whole-snapshot before digest → manifest read/identity 검증 → 같은 shared entry/byte budget과 allocation-before-size gate를 적용한 기존 production execution-tree grammar로 app/worker/authority 및 optional full-local 실제 component bytes 재계산 → bounded whole-snapshot after digest이며 before/after가 exact 같아야 한다. 따라서 manifest와 accepted snapshot bytes 사이의 A→B 치환 및 component 확장도 read 전에 fail closed한다. duplicate reference, malformed/escaping reference, referenced child 부재, manifest/physical-content mismatch는 fail closed한다. descriptor가 참조하지 않는 retained snapshot child는 name digest, directory lstat identity, 최대 1 MiB known `evidence.json` digest 또는 exact absent sentinel만 수집하고 내부 payload bytes는 열거나 hash하지 않는다. release root 아래 unrelated legacy build/cache bytes와 retained snapshot 내부 cache bytes는 active production mutation authority가 아니다.
...
- selection 없이 candidate는 실행 시점 `origin/master`가 가리키는 exact full SHA여야 한다. selection이 있으면 verified selected ancestor SHA를 사용하며 current master가 selection의 observed master를 계속 포함해야 한다. 두 경우 모두 selected SHA의 7개 `EXPECTED_RELEASE_CONTEXTS`의 모든 started instance는 trusted GitHub Actions App의 exact `success`여야 한다. 추가 non-required context는 terminal `success` 또는 exact `skipped`만 허용하고 `skipped`만 `intended_skip`으로 기록한다. 유일한 non-Actions started check 예외는 exact GitGuardian App `46505`/`gitguardian`/`GitGuardian`의 `GitGuardian Security Checks` 1개이며, empty external ID, exact head/suite/repository, completed/success만 허용한다. duplicate/tuple drift와 `neutral`, pending/queued, failure/cancelled, 그 밖의 non-Actions started check는 candidate를 거부한다. 동일 Actions context 복수는 distinct first-attempt run이 같은 workflow ID/path를 소유할 때만 허용한다.
...
- current-head GitHub check-suites, `check-runs?filter=all`, `actions/runs?head_sha=<release_sha>`와 remote `origin/master`는 trusted read-only adapter가 full pagination으로 build 전후 두 번 읽는다. 모든 Actions App `15368` suite는 zero-check도 canonical repository `netsus/homecook`과 exact 하나의 workflow run mapping을 가져야 한다. orphan/missing/multi-map/wrong-repository Actions suite, workflow-run search의 1,000-result boundary, page/count/ID 중복, exact SHA/repository 불일치와 `run_attempt!=1`은 fail closed한다. commit statuses는 exact empty여야 한다. candidate는 attestation과 같은 shared normalizer와 `EXPECTED_RELEASE_CONTEXTS`를 사용한다. exact GitGuardian allowlist projection은 safe CI evidence에 별도로 보존하고 `ci_snapshot_digest`로 봉인하며 completed reader가 같은 shared normalizer에 재입력한다. workflow run 없는 zero-check external suite는 canonical repository/head와 unique exact Netlify/GitGuardian/Vercel/Claude App owner tuple만 허용한다. 동일 Actions context 복수는 distinct run/suite IDs, 동일 workflow ID/path, 각 first attempt일 때만 정상 `push`/`schedule` 실행으로 허용하고 `rerun=0`으로 유지한다. 같은 run/suite 내부 중복과 cross-workflow owner collision은 거부한다. normalized projection과 suite/run/provenance set이 전후 exact match할 때만 create-only `ci-evidence.json`과 digest를 candidate에 묶으며 provider raw payload는 저장하지 않는다.
- `ci_check_summary_digest`는 shared normalizer가 만든 canonical summary만, `ci_snapshot_digest`는 optional intended skip과 allowlisted external suite를 포함한 전체 safe projection만 각각 독립 hashing한다. `ci_suite_run_set_digest`는 ID 오름차순의 exact closed `{app_id,app_name,app_slug,head_sha,id,repository}` suite tuple과 started check-run identity set을, `ci_workflow_run_provenance_digest`는 normalized run/suite/workflow/path/event/attempt/repository provenance를 별도로 묶는다. candidate collector와 stored/completed reader는 같은 exported suite tuple normalizer를 사용한다. candidate schema는 v2, bundle authority schema는 v2이며 이전 v1 authority는 current candidate로 재사용하지 않는다.
- completed reader는 `complete.json`, `candidate.json`, `candidate-identity.json`, `ci-evidence.json`, physical manifest, bundle authority manifest를 모두 current-owner exact mode `0400`, nlink-1 `O_NOFOLLOW` FD로 읽고 lstat/open/fstat/read/fstat/path-post identity와 containment를 검증한다. realpath를 먼저 적용하지 않고 authority root부터 original absolute lexical path의 각 directory segment를 `lstat`해 symlink가 아님을 확인하며, 각 segment를 `O_DIRECTORY|O_NOFOLLOW` FD로 읽기 종료까지 고정한다. 그 뒤의 realpath는 lexical canonical path와 같아야 하고 전체 chain을 post-read에 다시 검증한다. authority root/parent directory mode는 lifecycle에 따라 exact `0500` 또는 `0700`만 허용하며 group/world bit가 있는 `0444`/`0440`/`0040` file이나 `0555`/`0755` parent를 읽기 위해 자동 chmod하지 않는다. stored CI summary는 safe suite/check/workflow-run array를 shared normalizer에 다시 넣어 재계산하고 stored summary와 exact 비교한 뒤 candidate의 summary/snapshot/suite-run/workflow-provenance digest를 각각 재검증한다. hardcoded `rerun:0` 또는 all-success projection은 허용하지 않는다. candidate와 bundle의 repository/source ref/SHA/tree/build/toolchain/images/migration/artifacts/file inventory/physical·provenance·input digest도 exact 교차 비교하고, sealed private pnpm `files/index` 전체 inventory digest를 candidate root에서 다시 계산한다.
...
- dependency install과 Next build는 pinned Node clone이 trusted preload를 마친 뒤 같은 process 안에서 macOS deny-default profile을 적용하고, 그 이후 `process-fork`를 전면 거부한다. pnpm lifecycle script는 계속 `--ignore-scripts`로 금지한다. offline DNS projection은 pinned pnpm install stage에만 exact 활성화하고 Next/application build에는 주입하지 않는다. install profile에는 macOS가 pnpm/Node startup에서 읽는 local OS service인 exact `(allow mach-lookup (global-name "com.apple.SystemConfiguration.DNSConfiguration"))` 한 줄만 추가하며 build/runtime profile에는 이 allow를 넣지 않는다. 이는 external DNS나 network 권한이 아니고 `(deny network*)`는 두 stage 모두 유지한다. direct `dns.lookup`, `net.createConnection`, `tls.connect`, public IP connect와 HTTP는 계속 fail closed하며 synthetic net/tls shim으로 성공을 투영하지 않는다. pinned pnpm bytes는 바꾸지 않으며 root와 모든 `worker_threads`에서 CommonJS `dns|node:dns|dns/promises|node:dns/promises`, `process.getBuiltinModule`, ESM static/dynamic import로 얻은 모든 lookup/resolve/Resolver 사용은 같은 witness FD에 `network` attempt를 기록하고 즉시 fail closed한다. Worker가 caller-supplied `env`/`execArgv`를 사용해도 preload와 projection을 제거할 수 없다. rehearsal 전용 Next config는 `workerThreads=true`, `cpus=1`, `webpackBuildWorker=false`, parallel server compile/trace false를 고정하므로 정상 병렬 작업은 thread로만 수행한다. child, nested child, `setsid` escape는 생성 자체가 kernel에서 거부되며, 일반 `child_process`/signal API 시도는 preload witness pipe에도 기록되어 호출자가 오류를 삼켜도 stage가 실패한다. Node startup 중 발생하던 platform noise를 숨기기 위해 no-log를 넓히지 않는다. exact `(deny mach-lookup (global-name "com.apple.diagnosticd") (with no-log))` 한 줄만 `no-log` 예외이고 DNSConfiguration allow에는 `with no-log`를 붙이지 않는다. logd/notification/opendirectory/shared-memory/passwd/dtracehelper 및 다른 file/network/Mach/process/signal denial은 모두 관찰 실패다. macOS root alias metadata와 exact `zoneinfo/posixrules`, source/global/production/Docker/network, install/build profile 분리와 index-seal 재사용 금지 계약은 유지한다. canonical stage capability policy text/digest, install allow count 1·build allow count 0, 두 stage의 exact audit digest와 denial/process-attempt 0은 candidate와 bundle manifest에 교차 결합한다.
- 각 stage root는 candidate-private nlink-1 mode `0500` Node clone이다. 동일 compiled N-API witness는 parent가 untrusted child spawn 전에 controller mode로 load/pin하고, child mode에서는 untrusted pnpm/Next JS 전에 스스로 `SIGSTOP`한다. parent는 정지된 direct child의 audit token, pidversion, microsecond start, PID/PPID/PGID와 `proc_pidpath_audittoken` executable dev/inode/size/ctime/SHA-256을 native memory에 등록한 뒤 exact instance에만 `SIGCONT`를 보낸다. 그 뒤 child가 deny-default profile을 적용하고 같은 identity를 witness pipe에 단회 보고한다. 초기 witness timeout/overflow/malformed/mismatch와 실행 timeout을 포함한 모든 live-child 실패는 이미 pinned된 native controller가 `proc_signal_with_audittoken`으로 exact instance에만 `SIGKILL`을 보내고 parent가 exit/reap을 기다린 뒤 반환한다. 별도 pathname executable signal helper와 post-resume raw PID/PGID kill fallback은 없다. audit token 확보 자체가 실패하면 untrusted code를 한 줄도 실행하지 않은 `SSTOP` 상태, direct-child PPID, unreaped child handle을 native controller가 함께 확인한 경우에만 pre-resume abort하고 exit/reap을 기다린다. child가 reap된 뒤 registry를 해제하므로 같은 숫자의 재사용 PID는 signal 대상이 될 수 없다. parent는 spawn 전후 exact clone/witness/preload bytes와 보고값을 교차 검증하며 path/basename 또는 `lsof` 빈 결과로 실행 vnode를 추정하지 않는다. witness 누락·malformed·path swap·bytes drift·root 외 process identity·process attempt 1개 이상은 fail closed다. unified log event filtering과 audit digest의 기존 v3 exact-instance 결합은 유지한다. legacy polling observer는 회귀 fixture의 fail-closed 검증에만 남고 실제 install/Next authority가 아니다.
- approved local pnpm v10 store는 broad writable `build-home` 밖의 `<runRoot>/pnpm-store/v10`에 `files/index` 전체를 APFS clone-or-copy로 create-only materialize한다. install phase에서만 exact candidate-owned `index/projects/tmp`를 writable로 열고 `files`, source/global store와 unknown child는 deny한다. install 종료 직후 held parent/root FD를 확인하고 exact `index/projects/tmp`를 store/completed candidate 밖 UUID quarantine으로 atomic rename한다. moved identity가 held FD와 일치할 때만 files/index child modes를 열린 FD의 `fchmod`로 봉인하고 index만 exact store child로 복귀시킨다. scratch는 untrusted sandbox process tree가 전부 종료될 때까지 deletion-free deferred quarantine에 유지한다. 그 뒤 full path/type/mode/owner/device/inode/ctime/mtime ledger를 만들고 observer hook 이후에도 동일한 경우에만 inherited parent FD를 받은 fixed actual CommandLineTools Python app executable(`/usr/bin/python3` xcrun shim 아님) cleanup helper가 descriptor-relative traversal로 제거한다. root와 각 child는 source name으로 직접 삭제하지 않고 같은 verified parent 안의 unguessable tombstone name으로 원자 claim한 뒤 device/inode/type/owner/mode/size identity를 다시 확인하며, 확인 뒤 source name이 새 inode로 교체되면 새 inode를 삭제하지 않고 fail closed한다. root/nested/swap+restore mismatch나 cleanup failure는 fail closed하고 unowned path를 삭제하지 않으며 cleanup inventory/tool digest를 authority에 결합한다. 성공 경로는 quarantine residue 0을 요구한다. Next build는 exact `files,index` sealed child set과 store write root가 없는 새 profile에서만 실행한다.
- authoritative Vitest release suite는 worker 시작 전에 current-user mode `0700`의 짧은 `$HOME/.cache/hcv/r-*` root를 create-only로 만든다. 각 worker는 그 아래 별도 `w<pool-id>-*` private root를 `TMPDIR`로 받아 기존 `os.tmpdir()` call도 run/worker-owned 경계 안에서 동작한다. suite owner는 root와 parent의 owner/device/inode를 열린 FD와 함께 고정하고 정상, assertion failure, timeout, `SIGHUP`/`SIGINT`/`SIGTERM` 종료에서 같은 inode만 sibling tombstone으로 atomic claim한 뒤 제거한다. root claim 뒤에도 각 nested entry를 no-follow identity로 고정하고 같은 verified parent 안의 unguessable tombstone으로 원자 claim한 뒤 directory FD 또는 claimed non-directory inode만 제거한다. 원래 이름이 교체되면 replacement는 복원·보존하고 전체 cleanup을 fail closed하며, symlink target이나 hardlink의 외부 이름을 chmod/delete하지 않고 global `homecook-*` scan/delete를 수행하지 않는다. 별도 Vitest subprocess 회귀는 success/assertion failure/세 signal 종료의 출력된 exact suite·worker root ENOENT와 replacement inode/unrelated sibling 보존을 자동 증명한다. 실제 macOS build fixture도 별도 shared candidate cache가 아니라 이 suite root 아래에서만 생성한다.
- actual descriptor cleanup helper는 root parent FD도 상속받아 file/directory/root final remove 직전에 identity를 다시 확인하고 검출 가능한 post-claim replacement를 삭제하지 않은 채 fail closed한다. macOS/POSIX에는 writable parent의 exact-inode 조건부 `unlink/rmdir` syscall이 없으므로 adversarial same-UID writer의 마지막 identity-check 이후 pathname A→B→A는 cooperative boundary 밖이다. 이를 hard boundary로 과장하지 않으며 그 보장이 필요한 환경은 별도 UID/container/VM owner가 필요하다. 회귀는 실제 Python helper의 file/directory/root post-claim 교체에서 replacement와 parked original을 보존하는지 확인한다. Vitest signal cleanup은 자기 handler만 제거하고 다른 handler가 등록 상태를 유지한 채 호출되는 것도 독립 process에서 증명한다.
- release test scope의 단일 소스는 `scripts/config/local-mac-production-release-test-scope.json`과 `pnpm test:local-mac-production-release`다. 기존 authoritative 26개 파일을 그대로 유지하고 teardown/subprocess 2개를 union하여 exact 28개를 실행한다. 과거 `774 → 635`는 rulesets 15개와 rulesets-c2 133개, 합계 148개를 잘못 제외하고 신규 9개를 넣은 `774-148+9=635` scope regression이었다. runner는 실행 전에 exact 28개 file과 수집 test-name inventory SHA-256을 출력하며 fully-skipped fixture도 inventory mode로 수집한다. actual build test는 default release suite에서 explicit skip하고 `HOMECOOK_RUN_ACTUAL_RELEASE_BUILD=1`의 final exact-head command에서 한 번만 실행한다.
- 로컬 macOS 원증거는 비밀·raw provider payload 없이 canonical `homecook.local-mac-production-release-evidence.v2` JSON을 exact tested HEAD의 Git note `refs/notes/homecook-release-evidence`에 붙인다. `pnpm capture:local-mac-production-release-evidence -- --output <absolute-new-file>`은 clean final HEAD에서 exact 28-file suite와 actual-build selected test를 순서대로 각 1회 실행한다. release-suite child env는 `HOMECOOK_RUN_ACTUAL_RELEASE_BUILD`와 모든 Vitest teardown/suite/worker control env를 제거하고, actual-build child에만 exact flag를 다시 설정한다. raw command/provider output은 evidence에 보존하지 않은 채 runner `RELEASE_TEST_FILES/CASES/INVENTORY_SHA256`, Vitest totals, selected actual-build file/name, canonical stage capability policy text/digest, install/build audit digest·zero counts, explicit DNS/net/TLS/public-IP/HTTP 성공 0의 closed secret-free projection만 남긴다. release-suite는 현재 inventory contract의 exact `27 passed + 1 skipped` files 및 `846 passed + 2 skipped` tests split 외 값을 거부한다. Vitest summary가 skipped test identity를 출력하지 않는 현재 한계 때문에 skip identity 자체가 아니라 full test-name inventory digest와 exact skip counts를 결합하며, count가 같은 다른 skip identity는 이 projection만으로 구분하지 못한다. changed service/stage/policy digest, wildcard/additional Mach allow, egress 성공, stale output digest는 validator가 거부한다. stdout/stderr SHA-256은 이 canonical projection bytes에서 계산한다. note는 branch tree를 바꾸지 않으므로 final exact head를 자기참조 없이 결합한다. `pnpm verify:local-mac-production-release-evidence`는 current HEAD/tree, darwin-arm64, current tree scope manifest에서 같은 shared algorithm으로 다시 계산한 exact 28-file test-name inventory/count, release projection의 header/totals 교차 결합, actual-build exact command/result/projection, residue 0, unknown/missing field와 secret-shaped value 부재, projection/self digest를 검증한다. Git note는 서명되지 않은 로컬 self-attestation(자기 증명)이며 production trust anchor가 아니다. note ref를 fetch하지 못한 reviewer에게 evidence가 있다고 주장하지 않으며 PR body에는 exact fetch/verify command와 note object ID를 남긴다.
- seal 전 `file_inventory`의 `source_kind='generated_build'`는 app component의 `.next/**`와 `node_modules/**`만 허용한다. 두 root가 모두 nonempty여야 하며 exact filtered inventory의 `generated_build_inventory_digest`를 candidate schema, bundle authority manifest와 `bundle_manifest_digest`에 교차 결합한다. sealed private store의 portable `pnpm_store_snapshot_inventory_digest`는 exact `files,index` child set 전체를, `pnpm_store_final_index_inventory_digest`는 final `index` subtree의 path/type/mode/size/content만 candidate/bundle/schema/startup identity/builder authority에 각각 교차 결합하고 completed reader가 각 root에서 재계산한다. final index와 full store의 device/inode/owner/nlink/ctime/mtime physical identity digest는 candidate/bundle portable authority에 넣지 않고 exact candidate path에 묶인 create-only root-local physical authority에 별도 field로 결합해 reader가 재검증한다. 이 digest는 file/symlink뿐 아니라 candidate root, `bundles`, bundle root, 각 component root와 모든 하위 directory, `evidence`, `pnpm-store`, sealed store root와 모든 store directory의 path/dev/inode/uid/gid/mode/nlink/size/ctime identity를 함께 묶는다. 따라서 어느 내부 parent directory를 rename 또는 mutate한 뒤 원래 child bytes와 이름을 복구해도 ctime/physical identity drift로 다음 gate가 실패한다. 원본 authority는 exact sibling `<candidateRoot>.physical-authority.json`, R2 authority는 private run root의 `execution-candidate.physical-authority.json`이며 authority payload의 `authority_path_digest`도 그 exact canonical location에 결합한다. 둘 다 authority 자체의 `0400`, nlink 1, canonical path, stable `O_NOFOLLOW` FD read를 요구하며 같은 root용 authority bytes를 다른 private path로 복사해도 실패한다. full CAFS content는 원본 completed candidate read와 execution copy physical-authority 발급 같은 trust transition에서 검증한다. 그 뒤 반복되는 R2 stable-execution gate는 sealed file/dir FD·path·mode·owner·nlink·device·inode·size·ctime/mtime inventory와 manifest digest를 재검증하되 CAFS 7.7GB content를 다시 읽지 않는다. 따라서 정상 filesystem copy는 같은 portable authority를 유지하되 원본 physical seal을 복사본의 실행 권한으로 재사용할 수 없다.
...
#### R1 candidate failure evidence

candidate root를 예약한 뒤 실패하면 cleanup과 terminal readback이 모두 성공한 경우에만 exact create-only `failed.json` 하나를 남긴다. schema 위치는 `scripts/schemas/local-mac-production-rehearsal-candidate-failure.schema.json`이며 exact field는 `schema,status,reason_code,stage,error_code,path_digest,production_guard` 7개다. unknown/missing field는 거부한다.

- `schema`는 exact `homecook.local-mac-production-rehearsal-candidate-failed.v2`, `status`는 exact `failed`, `reason_code`는 exact `candidate_build_failed`다.
- `path_digest`는 legacy non-path disclosure field 이름을 유지하되 raw path는 입력에도 넣지 않는다. `{candidate_uuid,release_sha,release_tree}`의 canonical JCS SHA-256으로 후보 UUID와 head/tree만 결합하며, source evidence가 검증되기 전의 `release_tree`만 explicit `null`이다.
- `stage` allowlist는 `toolchain_preflight|source_prepare|production_guard_pre|ci_pre|environment_read|image_inspect|migration_collect|build_prepare|offline_install|next_build|artifact_assemble|bundle_seal|ci_post|production_guard_post|toolchain_postflight|manifest_finalize|immutable_finalize|internal`이다.
- `error_code` allowlist는 `authority_rejected|command_failed|execution_limit_exceeded|sandbox_policy_rejected|sandbox_evidence_unavailable|artifact_rejected|production_guard_rejected|create_only_collision|internal_error`다.
- 세부 `stage/error_code`는 module-private brand로 생성된 오류에서만 읽는다. caller가 오류 객체에 같은 이름의 property/code/name/message를 주입해도 신뢰하지 않는다. unknown, custom, revoked, forged raw 오류는 현재 allowlisted stage와 `internal_error`로 정규화한다.
- 최초 오류 분류는 catch 즉시 raw 오류와 분리해 고정한다. 후속 read-only `post_failure` production guard가 실패해도 최초 `stage/error_code`는 덮어쓰지 않고 `production_guard=unverified`만 기록한다. guard equality가 재검증된 경우에만 `verified`다.
- cleanup은 예약한 exact candidate root 안의 partial artifact만 제거하고 sibling/unowned path를 보존한다. cleanup 성공 후 marker를 `wx`, mode `0400`, nlink `1`로 쓰고 root를 mode `0500`으로 봉인한 다음 no-follow readback을 통과해야 한다. cleanup, marker, seal 또는 readback 실패 시 trusted marker를 발급하지 않고 public stderr는 exact `candidate_terminalization_failed`만 허용한다.
- raw error/cause/message/name/stack/code, argv, stdout/stderr, PID, secret/env/credential, provider payload, URL과 absolute path는 marker·CLI stdout/stderr·로그에 직렬화하거나 hash하지 않는다. bootstrap의 일반 public failure도 exact `candidate_build_failed`만 출력한다.
...
- public command는 `pnpm release:rehearsal:run -- --candidate <absolute-completed-candidate-root-or-candidate.json> --production-env-authority <absolute-private-env-authority> --json`이며 별도 `scripts/local-mac-production-rehearsal-run.mjs` entry를 사용한다. 원본 candidate를 split 2 `readCompletedCandidateRoot`와 원본 root-local physical authority로 검증한 뒤 run-owned create-only `execution-candidate`에 실제 filesystem copy·seal한다. 복사 직후 portable content/bundle manifest를 먼저 검증하고 현재 copy의 physical identity를 다시 계산해 private run root에 execution-local create-only physical authority를 발급한다. 이어 같은 stable physical authority에서 container-portable authority를 create-only로 파생해 container path `/sealed-candidate`와 exact authority path `/sealed-candidate-authority/authority.json`, candidate/manifest/bundle/store digest에 결합한다. namespace root는 absolute lexical canonical current-user `0700` directory여야 하고 realpath가 lexical path와 exact 같아야 한다. approved stable private anchor는 canonical current-user mode-`0700` `$HOME` 하나이며 Docker에 mount하지 않는다. runner는 `$HOME → .homecook → rehearsal → runs`의 complete lexical chain부터 run root→execution candidate/container authority까지 각 directory를 `O_DIRECTORY|O_NOFOLLOW` FD로 연다. `$HOME`은 locator-only authority라서 lexical no-symlink, held FD와 pathname의 dev/inode, current-user owner, exact private mode만 고정하고 nlink/ctime/size는 identity digest에서 제외한다. 따라서 unrelated HOME child create/delete는 rehearsal을 무효화하지 않지만 HOME path replacement, symlink, owner 또는 mode drift는 실패한다. exact dev/inode/uid/gid/mode/nlink/ctime/size binding은 `.homecook`에서 시작해 `rehearsal/runs`, run root, execution candidate/container authority까지 유지하므로 `.homecook` 이하의 transient swap/symlink/swap+restore는 계속 탐지한다. staging/runtime-state trust transition 전후와 모든 candidate bind resource create 직전/직후에 FD와 lexical path chain을 재검증한다. ancestor swap, swap+restore, symlink segment, 다른 tree bind는 fail closed하며 create-returned ID를 이미 얻은 경우에는 immutable cleanup ledger로 rollback한다. path/identity digest는 run-local `runtime-state/candidate-path-authority.json`에 create-only로 남고 public evidence에는 raw path 없이 trusted-anchor/root identity digest로 결합된다. host physical authority와 container authority source는 resource 생성 전 반복 검증되며, copied/stale authority, byte/path/mode/content drift, read 중 swap, hardlink/symlink는 Docker/resource 생성 전에 실패한다. Docker bind, migration, app/worker/full-local probe는 이 검증된 run copy만 사용하고 원본 candidate를 실행 중 다시 열지 않는다. host 실행 중 반복 gate는 physical identity만 재검증해 host full CAFS scan을 원본 read와 execution-copy seal의 2 trust transition으로 제한한다. app, worker, full-local probe는 각 container start trust transition에서 자기 read-only mount를 1회 full 검증하며 checkout, rebuild, dependency reinstall, image re-resolution을 하지 않는다.
...
- `scripts/lib/local-mac-production-rehearsal-runner-adapters.mjs`는 sealed full-local Compose와 candidate manifest의 local digest-pinned image만 실행한다. generated override에서 7개 service 각각 `pull_policy: never`, CLI에서 `--no-build`, 개별 `docker run`에서 `--pull=never`를 강제한다. create/start 뒤 각 container `.Image`, configured digest reference, local image ID/platform/repo digest를 preflight authority와 다시 비교한다. generated override는 exact run/project/creation-nonce label을 추가하고 `auth-edge`, `auth-egress`, `data-internal`을 모두 internal network로 강제한다. canary 전 실제 network `Internal=true`, exact labels/name/ID와 모든 container attachment를 readback한다. pathname source authority는 Compose source/generated override/public env, full-local script/config/plugin bind, 각 exact secret file, execution candidate/container authority, app root, worker root, app/worker secret directory를 closed inventory로 열거한다. resolved Compose output은 hard-coded target→sealed-source mapping과 exact 비교하므로 같은 namespace의 wrong-tree source도 거부한다. 각 source의 trusted anchor→parent chain과 source directory는 `O_DIRECTORY|O_NOFOLLOW`, regular file은 `O_NOFOLLOW` FD로 열어 dev/inode/uid/gid/mode/nlink/ctime/size와 content digest를 고정하고 Docker create/inspect가 끝날 때까지 유지하되 HOME entry에는 위 locator projection을 동일하게 적용한다. 작은 config/secret/plugin directory는 모든 child directory/file FD와 content digest를 bounded inventory로 고정하며, 큰 candidate/app/worker tree는 existing root-local physical authority의 full metadata inventory를 create 전후 재사용해 host full CAFS content scan을 늘리지 않는다. network, volume, full-local service container, app, worker, sentinel network, no-bind sentinel container는 closed create-site inventory를 공유한다. 각 create가 exact returned ID(network/container는 64-hex, volume은 exact expected name)를 반환하면 JSON/inspect/post-source 검증보다 먼저 immutable cleanup ledger에 그 kind/ID/name을 기록하고, 이후 inspect identity/labels가 실패해도 cleanup은 그 exact ID만 소비한다. discovery는 알려지지 않은 resource를 ledger에 채택하지 않으며 pre-create source/path mismatch는 resource 0이다. Docker command의 exception, signal, truncation, generic nonzero와 Docker stdout JSON parse 실패는 public CLI에서 stable `docker_command_failed` 또는 `docker_output_invalid` reason으로만 변환한다. expected nonzero는 generated closed inventory의 command class, exact status, empty stdout, stderr matcher가 모두 일치할 때만 허용한다. resource absence는 container/network/volume inspect별 exact missing-resource stderr와 요청 resource ID가 일치하고 LF/CRLF 한 개만 선택적으로 정규화될 때만 status `1`을 허용하며 prefix/suffix/추가 줄/private path는 실패한다. startup identity pending은 전용 status `45`와 exact empty stdout/stderr만 허용하고 generic status `1`은 실패한다. worker evidence pending status `44`와 network probe status `41|42|43`도 exact empty stdout/stderr만 허용한다. raw Docker stdout/stderr, HOME/bind path, secret filename/value, provider-like payload는 CLI error, JSON, terminal artifact에 직렬화하지 않고 public evidence에는 기존 approved digest/closed bounded metadata만 남긴다. app, worker, full-local `postgrest-probe`에는 verified execution candidate root와 dedicated container-authority directory만 각각 `/sealed-candidate`, `/sealed-candidate-authority`에 read-only bind하고, 다른 host sibling authority 또는 broad run root는 mount하지 않는다. `postgrest-probe`의 기존 idle command와 PostgREST-only healthcheck는 generated override가 exact candidate-reader startup wrapper와 identity+PostgREST healthcheck로 교체한다. reader 성공 전에는 probe process/health가 성립하지 않고 `storage → api-gateway → auth-proxy`가 시작되지 않으며, full-local resource gate가 반환되지 않아 app/worker 시작에도 도달하지 않는다. launchd/LaunchAgent는 사용하지 않는다.
- app/worker wrapper와 full-local `postgrest-probe` child는 process start 전에 container 전용 `readCompletedCandidateContainerRoot`를 실행한다. reader는 host 절대 candidate path를 신뢰하지 않고 exact container root/authority locator와 portable candidate/manifest/bundle/store digest를 검증하며 full CAFS content를 container mount bytes에서 다시 읽는다. wrapper는 host completed manifest에서 미리 계산해 literal로 받은 closed RFC8785 expected startup identity와 self digest를 reader 결과 canonical bytes에 즉시 exact 비교한다. candidate+authority pair가 자체적으로 유효해도 expected pair와 다르면 app proxy/listener, real app child, worker child, probe idle loop를 spawn하기 전에 exit `70`으로 끝난다. 일치한 child만 candidate schema, approved SHA/tree, candidate/manifest/build-input/artifact/toolchain/image/migration digest, build/bundle과 migration head를 create-only tmpfs file에 남기고 실제 app/worker process 또는 full-local probe idle loop를 시작한다. generated probe healthcheck도 같은 expected identity canonical bytes를 다시 비교한 뒤에만 PostgREST readiness를 확인한다. host runner도 full-local resource create 직후 같은 startup identity file을 읽어 completed candidate manifest와 exact 비교하고, 이 gate가 끝나기 전에는 migration/fixture/downstream acceptance 또는 app/worker `startComponents`를 호출하지 않는다. host comparison은 defense in depth이며 app/worker의 첫 identity gate가 아니다. app/worker startup report도 같은 exact identity를 사용하며 누락 fallback이나 늦은 reader 재실행은 없다.
...
- cleanup은 closed create-site call이 돌려준 exact ID를 inspect/parse/post-check 전에 kind/name과 함께 고정한 immutable ledger만 역순 stop/remove한다. inspect identity/ownership label/creation nonce는 ledger record 뒤 별도 cross-check하며, label discovery는 ledger에 합치지 않는다. unknown/production/label-spoofed resource는 residue/blocker로만 남겨 건드리지 않는다. SIGINT/SIGTERM/timeout/output overflow는 async detached process group을 즉시 중단하고 cleanup으로 전환한다. secret file root는 container 종료 뒤 제거하고 run-owned residue와 secret-bearing persistent file count가 모두 0이어야 pass한다.
...
- `release_artifacts.kind`는 전체 array에서 unique여야 한다. current authority는 exactly one `current_descriptor`와 그 descriptor에 결합된 exactly one verified `referenced_snapshot:current:*`를 함께 가져야 한다. prepared identity가 있으면 exactly one matching `prepared_descriptor`만 허용한다. duplicate/missing/mismatch 조합은 ambiguity 또는 `surface:current_referenced_snapshot` finding으로 거부한다.
...
- descriptor duplicate/escape, snapshot symlink/path escape, known manifest descriptor-identity/component-content mismatch/unknown field/size/검증 중 치환, missing current referenced evidence, aggregate entry/deep-byte limit을 각각 fail closed하는 test
...
- listener/descriptor/referenced active snapshot pre/post 변화와 `lsof exit 0 + empty`, `present:true + process_name:absent`가 fail closed되는 test
- production/rehearsal namespace·port·Docker resource collision rejection
- 실제 macOS in-process sandbox witness에서 offline pnpm `node_modules` 생성과 Next `.next` production output 성공, adjacent build root/source/authority/production/symlink escape 실패, child/nested/detached process와 sibling signal 시도 실패
- `(pid,pidversion)` audit token과 microsecond start/executable identity 결합, registered PID의 wrong-name/wrong-token 및 실행 구간 밖 reuse event 제외, witness 누락과 lsof empty/path swap-and-restore fail-closed
- deferred quarantine actual cleanup 중 checked name 교체 시 replacement inode 보존, test success/failure/interruption 뒤 run-owned exact temp root만 cleanup하고 unrelated temp sibling 보존
- required expected context의 `skipped`와 optional context의 `neutral|pending|failure|cancelled` rejection, optional non-required `skipped`의 `intended_skip` projection
- 실제 current-master형 fixture의 필수 7개 success + same-SHA `push`/`schedule` first-attempt duplicate context acceptance, actual attempt 2·pending scheduled·failed historical·same-run duplicate·cross-workflow owner collision rejection
