...
- app/worker wrapper와 full-local `postgrest-probe` child는 process start 전에 container 전용 `readCompletedCandidateContainerRoot`를 실행한다. reader는 host 절대 candidate path를 신뢰하지 않고 exact container root/authority locator와 portable candidate/manifest/bundle/store digest를 검증하며 full CAFS content를 container mount bytes에서 다시 읽는다. 성공한 child만 SHA/tree/build/bundle/migration head의 closed RFC8785 identity JSON을 create-only tmpfs file로 남기고 실제 app/worker process 또는 full-local probe idle loop를 시작한다. probe healthcheck는 그 exact closed identity file과 PostgREST readiness를 함께 요구하며, host runtime evidence는 같은 startup identity file을 다시 읽어 누락 fallback이나 늦은 reader 재실행 없이 세 child report를 exact 비교한다.
