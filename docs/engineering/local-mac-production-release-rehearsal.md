...
- seal 전 `file_inventory`의 `source_kind='generated_build'`는 app component의 `.next/**`와 `node_modules/**`만 허용한다. 두 root가 모두 nonempty여야 하며 exact filtered inventory의 `generated_build_inventory_digest`를 candidate schema, bundle authority manifest와 `bundle_manifest_digest`에 교차 결합한다. sealed private store의 `pnpm_store_snapshot_inventory_digest`와 `pnpm_store_snapshot_identity_digest`도 candidate/bundle/schema에 교차 결합하고 completed reader가 실제 root에서 재계산한다. 따라서 writable build output과 dependency snapshot은 검증된 selected/current payload, private store inventory와 seal을 벗어나 재사용될 수 없다.
