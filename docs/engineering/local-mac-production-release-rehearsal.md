...
- public command는 minimal built-in-only `scripts/local-mac-production-rehearsal-candidate-bootstrap.mjs`를 trust launcher로 사용한다. launcher는 trusted Git/tar/Node와 자기 bytes를 first/last snapshot하고, fetched exact `origin/master` Git archive를 private read-only root에 materialize한다. import graph는 regex가 아니라 comments, strings, regex literals, template literals와 `${...}` expression을 구분하는 closed lexical scanner로 읽는다. static/bare/export-from literal import와 exact literal dynamic import만 허용하고 computed/unsupported syntax는 fail closed한다. relative `.mjs`와 exact JSON attribute가 있는 `.json`, toolchain lock 각각을 exact Git blob OID·Git mode·SHA-256과 대조하고 materialized file의 dev/inode/ctime/size/uid/gid/nlink/private mode identity도 pre/finalize 사이 exact 비교한다. 따라서 실행 중 bytes를 바꿨다가 복구해도 graph provenance drift로 실패한다. graph의 정렬된 `builder_input_digest`는 source evidence, candidate manifest, bundle authority manifest에 결합되어 completed reader가 교차 검증한다. worktree candidate module을 직접 실행하거나 immutable-bootstrap/module-graph verification 없이 CLI candidate branch를 호출하면 실패한다.
