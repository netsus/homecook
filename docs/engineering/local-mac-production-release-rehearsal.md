...
- dependency install과 Next build는 absolute trusted argv를 macOS `sandbox-exec` deny-default profile에서 실행하며 pnpm lifecycle script는 `--ignore-scripts`로 금지한다. deterministic env는 `CIRCLE_NODE_TOTAL=2`를 고정해 Next 15.5.21의 worker 수를 1로 제한하고 `NODE_DISABLE_COMPILE_CACHE=1`로 candidate-private compile cache를 끈다. 각 stage는 fixed `/bin/cp -c -p`로 pinned Node bytes와 metadata를 candidate-private nlink-1 mode `0500` APFS clone에 materialize하고 source/copy-tool/clone identity를 전후와 evidence digest에 결합한다. 고유 clone basename은 macOS kernel denial rate-limit bucket을 stage별로 분리하며, `process-exec`는 root/current owner·executable bit·group/world non-writable·canonical non-symlink 검증을 통과한 stage clone exact literal 하나만 허용하고 다른 executable spawn은 거부한다. process inspection과 unrestricted Mach lookup은 허용하지 않는다. signal은 sandboxed build가 직접 생성한 child에만 `(target children)`으로 허용하며 sibling/기존 process, launchctl, production process 제어는 계속 거부한다. macOS Node가 정상 startup 중 alias root를 확인할 때 관측된 exact literal `/etc`와 `/var`에는 `file-read-metadata`만 허용한다. broad `/private/etc`와 `/private/var/db` content subtree read는 제거하고 system content exception은 exact lexical `zoneinfo/posixrules` regular file과 current realpath 하나뿐이다. 그 file까지의 directory chain은 exact metadata literal만 허용하며 `/etc/hosts`, `zoneinfo/UTC`, directory listing과 다른 physical alias child는 실제 sandbox에서 deny로 고정한다. macOS denial fallback인 exact dtracehelper write, `com.apple.diagnosticd`/DNS/logd/notification/opendirectory Mach name, notification shared-memory read와 passwd read는 capability allow가 아니라 closed `deny ... (with no-log)`로 유지한다. `diagnosticd`는 sandbox 밖 success와 안쪽 nonzero deny를 실제 bootstrap lookup으로 고정하며 adjacent Mach lookup은 계속 감사 실패다. 그 외 denial은 하나라도 실패다. exact Git source는 parent verifier만 읽고 child는 미리 복제·봉인한 `build-work`만 사용한다. `node_modules`와 `.next`, candidate-private home/tmp 외에는 writable하지 않으며 normal pnpm Next link authority와 source/global/production/Docker/network deny는 유지한다. install과 Next build는 서로 다른 profile digest를 사용하고 install-phase guard는 index seal 뒤 재사용을 거부한다.
- 각 sandbox stage는 `sandbox-exec` root를 unique detached session/process group으로 시작한다. parent observer는 root exec 전부터 종료+flush까지 `ps`를 fail-closed polling해 PID/PPID/PGID/`lstart`와 root absolute command 또는 descendant `lsof` text executable bytes identity를 등록한다. registered descendant가 `ps` 뒤 종료되어 `lsof`가 status 0 + empty text를 반환하더라도 stage의 sole exact executable basename과 일치할 때만 그 pinned clone path의 bytes identity를 사용하며 arbitrary command fallback은 허용하지 않는다. `/bin/ps`와 `/usr/sbin/lsof` 자체도 stage 전후 bytes/identity exact 비교를 통과해야 하며 그 tool digest를 audit에 결합한다. PID 시작 토큰 재사용, root group 또는 registered lineage에서 벗어난 child, unavailable/truncated discovery, 실행 파일 미식별, root 종료 뒤 survivor를 실패시킨 뒤 시작 토큰이 같은 exact group/registered PID만 정리하고 residue 0을 재검증한다. root 종료 뒤 full-table 조회가 실패하면 이미 등록한 PID만 targeted 조회하고 exact `lstart`가 같은 대상만 제한 시간 내 신호하며, 재사용된 PID에는 신호하지 않는다. unified log live stream과 conservative full wall-clock `log show` 결과를 합쳐 sorted registered PID set에 속한 root/descendant denial을 전부 판정한다. exact stage clone만 `process-exec` 가능한 install/Next stage에서는 logged deny rule에 unique clone basename과 결합된 exact `(with message ...)`를 넣는다. 따라서 20ms poll 사이에 fork 후 종료한 child도 kernel event의 clone basename과 audit message가 모두 일치할 때만 같은 stage denial로 귀속하고, 같은 basename만 흉내 내거나 다른 message/basename을 가진 동시 system event는 섞지 않는다. audit digest는 stage, UTC start/end, root PID/PGID, observer tool identity, sorted PID+start+executable identity set과 denial count를 결합하며 fixed zero digest를 사용하지 않는다. test-only native fork/grandchild harness는 direct/nested/late file/network/Mach/signal deny enforcement를 실제 sandbox에서 확인하고, deterministic full-window fixture는 child/grandchild PID denial mapping과 sub-poll execution token mapping이 observer를 실패시키는지 고정한다. install/Next real test는 전체 process tree에서 recorded denial 0을 요구한다.
...
- 실제 macOS `sandbox-exec`에서 offline pnpm `node_modules` 생성과 Next `.next` production output 성공, adjacent build root/source/authority/production/symlink escape 실패, sibling process signal 실패
- 20ms poll보다 짧은 execution-scoped child denial 귀속과 unrelated event 제외, root 종료 뒤 full-table lookup 실패 시 registered exact-start child cleanup 및 PID reuse 무신호
