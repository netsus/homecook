...
- public command는 minimal built-in-only `scripts/local-mac-production-rehearsal-candidate-bootstrap.mjs`를 trust launcher로 사용한다. launcher는 trusted Git/tar/Node와 자기 bytes를 first/last snapshot하고, fetched exact `origin/master` Git archive를 private read-only root에 materialize한다. 동적 import 전에 CLI부터 재귀적으로 닫힌 relative `.mjs` import graph와 toolchain lock 각각을 exact Git blob OID·Git mode·SHA-256과 대조하고 실행 뒤 같은 graph를 다시 검증한다. graph의 정렬된 `builder_input_digest`는 source evidence, candidate manifest, bundle authority manifest에 결합되어 completed reader가 교차 검증한다. worktree candidate module을 직접 실행하거나 immutable-bootstrap/module-graph verification 없이 CLI candidate branch를 호출하면 실패한다.
