...
- seal 전 `file_inventory`의 `source_kind='generated_build'`는 app component의 `.next/**`와 `node_modules/**`만 허용한다. 두 root가 모두 nonempty여야 하며 exact filtered inventory의 `generated_build_inventory_digest`를 candidate schema, bundle authority manifest와 `bundle_manifest_digest`에 교차 결합한다. sealed private store의 portable `pnpm_store_snapshot_inventory_digest`는 exact `files,index` child set과 그 path, type, mode, size, content identity만 candidate/bundle/schema에 교차 결합하고 completed reader가 각 root에서 재계산한다. device/inode/owner/nlink/ctime/mtime를 포함한 physical identity digest는 candidate/bundle portable authority에 넣지 않고 exact candidate path에 묶인 create-only root-local physical authority로만 검증한다. 원본 authority는 exact sibling `<candidateRoot>.physical-authority.json`, R2 authority는 private run root의 `execution-candidate.physical-authority.json`이며 authority payload의 `authority_path_digest`도 그 exact canonical location에 결합한다. 둘 다 authority 자체의 `0400`, nlink 1, canonical path, stable `O_NOFOLLOW` FD read를 요구하며 같은 root용 authority bytes를 다른 private path로 복사해도 실패한다. full CAFS content는 원본 completed candidate read와 execution copy physical-authority 발급 같은 trust transition에서 검증한다. 그 뒤 반복되는 R2 stable-execution gate는 sealed file/dir FD·path·mode·owner·nlink·device·inode·size·ctime/mtime inventory와 manifest digest를 재검증하되 CAFS 7.7GB content를 다시 읽지 않는다. 따라서 정상 filesystem copy는 같은 portable authority를 유지하되 원본 physical seal을 복사본의 실행 권한으로 재사용할 수 없다.
