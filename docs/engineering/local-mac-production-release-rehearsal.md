...
- tool identity는 Git/GitHub/Node/pnpm/Docker/Supabase/launchctl/lsof/sandbox/candidate builder와 Next CLI를 first use 전에 owner/mode/realpath/bytes로 snapshot하고 마지막 use 뒤 재검증한다. candidate builder/CLI/config/tool lock은 current clean HEAD와 exact release Git object가 같을 때만 사용한다. `scripts/config/local-mac-production-rehearsal-toolchain-lock.json`은 darwin-arm64 Node `v22.13.1`, pnpm `10.32.1`, 공식 npm integrity와 Supabase CLI `2.110.0`의 exact binary digest, full-local exact service/image set을 canonical JCS로 고정한다.
- Compose `services` 전체를 strict closed subset으로 열거해 각 service가 exact `image@sha256`와 platform expression을 가지는지 lock과 비교한다. flow style, anchor/merge/extension, env image interpolation처럼 완전한 열거가 모호한 문법은 fail closed한다. tag-only/mixed/build/missing/extra service는 거부하며, candidate Docker 호출은 exact version 및 local `image inspect` argv allowlist뿐이다. pull 경로는 없다.
