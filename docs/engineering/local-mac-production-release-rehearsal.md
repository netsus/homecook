...
- production release root 전체를 recursive content authority로 취급하지 않는다. root 자체는 owner/mode/dev/inode/size/ctime/mtime의 metadata-only identity로 고정한다. validated canonical `current.json`/`previous.json`이 exact `execution-snapshots/<64-hex>`를 참조한 경우에만 그 snapshot root를 shared entry/depth/byte limit 안에서 deep digest한다. descriptor reference는 canonical app/worker path, unique snapshot digest, known `evidence.json` schema, descriptor와 exact 같은 release SHA/tree/promotion/rehearsal identity가 모두 맞아야 한다. manifest의 app/worker/authority 및 optional full-local digest는 기존 production execution-tree digest grammar로 실제 component bytes에서 다시 계산하고, bounded whole-snapshot deep digest 전후가 같아야 한다. duplicate reference, malformed/escaping reference, referenced child 부재, manifest/physical-content mismatch는 fail closed한다. descriptor가 참조하지 않는 retained snapshot child는 name digest, directory lstat identity, 최대 1 MiB known `evidence.json` digest 또는 exact absent sentinel만 수집하고 내부 payload bytes는 열거나 hash하지 않는다. release root 아래 unrelated legacy build/cache bytes와 retained snapshot 내부 cache bytes는 active production mutation authority가 아니다.
...
- descriptor duplicate/escape, snapshot symlink/path escape, known manifest descriptor-identity/component-content mismatch/unknown field/size, missing current referenced evidence, aggregate entry/deep-byte limit을 각각 fail closed하는 test
