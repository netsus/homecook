...
- current-head GitHub check-runs와 commit statuses 및 remote `origin/master`는 trusted read-only adapter가 full pagination으로 build 전후 두 번 읽는다. 각 REST item의 actual `head_sha`/`sha`, run ID, suite ID, trusted GitHub Actions integration ID를 보존한다. candidate는 `normalizeGitHubProductionReleaseCheckSummary`와 shared `EXPECTED_RELEASE_CONTEXTS`만 사용한다. 필수 7개 context는 exact `success`, 추가 non-required context는 terminal `success|skipped`만 가능하고 exact `skipped`만 `intended_skip`이다. `neutral`, pending/queued, failure/cancelled, success→success를 포함한 같은 context rerun은 fail closed한다. normalized projection과 suite/run set이 전후 exact match할 때만 create-only `ci-evidence.json`과 digest를 candidate에 묶으며 provider raw payload는 저장하지 않는다. completed candidate reader도 이 evidence file 하나만 허용하고 fatal UTF-8/JCS, closed identity/state, candidate의 세 CI digest를 다시 검증한다.
- `ci_check_summary_digest`는 shared normalizer가 만든 canonical summary만, `ci_snapshot_digest`는 optional intended skip을 포함한 전체 safe projection만 각각 독립 hashing한다. `ci_suite_run_set_digest`는 stored trusted App/suite/run identity의 exact ordered set을 묶고, 세 field를 서로의 alias로 사용하지 않는다.
...
- 실제 current-master형 fixture의 필수 7개 success + unique scope context + optional QA skip acceptance와 generic `scope` 중복 rerun rejection
