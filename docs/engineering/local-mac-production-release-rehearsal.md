...
- authoritative Vitest release suite는 worker 시작 전에 current-user mode `0700`의 짧은 `$HOME/.cache/hcv/r-*` root를 create-only로 만든다. 각 worker는 그 아래 별도 `w<pool-id>-*` private root를 `TMPDIR`로 받아 기존 `os.tmpdir()` call도 run/worker-owned 경계 안에서 동작한다. suite owner는 root와 parent의 owner/device/inode를 열린 FD와 함께 고정하고 정상, assertion failure, timeout, `SIGHUP`/`SIGINT`/`SIGTERM` 종료에서 같은 inode만 sibling tombstone으로 atomic claim한 뒤 제거한다. 원래 이름이 교체되면 replacement는 보존하고 held descriptor로 이동된 owned inode만 찾으며, nested child의 owner/device도 재귀 확인한다. symlink target이나 hardlink의 외부 이름을 따라 삭제하지 않고 global `homecook-*` scan/delete를 수행하지 않는다. 실제 macOS build fixture도 별도 shared candidate cache가 아니라 이 suite root 아래에서만 생성한다.
