...
- 현재는 `설치 + repo-local config + spec 고정 + dispatcher/status helper + direct Codex stage runner + budget-aware reviewer fallback` 단계까지를 다룬다.
...
- direct execution은 현재 `Codex executable stage`에만 연결되고, reviewer stage는 handoff artifact를 남기는 수준에서 멈춘다.
...
5. `pnpm omo:run-stage -- --sync-status`를 사용하면 fallback artifact 경로와 approval patch를 `.workflow-v2`에 같이 남긴다.
...
다음 phase에서는 product pilot과 external smoke artifact schema를 다룬다.
