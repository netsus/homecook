...
- 현재는 `설치 + repo-local config + spec 고정 + dispatcher/status helper + direct Codex stage runner` 단계까지를 다룬다.
...
다음 phase에서는 reviewer-stage handoff automation과 external smoke artifact schema를 다룬다.
