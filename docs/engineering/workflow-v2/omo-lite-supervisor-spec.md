...
- 현재는 `설치 + repo-local config + spec 고정` 단계까지를 다룬다.
- dispatcher script와 status sync helper가 아직 완성되지 않았으므로, 지금은 사람이 읽는 운영 규격이자 이후 자동화 구현의 기준으로 사용한다.
...
실제 stage dispatcher script와 status sync helper는 다음 phase에서 구현한다.
