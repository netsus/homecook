...
- `pnpm omo:status:brief -- --work-item <id>`: 현재 stage와 다음 액션 확인
...
OMO가 있어도 규칙 우선순위는 바뀌지 않는다. 기준은 여전히 v1 문서와 `AGENTS.md`다. OMO는 그 규칙을 자동으로 집행하고 다시 이어서 실행하게 해 주는 보조 시스템이다.
