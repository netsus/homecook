...
- 현재 baseline: `work-item closeout schema + tracked status projection helper`까지 구현됐다. README / acceptance / PR body projection은 아직 후속 단계다.
...
- 현재 baseline은 `status` projection helper와 validator hook만 구현됐다. README / acceptance / PR body projection은 아직 남아 있다.
