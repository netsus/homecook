...
7. legacy artifact에서 `commit.subject`가 없으면 migration 경로에서만 `pr.title`을 commit subject로 fallback 한다.
...
4. review stage는 review 기록과 route/merge 후처리까지 끝나야 완료로 본다.
