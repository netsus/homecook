...
### Alignment Rule With Promotion Ledger

auditor와 promotion ledger는 서로 다른 wording으로 같은 현실을 말하면 안 된다.

reset 현재 baseline에서는 최소 아래가 같이 맞아야 한다.

- replay summary status
- active auditor blocker family
...
- `backfill-required` incident는 primary open blocker와 별도로
  "secondary blocker / disposition pending" 성격이 드러나야 한다.
