...
slice `12b-shopping-pantry-reflect` 개발 전에는 promotion blocker 전체를 닫지 않는다. 대신 아래 운영 잠금을 만족한 상태에서 Codex-orchestrated rail로 slice를 시작한다.

### Current Mode

- `OMO autonomous promotion`: `not-ready`
  - 기준: `.workflow-v2/promotion-evidence.json`의 `promotion_gate.status`
  - 의미: OMO를 Codex 개입 없이 독립 기본값으로 승격할 준비는 아직 아니다.
- `Codex-orchestrated OMO rail`: `usable`
  - 기준: 10b/11/12a baseline report와 representative replay acceptance
  - 의미: Codex가 orchestration owner로 붙어 blocker를 분류/복구하고, OMO는 deterministic rail로 상태와 evidence를 투영하는 방식은 12b slice 진행에 사용할 수 있다.

이 두 상태를 섞지 않는다. 12b를 시작하기 위해 promotion gate를 `ready`로 만들 필요는 없지만, 12b 운영 기록은 promotion readiness 후속 판단에 재사용될 수 있어야 한다.

### 12b Report / Evidence Checklist
...
- `stale_recovery_count`: merged/current-head 현실과 runtime/report projection이 어긋나 자동 보정한 횟수
...
12b에서 바로 사람에게 올릴 수 있는 경우는 아래로 제한한다.

- credential 또는 secret이 필요하다.
