...
| 루트 디스크 여유 | 약 53 GiB | 공식 권장 80 GB보다 작아 **현재 blocker** |
...
2. issuer, audience, expiry, `sub`, `session_id`를 검증한다.
3. local data client를 request마다 만들고 다음 두 값을 분리해 보낸다.
   - `apikey`: 로컬 Supabase publishable key
   - `Authorization: Bearer <remote user JWT>`
4. 로컬 PostgREST/Storage가 remote JWKS 공개키로 JWT 서명을 확인한다.
5. PostgreSQL이 JWT의 `sub`를 `auth.uid()`로 읽어 기존 RLS를 적용한다.
...
| `provider` / stable provider subject | provider 충돌과 linking 검증 |
| `email_normalized` / verification state | 현재 bootstrap의 verified email 규칙 유지 |
| `remote_deleted_at` | 삭제와 재가입 경계 |
| `last_verified_at` / `sync_revision` | mirror freshness와 replay 판단 |

원본 access token, refresh token, raw provider payload, OAuth secret은 저장하지 않는다.
...
3. `auth.users` read를 mirror authority로 바꾸는 additive migration과 regression test를 먼저 만든다.
...
- 루트 디스크 여유 약 53 GiB이므로 공간 정리 또는 전용 외장 SSD가 먼저 필요하다.
...
5. persistent DB/Storage volume을 전용 SSD 경로에 둔다.
...
2. local PostgREST/Storage에 같은 JWKS를 설정한다.
3. remote user token을 local Data API에 전달하는 isolated smoke client를 만든다.
4. key sync, atomic reload, rollback, stale alert를 구현한다.
...
- expired, wrong-project, wrong-audience, malformed token 모두 거부
...
1. remote DB를 새 local isolated instance로 restore한다.
2. application schema/data/RLS/functions/triggers/grants를 검증한다.
3. `recipe-images` 객체를 local Storage로 복사한다.
4. signed URL 자체가 아니라 canonical object path를 기준으로 검증한다.
5. object count, byte sum, MIME, hash와 DB registry reference를 비교한다.
6. 같은 절차를 처음부터 다시 실행해 replay/idempotency를 확인한다.
...
2. remote Auth의 신규 identity maintenance gate를 먼저 닫는다.
3. 앱 사용자 mutation을 maintenance response로 막는다.
4. remote application DB와 Storage의 final inventory/dump를 만든다.
...
11. remote Auth 신규 identity gate를 다시 연다.
...
- [ ] 사용자가 hybrid contract-evolution을 명시 승인했다.
- [ ] 공식 5종 문서와 `CURRENT_SOURCE_OF_TRUTH.md`가 먼저 갱신됐다.
- [ ] 신규 workpack과 acceptance가 작성·독립 검토됐다.
- [ ] production capability가 `legacy`, canonical generation row가 `0`이다.
- [ ] remote JWKS가 asymmetric key를 제공한다.
- [ ] 디스크 여유가 최소 gate를 만족한다.
- [ ] off-Mac encrypted backup 위치가 준비됐다.
- [ ] PostgreSQL 17 restore rehearsal이 통과했다.
- [ ] Auth/Data client 분리 테스트가 RED부터 준비됐다.
- [ ] identity mirror와 `auth.users` dependency 대체안이 security review를 통과했다.
...
- [ ] 운영자가 final cutover window를 별도로 승인했다.
...
- 현재 Mac CPU/RAM: **충족**
- 현재 disk gate: **미충족 (약 53 GiB free)**
- 가장 큰 설계 blocker: **local `auth.users` 의존을 remote identity mirror로 대체하는 공식 계약**
- 구현 상태: **미착수**
