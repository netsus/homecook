...
| historical hybrid artifacts | hybrid docs/scripts/tests/migrations, package `test:hybrid-supabase:runtime`, `test:hybrid-supabase:postgres`, `test:hybrid-supabase:storage`, CI `hybrid-authority-runtime` | rollback/audit history로만 보존한다. 세 package key와 CI job은 제거하고 `scripts/verify-hybrid-supabase.mjs`, `scripts/hybrid-production-runtime.mjs`, `scripts/hybrid-production-data-migration.mjs`, `scripts/hybrid-remote-auth-mirror.mjs`, `scripts/run-hybrid-revoked-session-canary.mjs`, `scripts/sync-remote-auth-jwks.mjs` 같은 executable remote credential consumer는 import/env/file/network read·write 전에 unconditional forbidden tombstone으로 둔다. 필요한 pure historical helper는 `scripts/lib/`에만 남기고 신규 session prerequisite, release gate 또는 운영 fallback으로 호출하지 않는다. |
...
- backup encryption key는 archive와 분리한다. envelope는 isolated replacement environment의 credential environment가 보유한 Ed25519 issuer public key를 pin한다. `restore-platform`은 direct source backup Keychain account의 부재를 정확히 확인하고, second-medium credential로 escrow를 열어 얻은 key로 archive를 인증한 뒤에만 ownership token이 붙은 create-only 방식으로 Keychain에 등록한다. 등록 경쟁이나 기존 item은 덮어쓰지 않고 실패한다. restore 시작 전 exact Docker container/volume inventory를 snapshot하고 모든 replacement writer와 named volume에 같은 무작위 attempt token을 label로 남긴다. downstream restore/서명 실패 시 snapshot에 없었고 exact Compose project/service-or-volume/attempt-token label이 모두 일치하는 container·volume과 이번 attempt가 `wx`로 만든 뒤 device/inode/size/SHA-256 identity가 변하지 않은 restore/recovery manifest·HMAC sidecar, 그리고 token과 recovered value가 모두 일치하는 Keychain item만 삭제해 동일 target 재시도를 허용한다. preexisting production/dev/decoy, racing 또는 label-mismatch resource와 기존 artifact는 절대 삭제하지 않는다. 소유권 불일치나 cleanup 실패는 manual recovery required로 fail closed한다. actual restore 뒤에만 issuer private key로 recovery manifest를 서명하며 source backup-key HMAC만 있는 self-assertion은 거부한다. fixture 명칭은 `isolated_replacement_environment_verified`로 제한한다. Physical off-device production evidence is separately required; the fixture cannot satisfy it.
...
- RPO 목표는 24시간, RTO 목표는 4시간이다. `validate`, `start`, `status`는 readiness JSON 자체의 HMAC을 parse 전에 검증하고, readiness/archive/sidecar/restore/recovery/escrow-envelope artifact와 canonical owner·exact `0700` parent를 재검증한다. 매 gate마다 primary/off-Mac archive를 인증·복호화해 full metadata digest를 readiness와 결합하고, signed restore manifest와 signed isolated replacement environment recovery evidence 및 그 evidence가 가리키는 escrow envelope의 exact path/hash/device/HMAC을 재검증한다. envelope 삭제·변조·경로 바꿔치기·archive와 동일 device는 모두 fail closed다. `start`는 writer 기동 전 이 offline preflight를 통과하고, 기동 뒤 live Docker identity를 다시 확인하며 실패하면 이번 attempt가 새로 올린 exact writer service만 compensation stop한다.
