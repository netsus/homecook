...
- backup encryption key는 archive와 분리한다. 현재 Mac Keychain만으로는 single-Mac failure를 복구할 수 없으므로 off-Mac archive와도 다른 두 번째 매체/credential manager에 AES-256-GCM authenticated escrow envelope를 보존한다. envelope는 replacement credential environment가 보유한 Ed25519 issuer public key를 pin한다. production recovery manifest는 `restore-platform`의 actual clean restore가 끝난 뒤에만 해당 private key로 서명해 발급하며, source backup-key HMAC만 있는 self-assertion은 거부한다. fixture가 증명하는 범위는 `isolated_replacement_environment_verified`이고 physical replacement-Mac 주장은 하지 않는다. 실제 별도 매체/private issuer key가 없으면 production issuance/readiness는 fail closed한다.
