...
- backup encryption key는 archive와 분리한다. 현재 Mac Keychain만으로는 single-Mac failure를 복구할 수 없으므로 off-Mac archive와도 다른 두 번째 매체/credential manager에 AES-256-GCM authenticated escrow envelope를 보존한다. signed recovery evidence는 envelope의 canonical absolute path, SHA-256, 실제 filesystem device identity를 포함하고, envelope와 `.auth.json` sidecar의 존재·HMAC·독립 device를 매 gate에서 다시 검증한다. 교체 Mac에서 escrow open → 격리 Keychain adapter 등록/조회 → 실제 encrypted platform archive 인증·복호화 → disposable DB+Storage clean restore를 완료한 결과로만 recovery evidence를 발급해야 한다. 실제 별도 매체가 없으면 production issuance/readiness는 fail closed한다.
