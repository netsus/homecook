...
- backup encryption key는 archive와 분리한다. envelope는 isolated replacement environment의 credential environment가 보유한 Ed25519 issuer public key를 pin한다. `restore-platform`은 direct source backup Keychain account의 부재를 정확히 확인하고, second-medium credential로 escrow를 열어 얻은 key로 archive를 인증한 뒤에만 create-only 방식으로 Keychain에 등록한다. 등록 경쟁이나 기존 item은 덮어쓰지 않고 실패한다. actual restore 뒤에만 issuer private key로 recovery manifest를 서명하며 source backup-key HMAC만 있는 self-assertion은 거부한다. fixture 명칭은 `isolated_replacement_environment_verified`로 제한한다. Physical off-device production evidence is separately required; the fixture cannot satisfy it.
