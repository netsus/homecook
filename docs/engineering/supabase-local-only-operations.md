...
- backup encryption key는 archive와 분리한다. 현재 Mac Keychain만으로는 single-Mac failure를 복구할 수 없으므로 off-Mac archive와도 다른 두 번째 매체/credential manager에 authenticated escrow를 보존한다. 교체 Mac에서 escrow open → Keychain 재등록 → archive 인증·복호화 → clean isolated restore를 완료하고 signed recovery evidence를 발급해야 한다. 실제 별도 매체가 없으면 issuance/readiness는 fail closed한다.
...
- RPO 목표는 24시간, RTO 목표는 4시간이다. `validate`, `start`, `status`는 readiness JSON 자체의 HMAC을 parse 전에 검증하고, readiness/archive/sidecar/restore/recovery artifact와 canonical owner·exact `0700` parent를 재검증한다. 매 gate마다 primary/off-Mac archive를 인증·복호화해 full metadata digest를 readiness와 결합하고, signed restore manifest의 path/hash와 두 번째 매체의 signed replacement-Mac recovery evidence를 재검증한다. `start`는 writer 기동 전 이 offline preflight를 통과하고, 기동 뒤 live Docker identity를 다시 확인하며 실패하면 이번 attempt가 새로 올린 exact writer service만 compensation stop한다. 하나라도 unavailable/mismatch/stale이면 fail closed한다.
