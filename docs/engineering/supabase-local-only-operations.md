...
- backup encryption key는 archive와 분리한다. envelope는 replacement credential environment가 보유한 Ed25519 issuer public key를 pin한다. `restore-platform`은 source backup Keychain item 부재를 먼저 확인하고, second-medium credential로 escrow를 열어 얻은 key만 replacement Keychain에 등록·조회한 뒤 archive decrypt와 clean restore에 사용한다. actual restore 뒤에만 issuer private key로 recovery manifest를 서명하며 source backup-key HMAC만 있는 self-assertion은 거부한다. fixture 명칭은 `isolated_replacement_environment_verified`로 제한하고 physical Mac을 주장하지 않는다.
