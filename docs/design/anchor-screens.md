...
- 첫 화면에 며칠치 식단이 거의 안 보이거나, 설명/여백 때문에 day overview가 아래로 밀리면 위험 신호다.
- 주간 범위 이동 컨트롤이 planner 본문과 멀리 떨어지면 사용성이 급격히 떨어진다.
- 같은 날짜의 끼니가 여러 카드나 여러 축으로 흩어지면 "하루 식단을 한눈에 보기" 목표가 무너질 수 있다.
- 4~5컬럼 대응은 지원 자체보다 가독성 보존이 우선이다.
