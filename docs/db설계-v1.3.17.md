...
> 기준 문서: 요구사항 기준선 v1.7.14 / 화면정의서 v1.5.21 / API 설계 v1.2.22 / 유저 Flow맵 v1.3.20
>
>
> 작성: 킴실장
>
...
ingredients ─< piece_unit_weights
...
> 공공 데이터는 운영자 batch에서 raw artifact와 manifest를 외부 artifact storage에 불변 보존한 뒤, 검수 승인된 정규화 row만 아래 테이블에 승격한다. API key, 인증 query, cookie, secret, raw provider response는 아래 테이블과 로그에 저장하지 않는다.
...
| review_status | varchar(20) | NOT NULL | `pending / approved / rejected / needs_source_check` |
| created_at | timestamptz | NOT NULL | 생성 시각 |

```
UNIQUE (provider_code, dataset_name, source_version, manifest_sha256)
CHECK (review_status IN ('pending', 'approved', 'rejected', 'needs_source_check'))
```

- public calculation/catalog에는 `review_status='approved'` source만 사용할 수 있다.
- 농촌진흥청 계량자료는 원문 표 전체를 저장하지 않는다. 필요한 source URL·조회일·관측 사실·변환 결과만 assignment provenance로 연결한다.

## 2A-3. nutrition_source_items

| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| id | uuid | PK | normalized source item ID |
| source_id | uuid | FK → nutrition_sources, NOT NULL | source version |
| external_item_key | text | NOT NULL | source food code/품목제조보고번호 등 stable key |
| external_name | text | NOT NULL | 원본 식품명 |
| preparation_state | text | nullable | 생/조리/건조/가식부·형태 |
| source_basis_text | text | nullable | 기준량 원문. 파싱 실패를 100g으로 추정하지 않음 |
| stable_fingerprint | text | NOT NULL | deterministic identity hash |
| review_status | varchar(20) | NOT NULL | source item review 상태 |
| provenance_json | jsonb | NOT NULL, DEFAULT '{}' | source row 위치·대조 관계. secret/raw 전체 payload 금지 |
| created_at | timestamptz | NOT NULL | 생성 시각 |

```
UNIQUE (source_id, external_item_key)
UNIQUE (source_id, stable_fingerprint)
CHECK (review_status IN ('pending', 'approved', 'rejected', 'needs_review', 'needs_source_check'))
```
...
| source_item_id | uuid | FK → nutrition_source_items, nullable | 공공 source profile이면 필수, manual/recipe 계산이면 null 가능 |
| profile_kind | varchar(30) | NOT NULL | `ingredient_source / product_label / recipe_calculation` |
| basis_amount | numeric(12,4) | NOT NULL | 기준량 |
| basis_unit | varchar(20) | NOT NULL | `g / ml / serving / package / recipe` |
| version | int | NOT NULL, DEFAULT 1 | 대상별 immutable version |
| review_status | varchar(20) | NOT NULL | `pending / approved / rejected / self_reported` |
...
CHECK (review_status IN ('pending', 'approved', 'rejected', 'self_reported'))
```

- profile은 insert 후 basis/source/version을 UPDATE하거나 DELETE하지 않는다. 정정은 새 profile/version으로 수행한다.
- 공공 source profile은 operator review의 `approved`만 production 계산에 사용한다. private manual 제품은 `self_reported`로 저장해 공공 검수값으로 오해되지 않게 하며 해당 owner의 제품/플래너에서만 사용한다.
...
| nutrient_code | varchar(50) | FK → nutrient_definitions(code), NOT NULL | 영양소 |
| amount | numeric(14,6) | NOT NULL, CHECK >= 0 | 기준량당 값 |
| source_token | text | nullable | 값이 존재한 경우의 제한된 원문 숫자 token 감사용. 대량 raw payload 금지 |
| created_at | timestamptz | NOT NULL | 생성 시각 |

- **PK/UNIQUE**: `(profile_id, nutrient_code)`
- 값이 없으면 row 자체를 만들지 않는다. 결측·trace·미검출·빈값은 `amount=0`으로 정규화하지 않고 `nutrition_source_items.provenance_json`의 normalized missing reason으로만 제한 보존한다.
- 값 row는 profile과 함께 immutable하다.
...
| nutrition_profile_id | uuid | FK → nutrition_profiles, NOT NULL | 승인 profile |
| preparation_state | text | NOT NULL | 생/조리/건조/가식부 상태 |
| is_primary | boolean | NOT NULL, DEFAULT false | 해당 상태의 primary 여부 |
| review_status | varchar(20) | NOT NULL | `pending / approved / rejected / needs_review` |
| reviewed_at | timestamptz | nullable | 검수 시각 |
| created_at | timestamptz | NOT NULL | 생성 시각 |

```
UNIQUE (ingredient_id, nutrition_profile_id, preparation_state)
UNIQUE (ingredient_id, preparation_state) WHERE is_primary = true AND review_status = 'approved'
```

- 같은 표준명이라도 생것/조리/건조 profile을 서로 덮어쓰지 않는다. 자동 이름 일치만으로 `approved`로 만들지 않는다.
...
| is_active | boolean | NOT NULL, DEFAULT true | 신규 배정 가능 여부 |
| created_at | timestamptz | NOT NULL | 생성 시각 |
...
| conversion_profile_id | uuid | FK → measurement_conversion_profiles, NOT NULL | 계산용 대표 등급 |
| source_id | uuid | FK → nutrition_sources, NOT NULL | license/version이 확인된 관측 source |
| source_item_id | uuid | FK → nutrition_source_items, nullable | 공공 관측 source item |
| preparation_state | text | NOT NULL | 재료 형태/손질 상태 |
| observed_g_per_15ml | numeric(8,3) | NOT NULL | 감사·재분류용 관측값 |
| source_observed_unit | text | NOT NULL | 원래 계량단위 |
| source_observed_amount | numeric(10,3) | NOT NULL | 원 관측값 |
| source_url | text | NOT NULL | 공식 출처 URL |
| source_accessed_at | date | NOT NULL | 조회일 |
| assignment_reason | text | NOT NULL | 대표 등급 배정 근거 |
| review_status | varchar(20) | NOT NULL | `pending / approved / rejected / needs_review` |
| version | int | NOT NULL | 적용 version |
| created_at | timestamptz | NOT NULL | 생성 시각 |

```
CHECK (observed_g_per_15ml > 0)
CHECK (source_observed_amount > 0)
UNIQUE (ingredient_id, preparation_state, version)
```

- 가장 가까운 대표값과 차이가 `<= 2.5g/15mL`이고 재료/형태 mapping이 명확한 row만 승인 후보가 된다. 최종 `approved`는 사람 검수 결과다.
- 원문 표·문장·행/열 배치·페이지 이미지·전체 dataset은 저장하지 않는다.

## 2A-9. piece_unit_weights
...
| source_id | uuid | FK → nutrition_sources, NOT NULL | license/version이 확인된 관측 source |
| source_item_id | uuid | FK → nutrition_source_items, nullable | provenance |
| review_status | varchar(20) | NOT NULL | `pending / approved / rejected / needs_review` |
...
```
CHECK (weight_g > 0)
UNIQUE (ingredient_id, size_code, preparation_state, version)
```

### 영양 테이블 권한/불변 정책

- `nutrient_definitions`와 approved source attribution은 public read를 허용할 수 있다. 원본 ingestion/검수 테이블의 write는 operator/service-role 전용이다.
- private manual product의 profile/value는 소유자 scope route를 통해서만 생성·조회한다. 다른 사용자에게 직접 노출하지 않는다.
- 일반 사용자는 approved 공공 source/profile/assignment를 수정·삭제할 수 없다.
- 공공 source의 license와 provenance가 확인되지 않거나 `needs_source_check`이면 production 계산/catalog에서 제외한다.
...
## 4-6. recipe_nutrition_snapshots `v1.3.17 신규`

| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| id | uuid | PK | immutable snapshot ID |
| recipe_id | uuid | FK → recipes, NOT NULL | 대상 recipe |
| nutrition_profile_id | uuid | FK → nutrition_profiles, NOT NULL | 전체/base recipe 계산 결과 profile |
...
| scalable_values_json | jsonb | NOT NULL, DEFAULT '{}' | 인분 비례 영양 벡터 |
...
## 5-3. food_products `v1.3.17 신규`

| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| id | uuid | PK | 제품 ID |
| owner_user_id | uuid | FK → users, nullable | private manual 제품 소유자. public이면 null |
...
| external_product_key | text | nullable | 품목제조보고번호 또는 provider food code |
...
UNIQUE (source_type, external_product_key) WHERE external_product_key IS NOT NULL
```

- public catalog는 승인 source의 기준량·핵심 영양값·stable product key가 온전한 subset만 등록한다. 제품명+업체명 유사성만으로 merge하지 않는다.
- public 제품은 일반 사용자 read-only다. private manual 제품은 owner만 조회/수정/soft-delete한다.
- private manual 제품은 다른 사용자 검색에 나타나지 않는다. 삭제된 제품은 새 검색/entry 생성에서 제외하지만 기존 entry 참조는 보존한다.
...
## 5-4. food_product_nutrition_versions `v1.3.17 신규`
...
| label_basis_text | text | nullable | 제품 라벨 기준량 원문 |
...
- version/profile/value는 insert 후 수정·삭제하지 않는다. `food_products.current_nutrition_version_id`만 새 version을 가리키도록 원자적으로 전환한다.
- manual 제품 수정은 새 version을 만들며 기존 planner entry는 이전 version을 유지한다.

## 5-5. product_planner_entries `v1.3.17 신규`
...
- `food_products`: public active row + 본인 private row만 SELECT. private owner만 INSERT/PATCH/soft-delete. public write는 operator/service-role만 허용한다.
- `food_product_nutrition_versions`: public 제품 version 또는 본인 private 제품 version만 SELECT. 일반 사용자의 UPDATE/DELETE는 금지한다.
...
| nutrition_profiles 1 — N nutrition_values | 1:N | 영양소별 값; 결측 row 없음 |
...
| ingredient_nutrition_profiles | `(ingredient_id, preparation_state, is_primary)` | 재료 계산 profile 선택 |
| ingredient_conversion_assignments | `(ingredient_id, preparation_state, review_status)` | 승인 대표 등급 선택 |
...
| food_products | `(visibility, deleted_at, name)` | public/private active catalog 검색 |
| food_products | `(owner_user_id, deleted_at, updated_at DESC)` | 본인 private catalog |
...
| 36 | nutrient_definitions | 영양 공통 `v1.3.17` |
| 37 | nutrition_sources | 영양 provenance/license `v1.3.17` |
| 38 | nutrition_source_items | 영양 source item `v1.3.17` |
| 39 | nutrition_profiles | immutable 영양 profile `v1.3.17` |
| 40 | nutrition_values | 영양소별 값 `v1.3.17` |
| 41 | ingredient_nutrition_profiles | 재료 영양 연결 `v1.3.17` |
| 42 | measurement_conversion_profiles | 대표 부피 환산 등급 `v1.3.17` |
| 43 | ingredient_conversion_assignments | 재료별 대표 등급 검수 `v1.3.17` |
| 44 | piece_unit_weights | 승인 개당 중량 `v1.3.17` |
| 45 | recipe_nutrition_snapshots | 레시피 영양 snapshot `v1.3.17` |
| 46 | food_products | 완제품 catalog `v1.3.17` |
| 47 | food_product_nutrition_versions | 완제품 영양 version `v1.3.17` |
| 48 | product_planner_entries | 완제품 플래너 entry `v1.3.17` |
