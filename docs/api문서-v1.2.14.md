...
# 집밥 서비스 — API 설계 v1.2.14

> 기준 문서: 요구사항 기준선 v1.7.4 / 화면정의서 v1.5.11 / DB 설계 v1.3.10 / 유저 Flow맵 v1.3.11
> 작성: 킴실장
...
> v1.2.13 → v1.2.14: Recipe media/tags contract. `POST /api/v1/recipes/images` 이미지 업로드 endpoint 추가, YouTube register가 session thumbnail/tags를 서버에서 저장, manual `POST /recipes`가 current-user upload reference만 허용하고 tags는 서버 생성. 엔드포인트 수 59 → 60
...
**처리**: extraction_id로 `youtube_extraction_sessions` 조회 → 소유권 검증(user_id == current_user) → 만료/소비 검증 → client body의 `youtube_url`을 파싱한 값과 session의 canonical URL/video ID를 비교해 EXTRACTION_MISMATCH 검증 → 세션 `thumbnail_url`과 서버 생성 `tags`를 적용 → Postgres RPC `register_youtube_recipe_from_session` 원자적 실행
...
> **v1.2.14 총계**: 60개 (`POST /recipes/images` 이미지 업로드 endpoint 1개 추가, thumbnail_url/tags field 계약 보강)
