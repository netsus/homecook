...
- browser/public request는 mode/options/key version/digest를 지정할 수 없다. route는 authenticated user session으로 URL 또는 owner retry source를 normalized video ID로 확정한다. 이후 Next server-only client가 installed current release descriptor와 authenticated user binding의 canonical preimage를 만들고 external secret loader의 current/optional previous HMAC key로 `current_digest`/`previous_digest`를 계산한다. DB는 HMAC secret을 알지 못한다.
- internal enqueue call은 existing server-side JWT/attestation authority가 current `sub/session_id/generation`에 묶어 발급한 request-scoped enqueue JWT의 exact `youtube-extraction-enqueue` pre-request scope와 `youtube_extraction_enqueue` NOLOGIN role만 사용한다. enqueue RPC input은 `video_id, current_key_version, current_digest, optional previous_key_version, optional previous_digest, submission_mode`뿐이다. 브라우저 직접 RPC 금지이며 static cross-user token·generic service-role 호출도 금지다. DB는 current/previous version·rotation window·lowercase 64-hex format을 검증하고 active current/previous pair를 dual-read dedupe하며 새 row에는 current-write만 한다.
...
- maintenance/rotation과 경합한 accepted enqueue는 old/new 중 한 complete snapshot만 사용하며 mixed policy/key/options/digest는 fail closed한다.
