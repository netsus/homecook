...
- internal enqueue call은 `createRouteHandlerClient()` 사용자 세션으로 SECURITY DEFINER RPC를 호출하고 owner는 RPC의 `auth.uid()`에서만 도출한다. 별도 enqueue credential authority는 없다. RPC input은 `video_id, expected_policy_version, expected_policy_snapshot_digest, current_key_version, current_digest, optional previous_key_version, optional previous_digest, submission_mode`뿐이다. 브라우저 직접 RPC 금지이며 direct call은 invalid digest/policy mismatch로 거부된다. digest/descriptor는 response/log/browser bundle에 노출 금지다.
