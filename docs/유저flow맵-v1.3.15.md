...
> 기준 문서: 요구사항 기준선 v1.7.7 / 화면정의서 v1.5.14 / DB 설계 v1.3.12 / API 설계 v1.2.16
>
...
## ⑪-b 사용자 성장/배지/퀘스트 확인 (Gamification) `33c-gamification`
...
```
MYPAGE (Bottom Tab) → 계정 섹션
  ├─ ⑪-a compact progress 표시
  └─ Gamification surface
       ├─ 대표/최근 배지 row
       ├─ 배지 안내 버튼 → badge guide modal/bottom sheet
       ├─ active quest 1~2개
       └─ tutorial quest 1개
```
...
```
XP source action 성공
  ├─ 33a progress event append/projection update
  ├─ 33c badge/quest/notification projection attempt
  │    ├─ 성공: unseen notification 생성 또는 기존 notification 유지
  │    └─ 실패: 원래 source action 성공은 유지, gamification soft-fail 기록
  └─ FE notification refresh
       ├─ unseen XP notification 있음 → shell XP toast 표시
       └─ notification seen 처리 → 실패해도 core action rollback 없음
```
