...
- client/route는 mode/options를 지정할 수 없다. client/public route body는 key version/digest/descriptor도 지정할 수 없다. trusted Next server layer만 release expected-schema manifest의 installed app descriptor와 authenticated user binding을 canonical preimage에 사용한다. existing app external secret loader의 server-only allowlist가 current/optional previous HMAC key를 읽어 digest를 계산하고 `createRouteHandlerClient()` 사용자 세션으로 RPC를 호출한다. owner는 RPC의 `auth.uid()`에서만 도출한다. 브라우저 직접 RPC 금지이며 invalid digest/policy mismatch는 write 0이다. fingerprint HMAC key는 worker에는 금지하고 digest/descriptor는 response/log/browser bundle에 노출 금지다.
...
- enqueue RPC는 advisory lock 안에서 enabled policy plain SELECT를 수행하고 supplied `expected_policy_version`/`expected_policy_snapshot_digest`와 computed current row의 expected version/digest exact match를 먼저 검증한다. mismatch 또는 invalid digest/policy mismatch는 safe `POLICY_CHANGED`, insert/dedupe/budget 0이다. 그 뒤에만 current key/version/digest format, optional previous version/window/digest format → current+previous active row dual-read dedupe → active/daily budget → immutable current-write job INSERT 순서로 결정한다. active duplicate는 current 또는 previous stored pair가 일치한 한 row다. 새 row에는 `current_key_version/current_digest`만 저장한다. route count-then-insert와 DB secret access는 금지한다.
...
- enqueue RPC exact signature를 `authenticated`에만 `GRANT EXECUTE`하고 `PUBLIC`, `anon`, `service_role`, worker, credential-manager, manager에서 `REVOKE EXECUTE`한다. 브라우저 직접 RPC 금지이며 HMAC digest와 expected snapshot을 모르는 direct call은 invalid digest/policy mismatch로 write 0이다. enqueue owner는 policy plain SELECT, jobs owner-bound SELECT/INSERT와 allowlisted owner-bound budget-consume RPC EXECUTE만 허용하며 broad schema/table ownership, DELETE/TRUNCATE/TRIGGER/REFERENCES와 HMAC secret 접근은 0이다.
