...
> **2026-08-31 contract-evolution — 마케팅 수요검증 단일 세션 테이블**
>
> 사용자 승인(2026-08-31)에 따라 `.omx/plans/mumeok-weekly-nutrition-ad-landing.md`의 승인된 계획을 공식 계약으로 반영했다.
>
> `public.marketing_validation_sessions` 단일 테이블만 추가한다. 이 테이블은 첫 view부터 followup까지의 단계 timestamp와 attribution, result, lead status를 한 행에 보관한다.
>
> 필수 필드는 `id`, `campaign_key`, `creative_key`, `audience_key`, `utm_*`, `attribution_status`, `viewed_at`, `quiz_started_at`, `quiz_completed_at`, `solution_viewed_at`, `intent_choice`, `intent_clicked_at`, `quiz_result`, `quiz_answers`, `target_qualified`, `email`, `consent_version`, `consented_at`, `turnstile_verified_at`, `lead_submitted_at`, `lead_submission_status`, `planner_intent`, `planner_priority`, `followup_submitted_at`, `retention_until`, `created_at`, `updated_at`다.
>
> RLS는 enabled이고 anon/authenticated 직접 접근은 금지한다. service_role만 읽기/쓰기를 수행한다. unique lower(email) partial index와 `campaign_key, creative_key, created_at` index만 추가한다. raw IP, user-agent, 전체 referrer URL, cookie fingerprint, operational_events 재사용은 금지한다.
>
> `retention_until`의 기본값은 캠페인 종료 + 180일이다. turnstile/token/lead evidence는 별도 운영 로그가 아니라 행 자체의 최소 필드만 사용한다. preview/production lead gate는 `MARKETING_LEAD_PROTECTION_READY`, Turnstile secret/hostname, allowlisted origin, edge rule evidence가 없으면 fail closed다.
>
> 기존 테이블은 건드리지 않는다. 모든 새로운 analytics/export는 `public.marketing_validation_sessions`와 `docs/marketing/demand-validation-analysis.sql`만 기준으로 계산한다.

> **2026-08-27 contract-evolution — YouTube 실제 단계 진행바와 남은 시간 범위**
>
> 승인 계획은 `youtube-extraction-truthful-progress-eta-plan-20260826.md`, SHA-256 `e3bf440a3708b50e2430f1f2fda770fbe2a1f30bdaf3eb79fb6237affd5bbe60`이다. 신규 endpoint/status/error 없음. `youtube_extraction_jobs`에 nullable progress snapshot 5컬럼을 additive로 추가하고 private stage event table과 exact fenced progress RPC를 추가한다. 전체 테이블 수는 75개다.
>
> 공개 단계와 floor는 `queued=0`, `source_fetch=10`, `video_download=25`, `frame_extraction=45`, `model_analysis=65`, `finalizing=90`이다. 같은 attempt 안에서는 stage ordinal이 뒤로 갈 수 없다. stale job/worker/lease/permit/attempt update는 `applied=false`, write 0이다. progress write의 transient 실패는 extraction/finalize 결과를 failed로 바꾸지 않는다.
>
> `video_duration_seconds`는 nullable integer `1..86400`만 저장하며 범위 밖·미확인은 null이다. raw URL, video ID, title, transcript, frame, prompt, provider payload, user UUID는 progress event에 중복 저장하지 않는다. browser/anon/authenticated/service_role의 private table direct read/write는 0이고 restricted worker exact RPC만 `EXECUTE`한다.
>
> `confirmed_percent` 진행 하한은 시간만으로 증가하지 않는다. active 최대 confirmed floor는 90이고 active UI는 95를 넘지 않는다. `succeeded일 때만 100`이다. 첫 release는 stage telemetry promotion gate 전 numeric ETA를 만들지 않는다. 새 public endpoint, WebSocket/SSE, ML, 새 npm/pnpm 의존성은 없다. 이전 client는 additive status field를 무시할 수 있고 ETA 문제가 생기면 숫자 ETA만 숨긴다.
...
>
> table, column, constraint, trigger, RLS, function signature와 public projection은 변경하지 않는다. DB schema 영향은 정확히 N/A이며 이 버전 bump는 database authority·backup·검증 경계를 공식화하기 위한 것이다. 2026-07-30 hybrid/remote 문단은 historical recovery 기록으로만 보존한다.

> **2026-08-12 contract-evolution — YouTube durable queue, fenced worker, credential authority**
>
> 사용자는 2026-08-12 background extraction job, 기존 session의 `source_job_id`, global provider permit, private worker credential state와 최소권한 worker/credential-manager 역할을 승인했다. 이 문서는 additive target schema와 권한 불변식을 잠글 뿐 migration을 생성·적용하거나 production credential을 발급하지 않는다. 기준은 `origin/master@d38ee2e4a4c8cafc00dce713919c3f3e8df2bdda`다. PR #1343 review task `019ff598-233b-72c1-92f5-4372596ede7a`의 Findings 1~4,6을 반영했고, 수정 계획 전체는 독립 plan reviewer task `019ff4f7-806c-7151-b646-cab784606cde`가 `Verdict PASS`, `Findings 없음`, `차단 없음`, exact SHA-256 `b560b60ff758171e1d52ad56b2a63a2e1877cd762d1f691c9cea32c753f8d332`, line count `873`, baseline `origin/master@d38ee2e4a4c8cafc00dce713919c3f3e8df2bdda`로 재승인했다.

> **2026-08-22 contract-evolution — personal recipe write strict union의 DB 영향 N/A**
>
> `POST /recipes`의 personal-derived branch는 기존 `recipes` authority와 `revision`, `origin_recipe_id`, `deleted_at`, `visibility`만 재사용하고 새 public table/column/status/error/RLS/ACL를 만들지 않는다. server는 accessible public source를 fork, exact owner-private active source를 save_as_new으로 derived operation 처리하고, other-owner/deleted/quarantined source는 404, stale base revision은 no-write conflict로 fail closed한다. `base_recipe_revision`은 request authority이지 stored column이 아니며, `draft`와 `image_object_id`는 existing canonical draft/image provenance를 그대로 재사용한다. 이 버전 bump의 schema 영향은 N/A다.
...
| `user_id` | uuid | `users(id) ON DELETE CASCADE`, NOT NULL. payload가 아닌 enqueue/session authority에서 결정 |
| `youtube_video_id` | varchar(20) | normalized ID, NOT NULL. jobs에 원문/암호화 URL 저장 금지 |
| `video_title_snapshot` | varchar(160) | nullable. provider metadata 뒤 control 문자 제거·공백 정규화·길이 제한한 값만 1회 기록 |
...
| `status` | text | stored `queued | processing | succeeded | failed` |
| `attempt_count`, `max_attempts` | integer | 0 이상, `attempt_count <= max_attempts`; 기본 max 3, 실제 provider start만 count |
| `available_at` | timestamptz | queue/retry claim 가능 시각 |
| `lease_owner` | text | nullable opaque worker ID |
| `lease_generation` | bigint | claim마다 증가하는 fencing token, NOT NULL |
| `lease_expires_at`, `heartbeat_at` | timestamptz | nullable lease 회수/생존 근거 |
| `started_at` | timestamptz | nullable. 최초 provider attempt 시각을 retry/reclaim에도 유지 |
| `progress_attempt` | integer | nullable. snapshot이 속한 `attempt_count`; historical row는 전부 null |
| `progress_stage` | text | nullable. `queued | source_fetch | video_download | frame_extraction | model_analysis | finalizing` allowlist |
| `progress_stage_started_at` | timestamptz | nullable. current stage를 최초 정상 적용한 DB 시각 |
| `progress_updated_at` | timestamptz | nullable. 같은 attempt의 마지막 정상 progress apply 시각 |
| `video_duration_seconds` | integer | nullable. ETA bucket 전용 `1..86400`; 범위 밖·미확인은 null |
| `extraction_session_id` | uuid | nullable FK → `youtube_extraction_sessions(id)`, succeeded이면 NOT NULL |
| `error_code`, `error_message` | text | nullable allowlisted stable code + sanitized 사용자 문구. raw provider 내용 금지 |
| `completed_at` | timestamptz | succeeded/failed이면 NOT NULL |
| `completion_delivery_key` | text | terminal 전환 때 1회 생성, nullable UNIQUE |
| `completion_delivered_at`, `completion_seen_at` | timestamptz | nullable, 최초 시각만 멱등 기록 |
...
worker가 실제로 들어간 단계의 시각만 보존하는 private append-only table이다. `queued`는 job snapshot에만 있고 event table에는 저장하지 않으므로 attempt당 최대 5행이다.

| 컬럼 | 타입 | 제약/의미 |
| --- | --- | --- |
| `job_id` | uuid | FK → `youtube_extraction_jobs(id) ON DELETE CASCADE` |
| `attempt` | integer | 1 이상, 현재 job `attempt_count`와 exact match |
| `stage` | text | `source_fetch | video_download | frame_extraction | model_analysis | finalizing` allowlist |
| `entered_at` | timestamptz | DB clock authority, 최초 정상 적용 시각 |
| `video_duration_seconds` | integer | nullable `1..86400`; 해당 attempt에서 최초 확인된 bounded 값 |

- PK/UNIQUE는 `(job_id, attempt, stage)`다. 같은 stage 재보고는 event row를 늘리지 않고 job snapshot timestamp도 되돌리지 않는다.
- 같은 attempt의 stage ordinal은 뒤로 갈 수 없다. cache hit 또는 단계 생략은 앞으로 건너뛰는 것만 허용한다.
- historical row는 전부 null로 유지하고 backfill하지 않는다. 신규 enqueue만 `progress_attempt=0`, `progress_stage='queued'`, progress 시각=`created_at`으로 시작한다.
- private schema이며 browser, `anon`, `authenticated`, `service_role` direct read/write는 0이다. table/sequence privilege도 0이고 exact RPC owner만 function body에서 접근한다.
- user UUID, YouTube URL/ID/title, source text, transcript, frame, prompt, provider payload, token/cookie/secret을 저장하지 않는다.
- job 삭제와 함께 cascade된다. 별도 retention/compaction worker는 만들지 않는다.

### `report_youtube_extraction_progress`

- exact input은 `(job_id uuid, worker_id text, lease_generation bigint, permit_generation bigint, attempt integer, stage text, video_duration_seconds integer default null)`이다.
- exact return은 `TABLE(applied boolean)` 한 컬럼이다. current job/worker/lease generation/active permit generation/current attempt가 모두 일치하고 job이 non-terminal일 때만 `applied=true`다.
- stale job/worker/lease/permit/attempt, terminal job, stage 역행은 `applied=false`, write 0이다. duplicate same-stage는 멱등이며 기존 event/started timestamp를 바꾸지 않는다.
- `video_duration_seconds`는 nullable `1..86400`만 허용하고 범위 밖은 RPC validation 실패와 write 0이다.
- restricted worker role만 exact signature `EXECUTE`를 가진다. public/authenticated/service_role 및 다른 worker RPC owner의 `EXECUTE`는 0이다.
- stale/fence rejection은 authoritative no-write다. 반면 worker의 network/transient progress report 실패는 best-effort로 기록·계수하되 extraction child, heartbeat, finalize/fail 경로를 중단하거나 job을 failed로 바꾸지 않는다.
...
- 2026-09-18 실행 상태 확인: current policy version 2는 같은 options schema에서 `codexEffort="low"`, `selectorEffort="low"`를 사용한다. pipeline identity는 `5e80ffc32ab63ec1e4b015222692597e18bbce8520271a7130689dd138ff808c`, policy snapshot digest는 `5418cbb09d1ae090becd4e33a7c5c449ca2769e85443b9e8dca82f103a82fa17`이다. 미적용 Luna/Sol version 3 SQL은 `docs/engineering/archive/20260915180000_youtube_luna_sol_medium_pipeline.sql`에 원문 보존하며 활성 migration에 포함하지 않는다. 운영 DB와 이력은 변경하지 않았다.
...
- enqueue transaction의 canonical serialization authority는 stable policy advisory key 하나다. 순서는 transaction advisory shared lock → enabled policy plain SELECT → expected version/digest exact match → supplied current/previous key version·window·digest format 검증 → active dual-read dedupe → active/daily budget → current-write job INSERT다. policy row-level locking read는 금지한다. mismatch는 safe `POLICY_CHANGED`로 insert/dedupe/budget 0이며 policy의 `policy_key`, `policy_version`, `extractor_mode`, `pipeline_identity`, normalized `result_affecting_options`, `fingerprint_key_version`, computed `policy_snapshot_digest`를 job에 immutable snapshot으로 저장한다.
- client/route는 mode/options를 지정할 수 없다. client/public route body는 exact `{ youtube_url } | { retry_job_id }` union뿐이며 digest/key version/policy field는 `422 VALIDATION_ERROR` unknown field다. trusted Next server layer만 release expected-schema manifest의 installed app descriptor와 authenticated user binding을 canonical preimage에 사용한다. existing app external secret loader의 server-only allowlist가 current/optional previous HMAC key를 읽어 digest를 계산하고 `createRouteHandlerClient()` 사용자 세션으로 loopback RPC를 호출한다. owner는 RPC의 `auth.uid()`에서만 도출한다. fingerprint HMAC key는 worker에는 금지하고 digest/descriptor는 response/log/browser bundle에 노출 금지다.
- 정책 전환은 승인된 release migration만 수행한다. canonical write 순서는 같은 advisory key의 exclusive transaction lock → UPDATE/CAS다. initial bootstrap은 disabled singleton/roles/RPC → same release app/worker install → release expected-schema manifest와 exact current policy snapshot preflight → credential/snapshot attestation → exclusive lock/enable → enqueue publish 순서다. later rotation은 enqueue maintenance → drain old snapshot → exclusive lock → disable/lock → CAS update disabled → new app/worker install → manifest와 exact new snapshot preflight + credential/snapshot attestation → exclusive enable → resume 순서다. runtime policy mutation RPC는 없으며 direct table privilege는 0이다.
- transaction advisory shared/exclusive lock 때문에 retry/enqueue 경합은 old/new 중 한 complete snapshot만 사용한다. partial field 조합, disabled row enqueue, old HMAC/options 역복원은 fail closed다. worker는 job `policy_snapshot_digest`를 claim하며 current policy row를 실행 identity로 다시 읽어 이미 queued인 job을 바꾸지 않는다.
...
- `youtube_extraction_sessions.source_job_id uuid UNIQUE NULLABLE REFERENCES youtube_extraction_jobs(id) ON DELETE SET NULL`을 추가한다. legacy/sync historical session은 null을 허용한다.
- async succeeded job에는 같은 owner의 유효한 session이 정확히 하나 연결되고, async session의 canonical reverse linkage는 `source_job_id`다. job 쪽 FK만으로 대체하지 않는다.
- consumed가 TTL보다 우선한다. linked session이 `status='consumed'`이면 `recipe_id IS NOT NULL`이어야 하고, `consumed-after-TTL`도 status/list `succeeded` + exact `recipe_id`/`recipe_path` destination + `can_retry=false`, session-read `200`으로 projection한다. linked session이 unconsumed draft이고 `expires_at <= now()`인 경우만 `expired` + `can_retry=true`다.
- `finalize_youtube_extraction_job` 한 transaction이 session upsert, multi-recipe candidate ledger 전체 write, cooking-method child/draft completeness, job succeeded/link, completion delivery key를 함께 확정한다.
- worker payload의 user UUID는 authority가 아니다. locked job row의 `user_id`에서 session/candidate owner를 도출한다. 일부 candidate/session write 뒤 crash해도 같은 `source_job_id` replay가 완전한 결과 하나로 복구돼야 한다.
- 성공 session의 `expires_at`은 finalize에서 session 생성 시각 + 24시간이다.

### `youtube_extractor_permits`

| 컬럼 | 타입 | 제약/의미 |
| --- | --- | --- |
| `permit_key` | text | PK. 초기 global i031 provider permit singleton |
| `owner_id` | text | nullable worker owner |
| `permit_generation` | bigint | claim마다 증가, NOT NULL |
| `heartbeat_at`, `expires_at` | timestamptz | nullable current holder lease |

- claim은 row lock 안에서 owner/generation을 새로 발급한다. heartbeat/release는 exact current owner+generation만 허용하고 stale release가 새 holder를 해제하지 못한다.
- takeover는 `expires_at + 60초` grace 뒤에만 허용한다. permit heartbeat 실패는 active child `AbortSignal`과 새 child 시작 금지로 연결하고, 60초 안에 종료되지 않은 process tree는 정리한다.
- 현재 production provider-call 단일성은 single launchd executor + global permit + abort/grace로 보장한다. DB 결과 commit 단일성은 job/permit generation fencing으로 보장한다.

### Worker queue/RPC state machine

```text
queued -> processing -> succeeded
                    -> failed
processing -- lease expired + attempts remain --> queued/reclaim
processing -- lease expired + attempts exhausted --> failed/ATTEMPTS_EXHAUSTED
```

- `enqueue_youtube_extraction_job(video_id, expected_policy_version, expected_policy_snapshot_digest, current_key_version, current_digest, previous_key_version, previous_digest, submission_mode)` exact signature만 유지한다. nullable previous version/digest는 둘 다 null 또는 둘 다 non-null이어야 하고, non-null이면 policy row version/window와 일치해야 한다. RPC는 `user_id`, retry ID, URL, mode, pipeline, options, HMAC secret/key를 받지 않고 owner는 `auth.uid()`에서만 도출한다. DB는 HMAC secret을 알지 못한다.
- Next route는 public `{ youtube_url } | { retry_job_id }`를 owner session으로 처리한다. retry branch는 exact owner status read에서 terminal failed/expired와 `can_retry=true`를 확인해 이전 `youtube_video_id`만 가져온다. route는 release expected-schema manifest의 installed app descriptor와 bound user/video를 canonicalize하고 existing app external secret loader의 server-only allowlist에서 current/optional previous key를 읽어 lowercase 64-hex `current_digest`/`previous_digest`를 계산한다. `createRouteHandlerClient()` 사용자 세션으로 exact RPC를 호출하며 별도 enqueue credential authority/static cross-user token은 없다. current/previous fingerprint key lifecycle은 app release와 policy rotation에 맞추고 worker에는 금지한다.
- enqueue RPC는 advisory lock 안에서 enabled policy plain SELECT를 수행하고 supplied `expected_policy_version`/`expected_policy_snapshot_digest`와 computed current row의 expected version/digest exact match를 먼저 검증한다. expected policy mismatch는 safe `POLICY_CHANGED`, insert/dedupe/budget 0이다. 그 뒤에만 current key/version/digest format, optional previous version/window/digest format → current+previous active row dual-read dedupe → active/daily budget → immutable current-write job INSERT 순서로 결정한다. active duplicate는 current 또는 previous stored pair가 일치한 한 row다. 새 row에는 `current_key_version/current_digest`만 저장한다. route count-then-insert와 DB secret access는 금지한다. digest의 값 검사는 형식·회전 창·중복 제거 입력 검증일 뿐 호출자의 암호학적 진위를 증명하지 않는다.
...
- `fail_or_retry_youtube_extraction_job(job_id, worker_id, lease_generation, error_code)`은 allowlisted retryable code+attempt 잔여일 때만 backoff queued, 아니면 failed+delivery key를 확정한다.
- `mark_youtube_extraction_jobs_delivered(user_id, delivery_keys)`와 `mark_youtube_extraction_jobs_seen(user_id, job_ids)`는 authenticated owner terminal rows만 최초 시각을 기록한다. 타인 ID는 존재 여부를 노출하지 않고 무시한다.
...
- release 전환 상호계약 검증은 전환 전 terminal job의 retry가 이전 job에서는 youtube_video_id만 복사한다는 것, retry 시점의 `private.youtube_extraction_current_policy` complete snapshot으로 새 identity/options/fingerprint를 만든다는 것, 이전 HMAC/options 역복원 금지, 전환 후 current worker가 exact snapshot digest로 claim 가능함을 고정한다. policy rotation vs retry concurrency는 같은 canonical advisory key의 shared enqueue/exclusive UPDATE/CAS 아래 old/new 중 한 complete snapshot만 허용한다. 별도 negative test는 options-only rotation에서 old `allowed_snapshot_digest` worker claim/reaper/start/finalize 0건과 options-only rotation stale app의 `POLICY_CHANGED`·insert/dedupe/budget 0을 증명한다.
...
| 컬럼 | 타입 | 제약/의미 |
| --- | --- | --- |
| `credential_name` | text | singleton PK |
| `current_generation` | bigint | monotonic current worker credential generation |
| `current_jti_hash` | text | 목적별 keyed hash. raw JTI/token 아님 |
...
> **2026-08-08 contract-evolution 1A — session binding의 stable identity와 token evidence 분리**
>
> 기존 session authority table/function을 additive migration으로 교정한다. 과거 migration은 수정하지 않고, 공개 table/column/API를 추가하지 않는다.

## 0-SESSION-REFRESH. Binding authority 의미

- binding row의 stable key는 HMAC 처리한 Auth `session_id`다. JWT `iat`는 session identity가 아니며 access-token 회전 때 달라질 수 있는 evidence다. raw token·refresh token·raw `session_id`는 저장하지 않는다.
- record와 assert/PostgREST pre-request는 stable session hash, active user/provider/identity/epoch/generation, token `iat`/`exp`를 같은 의미로 검증한다.
- 같은 active session에서 새 `iat`/`exp`는 기존 값 이상일 때만 expiry/last-token evidence/last-seen을 단조 갱신한다. 동시 요청은 row lock 또는 동등한 원자적 조건부 update로 이전 evidence가 최신 값을 덮지 못하게 한다.
- 새 JWT `iat`가 기존 `session_issued_at`와 다르다는 이유만으로 stale 처리하지 않는다. 다만 이전 JWT 재사용, identity 생성 전 token, 다른 session/user/provider, revoked·missing·expired binding, stale epoch/generation은 mutation 없이 기존 error로 거부한다.
- missing/revoked binding은 callback·refresh·assert에서 암묵적으로 복구하지 않는다. 허용된 최초 callback/bootstrap만 새 binding을 만들 수 있다.
- DB function과 PostgREST pre-request는 같은 규칙을 사용해야 하며 한 경로만 exact-`iat` equality를 유지하는 부분 교정은 허용하지 않는다.
- 30일 inactivity/90일 absolute timeout 및 recent-auth receipt schema는 이 1A의 적용 완료 범위가 아니며 후속 1B 계약 이후 추가한다.

> **2026-08-04 contract-evolution — recipe snapshot entrypoint read projection**
>
> 기존 `recipes.revision`, `meals.revision`, owner-private recipe/content rows, managed image reference, `personal_recipe_v2`, `snapshot_v2_creation`을 재사용한다. 새 public table/column/endpoint/query parameter/env/status/error/capability field는 추가하지 않는다. 실제 migration 적용·capability 활성화는 이 docs PR의 범위가 아니다.

## 0-RSE. Owner editor / Meal revision / UI mode read authority

### Recipe detail revision과 owner-only edit context

- `recipes.revision` positive bigint가 `GET /recipes/{id}.revision`, owner edit `base_recipe_revision`, PATCH 성공 revision의 단일 authority다. 같은 detail read snapshot 안에서 이 값을 읽어 owner context와 어긋나지 않게 한다.
- owner editor read는 current caller/session/generation을 검증하고 `recipes.created_by = caller`, `visibility='private'`, `deleted_at IS NULL`인 exact row만 허용한다. 해당하지 않으면 row/context 0이며 other-owner private/deleted/quarantined 자원은 기존 404 non-disclosure를 유지한다.
- owner context의 `draft`는 mutable current row와 정렬된 child row를 기존 PATCH canonical input으로 투영한다. 상위 exact key는 `title`, `description`, `base_servings`, `ingredients`, `steps`다. `description`과 nullable scalar는 값이 없으면 명시적 `null`; array는 비어 있어도 `[]`다. owner context 자체가 허용되지 않으면 객체/field를 omit하지 `null`로 대체하지 않는다.
- `ingredients[]`는 `recipe_ingredients.sort_order ASC, id ASC`로 읽고 array order를 write order로 삼는다. exact key는 `ingredient_id`, `amount`, `unit`, `ingredient_type`, `display_text`, `component_label`, `scalable`, `food_product_id`, `food_product_nutrition_version_id`다. nullable은 명시적 `null`이고 row `id`, `sort_order`, joined `standard_name`, product name/brand는 generated/presentation duplication이므로 제거한다. product row는 current version을 재선택하지 않고 저장된 exact product/version pair를 유지한다.
- `steps[]`는 `recipe_steps.step_number ASC, id ASC`로 읽고 exact key `step_number`, `instruction`, `cooking_method_id`, `cooking_method_ids`, `ingredients_used`, `component_label`, `heat_level`, `duration_seconds`, `duration_text`를 투영한다. `cooking_method_ids`는 `recipe_step_cooking_methods.position ASC, method_id ASC`의 array며 legacy single-method row는 current `cooking_method_id`를 포함한 1개 array로 정규화한다. `ingredients_used`는 저장된 array를 그대로 유지한다.
- `image_object_id`는 `recipe_image_object_references(reference_type='recipe_thumbnail', consumer_id=recipe.id)`의 verified current object UUID다. 해당 reference가 없으면 `null`이다. `recipes.thumbnail_url`, signed URL, legacy/external URL을 object ID로 추측하지 않는다. same owner/generation/private attached reference만 투영한다.

### Meal revision read

- `GET /meals` owner/date/column scope에 이미 선택된 recipe Meal row의 `meals.revision` positive bigint를 각 `items[]`에 투영한다. `product_entries[]`에는 이 field를 추가하지 않는다.
- client가 보내는 `expected_meal_revisions` 객체는 selected Meal ID 전체와 exact 1:1이어야 하지만 hint일 뿐이다. `start_snapshot_v2_cooking_session` final transaction은 recipe row를 먼저 lock한 뒤 Meal UUID 오름차순 lock에서 owner/status/revision/no active claim/same recipe/same non-null content를 다시 검증하고 mismatch에서 session/claim write 0으로 종료한다.

### Server-only UI mode projection

- `recipe_snapshot_ui_mode` projection은 `legacy_v1 | snapshot_v2` 두 값만 갖는 server-only read model이다. 기존 capability authority에서 `personal_recipe_v2='on' AND snapshot_v2_creation='on'`을 한 번에 읽은 결과만 `snapshot_v2`로 투영하고, 나머지·null·unknown·read error는 `legacy_v1`으로 fail closed한다.
- projection read는 service-internal/server-render path만 실행한다. underlying setting/GUC를 따로 읽어 서로 다른 시점의 모드를 조합하지 않고, browser/`PUBLIC`/`anon`/`authenticated`의 direct execution·table read·raw capability response는 revoke/deny한다. raw capability name/value/revision은 HTML, browser prop, public JSON, cache key, telemetry에 저장하지 않는다.
- UI mode는 presentation consumer 선택일 뿐 write authority가 아니다. 각 personal write/snapshot-v2 start RPC가 같은 transaction 안에서 capability, owner/session/generation, revision, target/claim을 다시 검증한다. mode 선택과 RPC capability가 어긋나면 RPC가 fail closed하고 client fallback은 없다.
- R/R+1에는 두 creation capability가 외부에 off이며 mode는 `legacy_v1`이다. seeded/existing `snapshot_v2` row의 read/cancel drain은 mode projection과 무관하게 exact v2 namespace에서 유지한다. #8 R/R+1 증거와 R+2 공동 승인 전에 capability state를 바꾸지 않는다.
- #8 pantry/weight/batch/XP complete schema와 F0/account quarantine는 변경하지 않는다.

> **2026-08-01 contract clarification — full-local Auth flow ledger lookup authority**
>
> signed cookie에는 provider를 저장하지 않고 query provider도 authority로 신뢰하지 않으므로, callback이 exact `(attempt_hash, flow_kind)`로 ledger의 provider·authority·cutover epoch·expiry·terminal 상태를 읽는 server-only read RPC를 허용한다. 이 RPC는 HMAC hash와 flow kind 외 lookup key를 받지 않고 row를 변경하지 않으며, table 접근과 RPC 실행은 계속 browser/`anon`/`authenticated`에 닫힌다. public endpoint/status/error와 OAuth/session 책임은 바뀌지 않는다.
>
> **2026-08-01 contract-evolution — full-local Auth schema·identity·migration authority**
>
> local `auth.users=0`, private remote identity mirror와 two-system barrier를 active invariant로 둔 아래 2026-07-30 hybrid 설계는 역사 기록으로 보존한다. production authority는 같은 local PostgreSQL의 Supabase Auth `auth` schema와 application `public/private/storage` schema로 통합한다. 과거 migration 파일을 수정하지 않고 additive migration으로 전환한다.
>
> 이 계약 승인 시점에는 hybrid Data cutover가 실행되지 않았으므로 원격 Supabase Auth·`public/private/storage`와 application Storage가 유일한 migration source다. 기존 local rehearsal DB/object는 버리고 final maintenance fence에서 만든 같은 시점의 remote snapshot으로 fresh restore한다. 아래 hybrid 설계의 local authority 표현은 대체된 목표 상태이며 현재 배포 사실이 아니다.

## 0-FULL-LOCAL. DB와 Storage 불변식

- stable identity restore 대상은 remote `auth.users.id`, `auth.identities.user_id/provider/provider_id/identity_data` 중 self-hosted target version에서 지원·분류된 필드다. `auth.users.id`와 `public.users.id` 및 모든 owner FK의 UUID mismatch는 0이어야 한다.
- `auth.sessions`, `auth.refresh_tokens`, `auth.flow_state`와 version-gap relation/column은 restore manifest에서 `include|exclude|transform`으로 전수 분류한다. 기존 remote session/refresh/flow row는 local production promote 대상에서 제외하고 count 0을 확인한다.
- 기존 `private.remote_auth_identity_epochs`, remote JWKS/two-system barrier field와 RPC는 cutover 14일 동안 dormant recovery artifact로 둔다. dependency/use count 0과 별도 cleanup PR 전에는 삭제하지 않는다.
- `user_session_generation_bindings`는 local issuer와 verified `session_id` HMAC authority로 재정의한다. raw JWT/session ID를 저장하지 않고 callback/refresh 멱등 bind, logout/delete/quarantine/identity replacement 즉시 revoke, pre-request active check를 유지한다.
- 신규 `private.auth_flow_attempts`는 primary key `(attempt_hash, flow_kind)`, `provider`, `authority(remote|local)`, `cutover_epoch`, `issued_at`, `expires_at`, nullable `terminal_at`, `terminal_reason(success|error|cancelled|expired|cutover_rejected)`만 저장한다. `attempt_hash`는 nonce의 HMAC-SHA256이며 OAuth code/token/email/UUID/raw nonce를 저장하지 않는다.
- `auth_flow_attempts`는 `anon`/`authenticated`/browser의 table 접근을 모두 revoke하고 exact server-only insert/read/terminal/outstanding RPC만 허용한다. read는 signed cookie nonce의 HMAC인 exact `attempt_hash`와 cookie의 `flow_kind`로 한 행만 조회하고 provider·authority·cutover epoch·expiry·terminal 상태만 반환하며 raw nonce, query provider 또는 사용자 입력 UUID로 조회하지 않는다. TTL 900초, terminal transition 멱등, outstanding predicate는 `authority='remote' AND issued_at <= cutover_started_at AND terminal_at IS NULL AND expires_at > now()`다.
- local Auth JWT의 `sub`는 기존 RLS `auth.uid()` authority다. account-generation, owner lock, read-only, deletion/recreate와 cross-owner 금지 규칙은 완화하지 않는다.
...
>
> 이 버전은 새 public table을 추가하지 않고 `pantry_items`, `shopping_list_items`, 기존 product/version FK와 internal reader를 additive하게 확장한다. 기존 generic row는 그대로 유효하며 migration은 product명, brand, synonym, `display_text`로 identity를 추측하거나 backfill하지 않는다.

## 0-PIL-A. `pantry_items` tagged identity

추가/변경 컬럼:

| 컬럼 | 타입 | nullability / FK | 의미 |
| --- | --- | --- | --- |
| `ingredient_id` | uuid | nullable, FK → `ingredients(id)` `ON DELETE RESTRICT` | generic row의 canonical identity |
| `food_product_id` | uuid | nullable, composite FK의 첫 컬럼 | product row의 exact product |
| `food_product_nutrition_version_id` | uuid | nullable, composite FK의 둘째 컬럼 | pantry 선택 당시 immutable version |
...
- `food_product_nutrition_versions`의 existing `UNIQUE (product_id, id)`가 composite reference authority다. 전달된 version이 다른 product 소속이면 FK/route guard가 fail closed하며 current version으로 바꾸지 않는다.
- 조회 인덱스는 `(user_id, created_at DESC, id)`, partial `(user_id, ingredient_id) WHERE ingredient_id IS NOT NULL`, partial `(user_id, food_product_id, food_product_nutrition_version_id) WHERE food_product_id IS NOT NULL`을 둔다.
- existing owner RLS(`auth.uid() = user_id`)를 SELECT/INSERT/DELETE에 유지한다. product insert/read guard는 `public + visible + not deleted` 또는 exact owner private product만 허용하고 다른 owner private product는 row/count/error에서 non-inferable하게 처리한다.
- generic/product identity는 서로 overwrite하지 않는다. effective reader가 같은 ingredient로 dedupe해도 exact pantry row ID와 product/version은 유지한다.
...
- `(shopping_list_id, sort_order, id)`와 `(shopping_list_id, is_pantry_excluded)` index, `added_to_pantry`, exclude→uncheck, completed read-only 규칙은 그대로다.
- product source는 item 생성/reconcile 시 exact version을 pin한다. completion은 item pair를 사용하며 client-supplied product/version을 받지 않는다.
- 기존 non-null `ingredient_id` row는 generic legacy provenance다. all-null identity row는 migration preflight blocker다. compatibility reader에서 발견된 all-null row는 표시 기록을 보존하되 effective matching, pantry reflection, `added_to_pantry=true` 전환에서 제외한다.

## 0-PIL-C. shared effective-ingredient authority
...
- `pantry_items`와 `shopping_list_items`의 composite product/version FK는 `ON DELETE RESTRICT`다. current version 변경은 existing pair를 UPDATE하지 않는다.
- `food_product_nutrition_versions.product_id`의 aggregate-child FK는 account cleanup의 exact private aggregate 삭제를 위해 `ON DELETE CASCADE`로 전환한다. 일반 product delete는 계속 soft delete이며 ordinary authenticated role에는 product/version hard DELETE나 cleanup function execute grant가 없다.
- `food_product_ingredient_links.product_id`는 existing `ON DELETE CASCADE`, `ingredient_id`는 existing `ON DELETE RESTRICT`를 유지한다.
- immutable-version DELETE trigger는 UPDATE와 ordinary DELETE를 계속 거부한다. account cleanup SECURITY DEFINER가 owner lifecycle fence, exact session generation, `visibility='private'`, `source_type='manual'`, `owner_user_id=p_user_id`를 검증하고 transaction-local exact product token을 설정한 경우에만 cascade DELETE를 통과시킨다.
- account cleanup order:
  1. owner lifecycle/generation fence와 exact private product set을 lock한다.
  2. owner-private pantry references와 owner shopping lists/items를 먼저 제거한다.
  3. `product_planner_entries`, meal-log, private recipe/content/snapshot 등 같은 owner의 product/version reference를 owning successor 규칙대로 제거한다.
  4. reference count가 0인지 다시 확인한다. 남아 있으면 전체 transaction을 fail closed한다.
  5. private product를 hard delete한다. exact token 아래에서 private nutrition versions와 link가 aggregate cascade로 정리된다.
  6. owner user row를 정리하고 terminal receipt를 남긴다.
- owner-null public/shared product, immutable version, link/provenance는 위 private product set에 들어가지 않으며 보존한다. 일반 사용자가 FK cascade를 이용해 다른 owner나 shared 기록을 지울 수 없다.
- account-generation activation은 F0 + #3 joint gate 뒤 별도 production gate에서만 가능하다.

> **2026-07-30 contract-evolution — remote identity mirror와 local Data authority**
>
> 이 버전은 private mirror table 1개를 추가해 target table 총계를 **71개**로 확장한다. 2026-07-29 local-first 계약의 local `auth.users` authority와 원격 provider N/A 전제를 대체하며 public API shape는 변경하지 않는다.

## 0-HYBRID. `private.remote_auth_identity_epochs`

- PK는 `(issuer, owner_uuid, identity_created_at)`이다.
- 필수 컬럼은 `issuer`, `owner_uuid`, `identity_created_at`, `active_epoch boolean`, `remote_revision`, `remote_identity_digest`, `verified_at`, `deleted_terminal_at`, `deleted_terminal_reason`, `evidence_revision`, `created_at`, `updated_at`이다.
- `(issuer, owner_uuid) WHERE active_epoch` partial unique로 current epoch를 1개로 제한한다. deleted/identity-replaced terminal epoch는 active가 될 수 없다.
- access/refresh token, OAuth code/secret, raw provider payload, raw/hash email, raw/hash provider subject, nickname, profile image, cookie, JWT 원문, unbounded provider JSON은 저장하지 않는다.
- anon/authenticated direct SELECT/DML은 revoke한다. exact internal RPC만 remote verified session/Auth Admin evidence를 멱등 반영한다.
- profile bootstrap은 mirror와 분리한다. verified remote result의 exact allowlist를 일회성 internal RPC로 전달하고 `public.users`가 profile authority를 유지한다.

### `auth.users` replacement matrix

| 기존 dependency | hybrid replacement |
| --- | --- |
| runtime bootstrap의 `auth.users(id, created_at)` | active mirror epoch + verified `session_id` generation binding |
| cutover count/digest/lock | remote control-plane Auth digest ack + local mirror digest CAS |
| quarantine recovery raw metadata | remote verified allowlist를 일회성 bootstrap RPC로 전달 |
| `admin_members.user_id` | active mirror anti-join invariant와 admin activation check |
| `admin_members.granted_by` | nullable set-null 또는 비식별 grant snapshot |
| `admin_audit_logs.actor_admin_user_id` | nullable set-null + immutable historical audit snapshot |
| application function/policy/FK의 `auth.users` 참조 | compatibility migration 뒤 static inventory expected 0 |

- 기존 owner UUID와 `auth.uid()` RLS 의미는 유지한다. mirror는 epoch/lifecycle authority이며 local Auth user row로 사용하지 않는다.
- restore 후 local `auth.users=0`은 의도된 invariant다. stale/duplicate/deleted epoch, issuer mismatch, remote digest conflict는 personal write와 promote를 fail closed한다.
...
- deletion state machine은 `local_owner_fenced → local_cleanup_complete → remote_exact_epoch_verified → remote_delete_requested → terminal_deleted|terminal_already_absent|terminal_identity_replaced → mirror_terminal → lifecycle_complete`다. attempt/epoch/idempotency key가 같은 retry와 duplicate는 같은 terminal result를 반환하며 identity-replaced는 새 epoch를 삭제하지 않고 fail closed한다.
- 위 barrier 없이 `generation_active`, lifecycle/watermark backfill, local write cutover를 활성화하지 않는다. account-generation capability는 `legacy`를 유지한다.

### restore semantic gate

- 순서는 pre-data schema → hybrid compatibility/FK replacement → application data → post-data constraints/validation이다. 원격 `auth.*` data는 restore하지 않는다.
- post-data validation은 `pg_constraint`, `pg_depend`, `pg_proc`, RLS policy definition의 application-owned `auth.users` dependency 0을 요구한다.
- `public.users/admin_members/personal owner universe ↔ active mirror epoch` anti-join 0, duplicate active epoch 0, stale/deleted active epoch 0, historical audit nullable/snapshot 규칙을 exact SQL로 검증한다.
- isolated rehearsal에서 재현된 `public.users=5`, `admin_members` missing auth FK 1, `admin_audit_logs` missing actor 99는 compatibility 전 fail하고 교정 뒤 expected semantic result를 통과해야 한다.
- 첫 local write 뒤 env-only rollback은 금지한다. write freeze와 local delta export/restore 또는 forward-fix를 요구한다.

> **2026-07-29 contract-evolution — local-first MacBook DB authority**
>
> 새 table, column, index, RLS를 추가하지 않는다. 초기 production DB authority는 서버 MacBook의 local Supabase 하나뿐이며, 실제 사용자 row 대신 테스트 데이터만 존재하는 것으로 간주한다. 원격 Supabase migration, remote verifier, provider-managed maintenance barrier는 이번 초기 배포 계약에서 **N/A**다.

## 0-LOCAL-FIRST. 운영/복구 경계

- staging은 원격 clone이 아니라 **격리 local rehearsal DB**다. 재현 방식은 `clean migration replay + seed` 또는 `verified local backup restore` 둘 중 하나여야 하며, 두 경로를 섞어 authoritative state를 만들지 않는다.
- production cutover의 마지막 DB barrier는 local PostgreSQL에서 `LOCK TABLE auth.users IN SHARE ROW EXCLUSIVE MODE`를 잡는 절차다. 이 lock 아래에서 최종 확인, promote, rollback 여부를 끝내야 한다.
- old-path data나 legacy Storage reference의 irreversible delete는 이번 계약 범위가 아니다. 새 경로 안정화 후에도 영구 삭제는 별도 gate에서 다룬다.
- 향후 hosted Supabase, managed provider barrier, remote auth/storage 복귀가 필요하면 그 시점의 schema/운영 계약을 새 contract-evolution으로 다시 잠근다.

> **2026-07-26 contract-evolution — 완제품 통합 검색 정렬 authority 일치**
>
> 새 table/column/index/RLS를 추가하지 않는다. 통합 검색 DB 정렬 설명을 API v1.2.29의 exact integer/fixed cursor tuple과 일치시키고 `popularity`를 cursor authority에서 제외한다. query가 없으면 기존 stable browse 의미를 유지한다.

> **2026-07-26 contract-evolution — i031 추출 provenance 재사용**
>
> 새 table/column/index/RLS를 추가하지 않는다. i031 직접 추출은 기존 `youtube_extraction_sessions`와 `recipe_sources`의 JSON provenance 공간만 사용한다.

## 0-YT-I031. 저장 경계

- `youtube_extraction_sessions.extraction_meta_json.i031_extractor`에는 `status`, 모델/프롬프트/클라이언트 버전, execution signature, 단계별 소요 시간, model call 수, 선택 frame 수, frame manifest의 비가역 hash를 저장할 수 있다.
- `source_providers`에는 실제 사용한 text/caption provider와 `codex_vision_i031`을 기록한다. `extraction_methods`의 기존 `description | comment | caption` source 의미는 바꾸지 않고 `ocr` 또는 모델명을 추가하지 않는다.
- `raw_source_text`에는 기존 허용 공개 텍스트만 저장한다. raw video/frame, frame OCR 전체, Codex input/output raw payload, provider raw response, 쿠키, token, API key, Codex credential path를 DB에 저장하지 않는다.
- `recipe_sources.extraction_meta_json`으로 register 시 복제되는 i031 메타도 같은 allowlist를 적용한다.
- 중간 산출물은 DB/Storage에 저장하지 않고 요청별 OS 임시 디렉터리에서 성공·실패 후 정리한다.
- 기존 session 소유권, 24시간 TTL, draft/consumed 전이와 RLS를 그대로 유지한다.

> **2026-07-23 contract-evolution — account generation·recipe content·cooked batch·meal log·managed image schema**
>
> 사용자는 2026-07-23에 이 schema contract를 승인했다. 이 addendum은 아래 기존 문서의 target table 50개를 **70개**로 확장하고, 기존 public `recipe-images` direct mutation, Meal direct nutrition 단독 authority, UUID-only account bootstrap, `leftover_dishes.status` 단독 상태 해석과 충돌하는 부분을 대체한다. 기존 50개 table과 history/legacy row는 제거하지 않으며, 명시한 expand/compatibility/contract·joint cutover·tombstone gate를 통과하기 전 column/FK/route를 축소하지 않는다.

## A. 신규 table 20개와 target 총계

| # | 신규 table | 핵심 authority |
| --- | --- | --- |
| 51 | `recipe_content_snapshots` | immutable recipe content + exact nutrition snapshot pin |
| 52 | `recipe_change_previews` | owner/session-bound future-impact token state |
| 53 | `recipe_image_objects` | managed image registry와 permanent compact tombstone |
| 54 | `recipe_image_object_references` | object↔recipe thumbnail/recipe-book cover reference |
| 55 | `user_account_generation_watermarks` | owner별 append-only monotonic generation |
| 56 | `user_account_lifecycles` | generation state, deletion/quarantine durable result |
| 57 | `user_session_generation_bindings` | JWT session HMAC↔account generation binding |
| 58 | `account_generation_capability_state` | singleton `legacy|cutover_maintenance|generation_active` authority |
| 59 | `account_generation_cutover_attempts` | attempt state, authoritative population digest, promote result |
| 60 | `account_generation_cutover_staging` | owner별 분류/evidence/proposed action; canonical과 분리 |
| 61 | `legacy_account_delete_receipts` | legacy delete exact identity-epoch receipt |
| 62 | `legacy_external_write_attempts` | pre-cutover service external write lease/drain |
| 63 | `auth_identity_deletion_outbox` | exact identity epoch의 Admin deletion |
| 64 | `storage_object_deletion_outbox` | generation/cleanup별 Storage terminal processing |
| 65 | `mutation_idempotency_keys` | generation-scoped canonical key/payload/result |
| 66 | `image_upload_quota_counters` | generation별 rolling bytes/requests/active reservations |
| 67 | `cooking_session_meal_claims` | Meal당 active snapshot-v2 session 1개 |
| 68 | `food_product_ingredient_links` | approved product→representative ingredient relation |
| 69 | `cooked_batch_quantity_events` | append-only batch quantity/lifecycle ledger |
| 70 | `meal_log_entries` | 실제 섭취 source/quantity/timezone/nutrition evidence |

`account_generation_cutover_attempts`와 `account_generation_cutover_staging`은 서로 다른 table이다. 실패한 attempt의 proposed generation과 분류가 canonical watermark/lifecycle을 오염시키지 않도록 합치지 않는다.

## B. 기존 table 확장

| table | additive 변경 |
| --- | --- |
| `recipes` | `visibility`, nullable `origin_recipe_id`, nullable `deleted_at`, monotonic revision/`updated_at`; public/manual legacy row를 자동 private로 바꾸지 않음 |
| `recipe_ingredients` | nullable `food_product_id`, nullable `food_product_nutrition_version_id`; canonical `ingredient_id` 유지, approved primary represents link 일치 check |
| `recipe_nutrition_snapshots` | nullable `owner_user_id`; 기존 `recipe_id NOT NULL`, `ON DELETE RESTRICT`, ordinary unique `(recipe_id,input_hash,calculation_version)`와 current partial unique 유지 |
| `recipe_tags` | parent recipe visibility/deleted 상태에서 파생한 cached upper-bound projection; client visibility authority 금지 |
| `meals` | nullable `recipe_content_snapshot_id`, nullable `recipe_content_snapshot_origin=created|legacy_backfill`, server monotonic `revision`; direct nutrition은 단계별 mirror→null |
| `cooking_sessions` | `contract_version=legacy_v1|snapshot_v2`; v2-only `session_kind`, recipe/content snapshot, cooking servings, standalone expected recipe revision, idempotent completion state |
| `cooking_session_meals` | v2 planner의 `meal_revision_snapshot`; session recipe/content와 일치 |
| `leftover_dishes` | nullable content snapshot/cooking servings/finished·remaining g, `weight_status`, `batch_status`, `depleted_reason`, revision; 기존 row legacy 허용 |
| `pantry_items` | product row의 exact product/version과 approved effective ingredient reader를 지원하되 generic ingredient identity를 덮어쓰지 않음 |

- `POST /recipes`의 personal-derived branch는 위 `recipes` row authority를 재사용한다. `base_recipe_revision`은 request-time compare-only authority이고 stored column이 아니며, public source fork와 owner-private save_as_new는 같은 transaction 안에서 source row revision/visibility/deleted state를 검증한 뒤 derived operation으로만 분기한다. schema expansion이나 신규 DB object는 없다.
| `admin_audit_logs` | `actor_admin_user_id` nullable `ON DELETE SET NULL`; 신규 insert writer는 actor 필수 |
| `operational_events` | account cleanup 시 direct actor/target UUID와 metadata identifier scrub; action/result/time/hash는 정책대로 보존 |

## C. recipe content snapshot / preview / soft delete

### `recipe_content_snapshots`

- PK UUID, nullable `owner_user_id`, `recipe_id NOT NULL REFERENCES recipes(id) ON DELETE RESTRICT`, title, base servings, schema version, canonical ingredients JSON, steps JSON, nullable exact `recipe_nutrition_snapshot_id REFERENCES recipe_nutrition_snapshots(id) ON DELETE RESTRICT`, `content_hash`, created_at.
- ingredients에는 canonical ingredient ID, amount/unit, optional product ID/name/brand와 선택 당시 product nutrition version을 보존한다. 영양 vector/status/source는 저장하지 않는다.
- `UNIQUE NULLS NOT DISTINCT (recipe_id, content_hash, recipe_nutrition_snapshot_id, schema_version)`로 동일 content+nutrition pin을 재사용한다.
- private row는 snapshot owner=recipe owner, nutrition owner=recipe owner, 세 recipe ID가 같아야 한다. public/shared row는 owner null이며 사용자별 복사본을 만들지 않는다.
- direct update/delete는 모두 revoke+trigger 차단이다. 계산 writer current switch와 account cleanup의 transaction-local exact-owner guard만 allowlist 예외다.

### `recipe_change_previews`

- owner/generation, recipe, random token hash, session binding hash reference, base recipe revision, proposed content hash, target-set revision hash, target/claim summary, expires_at, consumed/result marker를 저장한다.
- owner+generation RLS read only; client direct INSERT/UPDATE/DELETE는 금지하고 preview/PATCH RPC만 쓴다. short TTL cleanup과 account cleanup으로 제거한다.
- PATCH는 token row를 lock하고 expiry/owner/session/base/content/target hash를 모두 다시 계산해 하나라도 다르면 mutation 없이 409다.

### recipe soft delete

- owner individual delete는 recipe lock 안에서 `deleted_at`만 멱등 기록한다. snapshot/FK detach와 hard delete는 금지한다.
- soft-deleted recipe는 public/owner selection, new Meal/session/snapshot/tag exposure에서 제외하지만 기존 history reader는 pinned snapshot을 읽는다.
- hard delete는 account cleanup 내부 guard에서 dependent records/private snapshots를 먼저 지운 뒤에만 허용한다.

## D. Meal content authority의 3단계 migration

1. **expand**: content-aware reader와 nullable columns/constraints를 먼저 배포한다. 미래 `registered|shopping_done` Meal의 existing direct nutrition N을 가리키는 content snapshot을 idempotent backfill한다. 세부 원본이 없으면 current projection+`legacy_backfill`을 쓰되 N을 바꾸지 않는다.
2. **compatibility mirror**: content가 있으면 direct N은 null 또는 `content.recipe_nutrition_snapshot_id`와 같아야 한다. 새 writer는 rollback용 direct N/origin을 DB-derived read-only mirror로만 기록하고 client DML을 막는다. direct-only legacy row는 fallback 가능하다.
3. **contract**: current+immediate-previous content-aware, old-shape write 한 호환 release 0, backfill/pair mismatch 0 뒤 controlled migration으로 direct N/origin을 null 처리한다. final check는 `content non-null → direct/origin null`, `content null → content origin null`이다.

contract 뒤 rollback floor는 content-aware release다. direct-only binary 배포와 direct pointer 재생성 migration을 금지한다.

## E. account generation security authority

### `user_account_generation_watermarks`

- `owner_uuid PK`, `last_account_generation > 0`, timestamps만 갖는 append-only 최소 authority다.
- auth/public user FK와 cascade를 두지 않는다. owner lock 아래 `last+1`만 허용하며 decrement/reuse/delete는 금지한다.

### `user_account_lifecycles`

- PK `(owner_uuid, account_generation)`, nullable quarantine-only `auth_identity_created_at_snapshot`, origin enum `runtime|cutover_active|cutover_recovery_approved|cutover_legacy_deleted|cutover_auth_without_profile_quarantined|cutover_public_without_auth_quarantined|cutover_personal_owner_quarantined|cutover_orphan_cleanup`, cutover evidence hash를 둔다.
- status는 `active|quarantined|deleting|cleanup_pending|complete`, activated/resolved/deleted times, revision, required/completed cleanup generation을 둔다.
- active/deleting identity lifecycle은 epoch 필수, identity 없는 orphan quarantine만 nullable다. owner당 active는 partial unique 1개다.
- account-delete initiation과 quarantine resolution의 versioned session HMAC, key version, canonical key/payload hash, durable result를 영구 보존한다. auth/public FK cascade와 general idempotency cleanup에서 제외한다.
...
- `account_generation_capability_state`는 정확히 1행이며 `legacy|cutover_maintenance|generation_active`, monotonic revision, nullable current attempt ID를 저장한다. protected transition RPC만 update한다.
- application mutation guard는 allowlisted `VOLATILE SECURITY DEFINER`, safe `search_path`(`pg_temp` last), transaction-scoped global shared advisory lock→capability row `SELECT ... FOR KEY SHARE` 순서다. `READ COMMITTED` 외 isolation은 거부한다.
- 모든 legacy/new mutation RPC, protected direct DML `BEFORE` trigger, authenticated Storage RLS predicate가 같은 state/fence를 사용한다. state legacy는 legacy path, maintenance는 orchestrator 외 전부 deny, active는 session-generation 검증 완료 internal RPC만 허용한다.
- service-role external write는 Storage RLS가 아니라 `legacy_external_write_attempts` start/finalize/cleanup wrapper와 120초 hard deadline으로 통제한다. maintenance 진입 후 신규 start와 late attach를 거부한다.

### Auth Before User Created Hook

- Hook entry wrapper는 `SECURITY INVOKER`, exposure `auth-hook-internal`, exact principal `supabase_auth_admin`이다.
- wrapper/guard schema USAGE와 wrapper EXECUTE만 `supabase_auth_admin`에 grant한다. PUBLIC/anon/authenticated/service_role은 wrapper와 guard EXECUTE를 모두 revoke한다.
- minimal guard만 dedicated NOLOGIN owner의 hardened SECURITY DEFINER다. shared advisory lock→plain capability SELECT 순서를 쓰며 table SELECT와 lock 최소권한만 가진다. INSERT/UPDATE/DELETE/TRUNCATE/REFERENCES/TRIGGER/column UPDATE와 비관리 membership/SET ROLE은 없다.
- legacy/active는 identity create 허용, maintenance는 fail closed다. pre-started Hook도 같은 advisory key의 exclusive transition과 직렬화한다.

## F. cutover attempts, staging, barrier

### `account_generation_cutover_attempts`

- attempt UUID, state, capability revision, staged/final count+ordered digest(auth/public/personal), auth barrier type/evidence, started/aborted/promoted time, result/error를 저장한다.
- attempt row 자체는 canonical user generation이 아니다. failed/aborted attempt는 감사용으로 보존하거나 policy에 따라 정리할 수 있지만 proposed generation을 watermark로 복사하지 않는다.

### `account_generation_cutover_staging`

- attempt ID+owner UUID key, auth identity epoch, proposed generation/action, evidence type/hash, validation state를 둔다.
- classification은 `active_candidate|legacy_deleted_confirmed|incomplete_bootstrap_recovery_approved|auth_without_profile_quarantined|public_without_auth_quarantined|personal_owner_without_identity_quarantined|approved_orphan_cleanup|classification_unresolved`다.
- 한 owner는 정확히 한 action을 가진다. evidence conflict/duplicate identity/owner ambiguity만 unresolved이며 한 건이라도 있으면 promote를 막는다.
- auth/public 양쪽에 없는 owner는 모든 personal table+registry/outbox universe로 찾는다. registry signal은 registry row의 owner/generation만 사용하고 parent recipe creator를 역추론하지 않는다.

### transition/promote

- `legacy→maintenance`는 global exclusive advisory xact lock으로 existing shared writer 종료를 기다린 뒤 state+attempt를 같은 transaction에 commit한다.
- maintenance에서 Auth Admin/import/dashboard create/delete freeze, Hook health, auth consumer off, Auth quiet window, external attempt 0, 15분 간격 Storage owner-signal inventory 2회와 writer fence를 요구한다.
- final transaction은 exclusive fence 뒤 migration-owner로 local `LOCK TABLE auth.users IN SHARE ROW EXCLUSIVE MODE` write barrier를 timeout 안에 확보한다. 이 lock은 전체 read freeze가 아니므로 Auth Admin/import/dashboard create/delete 동결, external attempt 0, 15분 간격 Storage owner-signal inventory 2회 일치를 별도로 유지한다. ordered `(id,created_at)` auth digest, public digest, personal-owner digest를 staging과 CAS하며 provider maintenance barrier는 향후 별도 contract-evolution 전까지 사용하지 않는다.
- 모든 분류/evidence/unresolved=0/union-zero를 재검증한 같은 transaction에서 watermark/lifecycle/outbox와 capability active를 만들며 그 전 canonical security row는 0이어야 한다.
- promote 전 실패는 app/DB/Storage adapter를 legacy로 복구하고 staging/external attempt를 purge, canonical 0 확인 뒤 state legacy로 되돌린다. promote 뒤 legacy bootstrap/delete/writer rollback은 금지하고 forward-fix한다.

## G. idempotency, session generation, lock order

### `mutation_idempotency_keys`

- owner, required generation, operation scope, canonical client UUID SHA-256 `key_hash`, canonical payload hash, `in_progress|succeeded|failed_retriable|failed_terminal|cancelled`, durable result/error, attempt token/count/lease/timestamps를 둔다.
- unique `(owner, account_generation, scope, key_hash)`. same key+payload는 최초 결과, different payload는 409다.
- active generation의 compact key/payload/terminal result/reference는 무기한 보존한다. 90일 뒤 attempt/lease/verbose error만 compact한다.
- account cleanup은 일반 non-image key를 지울 수 있으나 image compact key는 registry와 영구 보존한다. account-delete key는 lifecycle row가 authority다.

### common lock

- lock order는 항상 `global cutover shared fence → owner lifecycle advisory lock → recipe UUID ascending → Meal UUID ascending → resource row`다.
- all personal RPC는 JWT session binding expected generation=current active generation을 same transaction에서 검증한다. binding missing/revoked/quarantined/deleting/complete/stale는 mutation 0이다.
- service background RPC는 user session을 위조하지 않고 allowlisted actor+expected generation을 요구한다.
- Recipe PATCH/start/Meal/shopping RPC는 lock/check와 write를 한 transaction에 묶고 route pre-read는 UX 힌트일 뿐 authority가 아니다.

## H. cooking session v1/v2와 Meal claim

- existing/orphan row는 `legacy_v1`. 신규 v2 row만 `session_kind=planner|standalone`, recipe/content snapshot/cooking servings를 NOT NULL로 강제한다.
- planner v2는 session-meal 1개 이상, standalone은 0개다. planner session의 모든 Meal은 same recipe/content를 써야 한다.
- `cooking_session_meal_claims.meal_id PK`가 Meal당 active session을 하나로 제한한다. start transaction이 insert, cancel이 delete, complete가 consume하며 session row가 terminal history authority다.
- planner start는 recipe lock→Meal ascending row locks에서 owner/status/revision/no claim/same content를 검증하고 Meal의 content/servings를 session에 복사한다.
- standalone start는 recipe access/deleted/revision을 검증하고 current content/servings를 pin한다.
- v2 start/cancel/complete는 generation-scoped idempotency를 사용한다. complete는 exact owner `consumed_pantry_item_ids`를 recipe product/effective ingredient와 검증하고 selected row만 delete한다.
- v1과 v2 route/RPC는 contract_version predicate를 명시하고 다른 version ID를 404/409로 거부한다. v1 `consumed_ingredient_ids`는 legacy generic semantics로 유지한다.

## I. cooked batch와 append-only quantity ledger

### `leftover_dishes` 확장

- v2 completion은 `recipe_content_snapshot_id NOT NULL`, cooking servings, nullable finished/remaining g, revision을 저장한다. batch에 direct nutrition snapshot FK를 추가하지 않는다.
- `weight_status=known|missing|unrecoverable`, `batch_status=available|depleted`, depleted reason=`consumed|discarded|mixed|consumed_unweighed|discarded_unweighed|mixed_unweighed`다.
- available→reason null, depleted→reason non-null. known이면 `0<=remaining<=finished`; missing/unrecoverable이면 weight columns null이다.
- existing legacy row는 nullable content와 기존 status authority를 허용한다. reader migration 후 신규 status가 authority이고 legacy status는 compatibility projection이다.

### `cooked_batch_quantity_events`

- PK UUID, owner, batch `ON DELETE RESTRICT`, event type `consumed|discarded|adjustment|marked_unrecoverable|closed_unweighed|reversal`, nullable delta_g, reason, nullable meal-log entry `ON DELETE RESTRICT`, nullable `reverses_event_id`, operation ID/ordinal/payload hash, created_at.
- `(owner, operation_id, ordinal)` unique. 한 target event 직접 reversal은 partial unique 1개다.
- batch-source consumed와 그 reversal은 same owner/batch meal entry가 필수다. 다른 event는 entry null이다.
- consumed/discarded delta는 음수, weight reversal은 exact 반대값, adjustment는 signed+reason이며 적용 뒤 `0<remaining<=finished`다.
- `marked_unrecoverable`은 missing+available, weight null, quantity event 0에서 한 번만 가능하고 reversal target에서 DB check/RPC로 제외한다.
- `closed_unweighed`는 missing/unrecoverable+available과 closure reason을 요구하고 delta/entry는 null이다. current active terminal closure만 null-delta reversal 가능하며 후속 event가 있으면 409다.
- event update/delete는 일반·service direct DML 모두 금지한다. account cleanup internal guard만 link 정리 후 hard delete한다.
- `leftover_dishes` protected columns의 authenticated direct UPDATE grant/policy를 회수하고 guard-triggered row-lock RPC만 projection을 바꾼다. 매 mutation은 active-event full replay 결과와 cached projection을 비교한다.

## J. `meal_log_entries`

- owner+generation, nullable `consumed_at timestamptz`, required `consumed_local_date date`, required validated IANA `timezone_name_snapshot`, nullable `meal_plan_column_id ON DELETE SET NULL`, required `slot_name_snapshot`.
- source type `cooked_batch|food_product|ingredient`; source별 exact ID/FK와 pinned content/product nutrition version/ingredient profile evidence를 exact-one check로 강제한다.
- nullable `active_consumption_event_id ON DELETE RESTRICT`, nullable `deleted_at`, actual amount/unit, display name/brand, compact nutrition JSON, created/updated timestamps를 둔다.
- active batch entry는 자기 owner/batch의 non-reversed consumed event 하나를 가리킨다. product/ingredient entry는 event pointer null이다. deferred constraint/commit trigger가 entry↔event 쌍을 검증한다.
- POST는 entry ID 선생성→entry→consumed event→active pointer를 한 RPC로 commit한다. PATCH는 old active event reversal→replacement→pointer swap, DELETE는 reversal→pointer null→soft delete다.
- aggregate는 deleted row를 제외한다. 일반 user hard delete/event link direct edit는 금지하고 account cleanup만 pointer null→event delete→entry delete 순서다.
- product nutrition은 pin된 version의 direct basis relation만, ingredient는 approved profile+exact conversion/piece evidence만 허용한다. exact path 부재는 row를 만들지 않고 422다.
- `consumed_at`이 있으면 stored timezone 변환 date=`consumed_local_date`; unknown past time은 null이다. current timezone으로 backfill/regroup하지 않는다.

## K. 제품↔대표 재료와 검색 index

### `food_product_ingredient_links`

- product FK `ON DELETE CASCADE`, ingredient FK `ON DELETE RESTRICT`, relation, review status, `is_primary`, source/provenance, timestamps를 둔다.
- P0 production matching은 active approved primary `represents`만 사용하고 product당 최대 1개를 partial unique로 강제한다. `contains|substitute`가 존재해도 추천 matching에는 쓰지 않는다.
- private owner product cleanup은 product cascade로 link를 제거한다. owner-null public/shared product link는 개인정보를 복제하지 않고 보존한다.
- effective ingredient reader는 generic pantry ingredient와 approved product link ingredient의 DISTINCT union이다.

### 통합 검색

- food product/ingredient에 NFKC·lowercase·whitespace/punctuation-stripped compact searchable projection과 GIN/trigram/prefix index를 둔다.
- scope별 RLS/moderation predicate를 먼저 적용한 뒤 typed union 전체를 `(algorithm_version, match_bucket, coverage_bucket, quantized_score, source_partition, type_partition, created_at, stable_id)` integer/fixed tuple로 정렬한다. query가 있으면 relevance가 source/recency보다 먼저이며 `popularity`는 cursor authority가 아니다.
- query가 없으면 기존 stable browse 의미를 유지하며 relevance tuple을 새로 해석하지 않는다.
- opaque cursor는 이 정수 tuple과 stable ID를 encode한다. float similarity를 cursor boundary로 쓰거나 client merge하지 않는다.

## L. managed image registry/reference/quota

### `recipe_image_objects`

- nullable owner UUID+generation, bucket/path, raw SHA-256, bytes, actual MIME, server-derived visibility, state, upload attempt token, cleanup generation, upload lease expiry, unlinked cleanup after, first-404/quarantine/next terminal scan timestamps를 둔다.
- state=`pending_upload|uploaded_unlinked|attached_private|attached_public_shared|cleanup_pending|not_found_observed|deleted|verified_not_found`.
- private/pending/cleanup은 owner+generation 필수. owner-neutral public/shared는 둘 다 null인 conditional check다.
- auth/public/recipe cascade FK를 두지 않는다. bucket/path/generation/hash/terminal result와 compact image idempotency를 영구 보존한다.

### `recipe_image_object_references`

- object FK와 `reference_type=recipe_thumbnail|recipe_book_cover`, consumer ID를 unique 연결한다.
- service-owned URL은 object ID와 same-RPC attach/detach가 필수다. arbitrary external URL은 managed delete 대상으로 해석하지 않는다.
- reference가 있으면 stale cleanup으로 전환할 수 없다. private→public은 owner-neutral new object copy+reference atomic swap+old private outbox다.

### upload quota/lease/CAS

- `image_upload_quota_counters`는 `(owner,generation)` rolling request/byte buckets와 active pending/unlinked reservation을 관리한다.
- PUT 전 active session generation, 5MB, magic-byte/actual MIME/raw hash, rolling quota, active 20, global backlog breaker를 한 reservation transaction에서 검증한다.
- upload attempt lease 5분, external PUT timeout 120초, `upsert=false` same-path takeover다. metadata가 없거나 불일치하면 max 5MB service GET raw-byte rehash하며 ETag를 hash로 사용하지 않는다.
- finalize는 registry pending+idempotency in-progress+exact token+generation+cleanup generation CAS다. success는 uploaded-unlinked+24시간 attach grace이며 durable result에 signed URL을 저장하지 않는다.
- attach/cancel/scanner는 token을 조건부 폐기해 승자 하나만 만든다. reservation은 attach 또는 terminal cleanup의 unique release marker로 정확히 한 번 해제하고 rolling 사용량은 되돌리지 않는다.

## M. Storage/Auth outbox와 permanent terminal barrier

### `storage_object_deletion_outbox`

- bucket/path, owner/generation, cleanup generation, reason, state=`pending|processing|awaiting_not_found_recheck|succeeded|failed|dead_letter`, terminal result=`deleted|verified_not_found`, attempt/next time/lease token+expiry/error를 둔다.
- user FK/cascade 없이 snapshot 값을 보존한다. exact owner+generation+cleanup+lease+processing CAS로 complete/fail한다.
- first 404는 awaiting+observed time+15분 quarantine이고 normal claim에서 제외한다. due recheck에서 object 발견은 pending, 두 번째 independent 404만 terminal이다.

### `auth_identity_deletion_outbox`

- owner/generation, auth identity created-at snapshot, state=`pending|processing|succeeded|failed|dead_letter`, terminal result=`deleted|already_absent|identity_replaced`, lease/attempt/next/error를 둔다.
- delete 직전 Admin lookup의 created-at exact match만 삭제한다. absent는 success, newer identity는 삭제하지 않고 identity-replaced terminal이다.

### completion invariant / scanner order

- lifecycle complete는 auth outbox succeeded, cleanup generation 1..required의 연속 terminal, 각 object deleted/verified-not-found, Storage pending/processing/awaiting/failed/dead-letter 0, registry nonterminal 0을 모두 요구한다.
- `next_terminal_scan_at,id` SKIP LOCKED cursor로 permanent terminal tombstone을 공정하게 검사한다. 50건 limit에서도 151건 fixture가 starvation 없이 처리되어야 한다.
- terminal 뒤 늦은 object는 old generation registry tombstone이 새 outbox+required generation으로 lifecycle을 재개방한다. G2 quota/ownership에 붙이지 않는다.
- ordered tick은 `stale scanner → terminal tombstone scan → due 404 recheck → normal Storage drain → expected-owner signal union-zero → Auth deletion drain → lifecycle complete`다.

## N. Storage buckets / RLS / grants

- 신규 personal image는 private `recipe-images-private/{owner_uuid}/{generation}/{object_uuid}`에 저장하고 short signed read만 허용한다.
- public/shared image는 owner-neutral public bucket `shared/{object_uuid}.{ext}` path만 허용한다. 일반 authenticated upload에 public intent column/input을 주지 않는다.
- 기존 `recipe-images`와 신규 bucket의 anon/authenticated INSERT/UPDATE/DELETE policy/grant, browser direct remove를 회수한다. generation-aware server route+revoke된 internal RPC+service Storage client만 mutation한다.
- registry/reference/lifecycle/outbox/idempotency/quota/cutover table은 RLS enabled, anon/authenticated direct DML revoked다. 필요한 owner read projection과 exact internal RPC만 grant한다.
- application-owned SECURITY DEFINER는 exact signature allowlist, hardened search_path, minimal principal set을 가진다. provider/extension-managed function은 project migration으로 owner/ACL/search_path를 수정하지 않고 immutable baseline+Data API non-exposure로 검증한다.
- legacy service owner-null/UUID-path/reference signal union이 0이 아니거나 `owned_unverified|owner_path_unverified`가 있으면 generation/image joint activation을 막는다.

## O. account cleanup FK 순서와 보존

- account delete owner-lock transaction은 session binding revoke→lifecycle deleting→private image registry snapshot/outbox enqueue와 함께 다음 상대 순서를 지킨다.

```text
recipe_change_previews
→ meal_log_entries.active_consumption_event_id NULL
→ cooked_batch_quantity_events
→ meal_log_entries + non-image mutation_idempotency_keys
→ cooking_session_meal_claims
→ cooking_session_meals
→ cooking_sessions
→ meals
→ leftover_dishes(cooked batch)
→ private recipe_content_snapshots
→ private recipe_nutrition_snapshots
→ private personal recipes/ingredients/tags
→ pantry/product-planner 등 owner-private product references
→ food_product_ingredient_links / nutrition versions / reports-profiles / private products
→ profile/user row
```

- `meals.leftover_dish_id`의 existing NO ACTION FK를 SET NULL로 완화하지 않고 Meal을 batch보다 먼저 삭제한다. recipe content가 private product를 참조할 수 있으므로 private recipe/snapshot보다 product를 먼저 삭제하지 않는다.
- admin membership 제거, `granted_by`/audit actor SET NULL, operational user identifier scrub 뒤 Auth delete를 허용한다. isolated local rehearsal과 server-Mac local production의 `auth.users` inbound FK inventory에서 미분류 RESTRICT가 있으면 blocker다.
- public/shared recipe, owner-null snapshot, anonymized shared manual product/link/version은 기존 정책대로 보존한다.
- lifecycle/watermark/managed registry/image idempotency compact tombstone은 영구 보존한다. succeeded verbose outbox/quota/attempt detail만 Storage terminal+90일+terminal recheck 뒤 compact할 수 있다.

## P. migration/rollout/rollback gates

- F0는 additive schema/guard/inventory/dual-dispatch만 deploy하며 canonical backfill과 generation activation을 하지 않는다.
- account generation과 image direct-policy revoke는 #3 준비 뒤 joint maintenance/promote에서 같은 capability revision으로 전환한다. owner signal 0과 `owned_unverified=0` 전에는 activation하지 않는다.
- Meal은 expand→compatibility mirror→contract, images는 registry-aware reader→copy/swap→한 호환 release old path→irreversible delete, session은 v1/v2 dormant R→flag-off R+1→R+2 activation 순서를 지킨다.
- legacy orphan inventory는 known positive reference backfill+report-only이며 `storage_object_deletion_outbox` enqueue/delete가 0건이어야 한다. 실제 GC는 별도 contract+Manual Only manifest다.
- 모든 migration은 fresh isolated-local replay, existing-data upgrade, server-Mac local production read-only inventory/preflight, RLS/grant/role/ACL matrix, lock race, idempotency, FK cleanup fixture를 통과한다. final contract/destructive transition은 current+immediate-previous reader, old-shape 0, rollback floor와 recovery runbook evidence가 있어야 한다.
...
| 51 | recipe_content_snapshots | recipe content + exact nutrition pin `v1.3.23` |
| 52 | recipe_change_previews | 미래 영향 preview token `v1.3.23` |
| 53 | recipe_image_objects | managed image registry/tombstone `v1.3.23` |
| 54 | recipe_image_object_references | image consumer reference `v1.3.23` |
| 55 | user_account_generation_watermarks | append-only account generation `v1.3.23` |
| 56 | user_account_lifecycles | generation lifecycle/quarantine/delete `v1.3.23` |
| 57 | user_session_generation_bindings | session HMAC generation binding `v1.3.23` |
| 58 | account_generation_capability_state | cutover capability singleton `v1.3.23` |
| 59 | account_generation_cutover_attempts | cutover attempt/digest `v1.3.23` |
| 60 | account_generation_cutover_staging | attempt-scoped owner classification `v1.3.23` |
| 61 | legacy_account_delete_receipts | legacy exact-epoch delete receipt `v1.3.23` |
| 62 | legacy_external_write_attempts | legacy external write drain `v1.3.23` |
| 63 | auth_identity_deletion_outbox | Auth identity cleanup `v1.3.23` |
| 64 | storage_object_deletion_outbox | Storage cleanup/recheck `v1.3.23` |
| 65 | mutation_idempotency_keys | generation-scoped mutation idempotency `v1.3.23` |
| 66 | image_upload_quota_counters | upload quota/reservation `v1.3.23` |
| 67 | cooking_session_meal_claims | Meal active session claim `v1.3.23` |
| 68 | food_product_ingredient_links | approved product→ingredient relation `v1.3.23` |
| 69 | cooked_batch_quantity_events | append-only quantity/lifecycle ledger `v1.3.23` |
| 70 | meal_log_entries | 실제 식사 기록/evidence `v1.3.23` |
| 71 | youtube_extraction_jobs | YouTube durable queue/앱 내 알림 `v1.3.33` |
| 72 | youtube_extractor_permits | YouTube provider global permit `v1.3.33` |
| 73 | private.youtube_extraction_worker_credentials | 제한 worker credential generation authority `v1.3.33` |
