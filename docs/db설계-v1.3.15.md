...
> 기준 문서: 요구사항 기준선 v1.7.10 / 화면정의서 v1.5.17 / API 설계 v1.2.19 / 유저 Flow맵 v1.3.17
>
>
> 작성: 킴실장
>
...
| notification_type | text | NOT NULL | `level_up` / `xp_awarded` / `achievement_unlocked` / `badge_unlocked` / `quest_completed` |
...
| priority | integer | NOT NULL, DEFAULT 4 | 표시 우선순위. `level_up=1`, `achievement_unlocked/badge_unlocked=2`, `quest_completed=3`, `xp_awarded=4` |
...
- seen 처리는 멱등해야 한다.
- toast stack 우선순위는 서버가 정렬한다. 클라이언트는 `level_up > achievement_unlocked/badge_unlocked > quest_completed > xp_awarded` 순서를 바꾸지 않는다.
...
| 27 | user_achievement_awards | 사용자 성장/업적 `growth-achievement-album` |
| 28 | user_quest_progress | 사용자 성장/튜토리얼 퀘스트 `33c-gamification` |
| 29 | user_progress_notifications | 사용자 성장/notification `33c-gamification` |
| 30 | admin_members | Admin Foundation `v1.3.8` |
| 31 | operational_events | Admin Foundation `v1.3.8` |
| 32 | admin_audit_logs | Admin Foundation `v1.3.8` |
| 33 | youtube_extraction_candidates | YouTube 다중 후보 `2026-05-30` |
