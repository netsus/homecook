...
# 무엇을 먹든 서비스 — API 설계 v1.2.22

> 기준 문서: 요구사항 기준선 v1.7.14 / 화면정의서 v1.5.21 / DB 설계 v1.3.17 / 유저 Flow맵 v1.3.20
...
### Nutrition 공통 payload `v1.2.22`

레시피, 완제품, planner aggregate는 아래 shape를 재사용한다.
...
1. g/kg은 질량으로 직접 계산한다.
2. profile 기준이 `100mL`이고 입력이 mL/L/tbsp/tsp/cup이면 부피로 직접 계산한다.
3. 사용자가 입력한 정확한 g과 제품 표시 중량을 대표 등급보다 우선한다.
4. 그 외 부피는 `tbsp=15mL`, `tsp=5mL`, `cup=200mL`로 정규화한 뒤 승인 대표 등급으로 계산한다.
5. `개`는 재료·크기·손질 상태가 일치하는 승인 piece weight가 있을 때만 계산한다.
6. 변환 경로가 없으면 추측하지 않고 partial/unavailable을 반환한다.
...
- `nutrition`을 보낼 때는 5-6과 같은 전체 basis/value 규칙을 적용하며 새 version을 생성한다. 기존 version/value row를 UPDATE하지 않는다.
...
- unit은 current product version의 basis와 같은 물리 차원(`g↔g`, `ml↔ml`)이거나 같은 이산 단위(`serving`, `package`)여야 한다. serving/package와 g/ml 사이의 교차 환산은 MVP 저장 모델에 없으므로 허용하지 않으며, 임의 dimension conversion은 422 `NUTRITION_BASIS_MISMATCH`다.
...
| 422 | NUTRITION_BASIS_MISMATCH | 수량 단위와 label basis 변환 불가 |
