...
- 기존 manual create surface를 유지한다. `origin_recipe_id`가 public accessible recipe면 공개 원본을 수정하지 않고 owner-private fork를 만든다.
- personal image는 URL이 아니라 `image_object_id`를 받는다. recipe content write와 registry reference attach를 같은 RPC transaction에서 수행한다.
- response는 새 private recipe ID와 detail destination을 반환한다. client가 owner/visibility/public path를 선택하지 않는다.
