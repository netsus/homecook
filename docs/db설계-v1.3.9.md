...
> 기준 문서: 요구사항 기준선 v1.7.3 / 화면정의서 v1.5.10 / API 설계 v1.2.13 / 유저 Flow맵 v1.3.10
>
>
> 작성: 킴실장
>
...
> 신규 ingredient taxonomy는 이 컬럼을 대체하지 않는다. 코드/seed 기반 shared mapping source 또는 additive shadow metadata로 legacy label을 해석하며, production FK cutover는 이번 계약 범위 밖이다.
...
> - `label`은 짧은 표시 라벨 용도다. category taxonomy code나 분류 의미를 `label`에 과적재하지 않는다.
> - 조리방법 category가 필요하면 `GET /cooking-methods` v1 shape를 깨지 않는 optional additive metadata로만 검토한다.
...
1. `ingredients(standard_name, category, default_unit)` INSERT. `standard_name` conflict 시 기존 row 재사용
2. 생성/재사용된 ingredient row 조회
3. synonym이 null/빈값이면 `not_requested`
4. `lower(trim(p_synonym)) === lower(trim(p_standard_name))`이면 `skipped_same_as_standard`
5. 같은 normalized synonym이 다른 ingredient에 이미 있으면 best-effort advisory query로 `skipped_ambiguous`
6. 안전하면 `ingredient_synonyms(ingredient_id, synonym)` INSERT, `(ingredient_id, synonym)` conflict는 `already_attached`
