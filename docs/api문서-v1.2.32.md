...
- internal `AuthFlowLedgerStore`만 server credential로 ledger RPC를 호출할 수 있고 해당 credential을 사용자 데이터 query fallback에 사용하지 않는다.
...
> **v1.2.32 총계**: 102개 (full-local web-auth orchestration은 `/api/v1` 제품 API count 밖이며, pantry/shopping additive field와 기존 public endpoint count는 유지한다. active 101개 + 삭제된 `2-4` tombstone 1개. internal maintenance endpoint와 DB internal RPC는 제외)
>
