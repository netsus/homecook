...
- `auth_flow_attempts`는 `anon`/`authenticated`/browser의 table 접근을 모두 revoke하고 exact server-only insert/terminal/outstanding RPC만 허용한다. TTL 900초, terminal transition 멱등, outstanding predicate는 `authority='remote' AND issued_at <= cutover_started_at AND terminal_at IS NULL AND expires_at > now()`다.
