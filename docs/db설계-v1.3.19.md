...
> | 4 | piece weight는 evidence와 재료·크기·손질 상태가 모두 일치하는 active approved row만 소비하도록 고정한다 | `개→g` fail-closed |
...
> 기준 문서: 요구사항 기준선 v1.7.18 / 화면정의서 v1.5.24 / API 설계 v1.2.23 / 유저 Flow맵 v1.3.21
>
>
> 작성: 킴실장
>
...
- `개→g`는 ingredient, size code, preparation/edible state가 모두 일치하는 active approved row만 사용한다. 불일치·미승인·철회 시 `PIECE_WEIGHT_REQUIRED`로 fail-closed한다.
...
- snapshot 계산 payload는 `warnings_json`을 포함해 UPDATE/DELETE하지 않는다. 새 input 또는 calculation version은 새 row를 만들고 기존 row에서는 `is_current`만 false로 전환하는 원자 transaction을 사용한다.
...
- public dataset version은 운영자가 source/label을 검수한 직접 관계만 저장한다. private manual 제품 생성/수정 API는 MVP에서 relation 입력을 받지 않으며 해당 version의 `basis_relations_json`은 빈 배열이다. 승인 상태는 해당 immutable version의 profile/source 검수 경계를 따르며 client가 관계를 임의 주입할 수 없다.
...
| 49 | product_planner_entries | 완제품 플래너 entry `v1.3.17` |
