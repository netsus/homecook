...
- final transaction은 exclusive fence 뒤 migration-owner로 `LOCK TABLE auth.users IN SHARE ROW EXCLUSIVE MODE`를 timeout 안에 확보하거나 검증된 provider maintenance barrier를 사용한다. ordered `(id,created_at)` auth digest, public digest, personal-owner digest를 staging과 CAS한다.
...
- admin membership 제거, `granted_by`/audit actor SET NULL, operational user identifier scrub 뒤 Auth delete를 허용한다. local/remote `auth.users` inbound FK inventory의 미분류 RESTRICT는 blocker다.
...
- 모든 migration은 fresh local replay, existing-data upgrade, remote inventory/preflight, RLS/grant/role matrix, lock race, idempotency, FK cleanup fixture를 통과한다. final contract/destructive transition은 current+immediate-previous reader, old-shape 0, rollback floor와 recovery runbook evidence가 있어야 한다.
