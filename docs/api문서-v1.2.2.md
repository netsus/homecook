...
# 집밥 서비스 — API 설계 v1.2.2

> 기준 문서: 요구사항 기준선 v1.6.2 / 화면정의서 v1.2.2 / DB 설계 v1.3.1 / 유저 Flow맵 v1.2.2
> 작성: 킴실장
...
> 회원가입 완료 시 서버 자동 생성: meal_plan_columns ×4, recipe_books ×3
...
> 끼니 슬롯은 `아침 / 점심 / 간식 / 저녁` 4개 고정이다.  
> `POST /planner/columns`, `PATCH /planner/columns/{id}`, `DELETE /planner/columns/{id}`는 더 이상 제공하지 않는다.
...
> **v1.2.2 총계**: 47개 (`/planner/columns` CRUD 3개 제거)
> **v1.2.2 변경**: `PLANNER_WEEK`를 4끼 고정 슬롯으로 정리하고 planner column CRUD를 제거
