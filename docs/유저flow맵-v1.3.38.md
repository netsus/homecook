...
## 2026-09-22 후속 요청 — 재료 검색·교체·저장

한글 입력 → 250ms 입력 휴지 → 현재 문자열 자동 검색 → 재료 선택/교체 → 기존 양·단위 확인 → 개인 레시피 저장 → 저장된 상세 재조회로 이어진다. 조합 종료나 Enter는 검색 조건이 아니다. 검색 실패 시 입력·기존 선택을 유지해 재시도하며, 저장 실패 시 초안과 같은 요청의 중복 방지 키를 보존한다. 공개 원본은 수정하지 않고 개인 복제로 저장한다. 식사기록의 제품·재료 검색도 같은 입력 휴지 규칙을 사용한다.

## 2026-09-22 사용자 요청 — 베타 핵심 흐름 보완

레시피의 잘못된 재료 선택 → 이름을 확인하며 교체 → 기존 양 유지(제품 단위가 다르면 양 재입력) → 저장의 교정 흐름을 연결한다. 식사기록 → 해당 출처의 최근 음식 → 정확 ID로 현재 단위 확인 → 실제 양 확인 → 저장을 제공하며 재로그인 뒤도 동일하게 현재 정보를 확인한다. 삭제된 끼니 기록 수정은 현재 끼니 선택을 거친다. 요리 완료 → 식사기록 또는 남은요리 이동을 제공하고 요리 완료만으로 먹은 기록을 만들지 않는다. 모든 진입을 계획·장보기 경유로 통일하지 않는다.

레시피의 잘못된 재료 확인 → 편집에서 교정 → 기존 권한·revision 검증 후 저장, 최근 음식 선택 → 승인된 단위로 먹은 양 입력 → 실제 기록 저장, 요리 완료 → 식사기록 진입을 기존 흐름 안에서 연결한다. 단위/영양 근거가 없으면 임의 환산하지 않는다. 제품·식재료의 직접 식사기록 등 중간 진입을 유지하며 모든 단계를 강제하지 않는다.
...
## 2026-09-18 출시 전 복구 흐름

- 수동 등록 → 사용자·계정 세대·요청 키별 원자적 저장 → 응답 유실이면 결과 조회 → 같은 본문·같은 키 재요청 또는 원 작성자 초안 보관.
- 편집 중 세션 만료 → 명시적 재로그인 → 원작성 계정 확인 → 같은 계정만 초안 복원. 다른 계정에는 자동 행동을 적용하지 않는다.
- 끼니 장보기 → 이미 연결된 목록이 있으면 이어보기, 없을 때만 생성. 충돌 후에도 조회로 연결 상태를 회복한다.
- 이전 요리 → 식사기록 추가 → 수정/삭제 → 다먹음 이후에도 과거 기록 수정 허용. 실측 요리의 원장·완성 중량 기반 영양은 별도 보존한다.
- 재료 입력 → 공백/Unicode 표기 정규화 → 표준명 exact → 명시적 별칭 → 모호하면 후보 선택. 영양 누락을 0이나 임의값으로 보정하지 않는다.


## 2026-09-15 사용자 요청 — UI 수정 및 머지·배포 승인

- 요리계획 추가·레시피 저장·식사기록 추가 성공은 실제 저장 응답 뒤 확인 버튼이 있는 공통 알림창으로 안내한다. 실패 시 성공 안내를 표시하지 않는다.
- 요리계획의 개별 레시피 상태 텍스트를 제거한다. 상태 데이터와 장보기/요리 동작 조건은 유지한다.
- 끼니 상세는 데스크톱 두 열 카드, 모바일 한 열로 표시하고 칼로리·탄단지 수치와 열량 비율 그래프를 보여준다. 나트륨은 이 상세 UI에서 제외하며 값의 계산·저장은 유지한다. 불완전한 영양값으로 전체 비율을 만들지 않는다.
- 식사기록 모바일 끼니 영역을 날짜 카드 폭에 맞추고, 로그인 사용자의 음식 상세는 전체화면으로 표시한다. 기존 준비 안내를 해제하며 비로그인 로그인 안내와 기존 수정 권한 보호를 유지한다.
- 남은요리 상단 프로필은 공통 마이페이지 링크를 사용한다.
- 초기에는 로컬 편집만 진행했으며 후속 사용자 요청으로 커밋·머지·배포를 진행한다. 서버 오류 수정의 실제 운영 동작은 배포 후 확인 대상이다.
...
>
> 사용자 화면 흐름은 `ad click → Hero(a|b|c|d|default) → q1 → q2 → q3 → q4 → result → experience 1/5..5/5 → planner_homecook → packaged_food → planner_complete → beta_form → done`이다. Q1/Q2의 답변으로 조기 종료하지 않고 결과는 Q3 하나로 결정한다. 결과·체험·planner payoff는 모두 이메일 전에 공개한다.
>
...
>
> 광고 유입 사용자는 `/beta`에서 다음 흐름만 따른다.
>
> `ad click → hero → quiz → result → concept → intent → email → followup → done`
>
> `view`에서 서버는 UUIDv4 campaign session cookie `mumeok_validation_session`을 발급하고, 이후 action은 이 cookie와 단일 row를 통해서만 진행한다. 한 row 안에서 action은 `view → quiz_started → quiz_completed → solution_viewed → intent_selected → lead_submitted → followup_submitted` 순서를 지켜야 하며, 역순 전이는 409 fail closed다.
>
> 결과와 해결 아이디어는 이메일 전에 공개한다. `satisfied_control`은 불편을 억지로 만들지 않는 대조군으로 유지하고, `lead_submitted`는 `MARKETING_LEAD_PROTECTION_READY`, Turnstile, origin allowlist, edge rule evidence가 모두 확인될 때만 열어 둔다. lead 실패는 이메일 폼 retry 상태로만 되돌리고 결과를 숨기지 않는다.
>
> `target_qualified`와 result key는 `docs/marketing/quiz-content-spec.md`의 pure rule을 따른다. followup은 accepted/duplicate lead 뒤에만 허용하며, duplicate email은 generic success로 처리한다.

> **2026-08-27 contract-evolution — YouTube 실제 단계 진행바와 남은 시간 범위**
>
> 승인 계획은 `youtube-extraction-truthful-progress-eta-plan-20260826.md`, SHA-256 `e3bf440a3708b50e2430f1f2fda770fbe2a1f30bdaf3eb79fb6237affd5bbe60`이다. 신규 endpoint/status/error 없음. 흐름은 `enqueue queued=0 → source_fetch=10 → video_download=25 → frame_extraction=45 → model_analysis=65 → finalizing=90 → succeeded=100`이며 worker의 실제 stage 진입만 `confirmed_percent` 전진 근거다. 시간만으로 증가하지 않는다. confirmed floor는 최대 90이고 active UI는 95를 넘지 않는다. `succeeded일 때만 100`이다.
>
> `accepted → 기존 5초 polling → active status progress 복원 → 단계/range 갱신 → terminal redirect/notification`을 사용한다. ETA가 있으면 `약 2~4분 남음`, 없으면 `예상 시간 계산 중`, upper를 넘으면 `delayed=true`와 `예상보다 오래 걸리고 있어요. 추출은 계속 진행 중이에요.`를 표시한다. 새로고침·이탈·재로그인 뒤에도 DB snapshot으로 복구한다.
>
> 같은 attempt는 stage가 뒤로 갈 수 없고 cache skip은 앞으로 건너뛴다. retry backoff queued 동안은 이전 attempt snapshot을 숨기고, 새 attempt의 `source_fetch` 적용 뒤 frozen 최대 시도 3을 기준으로 UI가 `다시 분석 중 (2/3)`을 표시한다. progress 기록 실패는 best-effort로 격리해 extraction/finalize 성공·실패를 바꾸지 않는다. terminal `succeeded|failed|expired`는 `progress=null`이고 `succeeded일 때만 100`이다. 최초 release는 stage telemetry promotion gate 전 numeric ETA를 숨긴다.
>
> 이전 client는 additive `progress`를 무시할 수 있다. 새 public endpoint, WebSocket/SSE, ML, 새 npm/pnpm 의존성은 추가하지 않으며 ETA coverage가 나빠지면 숫자 ETA만 숨긴다.
...
→ 성공: session + candidates + job succeeded + delivery key 단일 finalize
→ 실패: bounded retry 또는 failed + safe error + delivery key

앱 shell foreground / 재로그인 / 재실행
...
```text
같은 fingerprint active job 존재
→ 새 row/provider 호출 0
→ 기존 job_id + deduplicated=true
→ queued/processing 상태 링크 표시

transient network/rate-limit + attempt 남음
→ backoff queued
→ permit 재획득 뒤 다음 attempt
...
rollback
→ enqueue 차단
→ 현재 identity queue drain
→ worker stop
→ provider permit release 확인
→ additive schema 호환이 검증된 이전 app release 설치
→ 기존 POST /recipes/youtube/extract 실제 성공 smoke
...
> **2026-08-08 contract-evolution 1A — access-token 회전 flow**
>
> production app/Auth/callback은 `.kr` 단일 origin을 사용한다. access token 갱신과 로그인 세션 교체를 구분하고, callback·server refresh·browser-first refresh 뒤 첫 protected request가 같은 session authority를 사용한다.

## 0-SESSION-REFRESH. Stable session → rotating JWT

```text
로그인 성공 또는 기존 active session
→ Supabase Auth가 같은 session_id로 새 access token(iat/exp 증가) 발급
→ browser cookie 또는 server refresh 결과 갱신
→ 첫 protected request가 stable session_id binding 조회
→ same user/provider/identity + active binding + monotonic iat/exp 확인
→ binding의 last token evidence / expiry / last-seen 갱신
→ planner read/write, pantry read, recipe save, YouTube extract 계속
```

```text
missing/revoked/expired binding 또는 session/account/provider/identity mismatch
→ binding 자동 생성·복구 금지
→ mutation 0
→ 기존 ACCOUNT_SESSION_STALE 계열 처리
→ LOGIN 안내와 allowlisted return-to-action
```

- 늦게 도착한 이전 JWT는 최신 token evidence를 되돌리지 못한다. 같은 session의 최신 JWT만 단조 갱신한다.
- callback과 refresh는 동일한 record 의미를, API gateway와 PostgREST pre-request는 동일한 assert/conditional-renew 의미를 사용한다.
- 별도 public refresh route는 만들지 않는다. OAuth 성공 callback은 `https://app.mumeok.kr/auth/callback`, link callback은 `https://app.mumeok.kr/auth/link/callback`만 허용한다.
- 30일 inactivity, 90일 absolute, recent-auth 10분 flow는 후속 1B에서 확정한다.

> **2026-08-04 contract-evolution — recipe snapshot entrypoint context flow**
>
> 기존 `RECIPE_DETAIL`, `MEAL_SCREEN`, `COOK_MODE`와 endpoint/status/error를 유지하고 실제 page entrypoint에서 owner edit 문맥과 Meal revision을 소비하는 최소 additive flow만 연결한다. `personal_recipe_v2` 및 `snapshot_v2_creation`의 raw 값은 server-only이며 browser/public API에 노출하지 않는다.

## 0-RSE. Server projection → detail/edit/start flow

```text
RECIPE_DETAIL 또는 MEAL_SCREEN page server loader
→ personal_recipe_v2 + snapshot_v2_creation을 같은 server-only authority에서 함께 읽음
→ 두 capability 모두 active: snapshot_v2 consumer 선택
→ 그 외/off/missing/unknown/read error: legacy_v1 consumer 선택
→ raw capability는 직렬화하지 않음
```

### Owner RECIPE_DETAIL edit

```text
GET /recipes/{id}
→ 모든 성공 detail에 revision
→ exact owner-private current에만 edit_context={base_recipe_revision,draft,image_object_id}
→ 같은 base revision + full draft로 future impact preview
→ 전체 반영 | 기존 계획 유지 중 하나 선택
→ 같은 draft/revision/token/image identity로 PATCH
→ 성공이면 latest detail/revision 재조회
→ stale/claim 409면 dialog 유지 + 최신 영향 재확인으로 focus
```

- non-owner/public/비로그인에서 `edit_context`는 omit된다. 삭제·다른 owner private recipe는 기존 404 non-disclosure며 empty/null editor context로 존재를 암시하지 않는다.
- loading은 save를 disabled하고, zero impact를 빈 상태로 명시하며, error를 성공처럼 닫지 않는다. unauthorized는 편집 action으로 return-to-action하고 public/non-owner는 read-only/fork 경계를 유지한다.

### Standalone / planner cooking start

```text
RECIPE_DETAIL의 기존 요리하기 CTA 1개
→ legacy_v1이면 기존 독립 요리
→ snapshot_v2이면 recipe revision을 expected_recipe_revision으로 start
→ exact success 전에는 현재 화면 유지
→ success session_id/contract_version/mode로 COOK_MODE

MEAL_SCREEN의 기존 Meal 요리하기 CTA 1개
→ GET /meals.items[].revision으로 selected Meal expected_meal_revisions 구성
→ start RPC가 transaction 안에서 owner/status/revision/claim/same recipe+content 최종 검증
→ exact success 전에는 pending·중복 제출 차단·현재 화면 유지
→ error/409면 inline retry, 성공이면 explicit snapshot_v2 COOK_MODE
```

- client가 보낸 revision은 stale detection hint일 뿐이며 authorization이 아니다. response body shape로 version을 추측하거나 legacy parser로 fallback하지 않는다.
- creation off인 R/R+1은 실제 page CTA에서 `legacy_v1`을 선택하고 new personal/v2 creation을 0으로 유지한다. direct seeded/existing v2 URL read/cancel drain은 유지하며 #8 증거와 R+2 공동 승인 전에 activation하지 않는다.
- #8 pantry/weight/batch/XP complete, F0/account quarantine는 이 flow에 합치지 않는다.

> **2026-08-01 contract-evolution — full-local Auth/Data/Storage flow**
>
> 아래 2026-07-30 remote Auth → local mirror/gateway 흐름은 역사 기록으로 보존하되 active production flow는 self-hosted local Auth → local DB/Storage로 대체한다. 공개 제품 흐름과 기존 error mapping은 유지한다.

## 0-FULL-LOCAL. 로그인·link·데이터·cutover 흐름

```text
사용자가 Google/Naver/Kakao login 또는 link 선택
→ POST /auth/flow/start same-origin/CSRF/provider/link-session 검증
→ private auth_flow_attempts insert + signed HttpOnly cookie 발급
→ browser signInWithOAuth 또는 linkIdentity 시작
→ provider가 https://auth.<owned-domain>/auth/v1/callback으로 복귀
→ local Auth가 provider code 교환, local auth.users/auth.identities 갱신
→ https://app.<owned-domain>/auth/callback 또는 /auth/link/callback 복귀
→ signed cookie HMAC ledger lookup + provider/flow/authority/cutover epoch exact 비교
→ local code exchange/getUser/session 검증
→ session_id HMAC binding 생성 또는 갱신
→ 기존 return-to-action
```

- provider SDK가 redirect 전에 실패하면 `/auth/flow/cancel`이 ledger를 `cancelled` terminal로 닫고 cookie를 제거한다. callback 없이 떠난 흐름은 900초 expiry로 닫는다.
- callback은 query의 provider 값을 authority로 사용하지 않는다. ledger provider와 실제 local Auth identity가 다르거나 cookie가 remote/old epoch면 기존 안전한 auth error로 실패하고 application write는 0이다.
- 사용자 데이터 요청은 Homecook server → loopback Supabase gateway → PostgREST/Storage 순서다. local JWT `sub`가 기존 `auth.uid()`와 일치하고 active session binding/account generation/RLS를 모두 통과해야 한다.
- logout/delete/quarantine/identity replacement는 app-owned binding revoke → local Auth session/identity 처리 → Homecook cookie 만료 순서이며, 아직 `exp`가 남은 이전 JWT도 개인 mutation을 수행할 수 없다.
- cutover는 신규 login/link와 application mutation 차단 → remote outstanding flow 최대 960초 drain → expired terminal → outstanding 0 → cutover epoch/HMAC version 회전 → final Auth/DB restore와 S3 copy → provider callback 전환 → old cookie 만료 → 운영자 승인 → local login/write 개방 순서다.
- 첫 local Auth state mutation 전에는 provider callback/app env를 remote로 복구할 수 있다. 그 뒤에는 login/write freeze와 Auth/application/Storage delta를 함께 다루며 env-only rollback은 금지한다.
- Auth/DB/Storage/Cloudflare 장애는 partial write 없이 기존 error/retry/read-only 상태로 fail closed한다. local Auth down에서 remote Auth로 자동 fallback하지 않는다.

> **2026-07-31 contract-evolution — product pantry / shopping provenance 흐름**
>
> 신규 endpoint, public status, public error code 없이 기존 PANTRY, shopping completion, pantry-match, HOME pantry-cleanout 흐름을 additive contract로 확장한다.

## 0-PIL. exact product pantry 흐름

1. 사용자가 generic ingredient를 고르면 기존 `POST /pantry.ingredient_ids[]`를 사용한다. product를 고르면 검색 응답의 exact product ID와 선택 당시 immutable nutrition version ID를 `product_items[]` pair로 보낸다.
2. server는 authenticated owner, product visibility/ownership, version 존재와 `(product_id, version_id)` 소속 일치를 검증한다. mismatch는 전체 product pair를 기존 `422 VALIDATION_ERROR`로 거부하며 current version으로 대체하지 않는다.
...
1. 일반 delete는 pantry/shopping composite product/version FK에서 `ON DELETE RESTRICT`로 fail closed한다.
2. account cleanup 전용 authority가 owner lifecycle fence와 exact private ownership을 검증한다.
3. 해당 owner의 private product를 참조하는 pantry, shopping, product planner, meal-log, recipe/content reference를 먼저 제거한다.
4. 그 뒤에만 private product를 hard delete하고 private immutable version/link를 정리한다. 일반 사용자는 이 경로나 cascade를 호출할 수 없다.
5. owner-null public/shared product, immutable version, link/provenance는 보존한다. account-generation activation은 F0 + #3 joint gate 전까지 닫혀 있다.

> **2026-07-30 contract-evolution — remote Auth → local Data/Storage 흐름**
>
> 이 흐름은 2026-07-29 local-only production의 `원격 프로젝트 없음`, local Auth barrier 흐름을 대체한다. 화면/API shape는 유지한다.

```mermaid
flowchart LR
  A["Google/Naver/Kakao"] --> B["원격 Supabase Auth"]
  B --> C["Homecook callback\nremote session 검증"]
  C --> D["private identity epoch\n멱등 mirror"]
  D --> E["claim-verifying gateway"]
  E --> F["local PostgREST/Storage\n기존 RLS"]
  F --> G["기존 화면/API response"]
  C -. "claim/mirror 실패" .-> X["fail closed"]
  E -. "issuer/session 실패" .-> X
```
...
>
> 실제 고객 없이 테스트 데이터만으로 서버 MacBook의 local Supabase + Next.js를 초기 production으로 두는 흐름을 아래로 고정한다. `docs/화면정의서-v1.5.29.md`와 `docs/api문서-v1.2.29.md`의 route/UI/API 계약은 바꾸지 않으며, 원격 Supabase migration/provider barrier 단계는 이번 흐름에서 제거한다.

## ⓪-LOCAL-FIRST. 격리 rehearsal → local production → 재현 흐름

```mermaid
flowchart LR
  A["격리 local rehearsal\n(다른 MacBook 또는 clean local reset)"] --> B["exact source checkout\n+ dependency install"]
  B --> C["local Supabase reset 또는 backup restore"]
  C --> D["seed 또는 동일 test-data restore\n+ Storage 복원"]
  D --> E["production env 재발급\n+ Next.js build"]
  E --> F["local auth.users SHARE ROW EXCLUSIVE barrier"]
  F --> G["launchd 재설치\n+ 127.0.0.1 smoke"]
  G --> H["server MacBook local-only production"]
  F -. 실패 .-> R["local rollback 또는 clean restore"]
  G -. 실패 .-> R
```

- rehearsal은 production 원격 복제본이 아니라 local 격리본이다. 같은 데이터를 재현하려면 backup restore를, 깨끗한 기준선을 재현하려면 전체 migration replay 뒤 seed를 사용한다.
- cutover의 마지막 DB 장벽은 local DB 안의 `auth.users` **write barrier**이며, remote verifier/provider quiet window는 없다. 이 lock은 전체 read freeze가 아니다.
- quiet window는 최초 cutover 1회성 운영 창이다. 같은 작업 안에서 시간이 늘어나도 별도 사용자 재승인을 다시 요구하지 않는다. 다만 Auth Admin/import/dashboard 동결 해제, 새 external/auth write, restart/restore, lock 상실, digest 불일치, production secret 교체가 발생하면 즉시 중단하고 Auth quiet 관측과 15분 간격 Storage inventory 2회를 처음부터 다시 수집한다.
- old-path irreversible delete는 이 흐름에 포함되지 않는다. 새 경로 정상화와 영구 삭제를 같은 change로 묶지 않는다.
- 향후 hosted provider나 managed maintenance barrier가 필요해지면 이 flow를 직접 수정하지 말고 새 contract-evolution을 먼저 연다.

> **2026-07-26 contract-evolution — i031 localhost 직접 추출 흐름**
>
> `YOUTUBE_RECIPE_EXTRACTOR_MODE=i031_codex_vision`인 localhost에서는 기존 YT_IMPORT Step 2 내부를 다음 흐름으로 대체한다. 다른 모드는 기존 여정을 유지한다.

```mermaid
flowchart LR
  A["임의 YouTube URL"] --> B["기존 validate 미리보기"]
  B --> C["i031 preflight"]
  C --> D["설명란·작성자 댓글·transcript 수집"]
  D --> E["영상 임시 다운로드·adaptive keyframe 생성"]
  E --> F["gpt-5.6-sol low frame selector"]
  F --> G["gpt-5.6-sol low final recipe extraction"]
  G --> H["JSON 검증·기존 재료 표준명 매핑"]
  H --> I["기존 검수 화면"]
  C -. 실패 .-> X["명시적 extract error"]
  D -. 실패 .-> X
  E -. 실패 .-> X
  F -. 실패 .-> X
  G -. 실패 .-> X
...
- preflight는 exact Codex CLI/version/login과 media/source 도구를 검증한다. `YOUTUBE_API_KEY`는 server-only이며 Gemini key는 사용하지 않는다.
- selector와 final 호출은 i031 prompt/client/version을 그대로 사용한다. 모델 alias의 비결정성 때문에 같은 URL의 문구와 점수가 매 실행 완전히 같다고 보장하지는 않는다.
- 단계별 timeout과 전체 timeout을 넘으면 subprocess를 종료하고 임시 디렉터리를 정리한 뒤 오류를 반환한다.
- 성공 결과만 기존 draft 세션에 저장한다. 기존 matcher가 확정하지 못한 재료는 사용자가 검수한다.
- 오류 시 Gemini, legacy parser 또는 특정 영상 fixture로 우회하지 않는다. 사용자는 같은 화면에서 재시도할 수 있다.
- localhost 실제 확인은 고정 평가 영상 외 임의 공개 YouTube 레시피 URL 1개 이상으로 수행하고, URL/영상 ID가 코드와 test fixture allowlist에 하드코딩되지 않았음을 검사한다.
...
3. `GET /recipes/{id}`는 모든 성공 detail에 `revision`을, exact owner-private current에만 `edit_context={base_recipe_revision,draft,image_object_id}`를 제공한다. client는 이 context의 `base_recipe_revision + 전체 draft`를 변환 없이 impact preview로 보낸다. non-owner/public에는 `edit_context`를 omit한다. `v1.3.31 갱신`
4. `새 레시피로 저장`은 `POST /recipes` personal-derived branch로 처리하고, source owner/current revision/draft/image_object_id를 재검증한 뒤 새 detail route로 이동한다. 기본 저장은 여전히 같은 ID의 PATCH다.
...
1. #8 R/R+1 증거와 R+2 공동 승인 뒤 server page loader가 `personal_recipe_v2` 및 `snapshot_v2_creation`을 함께 읽어 둘 모두 active일 때만 `snapshot_v2` creation consumer를 선택한다. raw capability는 browser/public response에 노출하지 않고, 그 외 상태는 `legacy_v1`으로 fail closed한다. `v1.3.31 갱신`
2. planner start는 owner-scoped `GET /meals.items[].revision`으로 selected Meal ID 전체의 `expected_meal_revisions`를 보낸다. recipe lock→Meal UUID lock에서 owner/status/pin/revision/claim을 최종 검증하고 session+session-meal+claim을 한 transaction으로 만든다. client revision은 hint이다. `v1.3.31 갱신`
3. standalone start는 `GET /recipes/{id}.revision`을 expected revision으로 보내고 recipe access/deleted/revision을 transaction에서 확인해 current content와 servings를 pin한다. `v1.3.31 갱신`
...
2. 모든 existing/new personal writer, direct DML, Storage policy, auth inbound FK를 isolated local rehearsal과 server-Mac local production에서 inventory한다.
...
1. exclusive fence와 local `auth.users SHARE ROW EXCLUSIVE` write barrier를 확보한다. Auth Admin/import/dashboard create/delete 동결, external attempt 0, 15분 간격 owner-signal inventory 2회 일치는 lock과 별도로 유지한다. provider barrier는 향후 별도 contract-evolution 전까지 사용하지 않는다.
...
> 아래 Step 2의 기존 parser→조건부 Gemini/visual 보강은 미설정 또는 `legacy` 모드에만 적용한다. `i031_codex_vision`에서는 문서 맨 앞 i031 흐름이 Step 2를 대체하고 legacy/Gemini/visual fallback을 호출하지 않는다.
...
Step 2) 공개 텍스트 우선 추출
  ...
  │ legacy mode에서만, 그래도 재료/스텝이 부족하면 조건부 Gemini structured fallback으로 공개 텍스트 JSON 구조화
  │ legacy mode에서만, 수량이 계속 부족하면 조건부 visual quantity enrichment
  │ i031 mode는 문서 맨 앞 source→frames→selector→final 흐름으로 이 세 줄을 모두 대체
  ...
  │ UI: retained legacy sync consumer에서만 indeterminate spinner/progress
  │ background accepted consumer는 §0-YT-ASYNC의 실제 6단계 progress 사용
  │
  ▼
Step 3) 결과 검수/수정
  │ ├─ 다중 후보 목록 (필요 시)
...
- **Visual Quantity Enrichment 흐름 (`legacy` only)** `v1.3.12 추가`:
    ...
    - `legacy` visual enrichment 비활성화/실패 시 public-text-only 결과로 fallback한다. i031 strict 실패에는 이 fallback을 적용하지 않는다.
...
## 2026-09-06 사용자 승인 — Planner 미리보기 흐름

비로그인 방문 → 요리 계획/식사 기록의 공개 예시 조회 → 추가/수정/삭제 선택 → 로그인 안내 → 기존 로그인 및 개인 플래너. 개인 데이터의 비로그인 조회는 허용하지 않는다. YouTube 신규 추출 진입 → 준비 중 안내 → 기존 화면 복귀. 기존 추출 결과 조회와 로그인 후 소유자 작업 흐름은 보존한다. API/DB/도메인 상태 전이는 변경하지 않는다.
