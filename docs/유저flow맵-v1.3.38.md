...
- 이번 작업은 로컬 편집 단계이며 사용자 요청에 따라 테스트·빌드·머지·배포를 보류한다. 서버 오류 수정의 실제 동작 검증은 별도 후속 확인 대상이다.
...
flowchart LR
  ...
  E --> F["gpt-5.4-mini frame selector"]
  F --> G["gpt-5.4 final recipe extraction"]
