...
네이버 개발 중 테스트 ID 제한을 확인하고 사용자 지정 ID를 등록했다. full-local 계정 복구의 중단된 내부 RPC와 계정 연결 서버 설정·재시도 UI를 복구한다. 본인 세션·소유권·identity·중복 요청 보호는 유지한다. 사용자는 기존 `app.mumeok.kr`에서 검증된 수정을 반영하며 확인하는 방식을 선택했다. 구현·검증·실제 반영 상태는 [로그인 복구 기록](../engineering/auth-repair-20260926.md)을 따른다.
