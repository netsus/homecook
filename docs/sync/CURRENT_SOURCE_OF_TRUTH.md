...
식사기록의 `요리한 음식` 목록은 요리계획에서 사용하던 남은요리 카드와 명시적인 `추가` 버튼을 사용한다. 별도 남은요리 탭은 두지 않는다. 추가 화면에서 먹은 날짜·끼니·실제 섭취 g을 선택한다. 이전 요리는 영양·잔량을 추정하지 않고 섭취 기록만 남긴다. 요리계획 추가 옵션에서는 남은요리를 제거한다. 식사 상세는 처음 기록한 수량을 숫자 입력칸에 표시하며 상세 화면 안에서 수정·성공 알림·최신 영양 갱신까지 완료한다. YouTube 추출 재료가 `TO_TASTE`여도 수량 입력이나 단위 선택을 시작하면 `QUANT`로 전환해 숫자 입력과 브라우저 증감 버튼이 동작한다.
...
- 요리모드 500 수정 후보: legacy cook-mode 조회에서 존재하지 않는 `cooking_session_meals.recipe_content_snapshot_id` 선택을 제거했다. snapshot 읽기 RPC 두 개의 scope 수정 후보는 최신 DB wrapper 충돌 위험으로 적용 대상에서 제거하고 `docs/engineering/recipe-snapshot-entrypoint-scope-followup.md`에 후속 작업으로 남긴다. 최신 master의 장보기·YouTube 수정은 보존하며 실제 운영 결과 확인은 남아 있다.
