...
- suite ID/App owner/repository/head는 candidate `ci_suite_run_set_digest`와 production `all_check_suite_authority_digest`에 봉인한다. workflow run 없는 zero-check external suite는 canonical repo/head와 unique exact `Netlify(13473/netlify)`, `GitGuardian(46505/gitguardian)`, `Vercel(8329/vercel)`, `Claude(1236702/claude)` tuple만 허용한다. Claude tuple은 installed GitHub App evidence일 뿐 actor 사용 권한이 아니다.
