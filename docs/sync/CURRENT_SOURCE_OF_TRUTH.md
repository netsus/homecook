...
식사기록 추가창은 `남은 요리`를 별도 선택 항목으로 제공하고, 선택한 cooked batch의 남은 중량 안에서 실제 섭취량을 g으로 기록한다. 식사 상세는 처음 기록한 수량을 숫자 입력칸에 표시하며 상세 화면 안에서 수정·성공 알림·최신 영양 갱신까지 완료한다. YouTube 추출 재료가 `TO_TASTE`여도 수량 입력이나 단위 선택을 시작하면 `QUANT`로 전환해 숫자 입력과 브라우저 증감 버튼이 동작한다. API와 DB 구조는 변경하지 않는다.
