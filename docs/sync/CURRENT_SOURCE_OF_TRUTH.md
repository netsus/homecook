...
- 요리모드 500 수정 후보: legacy cook-mode 조회에서 존재하지 않는 `cooking_session_meals.recipe_content_snapshot_id` 선택을 제거했다. snapshot 읽기 RPC 두 개의 scope 누락은 `20260915060000_recipe_snapshot_entrypoint_read_scope.sql`에 작성했으나 DB에는 적용하지 않았다. 장보기 생성 오류와 YouTube 503의 실제 원인은 미확정이며 운영 응답/워커 기록 확인이 남아 있다.
