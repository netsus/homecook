...
## 2026-09-26 사용자 요청 — 로그인·계정 복구·연결 복구

네이버 개발 중 테스트 ID 제한을 확인하고 사용자 지정 ID를 등록했다. full-local 계정 복구의 중단된 내부 RPC와 계정 연결 서버 설정·재시도 UI를 복구한다. 본인 세션·소유권·identity·중복 요청 보호는 유지한다. 사용자는 기존 `app.mumeok.kr`에서 검증된 수정을 반영하며 확인하는 방식을 선택했다. 운영 ledger 185개·웹 `5d05a180b`에 반영했고 사용자가 Google 계정 복구·네이버 로그인·계정 연결 성공을 확인했다. 검증과 실제 반영 내용은 [로그인 복구 기록](../engineering/auth-repair-20260926.md)을 따른다.

## 2026-09-22 사용자 요청 — D01 검색 503·저장 500 복구

`곰곰 치즈` 검색의 503은 검색 쿼리 지연과 전송 제한, 개인 복제 POST의 500은 DB 함수에 없는 인자 전달을 각각 복구했다. 한글 조합 종료를 기다리던 검색은 사용자 후속 요청에 따라 입력이 250ms 멈추면 자동 실행한다. 기존 색인·검색 기준을 재사용하고 권한·세션·영양 버전·원본 revision·중복 방지는 유지한다. 요구사항·화면·Flow의 같은 날짜 후속 항목이 초기 D06 규칙을 대체한다. 운영 ledger 184개·웹 d9df7878 반영 후 검색·재료 교체·개인 복제 저장·새로고침을 확인했으며 상세는 [오류 복구 기록](../engineering/recipe-search-save-repair-20260922.md)을 따른다.

## 2026-09-22 사용자 요청 — 베타 테스트 화면 안내와 응답 지연 최적화

D01~D07 확인 위치와 운영 응답 지연의 측정·한계는 [화면 안내와 성능 기록](../engineering/beta-response-time-20260922.md)을 따른다. 인증·초기화 이후의 독립 플래너 조회와 레시피 조리 단계 조회를 병렬화하고 운영 웹에 반영했다. API 응답·사용자 범위·내용 고정 데이터 검증·세션 liveness는 유지하며 새 인증 캐시를 만들지 않는다. 큰 지연은 외부 접속 구간에서 관측됐고 앱 변경 전부터 회복됐으므로 회복 전체를 최적화 효과로 표현하지 않는다. Caddy HTTP/1.1 비교는 효과가 없어 원본으로 복구했고 DB·DNS·캐시·TLS 정책은 바꾸지 않았다.

## 2026-09-22 사용자 요청 — 베타 기능 운영 반영

기존 계획 1건의 레시피 내용 고정 데이터 보완, 운영 웹 배포·DB 적용·새 모드 활성화를 완료했다. 변경 전 전체 암호화 백업과 별도 장치 복사·실제 격리 복원을 확인하고 정확한 기존 full-local 자원에서 수행했다. 과거 영양 `NULL`과 기존 기록·권한을 보존했으며, 새로운 내용 고정 값은 현재 원문 기준의 `legacy_backfill`이다. 최종 ledger 183개, `snapshot_v2/on/on`, 누락 계획 pin 0이며 실제 편집 진입·검색·요리 시작/조회/취소를 확인했다. 운영에서 발견한 오래된 함수 정의·읽기 전용 오류와 검증 범위는 [베타 기능 운영 반영 기록](../engineering/beta-rollout-20260922.md)을 따른다. 아래 구현 작업의 ‘운영 미반영’ 상태는 후속 실행 전의 역사 기록이다.

## 2026-09-22 사용자 요청 — 베타 흐름 공백과 문서 상태 보완

10월 12일 소규모 베타를 위해 기존 레시피 교정·제품/재료 선택·최근 음식·검색 오류·한글 입력·요리 완료 후 기록 연결을 완결하고, 과거 계획의 승인/병합 상태와 현재 안내를 바로잡는다. 현재 공식 5종의 같은 날짜 addendum을 따르며 구현·검증·배포 상태는 `docs/engineering/beta-flow-gaps-20260922.md`에서 구분한다. 모든 단계를 강제하지 않고 기존 권한·소유권·read-only·상태 전이·영양 근거 보호를 유지한다. 과거 CI·Stage·workpack/closeout은 신규 작업 조건이 아니다.
...
## 2026-09-15 사용자 요청 — 끼니·식사기록 compact UI와 legacy 영양 복구

요리계획 끼니 영양은 상단 `계획 영양`·계산 방법·하단 비율 설명을 제거하고 열량, bar, `탄/단/지 + 값`을 한 줄형으로 표시한다. 확인 안내는 우측 상단 작은 `i` 버튼으로 연다. 식사기록에서 기록이 없는 날짜는 하루 영양 placeholder를 렌더링하지 않는다. 완성 중량이 없는 legacy cooked batch는 현재 recipe 영양 snapshot이 있고 모든 정량 재료가 g/kg일 때 재료 합산 중량을 기준으로 실제 섭취 g 영양을 계산하며, legacy 전용 수정 RPC가 같은 계산과 revision/idempotency 보호를 유지한다. 식사 상세 모바일에는 명시적 뒤로가기와 식사기록 활성 하단 탭을 표시한다. 모바일 상단 프로필은 제거하고 법적 문서는 마이페이지 복귀와 마이 활성 하단 탭을 제공한다. 중복 `서브웨이 뺨치는 오이 참치 샌드위치` 한 건과 `스팸 김치볶음밥`은 soft delete한다.
...
## 2026-09-15 사용자 요청 — 식사기록 남은요리·상세수정·YouTube 수량

식사기록의 `요리한 음식` 목록은 요리계획에서 사용하던 남은요리 카드와 명시적인 `추가` 버튼을 사용한다. 별도 남은요리 탭은 두지 않는다. 추가 화면에서 먹은 날짜·끼니·실제 섭취 g을 선택한다. 당시에는 이전 요리의 영양·잔량을 추정하지 않고 섭취 기록만 남겼다. 이 중 영양 계산 제한은 같은 날 후속 「끼니·식사기록 compact UI와 legacy 영양 복구」 승인으로 대체됐다. 현재는 recipe 영양 snapshot과 모든 정량 재료의 g/kg 중량이 있을 때만 재료 합산 중량 기준으로 섭취 g 영양을 계산한다. 조건이 부족하면 영양을 추정하지 않으며 잔량 추정 금지는 유지한다. 요리계획 추가 옵션에서는 남은요리를 제거한다. 식사 상세는 처음 기록한 수량을 숫자 입력칸에 표시하며 상세 화면 안에서 수정·성공 알림·최신 영양 갱신까지 완료한다. YouTube 추출 재료가 `TO_TASTE`여도 수량 입력이나 단위 선택을 시작하면 `QUANT`로 전환해 숫자 입력과 브라우저 증감 버튼이 동작한다.

## 2026-09-15 사용자 요청 — 요리 완료·식사기록 빠른 복구

운영 DB에 새 세션 권한 요리 완료 RPC가 아직 없으면 `PGRST202`에 한해 기존 인증 사용자·소유권 검증 RPC로 완료한다. 다른 서버 오류에는 우회하지 않는다. 식사기록 제품·재료 검색은 입력 후 250ms에 자동 실행하고 이전 요청을 취소한다. 재료의 빈 기본 단위는 `g`로 처리하며 재료는 `g/kg`, 제품은 영양 기준과 승인된 basis relation에 포함된 단위를 선택 목록으로 제공하고 단위 변경 시 같은 양에 맞춰 환산한다. 공개 DB 구조는 바꾸지 않는다.
...
- 요리모드 500 수정 후보: legacy cook-mode 조회에서 존재하지 않는 `cooking_session_meals.recipe_content_snapshot_id` 선택을 제거했다. 당시 snapshot 읽기 RPC 두 개의 scope 수정 후보는 최신 DB wrapper 충돌 위험으로 적용 대상에서 제거했다. 이후 20260919000000 migration으로 기존 wrapper를 보존하며 해결·적용했으므로 현재 미해결 목록은 아니다. 경위는 `docs/engineering/recipe-snapshot-entrypoint-scope-followup.md`를 따른다. 최신 master의 장보기·YouTube 수정은 보존하며 실제 운영 결과 확인은 남아 있다.
...
2026-09-11 독립 검토 수정(R2C-001/002/003, S01 부분 수용)은 상세 계약 §11에 기록한다. 내부 단일 RPC·제어 lease, 삭제 쿠키 만료/명시 재시작·기존 쿠키 복원, Q1 범위와 Q2 선택지의 API/DB 동기화를 공식 5종에 반영했다. 내부 RPC는 고객 공개 API 총계에 추가하지 않는다.
...
- 모든 Actions App `15368` suite는 started check 0개여도 canonical repository `netsus/homecook`과 exact 하나의 workflow run에 1:1 결합돼야 한다. orphan/missing/multi-map/wrong-repository suite를 차단하고 self/manifest/schema의 `workflow_run_attempt`는 exact `1`이다.
- non-excluded started check는 GitHub Actions App `15368`이거나 exact GitGuardian tuple `(app.id=46505, app.slug=gitguardian, app.name=GitGuardian, check name=GitGuardian Security Checks)` 1개여야 한다. Actions check는 exact 하나의 workflow run에 suite ID로 결합하고 각 run은 `run_attempt=1`, terminal success, exact repository/head SHA와 nonempty workflow ID/path/event를 가져야 한다. GitGuardian은 check/suite App identity, empty `external_id`, exact head와 suite repository `netsus/homecook`, completed/success를 모두 만족해야 한다. commit statuses는 current contract에서 exact empty다.
...
- 모든 raw expected context instance는 success여야 한다. optional은 exact skipped만 intended skip이다. allowlisted GitGuardian도 success만 허용하며 duplicate, 이름/App/repository/SHA/suite/external-id 불일치와 neutral/pending/queued/failure/cancelled, 그 밖의 non-Actions started check는 차단한다. started check가 0인 다른 external suite는 complete suite digest에는 남지만 terminal gate에는 추가하지 않는다.
...
- suite authority는 ID 오름차순의 exact closed `{app_id,app_name,app_slug,head_sha,id,repository}` tuple 하나로 정규화하며 candidate collector/stored readback의 `ci_suite_run_set_digest`와 production `all_check_suite_authority_digest`가 같은 tuple을 사용한다. workflow run 없는 zero-check external suite는 canonical repo/head와 unique exact `Netlify(13473/netlify)`, `GitGuardian(46505/gitguardian)`, `Vercel(8329/vercel)`, `Claude(1236702/claude)` tuple만 허용한다. Claude tuple은 installed GitHub App evidence일 뿐 actor 사용 권한이 아니다.
