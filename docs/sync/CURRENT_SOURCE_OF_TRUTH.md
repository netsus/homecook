...
| 문서 | 변경 내용 |
|------|----------|
| 요구사항 v1.7.31 | `/menu/add/youtube` durable extraction, Next-only external HMAC/dual-read-current-write enqueue, exact i031 initial policy, complete snapshot claim, bootstrap/rotation, consumed-over-TTL과 Quick Import 호환을 잠근다 |
| 화면정의서 v1.5.35 | app shell toast/badge/list, browser digest/RPC 금지, server-only enqueue authority, exact snapshot claim, consumed-after-TTL, `can_retry=true` 전용 retry와 Quick Import UI 비변경을 잠근다 |
| 유저 Flow맵 v1.3.33 | enqueue→이탈→reaper-before-claim worker→notification→review/register, route HMAC→restricted RPC→dual-read/current-write, snapshot attestation, initial bootstrap/later rotation, `sync_wait`, rollback 순서를 고정한다 |
| DB v1.3.33 | jobs/session linkage, non-secret `private.youtube_extraction_current_policy`, exact i031 options, external HMAC trust boundary, enqueue role/owner, `policy_snapshot_digest`, unconsumed-only expiry, fencing/finalize와 exact ACL/pre-request/rotation을 잠근다. 총 74개 |
| API v1.2.38 | 신규 보호 endpoint 6개, exact public union과 server-only HMAC/scoped enqueue, current policy/snapshot digest, consumed-over-TTL `succeeded`, `can_retry`와 safe error/cursor/dedupe, 기존 POST/Quick Import 호환을 잠근다. 총 108개 |
...
> exact head `e23f9491a76dffe62957adf135d4e43d0bbacab6`의 다음 독립 review는 HMAC/enqueue authority, exact options/initial policy, worker complete snapshot, bootstrap/rotation 분리 4건을 blocking finding으로 냈다. 이 revision은 DB 밖 external current/previous HMAC loader, `youtube-extraction-enqueue` scope + `youtube_extraction_enqueue` NOLOGIN role, dual-read/current-write, workpack 33 exact i031 manifest/options, `policy_version`/`policy_snapshot_digest`/`allowed_snapshot_digest`, disabled initial bootstrap와 drained later rotation을 공식 5종·CSoT·contract test에 동기화한다. 제품 구현/migration 없이 Draft를 유지하고 successor exact-head 독립 재검토와 current-head CI 전에는 Ready/merge하지 않는다.
