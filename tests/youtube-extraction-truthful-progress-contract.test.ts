...
describe("YouTube truthful extraction progress contract evolution", () => {
  it("records the approved additive contract in every official document", () => {
    ...
    expect(read(officialTuple.requirements)).toContain("# 요구사항 기준선 v1.7.36");
    expect(read(officialTuple.screens)).toContain("# 화면정의서 v1.5.40");
    expect(read(officialTuple.flow)).toContain("# 유저 Flow맵 v1.3.38");
    expect(read(officialTuple.db)).toContain("# DB 설계 v1.3.38");
    expect(read(officialTuple.api)).toContain("# API\\_설계\\_v1.2.43");
