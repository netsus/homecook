...
describe("pi recipe extractor MVP runner", () => {
  ...
  it("moves timeline frame collection error refs from evidence to uncertainties", async () => {
    ...
    const sourcePacket = {
      video: { videoId: "case-frame-error-ref", title: "테스트", description: "00:00 양파 볶음" },
      captions: {
        segments: [
          { text: "양파를 썬다", startMs: 0, durationMs: 2000 },
        ],
      },
      authorComments: [],
    };
    const timeline = normalizeVideoTimeline({
      events: [{
        eventId: "e1",
        segmentId: "s1",
        timeRange: { startSec: 0, endSec: 20 },
        action: "프레임 수집 실패 구간이지만 설명란과 자막으로 양파를 확인한다",
        candidateAssignments: [{ candidateId: "r1", status: "unclear", reason: "frame failed" }],
        evidence: ["description:1", "transcript:0s", "timeline-frame-ledger:error:s1-w1"],
        confidence: 0.4,
      }],
    }, { videoId: "case-frame-error-ref", sourcePacket });

    expect(timeline.events[0].evidence).toEqual(["description:1", "transcript:0s"]);
    expect(timeline.events[0].uncertainties).toContain(
      "timeline-frame-ledger:error:s1-w1 frame collection failed; not usable as evidence",
    );
    expect(() => assertValidVideoTimeline(timeline, {
      allowedCandidateIds: ["r1"],
      allowedEvidenceRefs: timelineAllowedEvidenceRefs({ sourcePacket, timelineFrameLedger: { windows: [] } }),
  ...
  it("falls back from video timeline ledger for multi-candidate holistic runs", async () => {
    const { runPiExtraction } = await import(runnerModuleUrl);
    const caseDir = path.join(workdir, "notebooks/recipe_loop_data/train/case-a");
    const holisticResponsePath = path.join(workdir, "fixtures/holistic-timeline-draft.json");
    ...
    writeJson(understandingResponsePath, {
      globalStory: "두 개의 레시피가 이어지는 영상이다.",
      dishStories: [{
        candidateId: "storyboard-1",
        title: "양파 볶음",
        plainStory: "양파 볶음 뒤에 감자 무침이 이어지는 다중 레시피 흐름이다.",
        mainIngredients: ["양파"],
        stepOutline: ["양파를 볶는다"],
        sourceRefs: ["description:1"],
        confidence: 0.8,
      }],
      crossDishNotes: ["multi-candidate fallback에서는 이해 메모를 log-only로 남긴다."],
      uncertainties: [],
    });
    writeJson(holisticResponsePath, {
      recipes: [
        {
          candidateId: "storyboard-1",
          title: "양파 볶음",
          timeRange: { startSec: 0, endSec: 60, basis: "description-timeline-storyboard" },
          ingredients: [{ name: "양파", amount: null, unit: null, amountBasis: null, evidence: ["description:1"] }],
          steps: [{ text: "양파를 볶는다.", evidence: ["description:1"], confidence: 0.7 }],
          visualNeeds: [],
          uncertainties: [],
        },
        {
          ...
          title: "감자 무침",
          timeRange: { startSec: 60, endSec: 120, basis: "description-timeline-storyboard" },
          ingredients: [{ name: "감자", amount: null, unit: null, amountBasis: null, evidence: ["description:2"] }],
          steps: [{ text: "감자를 무친다.", evidence: ["description:2"], confidence: 0.7 }],
          visualNeeds: [],
          uncertainties: [],
        },
      ],
    ...
    const result = await runPiExtraction({
      split: "train",
      ids: "case-a",
      "out-tag": "pi-holistic-timeline-ledger-multi-fallback-fixture",
      mode: "holistic-draft",
      "source-packet-only": true,
      "compact-source-packet": true,
      "visual-frames": false,
    ...
    expect(result.failures).toBe(0);
    const outDir = path.join(caseDir, "runs/pi-holistic-timeline-ledger-multi-fallback-fixture");
    const manifest = JSON.parse(readFileSync(path.join(outDir, "file-access-manifest.json"), "utf8"));
    const understandingUsage = JSON.parse(readFileSync(path.join(outDir, "video-understanding-usage.json"), "utf8"));
    const understandingAudit = JSON.parse(readFileSync(path.join(outDir, "video-understanding-audit.json"), "utf8"));
    const draftPrompt = readFileSync(path.join(outDir, "holistic-draft-prompt.txt"), "utf8");

    expect(manifest.holisticVideoTimelineLedgerRequested).toBe(true);
    expect(manifest.holisticVideoTimelineLedgerEnabled).toBe(false);
    expect(manifest.holisticVideoTimelineLedgerFallback).toBe(true);
    expect(manifest.holisticIntegratedUnderstandingRequested).toBe(true);
    expect(manifest.holisticIntegratedUnderstandingStageEnabled).toBe(true);
    expect(manifest.holisticIntegratedUnderstandingEnabled).toBe(false);
    expect(manifest.stages.map((stage: { name: string }) => stage.name)).toEqual(["video-understanding", "holistic-draft"]);
    expect(understandingUsage).toMatchObject({
      usable: false,
      rejectedStoryCount: 1,
      rejectedStories: [{
        reasons: expect.arrayContaining(["multi_candidate_understanding_injection_disabled"]),
      }],
    });
    expect(understandingAudit).toMatchObject({
      summary: {
        storyCount: 1,
  ...
  it("validates recipe boundary plans as source-ordered candidate partitions", async () => {
    const {
      normalizeRecipeBoundaryPlan,
      assertValidRecipeBoundaryPlan,
    } = await import(holisticModuleUrl);
    const candidates = Array.from({ length: 6 }, (_, index) => ({
      candidateId: `s${index + 1}-w1`,
      title: `구간 ${index + 1}`,
      timeRange: { startSec: index * 30, endSec: (index + 1) * 30 },
    }));
    const allowedEvidenceRefs = ["title", "description:1", "description:2", "event:e1"];
    const normalized = normalizeRecipeBoundaryPlan({
      recipeUnits: [{
        recipeUnitId: "r1",
        title: "마들렌",
    ...
    expect(() => assertValidRecipeBoundaryPlan(normalized, {
      allowedCandidateIds: candidates.map((candidate) => candidate.candidateId),
      allowedEvidenceRefs,
    })).not.toThrow();
    expect(normalized.recipeUnits).toHaveLength(1);
    expect(normalized.recipeUnits[0].timeRange).toMatchObject({ startSec: 0, endSec: 180 });

    expect(() => assertValidRecipeBoundaryPlan({
      ...normalized,
      recipeUnits: [{
        ...normalized.recipeUnits[0],
        candidateIds: ["s1-w1", "s1-w1"],
      }],
    }, {
      allowedCandidateIds: candidates.map((candidate) => candidate.candidateId),
      allowedEvidenceRefs,
    })).toThrow(/duplicate candidate assignment/u);

    expect(() => assertValidRecipeBoundaryPlan({
      ...normalized,
      recipeUnits: [{
        ...normalized.recipeUnits[0],
        candidateIds: ["s1-w1", "unknown"],
      }],
      skippedCandidates: candidates.slice(1).map((candidate) => ({
        candidateId: candidate.candidateId,
        reasonCode: "not_part_of_recipe",
        evidence: [],
      })),
    }, {
      allowedCandidateIds: candidates.map((candidate) => candidate.candidateId),
      allowedEvidenceRefs,
    })).toThrow(/unknown candidateId/u);

    expect(() => assertValidRecipeBoundaryPlan({
      ...normalized,
      recipeUnits: [{
        ...normalized.recipeUnits[0],
        dishIdentityEvidence: ["description:999"],
      }],
    }, {
      allowedCandidateIds: candidates.map((candidate) => candidate.candidateId),
      allowedEvidenceRefs,
  ...
  it("keeps candidate integrated brief scoped to the current recipe unit", async () => {
    const {
    ...
    const payload = buildCandidateIntegratedBrief({
      recipeUnitUnderstandingState: state,
      candidateId: "r1",
    ...
    const prompt = buildCandidateFirstHolisticDraftPrompt({
      ...
      candidateIntegratedBrief: payload?.brief,
    });

    expect(payload?.brief.candidateId).toBe("r1");
    expect(prompt).toContain("[CANDIDATE_INTEGRATED_BRIEF]");
    expect(prompt).not.toContain("[RECIPE_UNIT_UNDERSTANDING_STATE]");
    expect(prompt).not.toContain("[CANDIDATE_UNDERSTANDING_AUDIT]");
  ...
  it("passes source-backed amounts through the candidate integrated brief contract", async () => {
    const {
      buildCandidateFirstHolisticDraftPrompt,
      buildCandidateIntegratedBrief,
      buildRecipeUnitUnderstandingState,
    ...
    expect(payload?.brief.sourceBackedAmounts).toEqual([
      expect.objectContaining({
        ingredient: "양파",
        amount: "1",
        unit: "개",
  ...
  it("trims large candidate integrated brief into the default prompt budget while preserving minimum memory", async () => {
    const {
      buildCandidateIntegratedBrief,
    } = await import(holisticModuleUrl);
    const longReason = "아주 긴 설명 ".repeat(80);
    const state = {
      schemaVersion: 1,
      kind: "recipe-unit-understanding-state",
      videoId: "case-a",
      units: [{
        ...
        dishIdentity: {
          ...
          name: "긴 설명이 많은 레시피",
          summary: longReason,
          evidence: ["description:1", "transcript:1s", "event:e1"],
          uncertainties: [longReason],
        },
        ingredientIdentityState: Array.from({ length: 14 }, (_, index) => ({
          surfaceName: `재료${index}`,
          resolvedName: `재료${index}`,
          nameStatus: index < 10 ? "source_named" : "generic_visual_descriptor",
          finalNameAllowed: index < 10,
          role: longReason,
          evidence: ["description:1", "transcript:1s", "event:e1"],
          unresolvedQuestion: longReason,
        })),
        sourceBackedAmounts: Array.from({ length: 8 }, (_, index) => ({
          ingredient: `재료${index}`,
        ...
        coreStepFlow: Array.from({ length: 10 }, (_, index) => ({
          order: index + 1,
          action: `${index + 1}번째 단계 ${longReason}`,
          evidence: ["event:e1", "transcript:1s"],
        })),
        unresolvedIdentityQuestions: [longReason, longReason],
      }],
    };
    ...
    expect(payload?.bytes).toBeLessThanOrEqual(1400);
    expect(payload?.budgetExceeded).toBe(false);
    expect(payload?.truncated).toBe(true);
    expect(payload?.brief.sourceBackedAmounts.length).toBeGreaterThanOrEqual(1);
    expect(payload?.brief.coreStepFlow.length).toBeGreaterThanOrEqual(1);
    expect(payload?.brief.understandingOrientation.rawStoryInjected).toBe(false);
  });
  ...
  it("uses recipe boundary units instead of raw timeline windows for candidate-first drafts", async () => {
    ...
    writeJson(timelineResponsePath, {
      events: Array.from({ length: 6 }, (_, index) => ({
        ...
        segmentId: `s${index + 1}`,
        timeRange: { startSec: index * 30, endSec: (index + 1) * 30 },
        action: ["재료 소개", "버터 녹이기", "반죽 섞기", "팬닝", "굽기", "완성"][index],
        visibleIngredients: index < 5 ? ["버터", "밀가루"] : ["마들렌"],
        candidateAssignments: [{ candidateId: `storyboard-${index + 1}`, status: "supporting", reason: "same madeleine flow" }],
        evidence: [`description:${index + 1}`],
        confidence: 0.8,
      })),
    });
    writeJson(understandingResponsePath, {
      globalStory: "하나의 마들렌을 처음부터 끝까지 만드는 영상이다.",
      dishStories: Array.from({ length: 6 }, (_, index) => ({
        candidateId: `storyboard-${index + 1}`,
        title: "마들렌",
        plainStory: "마들렌 조리의 한 단계이다.",
        mainIngredients: ["버터", "밀가루"],
        stepOutline: [["재료 소개", "버터 녹이기", "반죽 섞기", "팬닝", "굽기", "완성"][index]],
        sourceRefs: [`description:${index + 1}`, `event:e${index + 1}`],
        confidence: 0.8,
      })),
      crossDishNotes: ["새 요리 시작 신호가 없다."],
      uncertainties: [],
    });
    writeJson(boundaryResponsePath, {
      recipeUnits: [{
        recipeUnitId: "r1",
        title: "마들렌",
        candidateIds: ["storyboard-1", "storyboard-2", "storyboard-3", "storyboard-4", "storyboard-5", "storyboard-6"],
        dishIdentityEvidence: ["title", "description:1", "event:e1"],
        stageSummary: ["재료 소개", "반죽", "굽기", "완성"],
        reason: "새 레시피 시작 신호 없이 같은 마들렌 조리가 이어진다.",
        confidence: 0.8,
      }],
      skippedCandidates: [],
      uncertainties: [],
    ...
    const result = await runPiExtraction({
      split: "train",
      ids: "case-a",
      "out-tag": "pi-holistic-boundary-unit-fixture",
      mode: "holistic-draft",
      "source-packet-only": true,
      "compact-source-packet": true,
      "visual-frames": false,
      ...
      executePi: async (command: string[]) => {
        const promptPath = command[command.length - 1].replace(/^@/u, "");
        if (promptPath.includes("recipe-unit-drafts/01-r1")) {
          return {
            stdout: JSON.stringify({
              recipes: [{
                candidateId: "r1",
                title: "마들렌",
                timeRange: { startSec: 0, endSec: 180, basis: "recipe-boundary-plan" },
                ingredients: [
                ...
                steps: [
                  ...
                  { text: "반죽을 섞어 굽는다.", evidence: ["event:e3", "event:e5"], confidence: 0.8 },
                ],
                visualNeeds: [],
                uncertainties: [],
              }],
    ...
    expect(result.failures).toBe(0);
    const outDir = path.join(caseDir, "runs/pi-holistic-boundary-unit-fixture");
    const output = JSON.parse(readFileSync(path.join(outDir, "result.json"), "utf8"));
    const manifest = JSON.parse(readFileSync(path.join(outDir, "file-access-manifest.json"), "utf8"));
    ...
    expect(candidateDrafts.candidates[0].candidateIntegratedBriefInjected).toBe(true);
    expect(candidateDrafts.candidates[0].candidateIntegratedBriefBytes).toBeGreaterThan(0);
    expect(candidateDrafts.candidates[0].candidateIntegratedBriefBudgetExceeded).toBe(false);
    expect(candidateDrafts.candidates[0].candidateIntegratedBriefFailOpen).toBe(false);
    ...
    expect(candidateDrafts.candidates[0].recipeUnitUnderstandingStatePromptSection).toBe("CANDIDATE_INTEGRATED_BRIEF");
    ...
    expect(Math.abs(candidateDrafts.candidates[0].candidatePromptDeltaBytes)).toBeLessThanOrEqual(4000);
    ...
    expect(unitPrompt).toContain("[CANDIDATE_INTEGRATED_BRIEF]");
    expect(unitPrompt).not.toContain("[CANDIDATE_UNDERSTANDING_AUDIT]");
    expect(unitPrompt).not.toContain("[RECIPE_UNIT_UNDERSTANDING_STATE]");
    expect(unitPrompt).toContain("\"kind\": \"candidate-integrated-brief\"");
    ...
    expect(unitPrompt).toContain("\"sourceNamedIngredientNames\"");
  ...
  it("fails open when recipe unit understanding state prompt delta exceeds budget", async () => {
    ...
    expect(candidateDrafts.candidates[0].recipeUnitUnderstandingStatePromptGuardEnabled).toBe(true);
    expect(candidateDrafts.candidates[0].candidateIntegratedBriefInjected).toBe(false);
    expect(candidateDrafts.candidates[0].candidateIntegratedBriefBudgetExceeded).toBe(true);
    expect(candidateDrafts.candidates[0].candidateIntegratedBriefFailOpen).toBe(true);
  ...
  it("does not silently fall back to raw candidates when recipe boundary validation fails", async () => {
    ...
    const timelineResponsePath = path.join(workdir, "fixtures/video-timeline-response.json");
    const understandingResponsePath = path.join(workdir, "fixtures/video-understanding-response.json");
    const boundaryResponsePath = path.join(workdir, "fixtures/recipe-boundary-bad-response.json");
    const source = makeSource("case-a");
    source.video.durationSeconds = 60;
    source.video.title = "두 구간 한 요리";
    source.video.description = ["00:00 첫 구간", "00:30 둘째 구간"].join("\n");
    writeJson(path.join(caseDir, "source.json"), source);
    writeJson(path.join(caseDir, "golden.json"), { shouldNotRead: true });
    writeJson(timelineResponsePath, {
      events: [
        {
          ...
          timeRange: { startSec: 0, endSec: 30 },
          action: "첫 구간",
          candidateAssignments: [{ candidateId: "storyboard-1", status: "supporting", reason: "first" }],
          evidence: ["description:1"],
          confidence: 0.8,
        },
        {
          ...
          timeRange: { startSec: 30, endSec: 60 },
          action: "둘째 구간",
          candidateAssignments: [{ candidateId: "storyboard-2", status: "supporting", reason: "second" }],
          evidence: ["description:2"],
          confidence: 0.8,
        },
      ],
    ...
    writeJson(boundaryResponsePath, {
      recipeUnits: [{
        ...
        title: "첫 구간",
        candidateIds: ["storyboard-1"],
        dishIdentityEvidence: ["description:1"],
      }],
      skippedCandidates: [],
      uncertainties: [],
    });
    ...
    const result = await runPiExtraction({
      split: "train",
      ids: "case-a",
      "out-tag": "pi-holistic-boundary-invalid-fixture",
      mode: "holistic-draft",
      "source-packet-only": true,
      "compact-source-packet": true,
      "visual-frames": false,
      ...
      projectRoot: workdir,
      executePi: async () => {
        throw new Error("candidate draft should not run after invalid boundary plan");
      },
    });

    expect(result.failures).toBe(1);
    const outDir = path.join(caseDir, "runs/pi-holistic-boundary-invalid-fixture");
    const failure = JSON.parse(readFileSync(path.join(outDir, "recipe-boundary-failure.json"), "utf8"));
    expect(failure.code).toBe("recipe_boundary_plan_failed");
    expect(failure.message).toContain("missing candidate assignment");
    expect(() => readFileSync(path.join(outDir, "candidate-drafts.json"), "utf8")).toThrow();
  });
  ...
  it("runs opt-in integrated video understanding before holistic drafting", async () => {
    ...
    const understandingUsage = JSON.parse(readFileSync(path.join(outDir, "video-understanding-usage.json"), "utf8"));
    const understandingAudit = JSON.parse(readFileSync(path.join(outDir, "video-understanding-audit.json"), "utf8"));
    const draftSourcePacket = JSON.parse(readFileSync(path.join(outDir, "holistic-draft-source-packet.json"), "utf8"));
    ...
    expect(manifest.holisticIntegratedUnderstandingUsable).toBe(true);
    expect(manifest.holistic.integratedUnderstandingEnabled).toBe(true);
    expect(understandingUsage).toMatchObject({
      usable: true,
      acceptedStoryCount: 1,
    ...
    expect(understandingAudit).toMatchObject({
      ...
      summary: {
        storyCount: 1,
        orientationStoryCount: 1,
      },
      storyAudits: [{
        candidateId: "whole",
        draftRole: "orientation",
        supportedRefs: expect.arrayContaining(["description:2", "description:3", "event:e1"]),
        supportedMainIngredients: expect.arrayContaining(["양파", "간장"]),
      }],
    });
    expect(draftSourcePacket.videoUnderstandingHints).toBeUndefined();
    expect(draftSourcePacket.candidateSourcePackets[0].understanding).toBeUndefined();
    expect(manifest.stages.map((stage: { name: string }) => stage.name)).toEqual([
      "video-timeline",
      "video-understanding",
      "holistic-draft",
    ...
    expect(draftPrompt).toContain("[VIDEO_UNDERSTANDING_AUDIT]");
    expect(draftPrompt).toContain("source-aligned");
  ...
  it("skips weak integrated video understanding before holistic drafting", async () => {
    const { runPiExtraction } = await import(runnerModuleUrl);
    const caseDir = path.join(workdir, "notebooks/recipe_loop_data/train/case-a");
    const timelineResponsePath = path.join(workdir, "fixtures/video-timeline-response.json");
    const understandingResponsePath = path.join(workdir, "fixtures/video-understanding-response.json");
    ...
    writeJson(understandingResponsePath, {
      globalStory: "근거가 부족한 통합 이해다.",
      dishStories: [{
        candidateId: "unknown",
        title: "상상 요리",
        plainStory: "없는 후보를 근거 없이 요약한 흐름이다.",
        timeRange: { startSec: 0, endSec: 60, basis: "video-timeline" },
        mainIngredients: ["감자"],
        stepOutline: ["감자를 굽는다"],
        sourceRefs: ["description:999"],
        uncertainties: [],
        confidence: 0.8,
      }],
      crossDishNotes: [],
    ...
    const result = await runPiExtraction({
      split: "train",
      ids: "case-a",
      "out-tag": "pi-holistic-integrated-understanding-weak-fixture",
      mode: "holistic-draft",
      "source-packet-only": true,
      "compact-source-packet": true,
      "visual-frames": true,
    ...
    expect(result.failures).toBe(0);
    const outDir = path.join(caseDir, "runs/pi-holistic-integrated-understanding-weak-fixture");
    const manifest = JSON.parse(readFileSync(path.join(outDir, "file-access-manifest.json"), "utf8"));
    const understanding = JSON.parse(readFileSync(path.join(outDir, "video-understanding.json"), "utf8"));
    const understandingUsage = JSON.parse(readFileSync(path.join(outDir, "video-understanding-usage.json"), "utf8"));
    const understandingAudit = JSON.parse(readFileSync(path.join(outDir, "video-understanding-audit.json"), "utf8"));
    ...
    expect(manifest.holisticIntegratedUnderstandingEnabled).toBe(true);
    expect(manifest.holisticIntegratedUnderstandingUsable).toBe(false);
    expect(understanding.dishStories).toEqual([]);
    expect(understanding.uncertainties).toContain("video understanding was not injected into draft because no story passed the source-backed quality gate");
    expect(understandingUsage).toMatchObject({
      usable: false,
    ...
    expect(understandingUsage.rejectedStories[0].reasons).toEqual(
      expect.arrayContaining(["unknown_candidate_id", "missing_supported_source_refs"]),
    );
    expect(draftSourcePacket.videoUnderstandingHints).toBeUndefined();
    expect(draftPrompt).not.toContain("[VIDEO_UNDERSTANDING]");
    expect(draftPrompt).not.toContain("없는 후보를 근거 없이 요약한 흐름이다.");
    expect(manifest.stages.map((stage: { name: string }) => stage.name)).toEqual([
      "video-timeline",
      "video-understanding",
      "holistic-draft",
  ...
  it("logs but does not inject integrated video understanding for multi-candidate timelines", async () => {
    const { selectUsableVideoUnderstanding } = await import(holisticModuleUrl);
    const selected = selectUsableVideoUnderstanding({
      kind: "video-understanding",
      videoId: "case-multi",
      globalStory: "여러 요리가 있는 영상이다.",
      dishStories: [{
        candidateId: "r1",
        title: "양파 볶음",
        plainStory: "양파를 썰고 팬에 볶는 흐름이다.",
        mainIngredients: ["양파"],
        stepOutline: ["양파를 썬다", "팬에 볶는다"],
        sourceRefs: ["description:1", "event:e1"],
        confidence: 0.8,
      }],
      uncertainties: [],
    }, {
      refs: ["description:1", "event:e1"],
      candidateTimelineIndex: {
        candidates: [
          { candidateId: "r1" },
          { candidateId: "r2" },
      ...
      timelineEvents: [{ ref: "event:e1" }],
    });

    expect(selected.usage).toMatchObject({
      usable: false,
      acceptedStoryCount: 0,
      rejectedStoryCount: 1,
      reason: "video understanding was logged but not injected because multi-candidate story injection is not yet reliable enough",
    });
    expect(selected.usage.rejectedStories[0].reasons).toContain("multi_candidate_understanding_injection_disabled");
    expect(selected.understanding.dishStories).toEqual([]);
    expect(selected.audit).toMatchObject({
      ...
      storyAudits: [{
        ...
        revisionNotes: expect.arrayContaining([
          "다중 후보 understanding 주입은 아직 비활성화되어 draft에는 방향키로 넣지 않는다.",
        ]),
      }],
    });
  });

  it("audits video understanding ingredients without treating unsupported story parts as evidence", async () => {
    const { auditVideoUnderstandingAlignment } = await import(holisticModuleUrl);
    const audit = auditVideoUnderstandingAlignment({
      kind: "video-understanding",
      videoId: "case-a",
      globalStory: "양파 요리 흐름이다.",
      dishStories: [{
        candidateId: "whole",
        title: "양파 볶음",
        plainStory: "양파를 썰고 간장으로 간하지만 감자는 확인되지 않는다.",
        mainIngredients: ["양파", "간장", "감자"],
        stepOutline: ["양파를 썬다", "감자를 굽는다"],
        sourceRefs: ["description:2", "event:e1", "description:999"],
        confidence: 0.8,
      }],
    }, {
      refs: ["description:2", "event:e1"],
      entries: [
        { ref: "description:2", type: "description", text: "양파 1/2개와 간장 1큰술" },
      ],
      timelineEvents: [
        { ref: "event:e1", type: "timeline-event", text: "양파를 썰고 간장을 넣는다 / ingredients:양파, 간장" },
      ],
      candidateSourcePackets: [{
        candidateId: "whole",
        sourceEntries: [
    ...
    expect(audit.storyAudits[0]).toMatchObject({
      candidateId: "whole",
      supportedRefs: ["description:2", "event:e1"],
      unsupportedRefs: ["description:999"],
      supportedMainIngredients: ["양파", "간장"],
      unsupportedMainIngredients: ["감자"],
    });
    expect(audit.storyAudits[0].stepAlignment).toEqual([
      expect.objectContaining({
        step: "양파를 썬다",
        status: "source-aligned",
        matchedRefs: expect.arrayContaining(["event:e1"]),
      }),
      expect.objectContaining({
        step: "감자를 굽는다",
        status: "weak",
    ...
    expect(audit.storyAudits[0].revisionNotes).toContain("source에서 확인되지 않는 재료는 draft에서 확정하지 않는다: 감자");
  });
  ...
  it("fails open from video understanding timeout and still writes a candidate-first result", async () => {
    const { runPiExtraction } = await import(runnerModuleUrl);
    const caseDir = path.join(workdir, "notebooks/recipe_loop_data/train/case-a");
    const timelineResponsePath = path.join(workdir, "fixtures/video-timeline-response.json");
    const boundaryResponsePath = path.join(workdir, "fixtures/recipe-boundary-response.json");
    ...
    const timeoutError = new Error("fixture video understanding timeout") as Error & { piExecution?: Record<string, unknown> };
    timeoutError.piExecution = {
      code: null,
      signal: "SIGTERM",
      killed: true,
      timedOut: true,
    ...
    const result = await runPiExtraction({
      split: "train",
      ids: "case-a",
      "out-tag": "pi-video-understanding-timeout-fallback-fixture",
      mode: "holistic-draft",
      "source-packet-only": true,
      "compact-source-packet": true,
      "visual-frames": false,
      ...
      "holistic-enable-recipe-unit-working-memory": true,
      "holistic-enable-recipe-unit-understanding-state": true,
      "holistic-video-understanding-timeout-ms": "1",
      "holistic-video-timeline-response-json": timelineResponsePath,
      "holistic-recipe-boundary-response-json": boundaryResponsePath,
    }, {
      projectRoot: workdir,
      executePi: async (command: string[]) => {
        const promptPath = command[command.length - 1].replace(/^@/u, "");
        if (promptPath.includes("video-understanding-prompt.txt")) {
          throw timeoutError;
        }
        if (promptPath.includes("recipe-unit-drafts/01-r1")) {
          return {
            stdout: JSON.stringify({
              recipes: [{
                ...
                ingredients: [
                  { name: "버터", amount: "1", unit: "개", amountBasis: "stated", evidence: ["description:2"] },
                  { name: "밀가루", amount: "100", unit: "g", amountBasis: "stated", evidence: ["description:4"] },
                ],
                steps: [
                  { text: "버터를 녹인다.", evidence: ["event:e1"], confidence: 0.8 },
                  { text: "반죽을 섞는다.", evidence: ["event:e2"], confidence: 0.8 },
                ],
                visualNeeds: [],
                uncertainties: [],
              }],
    ...
    expect(result.failures).toBe(0);
    const outDir = path.join(caseDir, "runs/pi-video-understanding-timeout-fallback-fixture");
    const output = JSON.parse(readFileSync(path.join(outDir, "result.json"), "utf8"));
    const failure = JSON.parse(readFileSync(path.join(outDir, "video-understanding-failure.json"), "utf8"));
    const usage = JSON.parse(readFileSync(path.join(outDir, "video-understanding-usage.json"), "utf8"));
    const manifest = JSON.parse(readFileSync(path.join(outDir, "file-access-manifest.json"), "utf8"));
    const candidateDrafts = JSON.parse(readFileSync(path.join(outDir, "candidate-drafts.json"), "utf8"));
    ...
    expect(failure).toMatchObject({
      ...
      reason: "video_understanding_timeout",
    });
    ...
    expect(manifest.holisticVideoUnderstandingTimeoutCount).toBe(1);
    expect(manifest.holisticVideoUnderstandingFailureCount).toBe(1);
    expect(candidateDrafts.candidates[0].candidateIntegratedBriefInjected).toBe(true);
  });
