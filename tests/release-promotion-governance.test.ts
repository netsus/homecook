...
describe("release promotion governance docs", () => {
  it("routes release promotion authority to the canonical runbook", () => {
    ...
    expect(releaseRunbook).toContain("promotion lock, adapter, install, Docker, LaunchAgent, DB, pointer mutation은 0");
    expect(releaseRunbook).toContain("production snapshot도 실패 시 자동 삭제하지 않는다");
    expect(releaseRunbook).toContain("frozen scratch");
  ...
  it("separates untagged exact-SHA rehearsal from production authority", () => {
    ...
    expect(rehearsalContract).toContain("상태: **canonical / post-#1475 server-integration repair author complete / fresh independent review pending**");
