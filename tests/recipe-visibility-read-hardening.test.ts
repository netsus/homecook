...
describe("recipe visibility read hardening migration", () => {
  ...
  it("keeps first-404 cleanup durable and lease-bound without activating Storage", () => {
    ...
    expect(sql).toMatch(
      /revoke all on function public\.claim_recipe_image_cleanup\(\s*integer,\s*uuid,\s*timestamp with time zone\s*\)[\s\S]*from public, anon, authenticated, service_role[\s\S]*grant execute on function public\.claim_recipe_image_cleanup\(\s*integer,\s*uuid,\s*timestamp with time zone\s*\)[\s\S]*to service_role/i,
    );
    for (const functionName of [
