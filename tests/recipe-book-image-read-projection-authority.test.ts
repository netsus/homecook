...
describe("recipe book image read projection authority", () => {
  it("projects durable cover identity without issuing or persisting read URLs", async () => {
    ...
    expect(migration).toMatch(
      /create or replace function public\.read_recipe_book_image_projections\(\s*p_book_ids uuid\[\]\s*\)/i,
    );
    expect(migration).toMatch(
      /returns table\s*\([\s\S]*book_id uuid[\s\S]*legacy_cover_image_url text[\s\S]*image_object_id uuid[\s\S]*bucket_id text[\s\S]*object_path text[\s\S]*visibility text[\s\S]*state text/i,
    );
