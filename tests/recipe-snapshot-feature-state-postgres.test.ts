...
import { buildFeatureStateMutation, FEATURE_STATE_SELECT } from "../scripts/lib/recipe-snapshot-feature-state.mjs";
...
function sql(query: string) {
  return run(join(bin, "psql"), ["-X", "-qAt", "-v", "ON_ERROR_STOP=1", "-h", root, "-p", "55439", "-U", "postgres", "-d", "postgres"], query);
}
