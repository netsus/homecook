...
const IMAGE_LEGACY_VISIBILITY_MIGRATION_MANIFEST_PATH =
...
describe("recipe visibility security function inventory", () => {
  ...
  it("classifies the lifecycle completion candidate read authority", () => {
    expect(
      existsSync(IMAGE_LIFECYCLE_COMPLETION_CANDIDATE_MANIFEST_PATH),
    ).toBe(true);
    if (
      !existsSync(IMAGE_LIFECYCLE_COMPLETION_CANDIDATE_MANIFEST_PATH)
    ) {
    ...
    const manifest = JSON.parse(
      readFileSync(
        IMAGE_LIFECYCLE_COMPLETION_CANDIDATE_MANIFEST_PATH,
        "utf8",
      ),
    ) as {
      functions: Array<Record<string, unknown>>;
  ...
  it("classifies the legacy visibility migration authorities", () => {
    expect(existsSync(IMAGE_LEGACY_VISIBILITY_MIGRATION_MANIFEST_PATH)).toBe(
      true,
    );
    if (!existsSync(IMAGE_LEGACY_VISIBILITY_MIGRATION_MANIFEST_PATH)) {
      return;
    }

    const manifest = JSON.parse(
      readFileSync(IMAGE_LEGACY_VISIBILITY_MIGRATION_MANIFEST_PATH, "utf8"),
    ) as {
      functions: Array<Record<string, unknown>>;
    };
  ...
  it("validates every additive manifest without a live database", () => {
    ...
    expect(output).toContain(
      "recipe-image-legacy-visibility-migration:2 pre-deployment additive application functions",
    );
    expect(output).toContain(
