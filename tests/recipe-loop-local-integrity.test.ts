...
`);
  ...
  it("keeps production extraction free of recipe-specific recovery patches", () => {
    ...
    expect(source).not.toMatch(
      /recoverLowTailVisualDefaults|recoverTitleAnchoredIngredientStepMentions|CANDIDATE_TOKEN_RULES|allowedIngredientNamesForCandidate|interpretationNotesForCandidate/,
    );
    expect(source).not.toContain("김치와 돼지고기를 3대 1 비율로 준비한다.");
    expect(source).not.toMatch(/titleHint가 맥적이면|\"recipeTitle\": \"닭갈비\"/);
  });

  it("does not invent amounts for source-mentioned soup seasonings", async () => {
    const { extractRecipeFromSources } = await import(recipeExtractionModuleUrl);
    const result = await extractRecipeFromSources(
      {
        video: {
          ...
          title: "채소국",
          description: "설탕과 양파를 넣고 고춧가루로 색을 낸 뒤 소금이나 새우젓으로 간을 맞춘다.",
        },
        transcript: null,
        authorComments: [],
        youtubeUrl: null,
        ...
        llm: {
          generate: async () => ({
            ...
            json: {
              recipes: [{
                title: "채소국",
                ingredients: [{ name: "채소", amount: null, unit: null, amountBasis: null }],
    ...
    const recovered = result.recipes[0].ingredients.filter((ingredient: { name: string }) =>
      ["설탕", "양파", "고춧가루", "새우젓"].includes(ingredient.name));
    expect(recovered).toHaveLength(4);
    expect(recovered).toEqual(expect.arrayContaining([
      expect.objectContaining({ name: "설탕", amount: null, unit: null, amountBasis: null }),
      expect.objectContaining({ name: "양파", amount: null, unit: null, amountBasis: null }),
      expect.objectContaining({ name: "고춧가루", amount: null, unit: null, amountBasis: null }),
      expect.objectContaining({ name: "새우젓", amount: null, unit: null, amountBasis: null }),
    ]));
  });

  it("does not duplicate a generic seasoning when a specific form already exists", async () => {
    const { extractRecipeFromSources } = await import(recipeExtractionModuleUrl);
    const result = await extractRecipeFromSources(
      {
        video: { videoId: "seasoning-form", title: "채소국", description: "고춧가루를 넣고 끓인다." },
        transcript: null,
        authorComments: [],
        youtubeUrl: null,
      },
        ...
        llm: {
          generate: async () => ({
            ...
            json: {
              recipes: [{
                title: "채소국",
                ingredients: [{ name: "고운 고춧가루", amount: null, unit: null, amountBasis: null }],
                steps: ["고운 고춧가루를 넣고 끓인다."],
              }],
            },
          }),
        },
    ...
    expect(result.recipes[0].ingredients.filter((ingredient: { name: string }) =>
      ingredient.name.includes("고춧가루"))).toHaveLength(1);
  });
});
