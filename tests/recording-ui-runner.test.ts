...
describe("R2 UI isolated migration ordering", () => {
  it("removes all four from fresh startup and returns the exact dependency order", async () => {
    const { root, directory } = await fixture(4);
    expect(await stageUiRound2Migrations(root)).toEqual(["migration-0", "migration-1", "migration-2", "migration-3"]);
