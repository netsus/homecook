...
const finalizerPath = "scripts/finalize-cooking-meal-log-stage4-evidence.mjs";
...
async function listenLoopbackJsonServer(
  body: string,
  {
    status = 200,
  }: {
...
describe("cooking meal-log Stage 4 real-stack capture harness", () => {
  it("exposes an exact-head state proof finalizer", () => {
    expect(
      (stage4Isolated as unknown as Record<string, unknown>).finalizeStage4Proof,
    ).toBeTypeOf("function");
  });

  it("provides a create-only Stage 4 evidence finalizer CLI", () => {
    expect(existsSync(join(root, finalizerPath))).toBe(true);
    expect(read(finalizerPath)).toContain("finalizeStage4Proof");
  });

  it("uses the mobile-visible manual create title for capture readiness", () => {
    const source = read(scriptPath);
    const manualStart = source.indexOf('id: "MANUAL_RECIPE_CREATE"');
    const plannerStart = source.indexOf('id: "PLANNER_WEEK"');

    expect(manualStart).toBeGreaterThan(-1);
    expect(plannerStart).toBeGreaterThan(manualStart);
    expect(source.slice(manualStart - 100, plannerStart)).toContain(
      'expected: ["직접 등록"]',
    );
  });

  it("requires a canonical rehearsal-only activation run before the authority preflight", () => {
    expect(buildStage4CanonicalActivationSql).toBeTypeOf("function");
    expect(assertStage4CanonicalActivationOutput).toBeTypeOf("function");

    const sql = buildStage4CanonicalActivationSql();
    expect(sql.trimStart()).toMatch(/^begin;\s*create temporary table/iu);
    expect(sql.trimEnd()).toMatch(
      /select payload from pg_temp\.stage4_canonical_activation_result;\s*commit;$/iu,
    );
    expect(sql).toContain("set_config('request.jwt.claims', '{\"role\":\"service_role\"}', true)");
    expect(sql).toContain("begin_account_generation_cutover");
    expect(sql).toContain("stage_account_generation_cutover_owner");
    expect(sql).toContain("set_account_generation_cutover_snapshot");
    expect(sql).toContain("promote_account_generation_cutover");
    expect(sql).toContain("start_full_local_auth_cutover");
    expect(sql).toContain("expire_and_count_remote_auth_flows");
    expect(sql).toContain("activate_full_local_auth_authority");
    expect(sql).toContain("storage_terminal");
    expect(sql).toContain("owner_signal_union_zero");
    expect(sql).toContain(STAGE4_RESERVED_AUTH_ISSUER);
    expect(sql).not.toMatch(/update\s+private\.full_local_auth_control/iu);
    expect(sql).not.toMatch(/update\s+public\.account_generation_capability_state/iu);

    expect(assertStage4CanonicalActivationOutput([
      "BEGIN",
      "CREATE TABLE",
      "DO",
      JSON.stringify({
      ...
      cutover: {
        auth_cutover_epoch_after: 2,
        auth_cutover_epoch_before: 1,
        capability_revision_after: 3,
        capability_revision_before: 1,
        hmac_key_version_after: 2,
        hmac_key_version_before: 1,
      },
      guard_evidence: {
        owner_signal_union_zero: true,
        storage_terminal: true,
      },
      local_issuer: STAGE4_RESERVED_AUTH_ISSUER,
      mode: "rehearsal_only",
      owner_counts: {
        staged: 3,
        unexpected_outside_union: 0,
      },
      verification: {
    ...
    expect(() => assertStage4CanonicalActivationOutput(JSON.stringify({
      account_generation_capability: "generation_active",
      auth_authority: "local",
      auth_counts: {
        auth_only: 0,
    ...
    }))).toThrow(/auth flow|outstanding|rehearsal/iu);

    const runner = read(runnerPath);
    expect(runner).toContain("buildStage4CanonicalActivationSql");
    expect(runner).toContain("assertStage4CanonicalActivationOutput");
    expect(runner.indexOf("canonical-activation-rehearsal-complete"))
      .toBeLessThan(runner.indexOf("local-session-authority-ready"));
  });

  it("shares one Stage 4 attestation secret between the proxy and Next server", () => {
    const runner = read(runnerPath);
    expect(runner).toContain("const STAGE4_SESSION_ATTESTATION_SECRET =");
    expect(runner.match(/STAGE4_SESSION_ATTESTATION_SECRET/gu)?.length)
      .toBeGreaterThanOrEqual(3);
    expect(runner).not.toContain(
      "commandEnv.HOMECOOK_SESSION_ATTESTATION_HMAC_KEY_V1",
    );
  ...
  it("fails closed before browser capture when local session authority is inactive", () => {
    ...
    expect(assertRuntimeAuthority(
      "local|generation_active|https://auth.stage4.homecook.invalid/auth/v1\n",
    )).toEqual({
      account_generation_capability: "generation_active",
      auth_authority: "local",
  ...
  it("preserves the exact canonical activation blocker in a safe diagnostic failure", () => {
    expect(classifyStage4CanonicalActivationFailureOutput).toBeTypeOf(
      "function",
    );

    const safeFailure = classifyStage4CanonicalActivationFailureOutput({
      stderr: [
        "ERROR:  ACCOUNT_LIFECYCLE_MAINTENANCE",
        "CONTEXT:  PL/pgSQL function set_account_generation_cutover_snapshot(uuid,bigint,bigint,text,bigint,text,bigint,text,text,jsonb) line 3 at RAISE",
      ].join("\n"),
      stdout: "CREATE TABLE\n",
      timedOut: false,
    });
    ...
    expect(safeFailure).toMatchObject({
      ...
      message:
        "Stage 4 canonical activation blocked: ACCOUNT_LIFECYCLE_MAINTENANCE @ set_account_generation_cutover_snapshot",
    });
    expect(JSON.stringify(safeFailure)).not.toContain("CREATE TABLE");

    expect(classifyStage4StartFailure(Object.assign(new Error("ignored"), {
      safeFailure,
    }))).toEqual(safeFailure);
  });
  ...
  it("builds guarded PostgREST JWKS from loopback public auth keys plus one HS256 fallback key", async () => {
    const authJwks = {
      keys: [
        {
          alg: "ES256",
    ...
    const jwtSecret = "stage4-guarded-jwt-secret-0123456789abcdef";
    const authServer = await listenLoopbackJsonServer(JSON.stringify(authJwks));

    try {
      const jwksText = await buildStage4GuardedJwtVerificationJwks({
        authOrigin: authServer.url,
        jwtSecret,
      });
      const jwks = JSON.parse(jwksText);

      expect(authServer.requests).toEqual(["/auth/v1/.well-known/jwks.json"]);
      expect(jwks.keys).toEqual([
        ...
        },
        {
          ...
          kid: expect.stringMatching(/^stage4-guarded-hs256-/u),
          kty: "oct",
          use: "sig",
        },
      ]);
      expect(new Set(jwks.keys.map((key: { kid: string }) => key.kid)).size)
        .toBe(jwks.keys.length);
      expect(jwksText.includes("\n")).toBe(false);
    } finally {
  ...
  it.each([
    {
      expectedMessage: /loopback http only/iu,
      authOrigin: "https://auth.example.com",
      body: null,
      jwtSecret: "stage4-guarded-jwt-secret-0123456789abcdef",
      label: "remote auth origin",
    },
    {
      expectedMessage: /public verify keys/iu,
      authOrigin: null,
      body: JSON.stringify({
        keys: [{
          alg: "ES256",
          crv: "P-256",
          d: "private-stage4-sentinel",
          kid: "auth-es256",
          kty: "EC",
          use: "sig",
          x: "es256-x",
      ...
      sentinel: "private-stage4-sentinel",
    },
    {
      expectedMessage: /unique/iu,
      authOrigin: null,
      body: JSON.stringify({
        keys: [
          {
    ...
    },
    {
      ...
      jwtSecret: "stage4-guarded-jwt-secret-0123456789abcdef",
      label: "invalid verify key shape",
    },
  ])("rejects unsafe guarded JWKS input: $label", async ({
    authOrigin,
    body,
    expectedMessage,
    jwtSecret,
    ...
    if (authOrigin) {
      const failure = buildStage4GuardedJwtVerificationJwks({
        authOrigin,
        jwtSecret,
      });
      await expect(failure).rejects.toThrow(expectedMessage);
      if (sentinel) {
        await expect(failure.catch((error) => {
    ...
    const authServer = await listenLoopbackJsonServer(body ?? "{}");
    try {
      const failure = buildStage4GuardedJwtVerificationJwks({
        authOrigin: authServer.url,
        jwtSecret,
  ...
  it("writes guarded PostgREST with canonical JWKS instead of the raw shared secret", () => {
    const runner = read(runnerPath);
    const guardedStart = runner.indexOf("async function startStage4GuardedDataApi");
    const guardedEnd = runner.indexOf("function inspectStage4AuxiliaryContainers");
    const guardedSource = runner.slice(guardedStart, guardedEnd);

    expect(guardedSource).toContain("buildStage4GuardedJwtVerificationJwks");
    expect(guardedSource).toContain("PGRST_JWT_SECRET=${guardedVerificationJwks}");
    expect(guardedSource).not.toContain("PGRST_JWT_SECRET=${jwtSecret}");
  ...
  it("validates the server-side target attestation before capture", () => {
    const canonicalActivation = {
      account_generation_capability: "generation_active",
      auth_authority: "local",
      auth_counts: { auth_only: 0, intersection: 2, public_only: 0 },
      auth_flow_counts: { expired: 0, outstanding: 0 },
      cutover: {
        auth_cutover_epoch_after: 2,
        auth_cutover_epoch_before: 1,
        capability_revision_after: 3,
      ...
      local_issuer: STAGE4_RESERVED_AUTH_ISSUER,
      mode: "rehearsal_only",
      owner_counts: { staged: 2, unexpected_outside_union: 0 },
      verification: {
        auth_digest_verified: true,
        public_digest_verified: true,
        staged_owner_digest_verified: true,
    ...
    expect(projected.canonical_activation).not.toBe(canonicalActivation);
    expect(() => validateStage4TargetAttestation({
      ...valid,
      rehearsal_only: false,
    }, valid.app_origin)).toThrow(/rehearsal/iu);
    expect(() => validateStage4TargetAttestation({
      ...valid,
    ...
    }, valid.app_origin)).toThrow(/activation|rehearsal/iu);
    expect(() => validateStage4TargetAttestation({
      ...valid,
      qa_fixture_scope: [
    ...
    expect(() => validateStage4TargetAttestation({
      ...valid,
      auth_public_issuer: "https://example.supabase.co/auth/v1",
    }, valid.app_origin)).toThrow(/public Auth issuer|invalid/iu);
    expect(() => validateStage4TargetAttestation({
      ...valid,
  ...
  it("binds attestation to the exact official isolated Auth and Data environment", () => {
    ...
    expect(env).toMatchObject({
      AUTH_SUPABASE_EXPECTED_ISSUER: STAGE4_RESERVED_AUTH_ISSUER,
      AUTH_SUPABASE_JWKS_URL:
        "http://127.0.0.1:58101/auth/v1/.well-known/jwks.json",
      DATA_SUPABASE_URL: "http://127.0.0.1:4313",
      HOMECOOK_AUTH_AUTHORITY: "local",
      HOMECOOK_STAGE4_CAPTURE_MODE: "1",
  ...
  it("keeps the public issuer claim reserved while the internal Auth transport stays loopback", () => {
    const env = buildStage4ServerEnvironment({
      ambient: {
        AUTH_FLOW_HMAC_KEY: "stage4-isolated-auth-flow-hmac-key-20260821",
        HOMECOOK_ENABLE_ACCOUNT_QUARANTINE_QA_FIXTURE: "1",
      ...
      publicAuthUrl: STAGE4_RESERVED_AUTH_ORIGIN,
      serviceRoleKey: "isolated-service-role",
    } as Parameters<typeof buildStage4ServerEnvironment>[0] & {
    ...
    expect(env).not.toMatchObject({
      AUTH_SUPABASE_EXPECTED_ISSUER: "http://127.0.0.1:58101/auth/v1",
      NEXT_PUBLIC_AUTH_SUPABASE_URL: "https://auth.stage4.homecook.invalid",
    });
  });
  ...
  it("finalizes only exact-head state and private nondisclosure proof", () => {
    const sourceHeadSha = "a".repeat(40);
    const evidence = {
      kind: "playwright",
      path: ".artifacts/cml14/state-report.json",
      result: "passed",
      sha256: "b".repeat(64),
    ...
    const finalized = finalizeStage4Proof({
      captureManifest: {
        owner_boundary: {
          ...
          other_authenticated: true,
          private_nondisclosure: "pending",
        },
        quality: { quality_status: "passed" },
        real_local_stack: true,
        remote_linked_cloud_access: 0,
        source_head_sha: sourceHeadSha,
      },
      proofLedger: {
      ...
      stateMatrices: [{
        observed_state_candidate: "recipe-only",
        pending_states: ["loading", "recipe-only"],
        quality: { quality_status: "passed" },
        required_states: ["loading", "recipe-only"],
        screen: "HOME",
        source_head_sha: sourceHeadSha,
        verified_states: [],
      }],
    ...
    expect(finalized).toMatchObject({
      ...
      state_matrices: [{
        pending_states: [],
        screen: "HOME",
        verified_states: ["loading", "recipe-only"],
      }],
    });
  });

  it("rejects missing, stale, or unsafe Stage 4 proof", () => {
    const sourceHeadSha = "a".repeat(40);
    const base = {
      captureManifest: {
        owner_boundary: {
    ...
    expect(() => finalizeStage4Proof({
      ...base,
      proofLedger: {
        private_nondisclosure: privateProof,
        screens: { HOME: { states: { loading: [evidence] } } },
        source_head_sha: sourceHeadSha,
      },
    })).toThrow(/missing proof.*HOME:error/iu);
    ...
    expect(() => finalizeStage4Proof({
      ...
      proofLedger: {
        private_nondisclosure: privateProof,
        screens: {
          HOME: { states: { error: [evidence], loading: [evidence] } },
        },
        source_head_sha: "c".repeat(40),
      },
    })).toThrow(/source head/iu);
    ...
    expect(() => finalizeStage4Proof({
      ...
      proofLedger: {
        ...
        screens: {
          HOME: {
            states: {
              error: [{ ...evidence, path: "../secret.json" }],
              loading: [evidence],
            },
          },
        },
  ...
  it("retries one bounded INTERNAL_ERROR while the post-login profile settles", async () => {
    const responses = [
      {
        payload: {
          error: { code: "INTERNAL_ERROR", message: "safe public message" },
          success: false,
        },
        status: 500,
      },
  ...
  it("finishes both demo account bootstrap states before Stage 4 activation", () => {
    const seed = read("scripts/qa-seed-slices-01-05.mjs");
    expect(seed).toContain("async function markUserBootstrapComplete");
    expect(seed).toContain("user_bootstrap_version: 3");
    expect(seed).toContain("otherMyAddedBook");
    expect(seed).toContain("otherLikedBook");
    expect(seed).toContain("otherPlannerColumns");
    ...
    expect(seed.indexOf("await markUserBootstrapComplete(supabase, mainUserId);"))
      .toBeGreaterThan(seed.indexOf("const otherPlannerColumns"));
  });
});
