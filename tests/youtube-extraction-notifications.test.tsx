...
describe("YouTube extraction notification center", () => {
  ...
  it("resolves a server-cookie session before loading global notifications", async () => {
    render(
      <>
        <YoutubeExtractionNotificationTrigger />
        <YoutubeExtractionNotificationCenter
    ...
    expect(api.fetchYoutubeExtractionNotifications).toHaveBeenCalledWith("unseen-completed");
  });
  ...
  it("keeps a newly presented toast visible when a follow-up refresh reports it delivered", async () => {
    ...
    await waitFor(() => {
      expect(api.fetchYoutubeExtractionNotifications).toHaveBeenCalledTimes(2);
    });
    expect(screen.getByText("레시피 추출이 끝났어요")).toBeTruthy();
  });
  ...
  it("preserves the user's current focus when the closed center first mounts", async () => {
    const view = render(
      <>
        <button type="button">현재 페이지 작업</button>
        <button data-youtube-extraction-trigger="header" type="button">기존 알림</button>
      </>,
    );
    const currentAction = screen.getByRole("button", { name: "현재 페이지 작업" });
    currentAction.focus();
