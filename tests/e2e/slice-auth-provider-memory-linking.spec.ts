...
test("captures deterministic recent-provider, safe-error, and linked-provider evidence", async ({ page }) => {
  ...
  await captureTrackedEvidenceOnDemand(page, { path: join(evidenceDir, `LOGIN-recent-provider-${width}.png`), fullPage: true });
  ...
  await captureTrackedEvidenceOnDemand(page, { path: join(evidenceDir, `LOGIN-dialog-${width}.png`), fullPage: true });
  ...
  await captureTrackedEvidenceOnDemand(page, { path: join(evidenceDir, `LOGIN-safe-error-${width}.png`), fullPage: true });
  ...
  await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
  await captureTrackedEvidenceOnDemand(page, { path: join(evidenceDir, `MYPAGE-linked-error-${width}.png`), fullPage: true });
});
