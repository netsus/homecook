...
for (const q3 of ["pass", "eyeball", "track", "measure"] as const) {
  test(`live ${q3}: full flow, returning visitor and shared-cookie new tab`, async ({ page, context }, testInfo) => {
    ...
    await page.screenshot({ path: testInfo.outputPath("first-beta-form.png"), fullPage: true });
    ...
    await page.screenshot({ path: testInfo.outputPath("returning-result.png"), fullPage: true });
    ...
    await newTab.screenshot({ path: testInfo.outputPath("new-tab-beta-form.png"), fullPage: true });
...
for (const width of [320, 1280]) {
  test(`live ${width}px: full flow and reload`, async ({ page }, testInfo) => {
    ...
    await page.screenshot({ path: testInfo.outputPath(`beta-form-${width}.png`), fullPage: true });
  });
}
