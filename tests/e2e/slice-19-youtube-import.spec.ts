...
test.describe("Slice 19: YouTube Import", () => {
  ...
  test("oEmbed preview waits for explicit extraction before spending YouTube quota", async ({ page }) => {
    ...
    await expect(page.getByText("미리보기는 quota를 쓰지 않아요.")).toBeVisible();
  ...
  test("review: can add and remove ingredients", async ({ page }) => {
    ...
    const ingredientModal = await searchIngredient(page, "두부");
    await selectIngredient(ingredientModal, "두부");
  ...
  test("register button disabled without required fields", async ({ page }) => {
    ...
    const requirements = page.getByTestId("youtube-register-requirements").filter({ visible: true }).last();
  ...
  test("review: blocks save until needs-review and unresolved ingredients are resolved", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: YOUTUBE_REVIEW_HEADING })).toBeVisible({ timeout: 10000 });
    await expect(page.getByText("확인 필요한 재료", { exact: true }).filter({ visible: true }).last()).toBeVisible();
