...
const STAGE4_CAPTURE_NAMES = [
  ...
  "hero-d-390x844.png",
  "hero-d-1280x900.png",
  "hero-default-393x852.png",
  "result-homecook-passer-390x844.png",
  "result-eyeballing-master-390x844.png",
  "result-ingredient-tracker-390x844.png",
  "result-pro-measurer-390x844.png",
  "quiz-q1-390x844.png",
  "quiz-q2-390x844.png",
  "quiz-q3-390x844.png",
  "quiz-q4-390x844.png",
  "result-normal-eyeballing-master-390x844.png",
  "experience-1-390x844.png",
  "experience-1-loading-390x844.png",
  "experience-1-done-390x844.png",
  "experience-2-390x844.png",
  "experience-2-adjusted-390x844.png",
  "experience-3-390x844.png",
  "experience-3-confirmed-390x844.png",
  "experience-4-390x844.png",
  "experience-5-390x844.png",
  "planner-homecook-390x844.png",
  "packaged-food-390x844.png",
  "planner-complete-390x844.png",
  "beta-form-390x844.png",
  "beta-form-validation-error-390x844.png",
  "done-390x844.png",
  "quiz-q1-320x568.png",
  "quiz-q2-320x568.png",
  "quiz-q3-320x568.png",
  "quiz-q4-320x568.png",
  "turnstile-fail-closed-390x844.png",
  "reduced-motion-and-visible-focus.png",
  "planner-homecook-320x568.png",
  "planner-complete-320x568.png",
  "planner-homecook-393x852.png",
  "planner-complete-393x852.png",
] as const;
...
async function capture(browser: Browser, width: number, height: number, name: string, action?: (page: Page) => Promise<void>, routePath = MARKETING_BETA_PATH) {
  ...
  await expectStageImagesReady(page);
...
test.describe("marketing demand validation v2 /beta", () => {
  ...
  test("captures hero and result evidence", async ({ browser }, testInfo) => {
    ...
    for (const variant of ["default", "a", "b", "c", "d"] as const) {
      for (const [width, height, suffix] of [[320, 568, "320x568"], [390, 844, "390x844"], [1280, 900, "1280x900"]] as const) {
        ...
        await capture(browser, width, height, `hero-${variant}-${suffix}.png`, undefined, `${MARKETING_BETA_PATH}${query}`);
      }
    }
    await capture(browser, 393, 852, "hero-default-393x852.png");
    for (const result of ["homecook-passer", "eyeballing-master", "ingredient-tracker", "pro-measurer"] as const) {
      await capture(browser, 390, 844, `result-${result}-390x844.png`, undefined, `${MARKETING_BETA_PATH}?result=${result}`);
    }
  });
  ...
  test("captures narrow and responsive evidence and writes the manifest", async ({ browser }, testInfo) => {
    ...
    for (const [width, height, suffix] of [[320, 568, "320x568"], [393, 852, "393x852"]] as const) {
      await capture(browser, width, height, `planner-homecook-${suffix}.png`, async (page) => {
        await openMarketingLeadForm(page);
      ...
      await capture(browser, width, height, `planner-complete-${suffix}.png`, async (page) => {
        await openMarketingLeadForm(page);
        await page.getByRole("button", { name: "이전 화면" }).click();
        await expect(page.getByTestId("tomorrow-preview")).toBeVisible();
        for (const control of await page.getByRole("button", { name: /내일 .* 추가/ }).all()) await expect(control).toBeDisabled();
    ...
    const repositoryCaptures = STAGE4_CAPTURE_NAMES.map((captureName) => {
