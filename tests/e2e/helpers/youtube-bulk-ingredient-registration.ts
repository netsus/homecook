...
const BULK_EXTRACTION_ID = "ext-bulk-test";

const bulkExtractionDraft = {
  extraction_id: BULK_EXTRACTION_ID,
  title: "된장찌개 레시피",
  base_servings: 2,
...
export async function installBulkIngredientRoutes(page: Page) {
  ...
  await page.route("**/api/v1/recipes/youtube/extract", async (route) => {
    ...
    await route.fulfill({
      json: {
        success: true,
        data: bulkExtractionDraft,
        error: null,
      },
    });
  });

  await page.route(`**/api/v1/recipes/youtube/extractions/${BULK_EXTRACTION_ID}`, async (route) => {
    await route.fulfill({
      json: {
        success: true,
        data: {
          status: "draft",
          draft: bulkExtractionDraft,
          recipe_id: null,
          recipe_path: null,
        },
        error: null,
...
export async function openBulkReview(page: Page) {
  await page.goto(`${YOUTUBE_INGREDIENT_REGISTRATION_URL}?extractionId=${BULK_EXTRACTION_ID}`);
