    ...
    <article aria-label={itemTitle(item)} className="grid grid-cols-[48px_minmax(0,1fr)] gap-3 border-b border-[var(--wave1-border)] py-3 last:border-0 sm:grid-cols-[56px_minmax(0,1fr)] sm:py-4" data-youtube-job-id={item.job_id}>
      ...
      {destination || item.can_retry ? (
        <div className="col-span-2 flex min-w-0 flex-wrap gap-2 sm:col-start-2 sm:col-end-3">
          {destination ? (
            <Link className="inline-flex min-h-11 w-full min-w-0 items-center justify-center whitespace-normal break-keep rounded-full bg-[var(--brand-primary)] px-4 py-2 text-center text-sm font-bold leading-5 text-white sm:w-auto" href={destination}>
...
function YoutubeExtractionNotificationRuntime({
  ...
  const retainedToastDeliveryKeysRef = useRef(new Set<string>());
    ...
    () => items
      .filter((item) => item.seen_at === null
        && (item.delivered_at === null
          || retainedToastDeliveryKeysRef.current.has(item.delivery_key))
  ...
  const setUnseenItems = useCallback((nextItems: YoutubeExtractionNotificationItem[]) => {
    nextItems.forEach((item) => {
      if (item.seen_at === null && item.delivered_at === null) {
        retainedToastDeliveryKeysRef.current.add(item.delivery_key);
      }
    });
    setItems(nextItems);
  }, [setItems]);
  ...
  useEffect(() => {
    ...
    triggerBeforeOpenRef.current = null;
  ...
  const handleLoadMore = useCallback(async () => {
    ...
    if (view === "archive") {
      ...
      setUnseenItems(appendUniqueItems(items, page.items));
