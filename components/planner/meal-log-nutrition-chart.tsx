...
export function MealLogNutritionChart({ nutrition }: { nutrition: MealLogNutritionEvidence }) {
  const macros = [
    { label: "탄수화물", value: nutrition.carbohydrate_g, factor: 4, color: "#38bdf8" },
    { label: "단백질", value: nutrition.protein_g, factor: 4, color: "#34c7a2" },
    { label: "지방", value: nutrition.fat_g, factor: 9, color: "#f8b84e" },
  ];
        ...
        <div className="w-28 shrink-0 text-center sm:w-32">
          {available && energy > 0 ? (
            <svg aria-label={`${nutrition.calculation_status === "partial" ? "확인된 영양 기준 · " : ""}${macros.map((macro) => `${macro.label} ${format(macro.value!)}g`).join(" · ")} · 탄단지 열량 비율 · 총 칼로리 ${calorieLabel}`} className="h-28 w-28 sm:h-32 sm:w-32" role="img" viewBox="0 0 120 120">
              ...
              <text x="60" y="70" textAnchor="middle" fontSize="13" fontWeight="800" fill="#1e293b">{calorieLabel}</text>
            </svg>
          ) : <div className="flex min-h-28 flex-col justify-center rounded-full bg-sky-50 px-2"><span className="text-[11px] text-slate-500">총 섭취 열량</span><p className="mt-1 text-base font-extrabold text-slate-800">{calorieLabel}</p></div>}
        </div>
