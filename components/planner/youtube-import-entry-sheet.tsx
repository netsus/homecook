...
export function YoutubeImportEntrySheet({
  ...
  return (
    <AppBottomSheet
      ...
      bodyClassName="pb-5"
      description={isPrelaunchFeatureLocked() ? undefined : "링크를 붙여넣고 기존 유튜브 가져오기 화면에서 추출을 이어가요"}
      leadingAction={
      ...
      title="유튜브 가져오기"
    >
      {isPrelaunchFeatureLocked() ? <YoutubePreparationNotice onBack={onBack} /> : <>
      <label
      ...
      <div className="mt-5 flex">
        <Link
          className="flex h-[var(--control-height-lg)] flex-1 items-center justify-center whitespace-nowrap rounded-[var(--radius-sm)] bg-[var(--wave1-mint-contrast)] px-4 text-sm font-bold text-[var(--wave1-surface)] shadow-[var(--wave1-shadow-natural)] transition-colors hover:bg-[var(--wave1-mint-contrast-deep)]"
          href={importHref}
          onClick={onClose}
        >
          가져오기 화면 열기
