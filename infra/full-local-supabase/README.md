...
`FULL_LOCAL_BACKUP_READINESS_PATH` must be an absolute path in an owner-controlled, canonical external mode-0700 directory. Before readiness issuance, store the AES-256-GCM escrow envelope and its backup-key HMAC `.auth.json` sidecar on a second medium or credential manager that is distinct from both archives. The envelope pins an Ed25519 recovery-issuer public key. Only `restore-platform` may issue the production recovery manifest, using `--recovery-manifest`, `--escrow-envelope`, and a mode-0600 `--recovery-issuer-private-key` held by the replacement credential environment after the actual clean DB+Storage restore. `record-readiness` rejects backup-key-HMAC-valid self-assertions without that issuer signature. `pnpm verify:full-local-backup-key-recovery-drill` exercises the isolated replacement-environment adapter but deliberately reports `production_readiness_issued=false`; without a real distinct medium/private issuer key, production issuance remains blocked.
