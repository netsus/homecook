...
`FULL_LOCAL_BACKUP_READINESS_PATH` must be an absolute path in an owner-controlled, canonical external mode-0700 directory. Before readiness issuance, store the AES-256-GCM escrow envelope and its backup-key HMAC `.auth.json` sidecar on a second medium or credential manager that is distinct from both archives. The signed recovery evidence must bind the envelope's canonical absolute path, SHA-256, and observed device identity. On a clean replacement-Mac adapter, open that exact envelope, register/read the recovered key through the isolated Keychain adapter, authenticate/decrypt the actual encrypted platform archive, and complete the disposable DB+Storage clean restore. `pnpm verify:full-local-backup-key-recovery-drill` executes that real Docker backup/restore chain but deliberately reports `production_readiness_issued=false`; without a real distinct medium, production issuance remains blocked.
